...
- 도구의 현재 물리 경로는 `.agent_class/` 아래에 있지만, active canonical roots 는 `.registry`, `.unit`, `.workflow`, `.party`, `_workspaces` 다.
- 이 도구는 tracked file mutation 없이 현재 구조를 읽고 요약하고 검증한다.

## 현재 포함 도구

- `ui_sync.py`
- `sync-body-state` = 상태 보고용 명령. tracked file 을 쓰지 않고 현재 derive/write policy 를 출력한다.
- `resolve-loadout` = class/package, workflow, party surface 요약 명령이다.
...
- `derive-ui-state` = 6축 top-level (`species`, `units`, `classes`, `workflows`, `parties`, `workspaces`) 과 renderer surface (`overview`, `body`, `class_view`, `catalogs`, `ui_hints`) 를 함께 산출한다.
...
- `.registry/` cutover 이후에는 `.registry/species/**` 와 `.registry/classes/**` 를 canonical source 로 취급해야 한다.
...
- local smoke 는 private mount path 를 `--workspace-root` 또는 `SOULFORGE_LOCAL_WORKSPACE_ROOT` 로 주는 것을 권장한다. repo `_workspaces/` 를 그대로 스캔할 경우 `company/`, `personal/` 디렉터리는 project root 후보가 아니므로 warning 후 건너뛴다.
...
- `derive-ui-state --json` = `source`, `species`, `units`, `classes`, `workflows`, `parties`, `workspaces`, `diagnostics`, `ui_hints`, `overview`, `body`, `class_view`, `catalogs`
...
- 이번 phase에서는 도구 코드를 대규모로 바꾸지 않는다.
- `.agent_class/` 는 더 이상 owner root 가 아니므로, 후속 phase 에서 tool home 과 read contract 를 `.registry` 기준으로 정리한다.
