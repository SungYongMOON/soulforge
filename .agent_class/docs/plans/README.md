...
- 현재 계획 세트에는 class loadout resolve 확장, workspace project contract resolve 확장, UI derive state generator 단계, read-only UI prototype 단계, 첫 happy-path reference sample baseline 단계, 첫 invalid reference sample baseline 단계, 첫 unbound reference sample baseline 단계가 포함된다.
