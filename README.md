...
- **어디서 도는가 — 저장소를 읽고 운영을 판단하지 말 것**: HPP 서버 팩은 0.1.9가 운영 pin이고(정션 `current`→0.1.7·`previous`→0.1.6은 선언용이라 실행과 무관하며 다음 Owner 창에서 문장과 함께 정렬한다, 0.1.8은 설치만), **Bellows는 `current` 포인터를 따라가지 않고 payload 경로를 판본까지 직접 pin한다**. 2026-09-06 전환 영수증(`local-recovery/erp-019-cutover-20260906`) 기준: World Tree 서버(`Soulforge-MainNode-ERP`)는 `0.1.9`를 `-EnableMcp -EnableMcpReviewRead`(변형 B)로 돌리고, 연속 수집·PC 활동 두 작업도 `0.1.9`를 pin한다. 음성 ASR 작업은 인자가 `0.1.6`이지만 상주 감독기는 2026-09-01부터 `0.1.2`로 돌고 있어(싱글톤 lease) Owner 유지보수 창에서 정지·전환한다. Vigil 작업(`Soulforge-TeamOpsBoard-ReadOnly-v1`, 관측된 예약작업은 1개)은 팩이 아니라 `install/source-lanes/operations-lane-v4`(source commit `d0f95448`, 2026-09-06 전환, 이전 v3 보존)에서, Hiworks 전달기·Codex 보존 두 작업은 아직 `install/source-lanes/operations-lane-v2`에서(2026-09-06 밤 관측, 다음 lane 전환 창에서 v4로 통합), Tongs(MCP 문, 4311)는 `install/source-lanes/tongs-lane-v2`(예약작업 `Soulforge-Tongs-Loopback-v1`, 2026-09-06 등록, 심박 60초 자체 갱신)에서, Linear·Buzz·Slack 수집은 각 `install/source-lanes/<lane>-v1`에서 돈다. secure-work 사이클 시험(`sfx`)은 등록된 예약작업이 없고 `install/source-lanes/secure-work-lane-v1`에서 Buzz DM 트리거(Hermes 스킬)로만 on-demand 실행된다. 예외 1건: NAS DR 백업 작업은 아직 legacy checkout에서 실행된다(정비 대상). 그래서 **`main` = 도는 것이 아니다**: main은 운영 lane이 담은 커밋보다 앞서 있을 수 있고, 무엇이 도는지의 정본은 각 lane의 `LANE_MANIFEST.md`가 기록한 source commit이다. lane 무결성은 `node guild_hall/deployment_pack/tools/build_source_lane.mjs --verify <lane 루트>`로 언제든 재확인한다. 18080 로컬 모델 서버(강도담의 G2, `llama-server`)는 예약작업 없이 수동 기동 상태라 재부팅 뒤 재기동 주체가 정해지지 않았다(Owner 창). 이 문단은 예약작업 인자를 직접 읽어 대조한 관측(2026-09-06 밤)이며, 다음 전환 때도 문장을 먼저 고치지 말고 관측 뒤에 고친다.
- **전체 구조를 처음 읽는 순서**: 이 README → Master Map → 용어집 → `DOCUMENT_OWNERSHIP.md`·`TARGET_TREE.md` → plan 18(팀 파일럿 접속·출시 사다리) → plan 17(물리 구조) → plan 10(수집과 백업). 에이전트의 bounded 작업은 `AGENTS.md` → 실행 계약의 공통 절 → 조건부 읽기 표의 해당 owner만 확인한다. 이 전체 항법을 매 작업 선적재하지 않는다.
