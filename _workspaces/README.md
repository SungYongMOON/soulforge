...
- `_workspaces/gateway/` 는 mail fetch, intake inbox, gateway log 를 담는 workspace-level local-only ingress site 다.
- `company/`, `personal` 분기는 새 정본에 포함하지 않는다.
- tracked sample 이 필요하면 `_workspaces/` 아래가 아니라 `docs/architecture/workspace/examples/` 아래에 둔다.
- 다른 PC 나 다른 LLM 이 runtime shape 를 따라야 하면 `docs/architecture/workspace/examples/gateway/` 의 public-safe mirror sample 을 먼저 읽는다.
...
- `_workspaces/gateway/` 는 mail fetch 와 project assignment 전 intake staging 을 함께 담는 ingress/staging site 다.
...
- 현재 `_workspaces/gateway/` 는 `gateway:intake` 첫 실행 시 필요한 intake/log 폴더가 자동으로 materialize 된다.
