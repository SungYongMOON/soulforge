...
11. When the user explicitly requests an Outlook draft, bind the requested control surface before applying it:
   ...
   - Apply `structure.tables.layout` from the readability preset in the Outlook Word editor; do not duplicate its width, layout, semantic-profile measurements, or Word enum constants in this bridge. Consume the preset-owned point-width enum name/value mapping exactly. Select width policy in this order: latest owner explicit width override, matching `request_work_three_column` semantic profile, then generic default. Save, close with save, reopen the same locked draft by StoreID/EntryID, and verify the actual table and column widths still satisfy the selected path before reporting success.
...
12. If the owner later gives a separate, current, explicit send or scheduled-send instruction, treat that as a distinct executor handoff for the exact current-request draft only. Carry forward the locked StoreID/EntryID, recipients and order, thread subject, body corrections, selected attachment, control surface, and logical signature; do not ask for them again unless the owner changed them, validation detects drift, or a context boundary has no trusted lock record. Resolve the draft by StoreID/EntryID, produce a pre-send revalidation result for all locked fields, and only then call `.Send()` exactly once. Poll Sent Items and Outbox once per second for at most 30 seconds. Correlate a result using the runtime-private locked subject, typed ordered recipient identities, attachment name/size/digest, normalized body digest, and send-start UTC time. Use direct normalized SMTP identity when available. If an Outlook personal distribution list (`MAPIPDL`) has no direct SMTP identity, resolve every member to SMTP, lowercase, deduplicate, sort, and hash the canonical member set into a runtime-private group fingerprint; preserve the group's top-level recipient position and never log or expose its member addresses. Stop before send if any group member cannot be resolved. Use `scripts/outlook_recipient_correlation.ps1` for this normalization. Zero matches at the bound is `unknown`, more than one match is `ambiguous`, and neither state permits automatic retry. Maintainers run `scripts/test_outlook_send_continuation.ps1` for the file-only at-most-once regression. This authoring launcher never treats draft approval as send approval and never sends a synthetic or evaluation sample.
