...
11. When the user explicitly requests an Outlook draft, bind the requested control surface before applying it:
   ...
   - Apply `structure.tables.layout` from the readability preset in the Outlook Word editor; do not duplicate its width, layout, or Word enum constants in this bridge. Consume the preset-owned point-width enum name/value mapping exactly. Use the preset's default verification path unless the latest owner correction explicitly selects another width, then use its owner-override verification path. Save, close with save, reopen the same locked draft by StoreID/EntryID, and verify the actual table width still satisfies the selected path before reporting success.
