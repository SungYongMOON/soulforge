...
- Preserve the model and reasoning profile the user specified. If none was specified, inherit the current/default profile under the tool contract. When selectors or effective-profile evidence are absent, report that limit rather than guessing the strongest worker or an upgrade/downgrade.
...
2. Declare the objective, success criteria, and stop conditions in the checkpoint or current plan. Reuse a matching app goal if the read tool is available. Create a new app goal only when explicitly requested or authorized by higher-priority instructions and permitted by the tool contract; otherwise continue with file-based tracking. Preserve conflicting active goals and resolve the conflict before a dependent action, without blocking independent preparation.
