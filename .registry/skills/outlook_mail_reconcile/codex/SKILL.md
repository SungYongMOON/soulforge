...
description: "Use when Codex should run the Soulforge Outlook mail reconciliation workflow for /outlook-reconcile: optionally refresh Outlook with Send/Receive, update project sent-mail history from Outlook sent-folder metadata, and cross-validate received-mail history against inbox metadata without raw mail export."
---
...
5. If the owner or automation policy requires mailbox refresh, trigger Outlook Send/Receive immediately before metadata collection and record the preflight result.
6. Use only workflow-approved Outlook metadata fields. Sent mail may produce a private project ledger delta; received mail is cross-validation only.
7. Route ambiguous project matches or uncertain rows to owner follow-up instead of automatic upsert.
8. Close with the workflow boundary review before claiming the run state.

## Boundary Rules

- Do not send composed mail, move mail, delete mail, mark read/unread, categorize, or edit Outlook mail. Outlook Send/Receive preflight is allowed only when the bound run policy explicitly asks for it.
