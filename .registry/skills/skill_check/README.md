...
- `skill_check/skill.yaml` 은 Soulforge skill package boundary 검토를 위한 active canonical skill entry 다.
...
- actual model, MCP, tool, attached skill package, install path 는 local `_workmeta/<project_code>/bindings/` owner 이며 tracked skill folder 에 적지 않는다.
