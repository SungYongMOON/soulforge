...
description: "Use when Codex should run the Soulforge dual deep research workflow: NotebookLM CLI Deep Research via the repo-defined nlm/notebooklm-mcp contract, Codex direct source research as an independent path, optional Gemini or GPT web Deep Research advisory lanes, comparison, common-claim dedupe, and delta handoff. Also use for Korean requests mentioning 노트북LM CLI, 제미나이 딥리서치, GPT 딥리서치, 심층 조사, 직접 조사 비교, 공통 결과 제거, or NotebookLM/Gemini/GPT와 Codex 조사 비교."
---
...
The workflow owns the actual procedure, including the fixed NotebookLM CLI-first command contract, calibrated profile policy, Codex independent research path, optional Gemini/GPT web Deep Research advisory lanes, comparison buckets, common-claim dedupe rule, output templates, and downstream handoff. Do not re-invent the NotebookLM CLI sequence or external web Deep Research sequence inside this skill.
...
4. Start with the workflow `declare_goal` step before NotebookLM, Codex, optional external Deep Research, comparison, sourcebound, or registration work.
...
7. Keep NotebookLM, Codex direct research, and optional Gemini/GPT Deep Research advisory lanes independent until the workflow comparison step.
...
- Do not treat NotebookLM output, Gemini output, GPT output, Codex output, or their agreement as source truth, owner approval, ontology acceptance, or canon promotion.
...
- Do not present the synthetic profile calibration as proof that real NotebookLM, Gemini, GPT, web, Drive, wiki, source truth, account state, or owner-decision behavior has been pilot-executed.
