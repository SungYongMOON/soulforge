...
supported_input:
  ...
  quality_grade: 없음
principles:
- 폴더 순서 = 탐색개발 기본 SE 수행 순서
- SRR/SFR/PDR/CDR/TRR/FCA/PCA spine 을 유지하되 contractor-specific 품질 게이트는 넣지 않는다.
- 탐색개발 단계에서는 개념, 요구도, 기술성숙도, 운용성 확인, 다음 단계 진입 판단 근거를 public-safe 공통 산출물로 다룬다.
- 완료판정은 03_Out 기준
root_naming:
...
- code: 30
  ...
  - id: 34
    name: 운용개념(CONOPS)
    desc: 운용 시나리오, 사용자 관점, 임무 환경 정리
    term: CONOPS
    source: 방위사업관리규정
    template: 없음
  - id: 35
    name: 탐색개발실행계획서
    desc: 탐색개발 범위, 일정, 예산, 핵심기술 검증 계획
    term: 탐색개발계획
    source: 방위사업관리규정
    template: 없음
  - id: 36
    name: 기술성숙도및TRA자료
    desc: 핵심기술, CTE, TRL/TRA 판단 근거
    term: TRL_TRA
    source: 방위사업관리규정
    template: 없음
  - id: 37
    name: 체계요구사항명세서(SSRS)_D
    desc: 요구도 분석을 반영한 체계요구사항 초안
    term: SSRS
...
- code: 90
  ...
  - id: 94
    name: 기본설계기술서_D
    desc: 시제 수준 기본설계, 구조/아키텍처 초안
    term: 기본설계
    source: SE기반 기술검토회의 가이드북
    template: 없음
  - id: 95
    name: 대안분석(Trade_Study)
    desc: 설계 대안, 비용/위험/성능 절충 분석
    term: Trade_Study
    source: SE기반 기술검토회의 가이드북
    template: 없음
  - id: 96
...
- code: 120
  ...
  - id: 126
    name: 구현및시제제작준비자료
    desc: 시제 제작/구현 준비, 제작 가능성, 자원 준비 자료
    term: 시제준비
    source: 방위사업관리규정
    template: 없음
  - id: 127
    name: 위험관리및잔여과제
    desc: 잔여 위험, 한계, 후속 보완 과제
    term: 위험관리
    source: 과학적사업관리 수행지침
    template: 없음
  - id: 128
...
- code: 150
  ...
  - id: 156
    name: 시험자원준비상태
    desc: 시제, 장비, 인력, 시험환경 준비 자료
    term: 시험준비
    source: 시험평가 가이드북
    template: 없음
  - id: 157
    name: 안전보안환경확인
    desc: 시험 관련 안전, 보안, 환경 점검 자료
    term: 안전보안
    source: 시험평가 가이드북
    template: 없음
  - id: 158
...
- code: 180
  ...
  - id: 184
    name: 개발시험결과(DT)
    desc: 탐색개발 시제 수준 개발시험 결과
    term: DT
    source: 시험평가 가이드북
    template: 없음
  - id: 185
    name: 운용성확인결과
    desc: 운용성 확인 결과와 판단 근거
    term: 운용성확인결과
    source: 방위사업관리규정
    template: 없음
  - id: 186
...
- code: 210
  ...
  - id: 214
    name: 제품형상식별서(PCI)_F
    desc: 시제/산출물 형상 식별 및 최종 상태 자료
    term: PCI
    source: SE기반 기술검토회의 가이드북
    template: 없음
  - id: 215
    name: 기준선및As_Built자료
    desc: 최종 기준선, 실제 구현 형상, 변경 반영 자료
    term: As_Built
    source: 방위사업관리규정
    template: 없음
  - id: 216
    name: 탐색개발최종보고서
    desc: 탐색개발 수행 결과와 핵심 근거 종합 보고서
    term: 최종보고서
    source: 방위사업관리규정
    template: 없음
  - id: 217
    name: 체계개발진입판단자료
    desc: 다음 단계 진입 여부, 잔여 리스크, 추천 조치
    term: 진입판단
    source: 방위사업관리규정
    template: 없음
  - id: 218
...
- 이 문서의 YAML(위)을 파싱해서 탐색개발 기본형 폴더 트리를 생성한다.
- public-safe 공통 variant 이므로 contractor-specific Q-gate, 프로젝트 고유 산출물, 품질등급 분기 규칙은 넣지 않는다.
- 게이트 spine 은 `SRR/SFR/PDR/CDR/TRR/FCA/PCA` 를 유지하되, 탐색개발 핵심 근거는 `TRL/TRA`, `운용성 확인`, `체계개발 진입 판단`에 둔다.
