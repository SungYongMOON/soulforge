...
- Regenerate `assets/compiled/*.json` with `scripts/export_variant_json.py` after editing a spec; `--check` guards drift.
...
- Read [`references/source_verification_v0.md`](references/source_verification_v0.md) (DRAFT, 2026-08-18) before trusting a variant's gate spine or task list for a business type: it records, task by task, which items are supported by DAPA/MND sources, which are prime-contractor contract items, and which are missing. Current verdicts: 체계개발 spine source-supported (17 required items missing); 탐색개발·선행연구 baselines borrow the 체계개발 naming and need re-basing; 운용연구개발 needs track separation; 응용연구 has a proposal only.
