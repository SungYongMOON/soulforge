...
- 2026-08-18 계층 분리: 같은 체계개발 스펙에서 `compiled/system_dev_common_no_grade.json`(방사청 공통 기준선 131, prime_contract 제외)과 `compiled/overlays/system_dev_lig_grade_a.prime.overlay.json`(LIG 계약 항목 14, overlay add)을 함께 내보낸다. 다른 발주처 과제는 공통 기준선 + 그 발주처 덧씌움으로 쓰고, LIG 항목은 섞이지 않는다. 드리프트 가드는 세 파일을 모두 대조한다.
