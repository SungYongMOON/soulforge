...
- future target `_workspaces`에는 Human/project authority가 accepted 한 exact canonical bytes만, future target `_workmeta`에는 그 canonical byte-lineage만 들어간다. Owner 채택(2026-09-12 legacy 평면 비정본 선언·새 평면 직접 구축 결정, 2026-09-21 한 종류씩 반영 결정)으로 지금 허용되는 것은 과제별 SE 폴더트리 고정 `020_MGMT` 관리 폴더의 관리 기록 하나씩뿐이다: 규칙·운영(021)·연락처(023)·작업 장부(025)·할일 장부(026)·메일 수신/발송 이력과 회신 현황(027), 그리고 공용 자료 폴더·project-less 일반업무 폴더의 같은 장부, 지식 폴더 아래 회사 PJT 관리 대장 사본까지이며 모두 pointer-only CSV/MD 기록(사본 1개)에 target `_workmeta` byte-lineage 기록을 짝으로 둔다. run 산출물, 장부가 아닌 raw worklog·task 원문, battle, collector 원본 출력, analytics, procedure-capture, 메일 본문, 첨부, secret은 여전히 target에 새로 쓰지 않으며, 위 목록 밖의 다른 target 기록은 별도 Owner 채택 전에는 쓰지 않고 일반 binding/materialization 경로는 W-AUTH, Canonical Empty-State Genesis, applicable Legacy Freeze 채택 그대로다.
