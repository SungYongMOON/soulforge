...
- 보호 대상 업무 데이터는 Soulforge root 아래 nested private repo `private-state/` 에만 commit/push 한다.
- active AI workspace 는 `Soulforge/` 하나로 보고, private 데이터 plane 도 같은 workspace 안의 `private-state/` 에서 함께 읽는다.
