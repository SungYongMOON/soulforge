...
5. [ ] Goal #2 후속: 공식 수집 조건·계약 provenance와 OpenAlex/S2 metadata client는
       2026-09-08 합성 HTTP/SQLite 범위에서 구현·검증. 현재 계정/상품·권리·예산은
       UNKNOWN/OFF이며 실수집·운영 활성화는 별도다. 다음은 버스트·주간 브리핑·상세
       관측 프로필. LLM 액션은 별도 경로/게이트가 갖춰진 뒤.
...
### Owner에게 남은 것
- KIPRIS 현재 상품·계정·키 사용 범위와 월초 초기화 시각/시간대 확인. 과거 신청·키 발급
  이력만으로 현재 권리나 제3자 제공 허가를 추정하지 않는다.
