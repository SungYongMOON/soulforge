...
CREATE TABLE IF NOT EXISTS mail_route_rule ( -- Outlook식 사용자 라우팅 규칙(메일→과제). 발신자/제목 메타로만 매칭(본문 미저장)
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  field TEXT NOT NULL,                 -- from(발신자) | subject(제목)
  pattern TEXT NOT NULL,               -- 매칭 값(소문자 비교)
  match TEXT NOT NULL DEFAULT 'contains', -- contains(포함) | equals(완전일치)
  project_id TEXT NOT NULL,            -- 라우팅 대상 과제
  enabled INTEGER NOT NULL DEFAULT 1,
  note TEXT,
  created_by TEXT,
  created_at TEXT
...
export class Store {
  ...
  // --- Outlook식 메일→과제 라우팅 규칙(사용자 정의, 2026-07-05 owner) ---
  // 우선순위: 사용자 규칙(여기, 인입 시 즉시) > 엔진 바인딩 exact/hint(수집 후 backfill 단계)
  // — Gmail '사용자 필터가 자동분류보다 우선' 원칙. 매칭은 발신자/제목 메타만(본문 미사용).
  mailRouteRules({ enabledOnly = false } = {}) {
    return this.db.prepare(`SELECT * FROM mail_route_rule ${enabledOnly ? "WHERE enabled=1" : ""} ORDER BY id ASC`).all();
  }
  addMailRouteRule({ field, pattern, match = "contains", project_id, note = null, created_by = null } = {}) {
    const f = String(field ?? "").trim(), p = String(pattern ?? "").trim(), m = String(match ?? "contains").trim();
    const proj = String(project_id ?? "").trim();
    if (!["from", "subject"].includes(f)) return { error: "field_invalid" };
    if (!Store.MAIL_RULE_MATCHES.includes(m)) return { error: "match_invalid" };
    if (!p) return { error: "pattern_required" };
    const target = this.db.prepare("SELECT id, class FROM core_project WHERE id=?").get(proj);
    if (!target) return { error: "project_not_found" };
    if (target.class === "inbox") return { error: "target_is_inbox" }; // 받은함→받은함 라우팅은 무의미
    const info = this.db.prepare(
      "INSERT INTO mail_route_rule(field,pattern,match,project_id,enabled,note,created_by,created_at) VALUES(?,?,?,?,1,?,?,datetime('now'))"
    ).run(f, p, m, proj, note ? String(note).slice(0, 200) : null, created_by || null);
    this._mailRouteCache = null;
    return { ok: true, id: Number(info.lastInsertRowid) };
  }
  deleteMailRouteRule(id) {
    const r = this.db.prepare("SELECT id FROM mail_route_rule WHERE id=?").get(id);
    if (!r) return { error: "rule_not_found" };
    this.db.prepare("DELETE FROM mail_route_rule WHERE id=?").run(id);
    this._mailRouteCache = null;
    return { ok: true, id };
  }
  // 인입 시 라우팅 대상 판정 — 첫 매칭 규칙(등록순) 승. 대상 과제가 사라졌으면 그 규칙은 건너뜀.
  _mailRouteTarget({ counterpart = null, subject = null } = {}) {
    if (!this._mailRouteCache) this._mailRouteCache = this.mailRouteRules({ enabledOnly: true });
    if (!this._mailRouteCache.length) return null;
    for (const r of this._mailRouteCache) {
      const hay = String((r.field === "from" ? counterpart : subject) ?? "").toLowerCase();
      const needle = String(r.pattern ?? "").toLowerCase();
      if (!needle) continue;
      if (r.match === "equals" ? hay === needle : hay.includes(needle)) {
        if (this.db.prepare("SELECT 1 FROM core_project WHERE id=?").get(r.project_id)) return r.project_id;
      }
    }
    return null;
  }
  // 'Outlook: 현재 폴더에 지금 적용' — 받은함(INBOX) 잔여 메일에 규칙 소급 적용. rule_id 지정 시 그 규칙만.
  // 이동은 run17 검증 경로(setMailProject: 승격 할일 동행 이동 + autosync write-through) 재사용.
  applyMailRouteRulesToExisting({ rule_id = null } = {}) {
    const rules = this.mailRouteRules({ enabledOnly: true }).filter((r) => rule_id == null || r.id === Number(rule_id));
    if (!rules.length) return { scanned: 0, moved: 0, items_moved: 0 };
    const rows = this.db.prepare(
      "SELECT id, counterpart, subject FROM core_mail WHERE project_id=? AND COALESCE(hidden,0)=0"
    ).all(INBOX_PROJECT_CODE);
    let moved = 0; let itemsMoved = 0; const byProject = {};
    for (const m of rows) {
      for (const r of rules) {
        const hay = String((r.field === "from" ? m.counterpart : m.subject) ?? "").toLowerCase();
        const needle = String(r.pattern ?? "").toLowerCase();
        if (!needle) continue;
        if (!(r.match === "equals" ? hay === needle : hay.includes(needle))) continue;
        const res = this.setMailProject(m.id, r.project_id);
        if (res.ok && !res.unchanged) {
          moved += 1;
          if (res.item_moved) itemsMoved += 1;
          byProject[r.project_id] = (byProject[r.project_id] ?? 0) + 1;
        }
        break; // 첫 매칭 규칙 승
      }
    }
    return { scanned: rows.length, moved, items_moved: itemsMoved, by_project: byProject };
  }
  ...
  ingestMail({ id, project_code = null, at = null, subject = null, counterpart = null, stage_code = null,
    ...
    // 사용자 라우팅 규칙(Outlook식): 받은함(INBOX)행 메일이 규칙에 맞으면 인입 시점에 그 과제로 직행.
    // 사용자 규칙 > 엔진 backfill(수집 후 단계) — 이미 실과제로 분류된 메일(existing 보존 포함)은 건드리지 않음.
    if (project_id === INBOX_PROJECT_CODE) {
      const routed = this._mailRouteTarget({ counterpart, subject: subj });
      if (routed) project_id = routed;
    }
  ...
  ensureMailLabel(name, color = "#195a8e") {
    const trimmed = String(name ?? "").trim();
    if (!trimmed) return { error: "label_name_required" };
    const existing = this.db.prepare("SELECT * FROM mail_label WHERE name=?").get(trimmed);
    if (existing) return { label: existing, existed: true };
    return this.createLabel(trimmed, color);
  }
  ...
  setMailLabelByRefs({ project, history_key, mail_id, label, color = "#195a8e", on = true } = {}) {
    const labelResult = this.ensureMailLabel(label, color);
    if (labelResult.error) return labelResult;
    const labelId = labelResult.label.id;
    const candidates = [
      String(mail_id ?? "").trim(),
      project && history_key ? `${project}:${history_key}` : "",
      history_key ? `mailcsv:${history_key}` : "",
    ].filter(Boolean);
    let applied = 0;
    for (const id of [...new Set(candidates)]) {
      if (!this.db.prepare("SELECT 1 FROM core_mail WHERE id=?").get(id)) continue;
      const result = this.setMailLabel(id, labelId, on);
      if (!result.error) applied += 1;
    }
    return { ok: true, label_id: labelId, applied };
  }
