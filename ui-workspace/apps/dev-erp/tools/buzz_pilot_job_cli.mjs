...
import {createBuzzPilotJob,BUZZ_PILOT_ROLES,validateBuzzPilotBinding} from '../src/buzz_pilot_job.mjs';
...
async function pinnedFile(file,maxBytes,expected,{executable=false,hashOnly=false}={}){
  ...
  if(!before.isFile()||before.isSymbolicLink()||before.size>maxBytes||before.nlink<(executable?1:1)
...
export async function runBuzzPilotCli(args,{input=process.stdin,output=process.stdout}={}){
  ...
  if(action==='issue')instruction=(await pinnedFile(path.resolve(args[6]),64*1024)).bytes;
  if(action==='append'){
  ...
  const db=new DatabaseSync(binding.control_db_path);
  try{
    db.exec('PRAGMA busy_timeout=5000; PRAGMA journal_mode=WAL; PRAGMA synchronous=FULL;');
    const workingBytes=createProtectedWorkingBytes({root:binding.evidence_root,repositoryRoot:binding.repository_root,
      storageClass:binding.storage_class,ownerApprovalRef:binding.owner_approval_ref,roles:BUZZ_PILOT_ROLES});
    const job=createBuzzPilotJob({db,workingBytes,binding:core,authorize:async()=>{
