...
현재 `/workbench.html`은 결속된 Buzz 업무 한 건의 실행·질문·결과 기록을 조회한다.
오늘 할 일·전체 대기·진행/막힘·담당·결과물·과제 자료를 통합한 일상 업무용 작업대는
아직 미완료다. 이 화면을 완성된 통합 작업대로 소개하거나 설치 검사로 그 사용성을 대신하지 않는다.
...
runtime DB에만 저장한다. 발행된 Buzz 파일럿 조회가 켜지면 같은 질문도 이 함에 들어온다.
native 질문 원문은 현재 권한을 재확인한 보호 reader로 표시하고 읽음·미루기만
현재 서버의 보기 상태에 저장한다. native 작업 DB와 원본 인증 DB는 바꾸지 않는다.
Buzz가 이미 전달한 질문의 정확한 대화 링크를 사용하며 별도 알림을 중복 발송하지 않는다.
MCP 요청용 추가 알림 transport는 여전히 별도 결속이 필요하다.
...
이 선택은 정확한 Buzz GET 두 경로와 응답 대기함 GET, 대기함의 읽음·미루기 POST에만
적용한다. POST는 같은 Owner 세션·과제 권한·CSRF를 요구하고 후보 서버의 보기 상태만
변경한다. 다른 API·업무 실행·원본 DB 쓰기·로그인 권한은 바꾸지 않는다.
...
정상적인 인증 소스 연결에서 로그인이 필요한 경우에는 서버가 같은 loopback 호스트와
설정된 원본 포트로 만든 `login_url`을 보내며 작업대의 로그인 링크가 이 주소를 사용한다.
설정 실패(503)에는 로그인 링크를 제공하지 않으며, 인증 소스 미설정 모드의 기존 안내는 유지한다.
쿠키·비밀번호를 복사하거나 후보 계정을 새로 만들어 이 절차를 대신하지 않는다.

로그인 뒤에도 상단 브랜드와 `World Tree로 돌아가기`는 같은 원본 서버를 가리킨다.
서버가 신뢰된 설정으로 만든 `home_url`만 사용하며 Host·query나 업무 관측값으로
목적지를 정하지 않는다. 인증 소스가 없는 일반 작업대는 자기 서버의 홈을 유지한다.
