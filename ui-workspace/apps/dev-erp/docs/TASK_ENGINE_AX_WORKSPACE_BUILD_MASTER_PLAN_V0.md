...
> `ok`, failed labels `0`, no-content `1`; a subsequent dry read found pending `0` and
> official task/project mutations `0`. The first ingress cycle recorded `ok`, errors `0`,
> and mail `ok`. 4192 remained HTTP-ready; its refreshed Watchtower projection shows the
> voice and ingress nodes healthy. Five unrelated usage-meter lanes were merely late,
> not stale/down, and remain outside this bounded voice/ingress repair.
>
...
> `continuous_lease.v1` record unchanged while adding separate Windows process-instance
> evidence for one bounded self-heal case: an expired same-host lease whose numeric PID is
> still live but belongs to a provably newer process. All missing, malformed, unbound,
> out-of-window, unresolved, matching, or older evidence remains fail-closed. Supervisor
> heartbeats preserve a bounded sanitized reason code, and stale/orphan instance evidence is
> cleaned without deleting active or recovery-marker evidence. Public validation and a fresh
> independent Level-3 review passed; installed runtime application, scheduler restart, and live
> private-state observation remain separate Owner-gated work.
>
> 2026-08-25 CURRENT: a separate public-synthetic, default-OFF, in-memory
> `CandidateExecutionCoordinator` foundation is GREEN without changing or wrapping the existing
> `TaskExecutionCore`/SQLite semantics. `AI 실행 후보` is only a GPT/ingress prefilter marker;
> exact versioned Role/Capability plus explicit actor→agent→Bot→Executor binding and
> `responsible_ceo_triage` produce the assignment packet. The coordinator validates exact packet
> consistency, opaque parent-child-grandchild coverage custody, one active slot per performing
> agent with cross-agent parallelism, Waiting/HOLD release, exact successor fencing, replay
> `NO_OP`/conflict `HOLD`, execute-entry deep snapshot, and metadata-only attribution/effect
> receipts. Coordinator 21/21, Hermes bot-submit Executor 16/16, existing Task
> Execution Core 28/28, and adjacent Agent Observation 295/295 are GREEN evidence. Official Done
> and mutation remain false; Synthetic effects are observed 0 while Hermes unmeasured effects are
> literal `UNKNOWN` and never `null`. Injected `runCommand` is only a trusted test seam. One
> Owner-approved team-only synthetic canary used the Codex Linear connector to create `AI 실행 후보`
> Todo `SON-59`, then exercised exact Role/Capability→assignment→Coordinator→default Hermes runner.
> The exact product-manager Bot/session/model completed the expected marker, same-process replay was
> `NO_OP`, 4192 observed one matched Bot `working→idle`, and Official Done/mutation remained false.
> This does not activate a live Linear collector, organization source, persistent ledger, scheduler,
> writer cutover, automatic assignment, real-work Work Brief route, or production claim.

> 2026-08-21 CURRENT: public-synthetic foundations were integrated without activating a live
> route. Engineering Engine release integrity binds the exact Git-index source set; one admitted
> project PDF can produce trusted RAG/Thin Wiki and a bounded claim-before-admission runner can
> persist one body-free candidate, while a read-only Preparation Module can verify a direct
> non-reparse source and build the existing authority packet without opening the PDF; an
> authentic-producer P5 Module can prepare a complete
> human-review candidate under a separate trusted request pin; and `backup_controller` has an
> offline Linear LB1 v1/v2 manifest/restore contract, exact feature-OFF Owner Gates, and an
> in-memory claim-before-read Bound Runner. Actual project PDF execution, operational
> RAG/Wiki persistence, P5 acceptance/generation advance, Linear/Drive/NAS, Task dispatch/write,
> MCP/runtime, scheduler, and AgentRun activation remain `HOLD`.

> 2026-08-20 CURRENT: a public-synthetic, feature-OFF Task Execution Core POC
> validates Linear Official Todo eligibility, one active AgentRun, append-only
> provider/run events, Waiting and terminal receipts, crash HOLD, and idempotent
> replay in a memory-only SQLite POC store with MockExecutor fixtures. No live route is
> wired to Linear, dev-ERP task truth, MCP, scheduler, files, mail, Slack, or sharing state.
> Arbitrary Executor sandboxing, P5-P8 acceptance, and operational Dispatcher/AgentRun
...
>
> 2026-08-19 CURRENT: the Team Ops Board and AI Usage Meter add the v3 snapshot
> contract (`unmeasured_request_daily`) with exact Antigravity request overlay
> projections. Producer evidence gates by `source.kind -> provider=antigravity`,
> classifies exact Gemini and Claude/GPT quota families while failing closed on
> unknown model identifiers, and preserves left-axis token totals while projecting
> dual-axis request activity, realtime 7-day model drilldowns, and cumulative distribution.
>
...
| 계획 기준일 | 2026-07-15 |
| 최신 CURRENT 관찰 | 2026-08-28 (public implementation validated + exact installed runtime applied): exact `continuous_lease.v1`을 유지한 채 Windows PID 재사용을 별도 process-instance evidence로만 판별하고, same-host·expired·strictly newer 조건에서만 stale lease 자동 복구를 허용한다. 그 밖의 증거 결손·불일치·시간창 이탈·probe 불명은 모두 fail-closed이며 supervisor heartbeat에는 경로·token 없이 bounded reason code만 남긴다. public validator와 fresh independent review가 PASS했고, Owner-authorized exact runtime 적용·두 supervisor 재시작 뒤 ingress fresh heartbeat `ok`/error `0`/mail `ok`, voice fresh health `ok`/failed `0`/no-content `1`을 관찰했다. 2026-08-25 default-OFF structure + one bounded live canary 관찰은 계속 유효하다: 기존 `TaskExecutionCore`/SQLite와 분리된 in-memory 구조가 exact Role/Capability·actor→agent→Bot→Executor binding, `responsible_ceo_triage`, nested custody fingerprint, per-agent slot/cross-agent parallelism, Waiting/HOLD release, successor/replay/fencing, execute-entry deep snapshot, metadata-only attribution/effect evidence를 검증했다. Coordinator 21/21·Hermes Executor 16/16·Task Core 28/28·Agent Observation 295/295 evidence에 더해, Owner 승인 team-only 합성 `SON-59`가 `AI 실행 후보`/Todo로 생성되고 exact 제품 총괄 `gpt-5.6-terra` session에 기본 runner로 전달됐다. marker digest 일치, 같은-process replay `NO_OP`, 4192 matched Bot 1·`working→idle`, Official Done/mutation false를 관찰했다. Linear label은 prefilter이고 connector는 authority가 아니다. Windows descendant process-tree kill·timeout 이후 상태·effect totals는 계속 `UNKNOWN`; live collector/organization source·persistent ledger·scheduler·writer·automatic assignment·실업무 Work Brief·production route는 `HOLD`다. 아래 누적 행은 이전 관찰 이력이다 |
| 누적 CURRENT 관찰 이력 (2026-08-20까지) | 2026-08-20 (public-synthetic Task Execution Core POC): Linear Official Todo를 읽기만 하는 feature-OFF 실행 하네스가 Candidate 차단, complete Work Brief·Executor·authority gate, task별 active AgentRun 1개, append-only provider/run event, Waiting·terminal receipt, crash recovery HOLD, idempotent replay를 격리 SQLite와 합성 fixture로 검증했다. 실제 Linear·dev-ERP task truth·MCP·scheduler·외부 effect·live backup·P5~P8 acceptance는 0이다. Linear CSV/Google Sheets snapshot과 API Comment/history/webhook/reconciliation의 범위 검토 및 feature-OFF offline LB1 manifest/restore contract까지 추가했지만 actual collector/storage/restore는 0이다. 2026-08-19 (public candidate, Team Ops Board / AI Usage Meter Antigravity request overlay): Antigravity unmeasured request count 30일 시계열(`unmeasured_request_daily`) 스냅샷 계약 v3 확장 및 양방향 검증기 구현 완료(상태: `PUBLIC_SYNTHETIC_IMPLEMENTED / RECONCILED_WITH_PROVIDER_DAILY / FAIL_CLOSED_UNKNOWN`). Meter/Board 공통으로 `source.kind -> provider=antigravity` 명시적 원천 증거 기반 게이팅, `ag_gemini` / `ag_claude_gpt` 2개 쿼터 패밀리 분류, 미확인 모델 ID 대상 fail-closed 차단, 원장 토큰 합계 보존, 우측 축 요청 오버레이 및 7일/30일 모델 드릴다운을 합성 검증함(private 경로·수치·토큰 미주장). 2026-08-19 (public main, Slack Archive Query MCP v0): Codex owner 검증(logical evidence ref: `post_development_review_gate_v0:20260819_slack_collector_recovery`, `claim_ceiling=validated_private`)을 통해 이전 batch writer-lease 장애는 정상 복구되어 scheduled read-only collector가 live 가동 중임을 확인(이 런타임 관찰은 현재 합성 쿼리 계층 diff와 독립적이며 private 경로·수치·시각·run ID 미주장). Chat 등 다운스트림 조회를 위한 Slack Archive Query MCP v0는 합성 fixture 기반 public-safe 구현·검증 완료(상태: `PUBLIC_SYNTHETIC_IMPLEMENTED / NOT_BOUND_TO_REAL_ARCHIVE / PARTIAL/HOLD`). 현재 live 수집 custody는 metadata/digest-only이며 custody→archive projector 및 본문 보존 정책 결정 전까지 live 본문 검색은 미제공 상태다. 실제 메시지 시각(`message_ts`) 기준 정렬, 수정/삭제 추적, thread 묶음, PARTIAL/HOLD fail-closed, exact binding scope check, 경로·secret·첨부 bytes 배제를 검증함. 2026-08-18 (08-18 밤 후속 2): P26-014 전 단계(SRR~PCA 7단계) 엔진 판단 1회씩 PASS(private run `ax_se_project_context_pilot_20260818_04`): 요구 104 = 충족 5/결손 4/불명 95, CDR 27(5/4/18) 27 정책 재실행 포함, 나머지 단계 관측 0 → 불명(관측 확대 backlog = 단계별 requirement 토큰). 2026-08-18 (public main, 08-18 밤 후속): 엔진 개발 매뉴얼 `guild_hall/engineering_engine/manual/`(9장) + ① 도출 기록 착지, ① 출처 정정(행 인용은 NPR·DoD SEG만, Handbook 미반영), 202행=셀 수/산출물 종류 100/must_have 124 명시. 2026-08-18 (public main, 08-18 밤): 규칙표 계층 분리(방사청 공통 131 + LIG 덧씌움 14) 및 ① 일반 SE 바닥층 착지(generic_se_base 229 task, NASA/DoD 근거) → 4층(일반SE·방사청·발주처·과제) 실체화. 2026-08-18 (public main, 08-18 저녁): R3 packet 생성기 착지·KVDS run 03(표준 25항목, 충족 5/결손 4/불명 16) PASS, 경로 길이 정책·정리 완료(260자 초과 4,463→2). 2026-08-18 (public main, 08-18 후반): 단계 규칙 원천 통일 설계+구현 착지(체계개발 스펙 v0.8·stage_rules 컴파일러·정본 13종 대조표), KVDS 120_CDR 첫 컴파일(엔진 요구 25, 규정 필수 누락 6 식별). 2026-08-18 (public `main@fe13cb40`): Owner 결정 D37·D38 확정, 요구사항 추적 R2 준비 조각(coverage input builder) 착지, KVDS 첫 커버리지 실측 1회 PASS(셀 충족 31/결손 95/미시도 44/미선언 52, private run `requirement_trace_coverage_pilot_20260818_01`, 아래 "맥락·세계수·할일" 행). 2026-08-17 (public `main@8d702252`): KVDS(P26-014) 첫 M2-2 Owner-frozen context pilot 1회 PASS(command)·domain UNKNOWN(satisfied 5/unknown 9, mission 후보 3, private run `ax_se_project_context_pilot_20260817_01`), 계약자료 텍스트 PDF 2건 admission+tracer 각 1회 PASS(private run `project_pdf_rag_pilot_20260816_01` 후속 `20260817_01`), ERP MCP feature-OFF 조회 확장 3종 착지(위 행). 아래 07-31 문장은 그 시점 관찰이다. 2026-07-31T08:55:04+09:00; `origin/main@ca63963421e3ac4792dd8aeb34dac518e3a9b5a4`에 AI 작업 결과 누락 복구 Phase G 운영 preflight 보강이 포함됐다. 상태는 `PHASE_G_PUBLIC_SYNTHETIC_ACCEPTED / FEATURE_OFF / LIVE_HOLD`다. runtime preflight v2는 active public·workmeta·private-state·automation control root와 complete Codex/Orca worktree inventory, backup/restore authority, immutable sanitized Git config evidence를 fail-closed로 결속한다. builder v2와 runner v4는 caller PASS summary와 forged/incomplete preflight를 거부하고 전체 evidence를 재검증하며, ledger publication 뒤 HOLD는 reconciliation 상태를 명시하고 cursor push 뒤 불명확 상태는 `cursor_unchanged=null`로 남긴다. fresh Level 3 및 최신 main 통합 재검증에서 runner 25/25·non-runner 42/42·path-policy 5 PASS/Windows symlink 1 SKIP·canon 136/0/0 PASS를 확인했다. 표시명 `AI 작업 결과 누락 복구 (매일)`의 tracked candidate는 `PAUSED`이며 `official_completion=false`, WorkSession/TaskDriver/ERP/MCP acceptance `false`, `claim_ceiling=operational_evidence_only`다. 설치 automation은 legacy `ACTIVE` 그대로이고 actual ledger coverage와 missing/generated/duplicate/conflict/HOLD 수는 `UNKNOWN`이다. seed ratification, single-writer host·epoch, runtime root, ACL·NAS·restore, private remote authority, one-shot 실행과 이후 `ACTIVE` 전환은 모두 사람 Owner 결정 전까지 `HOLD`다. actual private cursor·ledger writer·runtime·installed automation·network mutation은 `0`; `313`과 `317`은 실제 누락 수가 아니다. 이전 AI 작업 기록 A2 fresh Level 2+ `ACCEPT`는 manager second fresh `REVISE`로 supersede됐으며 Windows 최대 길이 ID·outside-temp sentinel·CURRENT truth 보정은 새 독립 manager 재검증 전까지 acceptance를 주장하지 않는다. 실제 HPP root·binding·writer·NAS policy/runtime은 만들거나 재관찰하지 않았으므로 아래 기존 private 수치는 2026-07-28 마지막 관찰값이며 날짜가 붙은 과거 행은 감사 이력이다 |
| 2026-08-11 local store validity monitoring | `PUBLIC_IMPLEMENTATION_VALIDATED / RUNTIME_EVIDENCE_SEPARATE`; continuous ingress와 Slack batch가 provider 성공과 분리해 기존 mail event file-set/tail, voice custody checkpoint·current/history file size·immutable receipt identity, Slack custody state/index를 exact `lane`+고정 `validation_scope`로 검증한 뒤 atomic sanitized receipt를 기록한다. Voice activity digest는 per-cycle timestamp가 아니라 검증된 custody identity만 비교하고, ingress store freshness는 관찰된 15분 poll+bounded validation duration에 맞춘 1,200초 period를 쓴다. Mail 전체 JSONL 및 Slack attachment/content byte 무결성은 주장하지 않는다. 새 입력이 없으면 healthy idle이며 collector 오류는 store 오류가 아니다. Watchtower의 세 store node만 독립 관측하고 provider·33개 data edge 전달은 계속 `UNKNOWN/HOLD / unreceipted`다. |
...
| 2026-08-06 회사메일 query-only MCP | `PUBLIC_IMPLEMENTATION_VALIDATED / PRIVATE_RUNTIME_STATE_SEPARATE`; central Hiworks event JSONL에서 exact configured mailbox ID만 읽는 stdio/loopback MCP를 구현했다. `status/search/read`만 제공하고 발송·삭제·읽음 변경·첨부 다운로드·raw/custody path 반환은 지원하지 않는다. stdio child environment와 tunnel child environment는 OS runtime allowlist 및 각 child의 최소 credential만 상속한다. 합성 mailbox isolation, malformed-row fail-closed, loopback guard, secret omission과 stdio tool discovery를 검증했다. 실제 계정, tunnel·mailbox ID, live 수집 수치, plugin 설치 및 연결 상태는 private runtime/evidence plane의 CURRENT이며 이 public 계획 문서는 이를 주장하지 않는다. public inbound listener와 mail mutation은 `0`; trusted local mailbox binding을 별도 운영 경계로 유지한다. |
| 2026-07-31 AI 작업 결과 누락 복구 Phase G | `PHASE_G_PUBLIC_SYNTHETIC_ACCEPTED / FEATURE_OFF / LIVE_HOLD`; public commit `ca63963421e3ac4792dd8aeb34dac518e3a9b5a4`가 `origin/main`에 포함됐다. mandatory automation-control root와 complete Codex/Orca worktree inventory, authoritative backup/restore surface, sanitized immutable Git config evidence를 fail-closed로 결속하고 builder/runner가 전체 preflight를 재검증한다. runner 25/25·non-runner 42/42·path-policy 5 PASS/Windows symlink 1 SKIP·canon 136/0/0을 통과했다. actual private ledger·cursor·runtime·ACL·NAS·installed automation·network mutation은 `0`; one-shot과 이후 `ACTIVE`는 사람 Owner 승인 전 `HOLD`다. |
| 2026-07-30 AI 작업 결과 누락 복구 candidate | `PHASE_E_PUBLIC_SYNTHETIC_ACCEPTED / FEATURE_OFF / LIVE_HOLD`; public commit `57e93fb3dbabfa4fa4e24bfe062c6e4d6195c75e`가 `origin/main`에 포함됐다. metadata projector는 승인된 집계·digest·public commit ref·시각·분류·완전성만 출력하고 시간 역전을 HOLD한다. local/HTTPS/SSH transport는 logical remote/ref와 authority fingerprint로 결합하며 raw URL·credential·대화형 인증과 실패 출력 누출을 차단한다. isolated runtime preflight는 active/forbidden-root 비중첩, ACL·NAS·restore·single-writer fencing 증거를 fail-closed로 확인하고, builder는 전체 preflight를 다시 검증한 뒤 표시명 `AI 작업 결과 누락 복구 (매일)`의 deterministic `PAUSED` candidate와 byte-exact rollback만 만든다. `official_completion=false`, WorkSession/TaskDriver/ERP/MCP acceptance `false`, `claim_ceiling=operational_evidence_only`를 유지하며 ledger push·fresh inclusion 뒤에만 cursor CAS가 가능하다. fresh Level 3 contract/planner 41/41·runner 19/19, manager path/canon 검증을 통과했다. 설치 automation은 legacy `ACTIVE` 그대로이고 operational completion은 `LIVE_HOLD`다. actual ledger coverage와 missing/generated/duplicate/conflict/HOLD 수는 `UNKNOWN`; seed ratification, single-writer host·epoch, runtime root, ACL·NAS·restore, private remote authority, one-shot과 이후 `ACTIVE`는 사람 Owner 결정이 필요하다. actual private/runtime/install/network mutation은 `0`; `313`과 `317`은 실제 누락 수가 아니다. Phase B/C 항목은 감사 이력으로 보존하며 기존 `five_field_session_capture_v0`, `five_field_log.jsonl`, `bounded_work` 내부 호환 이름은 유지한다. |
| 2026-07-30 AI 작업 기록 A2 local outbox candidate | `PUBLIC_SYNTHETIC / FEATURE_OFF`; A1 `soulforge.ai_work_record_event.v1`을 변경 없이 소비해 합성 임시 root에서 lifecycle, append-only publish, replay/conflict, fencing, pending·ack·correction, `local_persisted`를 검증한다. actual HPP physical root·private binding·NAS policy/runtime·hook·scheduler·network·ERP/MCP activation `0`; live NAS include/restore와 운영 writer는 별도 gate까지 HOLD다. |
...
| 2026-07-27 Codex 활동 데이터 명칭 보정 | `HISTORICAL_SUPERSEDED`; 당시 five-field proxy를 `Codex 업무 결과 요약`, H05 data class를 `Codex 실행·검증 증거`로 구분한 기록이다. 현재 owner-facing 용어는 아래 CURRENT 상태표와 공통 용어집을 따른다. 기존 `bounded_work`·`run_log`·H03/H05 내부 ID와 machine-local 경로는 무중단 호환을 위해 rename하지 않는다. |
| 2026-07-27 HPP Codex 작업 맥락 사건 기반 | `HISTORICAL_SUPERSEDED`; 당시의 혼합 표시 이름으로 남긴 구현 기록이다. exact private project allowlist 아래 서로 다른 실제 업무를 분리하고, 같은 업무를 이어가는 프로젝트 팀장 작업·자식·계속·검증 작업만 같은 ID에 명시적으로 연결하는 append-only HPP writer와 CLI를 구현했다. 시작·중간 checkpoint·완료 요약과 source/file/run ref만 저장하고 whole conversation·screen·keyboard·OS는 자동 수집하지 않는다. close는 ERP 공식 완료·H03 WorkSession·H05 exact run receipt·accepted context가 아니다. |
...
| 받은메일·owner 보낸메일 | `PRIVATE_RUNTIME_TRUTH / LOCAL_QUERY_MCP_IMPLEMENTED / PARTIAL_PROJECT_ROUTE` | public 구현은 exact configured mailbox ID 선필터 뒤 `status/search/read`만 제공하며 합성 isolation, malformed-row fail-closed, loopback guard와 stdio discovery를 통과했다. live 계정·event 수·최신 수신시각·connector 등록 상태는 private runtime/evidence plane에서만 확인한다. owner 보낸편지함은 별도 source다. | 전체 프로젝트 동일 writer 투영, 재분류 parity, H01 acceptance, trusted local mailbox binding, 첨부 bytes 조회 경계 |
...
| Slack 대화·첨부 | `LIVE_READ_ONLY_COLLECTION / SCHEDULED_COLLECTOR_LIVE_VALIDATED_PRIVATE / PUBLIC_SYNTHETIC_QUERY_MCP_IMPLEMENTED / NOT_BOUND_TO_REAL_ARCHIVE / PARTIAL/HOLD` | Codex owner 검증(logical evidence ref: `post_development_review_gate_v0:20260819_slack_collector_recovery`, `claim_ceiling=validated_private`)을 통해 이전 batch writer-lease 장애는 정상 복구되었고 scheduled read-only collector는 live 가동 중임을 확인했다(이 런타임 관찰은 현재의 합성 쿼리 계층 diff와 독립적이며, private 경로·수집량·시각·run ID는 미주장). 이번 변경은 Chat 등 다운스트림 조회를 위한 최초 Slack Archive Query 순수 읽기 모델·색인기·stdio MCP adapter v0(status/search/thread/timeline/attachment)를 public-safe하게 구현·합성 검증했다. 현재 live 수집 custody는 metadata/digest-only이며 custody→archive projector 결정 전까지 live 본문 검색은 미제공 상태다. 실제 Slack 메시지 시각(message_ts) 기준 정렬, 수정/삭제 추적, thread 묶음, PARTIAL/HOLD 보존, exact binding scope check, 경로·secret·첨부 bytes 배제를 검증했다 | backup 축적 형태와 Slack Archive MCP 준비도는 `PARTIAL/HOLD`다. thread reply·delete coverage는 현재 read-only polling 계약으로는 구조적으로 불가하며, custody→archive projector, source-arrival+attachment ref의 프로젝트 시간장부 자동 투영과 H07 acceptance는 계속 별도다 |
| 프로젝트 파일 관찰 | `LIVE_LOCAL_DELTA_OUTBOX / PARTIAL_HASH` | 14개 project의 compact inventory 29,326행, exact hash 14,132개, hash 대기 15,194개. 구방식 격리 뒤 최신 실제 실행은 변경 5건/무변경 29,321건, 최신 delta 합계 27,152 bytes, 결과 0, held 0, stale lock 0, 구경로 재생성 0이었다. 중복 구방식 full packet 347개/1,179,013,521 bytes는 독립 실행 검증 후 삭제됐다 | sole reconciler가 `파일 관찰`을 first-observed/revision/rename/copy/touch `파일 이력`으로 확정. `tool_pc` absence candidate만으로 delete 확정 금지 |
| AI 작업 결과 (임시 proxy) | `LIVE_BOUNDED_SUMMARY_OUTBOX / RECOVERY_FEATURE_OFF` | 마지막 private 관찰값은 결과 351건, 같은 occurrence를 가리키는 Codex-origin relation 349건, conflict 0이며 이번 public slice에서 재관찰하지 않았다. 내부 호환 ID는 `bounded_work`; public candidate는 source/record clock 분리와 full-record conflict HOLD를 합성 검증함 | private cursor owner/path와 isolated runner 승인, 정식 WorkSession/MCP receipt 연결. whole chat·screen·keyboard·OS 수집은 계속 OFF이며 이 요약을 실제 PC 활동이나 실행·검증 증거로 과대 해석하지 않음 |
| AI 작업 기록 A2 local outbox | `PUBLIC_SYNTHETIC_CANDIDATE / FEATURE_OFF` | 선택된 dedicated logical root와 A1 lifecycle·local persistence를 합성 임시 root에서만 검증한다. `local_persisted`는 로컬 저장 확인이며 `official_completion=false`; 실제 HPP·private·network·ERP 관찰값은 `0` | ingress/backup fixed policy와 protected pin의 별도 승인·synthetic restore acceptance, actual binding/writer 배치, controlled-path activation. 미지원 bypass는 `unobserved` |
| HPP Codex 작업 맥락 수집기 → HPP 로컬 업무 장부 | `LIVE_HPP_LOCAL_EVENT_OUTBOX / LEADER_GUIDANCE_SENT` | exact private 14-project allowlist 고정. 현재 활동 팀장 로컬 업무 10개를 11개 project code에 등록했고 P25-000/P26-038은 같은 팀장 로컬 업무를 공유한다. KVDS 제한 canary 정본 `LW-P26-014-CANARY-20260727-005`는 KST 업무 사건 3개·attached thread 1개·파일 SHA-256 ref 1개·검증 25/25 ref 1개·whole-chat 자동 수집 0으로 완료했다. 이전 canary 4건은 덮어쓰거나 삭제하지 않고 `superseded` 감사 이력으로 보존했다. 공개 `21fdad4d`와 같은 3개 실행 파일을 HPP D: runtime에 배치·hash 일치·status 조회 검증했고 등록된 팀장 로컬 업무 10/10에 다음 실제 업무부터 적용할 start/attach/checkpoint/finish/supersede 지침을 전달했다 | 팀장 ACK와 첫 실제 업무 적용을 관찰한다. ERP/WorkSession/H05/프로젝트 시간장부 promotion은 별도 reconciler 전까지 OFF |
| 실행·검증 증거 | `PILOT_ONLY / COMMON_RECEIPT_MISSING` | 중앙 ingress의 legacy `run_log` 파일 7개는 과거 pilot. project `runs/**`는 형식이 섞여 있어 전체 재귀 수집하지 않음 | 일반 Codex 작업용 exact 실행·검증 영수증/manifest와 H05 adapter |
| 프로젝트 시간장부 | `KVDS_V3_SHADOW_ONLY` | `P26-014` 269행 = mail 84 + voice 3 + `AI 작업 결과` proxy 182, 전부 KST. 최신 local proxy 200건보다 18건 뒤처짐 | KVDS에 Slack·reconciled 파일 이력 event·`실행·검증 증거`를 합친 V4, 이후 같은 writer를 다른 프로젝트에 확장 |
...
| Candidate Execution Coordinator foundation | `PUBLIC_SYNTHETIC / DEFAULT_OFF / ONE_SHOT_LIVE_CANARY_GREEN / NOT_OPERATIONAL` | 기존 Core/SQLite와 분리된 RoleCapabilityMatcher→`responsible_ceo_triage` AssignmentPolicy→Coordinator와 기본 OFF Hermes bot-submit Executor 구조다. `AI 실행 후보`는 prefilter only이고 exact actor→agent→Bot→Executor binding, nested custody fingerprint, per-agent slot, Waiting/HOLD release, successor/replay/fencing, execute-entry deep snapshot, stdin-only command, metadata-only attribution/effect evidence를 Coordinator 21/21·Executor 16/16로 검증했다. Owner 승인 합성 `SON-59` 1건은 Codex Linear connector→exact 제품 총괄 Bot/session→기본 runner를 통과했고 marker digest 일치, same-process replay `NO_OP`, 4192 matched 1·`working→idle`, Official Done/mutation false를 관찰했다 | Connector와 label은 authority가 아니며 이 one-shot은 live collector나 production route를 켜지 않는다. persistent multi-process replay/ledger, scheduler, writer cutover, automatic assignment, live organization source, 실업무 Work Brief는 `HOLD`; Windows descendant process-tree kill은 미확정이고 timeout은 `UNKNOWN`이다 |
| Candidate Execution authority admission | `PUBLIC_SYNTHETIC / DEFAULT_OFF / ADMISSION_ONLY / NOT_OPERATIONAL` | coordinator-native candidate/task/assignment/match packet을 별도 current verified Agent binding, assignment/authority/tool epoch, role/capability, Bot/profile/session/deployment, requested/observed model·effort와 exact 결속한다. focused 6/6, 관련 coordinator/core/executor 52/52; 성공은 frozen admission receipt이며 execute/claim/Linear mutation/Agent creation/Official Done은 0 | Forge 문자열 task_ref·brief_id를 coordinator-native structured TaskRef/WorkBriefRevisionRef로 바꾸는 별도 Forge/Linear admission seam, durable coordinator ledger, actual KVDS binding/runtime은 HOLD |
| Chat 1-hour Shadow Bot | `V0_4_1_PROJECT_CONTEXT_GATE_CONFIGURED / NEXT_RUN_READBACK_PENDING / PROVIDER_READBACK_NOT_INDEPENDENTLY_VERIFIED` | 기존 `업무 인입 감시` ID·매시간·활성 상태를 유지한 채 project scope, `SONATECH_DEV1_PROJECT_INSTRUCTIONS_V1_0_20260820`, `project_memory_inherited=DO_NOT_ASSUME`, `project_context_gate=FAIL_CLOSED`를 task prompt에 직접 저장했다. project context가 missing/stale/conflict/unclear이면 모든 외부 mutation을 0으로 두고 `HOLD_PROJECT_CONTEXT`한다. 새 origin/receipt/output은 `V041`이고 과거 `V040`도 application echo로 억제한다. v0.4.0 first receipt는 Linear 5건(신규 4·갱신 1), 다른 앱 0, `gmail_sent=0`을 보고했다 | first v0.4.1 Cycle Receipt에서 project fields와 HOLD effect=0을 검증하고 Bot 자기보고를 실제 provider/app readback과 대조한다. exact prompt digest·permission snapshot·prohibited-action 0·Gmail send 0은 계속 Owner/live gate |
...
| P5 authentic-producer context candidate + acceptance/query | `PUBLIC_SYNTHETIC / FEATURE_OFF / ACCEPTANCE_AND_QUERY_FOUNDATION_DONE / ACTUAL_ACCEPTANCE_HOLD` | P4/M2/timeline candidate에 exact review-content digest를 추가하고, registered-human reviewed membership set·coverage·supersession·writer epoch/current-pointer CAS를 검증하는 in-memory append-only acceptance manifest/receipt와 mandatory-ACL generation-pinned query를 구현했다. hostile wrong-generation manifest/receipt도 요청 generation과 exact fence한다. candidate 12/12, acceptance/query 31/31, readiness 34/34 | actual P4/M2/timeline input, registered reviewer identity evidence, HPP fenced writer/private persistence, generation 1개 human acceptance와 live query binding. current store advance는 synthetic-only이고 `writer_called=false`, ERP/P6 effect 0 |
| Linear LB1 backup foundation + Owner Gate | `PUBLIC_SYNTHETIC / FEATURE_OFF / RUNTIME_ADAPTER_FOUNDATION_DONE / ACTUAL_LB1_HOLD` | offline contract·Gate에 capability-allowlisted synthetic-only runtime reader/storage/claim binding을 추가했다. scope/target/authority/clock/epoch/expiry가 immutable binding이며 client return은 clone·sanitize된다. runtime adapters 22/22, Backup Controller 136/136 | Owner-approved credential/workspace/target/writer/reviewer refs와 authorized actual clients를 bind한 one-shot, stored-byte readback, human restore review. provider/network/scheduler/Task/P5 effect는 실제 실행 전 0/HOLD |
| Linear LB1 v2 Bound Runner | `PUBLIC_SYNTHETIC / FEATURE_OFF / RESULT_V3_EFFECT_EVIDENCE / STORED_BYTE_RESTORE_CANDIDATE` | v2 18개 복원 차원과 Gate full pin을 유지하고 runner result v3가 adapter getEffects baseline/delta를 authorization 뒤 수집한다. missing evidence는 `UNKNOWN`, malformed/counter mismatch와 역행 counter는 `HOLD`, exact synthetic-only attestation일 때만 zero를 emit한다. main LB1 51/51, runner 27/27 | actual one-shot·human restore acceptance는 여전히 0. zero effect는 adapter self-attestation+runner counter reconciliation claim ceiling이며 real provider behavior 증명이 아님 |
| Hermes proposal-runtime trial gate | `PUBLIC_SYNTHETIC / PROPOSAL_ONLY / INSTALL_HOLD` | exact immutable version/host/isolation digests, one-seat/account/project mapping, read/query/candidate tool allowlist, forbidden-tool declaration, delivery idempotency, memory/transcript/attachment custody, rollback/time window를 pure gate로 검증한다. proposal authority/effects는 false/0. focused Hermes 10/10, fresh Opus 5 `ACCEPT` | actual Hermes install/doctor, credential, MCP registration, delivery adapter call, scheduler/team rollout은 D31 Owner packet+Level 3 전 0/HOLD |
| Bounded mutation canary gate | `PUBLIC_SYNTHETIC / TRUSTED_PIN_CONSISTENCY_ONLY / ACTUAL_CANARY_HOLD` | exact `Project×TaskType×Action×Authority×PolicyRevision`, external trusted Approval/C5 pins, per-window tuple claim, sole writer/CAS/fencing, tuple-bound digest readback, non-destructive voided/superseded compensation와 terminal replay를 synthetic-only로 검증한다. canary 17/17, fresh Opus 5 `ACCEPT` | actual C5 quality/NO_ACTION evidence, exact first tuple/Owner approval, real sole writer/adapter/readback/rollback receipt 전 `actual_canary_readiness=false`; broad v0.4 permission은 canary receipt 대체 불가 |
| Three-worker WorkUnit comparison | `PUBLIC_SYNTHETIC / EVIDENCE_COMPARISON_ONLY / ACTUAL_COHORT_HOLD` | Codex·Gemini Flash·Grok Build 각각 1개 run을 exact Work Unit/input/constraint/completion/validator/policy/harness basis와 run-bound validator·external-review evidence로 결속한다. complete measurement와 causal timestamp만 비교하고 어떤 dimension tie도 `NO_SELECTION`; authority/deploy false. comparison 14/14 + evidence 2/2, fresh Opus 5 `ACCEPT` | actual three-provider runs/version/worktree binding·repeated sample/variance·transferable winner 판단은 별도 Owner packet과 fresh review 전 0/HOLD |
| 맥락·세계수·할일 | `M2_2_RUN03_FROM_COMPILED_STAGE_RULES_EXECUTED + R3_PACKET_GENERATOR_LANDED / OBSERVED / NOT_OPERATIONAL` | 2026-08-14 M2-2 public-synthetic 후보는 fresh Level 3 B/V+Claude 독립검토로 수락됐고(private receipt), 2026-08-17 KVDS Owner-frozen packet(120_CDR 14 requirement·관측 5·논리 역할 6·common 1)으로 첫 실제 pilot을 1회 실행해 command PASS·assessment UNKNOWN(satisfied 5/unknown 9)·mission 후보 3(acquire_requirement_evidence)·역할 후보를 얻었다(zero-write, effects 0). 같은 날 run 02는 03_Out 직접 확인으로 absence_confirmed 4건(HRS·SRS·HDD 최종본, STP)을 더해 satisfied 5/missing 4/unknown 5, mission 후보 3(CDRL 취합·3D STEP·발표자료 취합)을 냈고(assessment 여전히 UNKNOWN), P4 첫 조각 요구사항 ID 색인 seam(`guild_hall/rag/project_pdf_requirement_index.mjs`, profile v0/v0.1)이 pinned 요구사양서에서 118개 ID·33쪽·TBC 19·TBD 9를 색인했다(private receipt; 두 실행 모두 2026-08-18 Owner ratify). 2026-08-18 Owner가 D37(자동 추출 요구 ID = candidate만)·D38(Needs = 기존 stage_expected_artifact_policy 확장)을 확정했고, 요구사항 추적 R2 준비 조각 coverage input builder(`guild_hall/requirement_trace/coverage_input_builder.mjs`, `main@fe13cb40`)로 KVDS 첫 커버리지 실측을 1회 PASS했다: 118행 → 108 admitted / 보류 10(중복 4쌍·템플릿 예시·잘린 ID), Needs 후보 정책 14건(PETB만) → 셀 충족 31 / 결손 95(HRS·HDD·SRS·STP 부재) / 미시도 44(시험문서 슬롯 미관측) / Needs 미선언 52(SIS·DIV·VTE·ETR·OSS), 요구 단위 missing 12 / unknown 96(unknown 우선), 파일 존재=충족 semantics(Owner 08-17 발언 근거, inconclusive 변형 병기), claim observed·원장 writer 없음(private run `requirement_trace_coverage_pilot_20260818_01`). 같은 날 Owner 질문("KVDS만 끼워맞추는 게 아니라 공용")에 따라 단계 규칙 원천을 폴더트리 variant 스펙 하나로 통일하는 설계(`SE_STAGE_RULE_SOURCE_MODEL_V0.md`)와 구현(체계개발 스펙 v0.8 기계 필드+필수 17건, `stage_rules/` 컴파일러)을 착지했고, 방사청·국방부 정본 13종 대조표(`se_foldertree_generate/codex/references/source_verification_v0.md`)로 체계개발 spine은 부합·탐색/선행 기본형은 재기준 대상임을 확인했다. KVDS 120_CDR 첫 컴파일: 발주처 14슬롯 = 표준 5·강화 2·추가 7, 발주처 미요청 표준 11(규정 필수 6: ICD·사업중간점검·MRA·RAM 분석·핵심부품 성적서·CDR 회의록) → 엔진 요구 25(private run `se_stage_rules_kvds_120_cdr_20260818_01`). 이어 R3 packet 생성기(`stage_rules/pilot_packet_generator.mjs`)로 packet을 재생성해 runner 1회(run 03) PASS: 25 요구 = 충족 5 / 결손 4 / 불명 16, mission 후보 3(핵심부품 성적서·DBDD·ICD) — 엔진이 처음으로 표준 규칙 기반으로 판단(private run `ax_se_project_context_pilot_20260818_03`). M2-0 TARGET과 M2-1 exact-one-project/root admission 뒤, M2-2 후보가 별도 expected pilot grant, complete project-source reference manifest, explicit common→policy bindings, role-roster pin과 root-bound canonical packet을 한 zero-write command로 결속한다. command는 launch와 canonical packet만 읽고 project source-body는 읽지 않으며, 기존 deterministic AX·SE subject를 한 번 호출한다. RAG/Wiki/LLM call과 ERP/TaskDriver effect·authority는 0이고 source truth·freshness·actual-project approval은 여전히 증명하지 않는다 | Owner: D42~D45(규칙 원천 모델)·D37 후속(Needs 정책 후보 확인)·D40(중복 ID 4쌍)·시험문서 슬롯 관측; 다음 조각 = 표준 25항목에 대한 관측 확대(시험문서·ICD·사업중간점검·MRA·RAM·핵심부품 성적서 슬롯) → R2 원장 writer → R4 카드·ERP 뷰, 남은 unknown 5(STR·3D STEP·시험문서·CDRL·발표자료 취합) 관측, 이후 P4/M2-3 project-local RAG+thin Wiki(첫 조각: 요구사항 ID 색인 seam)·P5 accepted context generation·P6 TaskIntent 순(로드맵 순서), 이후 P5 context resolver·project-local RAG/thin Wiki·ERP accepted generation·TaskDriver 승격 |
...
| --- | --- | --- | --- | --- | --- |
| `C0` mutation default-OFF guard | `DONE_FOR_SYNTHETIC_SCOPE`: launcher의 4300 기반 auto-intake/autosync 암묵 기본값 제거; explicit opt-in 보존; actual Task 실행 route activation·write receipt 0 | 유지 회귀만; VF-2/3 public-synthetic foundation과 독립 | 합성 fixture만; live restart·외부 mutation 0 | RED 1/2→focused 2/2, launcher 13/13, core 290/290, fresh independent review `ACCEPT` | 완료 |
...
| `C2` actual Linear LB1 | `RUNTIME_ADAPTER_FOUNDATION_DONE_FOR_SYNTHETIC_SCOPE`: exact-scope reader/create-only storage/durable claim + runner result v3 effect evidence; 역행 counter는 attestation 없이 HOLD, actual clients/one-shot 0 | Owner-bound actual clients·credential/target/reviewer refs와 hourly writer quiescence 또는 non-quiesced snapshot 수락 결정을 고정한 one-shot 후 human restore review; PASS 뒤 `LB2` scheduler 별도 승인 | 현재 합성 only; actual 단계는 Linear read+승인 target create-only write | adapter 22/22, LB1 51/51, Backup Controller 136/136; actual manifest/stored-byte/human restore 미생성 | foundation 완료; actual `4~7일` |
| `C3` decision ledger | `DONE_FOR_PUBLIC_SYNTHETIC_SCOPE`: required-source/A0 Cycle Contract + branded in-memory append-only ledger + typed Portfolio Projection + live-only ShadowEvaluator 53/53; persistent writer·actual task binding 0 | `HB-D1` exact Chat task snapshot/prompt freeze, `HB-D2` private owner path·writer epoch·retention binding | 순수 memory/합성 receipt만; Scheduled Task·private ledger·외부 effect 0 | replay/NO_OP/correction/digest chain/project isolation/ground-truth separation PASS; fresh Opus 5 `ACCEPT` | foundation 완료; actual binding 별도 |
| `C4` actual P5 | `ACCEPTANCE_FOUNDATION_DONE_FOR_SYNTHETIC_SCOPE`: candidate content/review/coverage/supersession binding + append-only in-memory manifest/receipt/current-pointer CAS. actual writer/persistence 0 | actual P4/M2/timeline+registered reviewer+HPP fenced writer/epoch로 generation 1개 human acceptance | synthetic store only; actual accepted-context sole-writer만, ERP/P6 0 | acceptance/query 31/31 + candidate 12/12; actual generation receipt 미생성 | foundation 완료; actual `2~4일`, `C1` 뒤 |
| `C5` proposal shadow | C3 live-only evaluator + mandatory-ACL/no-fallback/uniform-no-leak Accepted Context Query synthetic foundation 구현; wrong-generation manifest/receipt substitution도 uniform `NOT_AVAILABLE`, actual accepted generation·corpus 없음 | actual generation current pointer/manifest/receipt binding + Reactive/SE proposal-only route와 실제 품질 관찰 | Official Task/Linear mutation 0 | current query foundation 31/31; live metric은 actual `C3+C4+HB-D5` 뒤 | `1~2주`, actual `C3+C4` 뒤 |
| `C6` mutation canary | `GATE_FOUNDATION_DONE_FOR_SYNTHETIC_SCOPE`: trusted approval/C5 pins, exact tuple, atomic per-window claim, sole coordinator/writer/CAS/fencing, digest readback, non-destructive compensation terminal replay 구현 | actual C5 evidence와 별도 Owner 승인 exact tuple/real adapter로 first one-action canary. tuple이 v0.4.0 hourly write surface 밖이거나 동일 sole-writer fence 아래임을 증명 | 현재 synthetic only; actual canary는 별도 Owner가 승인한 exact mutation 1건만 | canary 17/17, actual_canary_readiness=false, live effect 0 | foundation 완료; actual `1~2주`, `C0~C5` 뒤 |
...
- 실제 관찰: 기존 Chat Scheduled Task의 v0.4.0 first Cycle Receipt는 Linear effect 5·다른 앱 0·`gmail_sent=0`을 보고했지만 provider/app readback은 아직 독립 검증되지 않았다. 결과는 일반 채팅에 생성되므로 기존 예약에 v0.4.1 project-instruction fail-closed gate를 직접 저장했고 first v0.4.1 readback을 기다린다.
- repo 검증: P4 Preparation 18/18·Runner 13/13, Linear LB1 runtime 22/22·v2 51/51,
  P5 candidate 12/12, Task Core POC 28/28, C3 synthetic shadow 53/53 PASS. C3 persistent
  Decision Ledger는 아직 없고 C5 Accepted Context Query foundation은 존재한다.
...
| H05 run/log | `_workmeta/<project>/runs/**`는 위치 owner지만 모든 run에 공통인 manifest schema가 아니다. `soulforge.workflow_receipt.v1`은 현재 `workflow_id: report_authoring_v0` 전용이다. AI 작업 결과 writer/cursor candidate는 canonical full-record digest와 same-ID conflict HOLD, source/record clock 분리를 구현했지만 전용 raw/secret boundary validator와 H05 lane acceptance는 없다. | 승인된 exact schema+explicit ref allowlist만 입력으로 받고 run directory recursion, raw/stage log, transcript, arbitrary manifest를 거부한다. 최초 eligible 후보는 exact schema validation과 `job_id` uniqueness/immutability를 증명한 report-authoring receipt뿐이다. AI 작업 결과는 boundary validator·D26 lane owner 승인 전 native occurrence allowlist에서 제외한다. daily ledger/context는 어느 lane에서도 native occurrence가 아니며 H01~H05 원 source를 가리키는 derived relation ref로만 쓴다. |
...
| run/log / five-field | `TBD` | `five_field_session_capture_v0` candidate | **capture identity는 relation-only/ineligible** | full-record digest와 same-ID conflict HOLD는 public synthetic candidate로 구현됨. raw/secret boundary validator와 D26 source-to-lane owner 승인을 마치기 전 native occurrence 산입 금지; `session_ref`는 grouping ref |
...
| --- | --- | --- |
| P5A-0 source scope/timeline | six-lane annotation→append-only scope binding→minimal system receipt+isolated project timeline projection | public pure contract/synthetic only; no RAW open, project folder writer, LLM prompt, DB/service |
...
| `ui-workspace/apps/dev-erp/src/task_execution_{core,sqlite_event_store,fixture_adapters}.mjs` | Linear Official Task read-only eligibility, AgentRun claim/state, append-only Task/Run event, Waiting/terminal receipt 합성 POC | synthetic test only | POC docs/tests | 2026-08-20 public-synthetic/feature-OFF 착지; TaskDriver/P8 apply, live DB, Linear/ERP writer·MCP·scheduler authority 0 |
...
한 요청은 정확히 한 project grant를 고정한 뒤 필요한 common collection revisions를 명시적으로
추가한 Knowledge View를 사용한다. 아래 `scope=project|common`은 각 hit의 provenance를 뜻하며,
project와 common 중 하나만 보라는 뜻이 아니다. common inclusion은 explicit allowlist이고 fallback이
아니다. project가 없거나 둘 이상이면 retrieval 전에 `HOLD`한다.
...
#### Linear backup and live-task split lanes (core phase non-unlocking)

Linear 백업과 Linear 업무 실행은 서로 다른 개발선이다. 현재는 backup scope/model review,
feature-OFF offline LB1 manifest/restore contract, 합성 Task Execution Core POC와 별도의
feature-OFF in-memory CandidateExecutionCoordinator foundation이 존재한다. Coordinator GREEN은
`LT1/LT2`나 기존 P5→P8 순서를 unlock하지 않는다.
synthetic-only runtime adapter foundation과 runner result v3는 존재하지만, authorized actual
Linear/Drive clients, actual one-shot/human restore, webhook, scheduler, live `LinearTaskProvider`,
live candidate/Role/Capability source, Hermes Executor/dispatch, persistent coordinator ledger,
4192 result projection과 Linear writer는 모두 `0/HOLD`다.

| 단계 | 시작 Gate | 허용 범위 | 금지·비해제 |
| --- | --- | --- | --- |
| `LB0` 범위 설계 | 완료 | snapshot, Comment/history, Waiting, Completion, Evidence, restore coverage 모델 | 백업 기능 완료 주장 금지 |
| `LB1` one-shot read-only pilot | `linear_lb1_owner_gate` READY: exact Owner decision + backup target/storage-write authority + 최소 read-only Linear credential와 workspace/team/project scope + retention/RPO/partial-failure/human restore-acceptance + trusted packet pin | bounded snapshot/API read, 승인된 private target에 새 revision 저장, manifest/hash/coverage, restore check | Linear mutation, webhook 등록, scheduler, overwrite, P4~P8 unlock 금지 |
| `LB2` 예약 백업 | LB1 실제·합성 restore PASS + single-writer·partial/failure reconciliation + 별도 Owner activation | append-only snapshot/API reconciliation; webhook은 별도 승인된 경우만 | Sheets 단독 백업, partial 성공 처리, 삭제·미러링 금지 |
| `LT1` 실제 Task read pilot | `LB1` PASS + Work Brief immutable revision 규칙 + TaskRef scope + read-only binding 승인 | allowlist Task 1건의 `readTask()`와 Gate 입력 검증 | dispatch, claim, Executor, AgentRun, status mutation 금지 |
| `LT2` 실제 실행·write | accepted P5→P6→P7→P8 receipts, P9 one-project canary/rollback, P10 capability별 승인 | 승인된 단일 writer와 bounded AgentRun | Candidate 실행, Agent success=Done, Linear/dev-ERP 이중 writer 금지 |

`LB1/LB2`는 P4 지식 vertical과 병렬로 진행할 수 있지만 core phase acceptance가 아니며,
백업 완료를 Task 실행 준비 완료로 해석하지 않는다.
...
| `H05` | run/log history envelope / `H00` | exact-schema report-authoring workflow receipt refs → run occurrence+coverage adapter; AI 작업 결과는 digest/conflict contract가 있어도 boundary·lane 승인 전 relation-only, `runs/**` recursion 없음 | existing run/five-field public workflow modules, tests, READMEs, CHANGELOG | missing/partial run, report-authoring schema+`job_id` uniqueness/immutability, AI 작업 결과 same-ID collision reject, validator ref, H03/H05 lane uniqueness, raw/stage log exclusion; AgentRun not required | adapter OFF+revert / stop on unknown schema, unresolved five-field lane authority, directory scan, or raw artifact need / run owner; D26 exact run/log mapping 전 adapter 금지, D25 run/log coverage policy 전 lane acceptance 금지 |
...
current_evidence_refs:
  - task_engine_inventory.mjs(+test)는 C00Q formal receipt와 함께 main에 tracked(2026-08-17 확인); task_engine_migration_dry_run.mjs는 main/candidate에 없음
...
| HP-LABEL-02 | common clock semantics | source-native fact clock+native ref+basis+precision→`event_at`, 관측→`observed_at`, 인지→`known_at`, append→`recorded_at`, cutoff→`valid_at` lossless crosswalk. 공통 semantic/timeline label의 실제 저장 `occurred_at`은 전 lane `Asia/Seoul` `+09:00`; UTC `Z` 또는 다른 offset 저장 `0`. Offset 없는 PLAUD provider 절대시각의 owner-ratified UTC 해석만 허용한 뒤 KST로 저장하고 `start_seconds/end_seconds` 상대시간은 무변환 보존; host timezone 추정·single timestamp collapse `0` |
...
| HP-QUERY-17 | Knowledge View cardinality | exact project 0개 또는 2개 이상이면 retrieval/model call `0`; one project + explicit allowed common revisions만 허용 |
| HP-QUERY-18 | physical project isolation | foreign-project root/index/Wiki/context/run enumeration·read·hit·count·error/cache-key leak `0`; common source byte의 project-root 복제 `0` |
| HP-QUERY-19 | global catalog/body split | global view의 body·chunk·Wiki prose·run payload·private locator `0`; catalog grant로 body retrieval `0` |
...
| `D26-FX-09` | current five-field 두 record가 같은 current `id`지만 omitted field가 다름 | canonical full-record digest 차이로 HOLD; record는 relation-only/ineligible 상태 유지 | boundary·lane 승인 없이 H05 occurrence나 completeness에 산입 | H05/D26 |
...
| D29 | ERP UI/MCP primary query·accepted generation·Knowledge View ACL/no-fallback·team knowledge candidate authority | owner가 ERP UI/MCP primary read, files=audit snapshot, exact project 하나 + explicit approved common revisions, no implicit fallback, team candidate-only를 plan default로 채택했다. `project|common`은 hit provenance이며 배타적 query mode가 아니다. M2-1은 external expected grant와 strict descendant/non-overlap root metadata를 검증하고 권한을 false로 유지한다. M2-2 public-synthetic 후보는 이를 승격하지 않고 별도 pilot grant가 project-source manifest, common→policy bindings, roster pin과 root-bound canonical packet을 결속한 zero-write Engine call만 허용한다. source body membership·truth·freshness와 actual-project approval은 미검증이며 local path commitment를 durable/public authority로 쓰지 않는다. actual launch authority, accepted-generation owner, cursor/retention, candidate reviewer/approver/writer와 project ID crosswalk는 후속 확정이 필요하다 | Q0~Q10/P4/P5/P8/P9/P10 | schema/read-only/synthetic / accepted pointer advance·live endpoint·foreign enumeration·implicit fallback·local path commitment의 durable/public authority 사용·direct Wiki/RAG/canon/task write 금지 |
...
| `D36` / P5A-0·P5A 전 | six-lane source annotation, scope-binding lineage, confirmed project isolation, candidate/unassigned/common/restricted/conflict routing, exact SourceSpan/Event/Unit/membership/summary-revision/memory-candidate schemas, HPP timeline/context-writer identity/epoch, ERP projection manifest/parity/rollback owner, MCP/plugin proposal/query boundary | existing source annotations and v0 CSV/Markdown remain compatibility state; minimal system receipt is audit-only and never a cross-project LLM prompt; new private projection paths and ERP read model OFF | public contract, six-lane isolation/reclassification/stale-binding/replay/raw-negative/cross-project-leak fixtures; authentic-producer pre-acceptance candidate 10/10와 complete-request external-pin 회귀 | private folder materialization, live source read, DB migration, writer/MCP/plugin activation, bulk backfill | P5A-0 pure contract와 candidate builder PASS는 exact actual packet·registered-human review planning만 연다. accepted generation/P5B는 별도 acceptance receipt, P5C/P5D는 각각 별도 receipts 필요 |
...
| run/log common source schema and lane assignment | `runs/**`는 위치 owner일 뿐 공통 manifest가 아니고 daily-ledger/context projection을 source truth로 오인하면 H03/H05 중복이 생긴다. AI 작업 결과의 full-record conflict contract도 H03/H05 lane authority를 만들지 않는다 | `DEFER` | D26 + H05 report-authoring exact-schema/`job_id` immutability fixture + AI 작업 결과 boundary validator/owner lane 결정; projection event count와 directory recursion `0` |
...
| AC-28 | `VERIFIED_FOR_PLAN_SCOPE` | §3.4.8·§12 P5A-0→P5D·§17 D36과 `SOURCE_TIMELINE_ANNOTATION_V1`·`PROJECT_CONTEXT_GRAPH_MODEL_V0`에서 source-native timeline과 minimal system receipt를 project timeline/LLM prompt와 분리하고 confirmed project만 격리 투영한 뒤 SourceSpan→Event→Unit→Branch→ProjectContext, append-only short/medium/long summary revisions, memory candidate, HPP sole writer, ERP accepted-generation read model, MCP/plugin direct write `0`, one-project Slack+mail+voice thin Shadow를 고정 |
...
공통 라벨에 실제 저장되는 `occurrence.occurred_at`은 입력 창구와 원래
offset에 관계없이 모두 `Asia/Seoul`의 `+09:00`으로 정규화한다. 수신·완료·
...
프로젝트 시간장부(project timeline) Shadow는 KVDS의 mail·voice·`AI 작업 결과` proxy만
포함한다. Slack
...
```text
메일 / Slack / 음성 / WorkSession / 파일 이력 / 실행·검증 증거
                    |
                    v
     ...
     KST time(+09:00) / occurrence / actor / project-state / label
                    |
                    v
...
3. AI 작업 결과 (임시 proxy)
   ...
   - 이 데이터는 Codex가 작성한 업무 종료 요약이지 PC 사용 감시, 정식
     WorkSession 또는 프로그램이 자동 생성한 실행·검증 영수증이 아니다.
...
4. HPP 로컬 업무 장부
   - `HPP Codex 작업 맥락 수집기`가 exact private project allowlist와 pinned
     binding 아래 명시적으로 전달된 시작·task 연결·checkpoint·종료를
     append-only `업무 사건`으로 저장한다.
   - 같은 실제 업무를 이어가는 task만 같은 `로컬 업무`의 wire field
     `work_id`를 공유한다. `begin_work.work_id=null`일 때 생성되는 값은
     `LW-<project>-<digest>` 형식이지만(`<project>`는 `project_code`)
     caller-supplied ID 전체에
     `LW-*`를 강제하지 않는다.
   - 내부 path `codex_work_context`, schema, operation, module, CLI 이름은
     유지한다. 이 장부는 ERP task, H03 WorkSession, H05 acceptance,
     accepted ProjectContext, 프로젝트 시간장부가 아니다.

5. 프로젝트 파일
   - 같은 collector가 14개 프로젝트 폴더의 정책 허용 범위에서 positive
     `파일 관찰` 29,326개와 exact hash 14,132개를 모았고, 15,194개는 hash
     대기다. sensitive/temp/symlink/policy-excluded 경로는 열거하지 않는다.
   - 최초 compact inventory 뒤 30분 주기에는 전체 packet을 반복 저장하지
     않고 변경 파일 관찰과 비권위 `absence_candidate_only`만 append한다.
     non-exact hash queue reason은 `pending`으로 정규화하며 최신 실행의
     14개 delta 합계는 27,152 bytes였다.
   - 구방식 full packet 347개/1,179,013,521 bytes는 모든 순차 상태 변화가
     `0`임을 확인하고 exact 격리 상태에서 새 collector 14/14 재실행을 통과한
     뒤 삭제했다. compact inventory와 delta, project 원본은 유지했다.
   - `guild_hall/file_activity`의 first-observed/revision/rename/copy/touch/
     delete/restore `파일 이력` reconciler 계약과 synthetic test는 존재한다.
...
6. 실행·검증 증거
   ...
   - H05는 exact `report_authoring_v0/workflow_receipt.json` 실행·검증
     영수증만 allowlist한 feature-OFF 기반이다. 일반 Codex 작업용 공통
     실행·검증 영수증은 아직 없다.
...
8. 프로젝트 시간장부
   ...
   - 269행은 mail 84, voice 3, `AI 작업 결과` proxy 182이며 모두
     KST다.
...
- HPP 파일 관찰을 sole reconciler가 immutable 파일 이력 event로 확정
- 일반 Codex 작업용 common 실행·검증 영수증/manifest와 H05 adapter를 구현해
  `실행·검증 증거`로 저장
- KVDS V4에서 mail·voice·WorkSession·Slack·파일 이력·실행·검증 증거를
  함께 replay한 뒤
  같은 timeline writer를 나머지 프로젝트로 확장
- mail/slack/voice에서 나온 요청·약속·결정 후보를 최근 맥락과 대조하는
  P5 context resolver
- accepted P5 context에서 P6 AX 책임공학 TaskIntent 후보를 만드는 단계(2026-07-29 authority decision 순서: P5→P6→P7→P8)
- P6 TaskIntent 후보를 P7 TaskDriver·P8 원자적 ERP 기록으로 승격하는 owner/role 승인
