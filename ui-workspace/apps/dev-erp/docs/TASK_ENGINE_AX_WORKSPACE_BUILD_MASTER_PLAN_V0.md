...
| 맥락·세계수·할일 | `M2_2_PUBLIC_SYNTHETIC_PILOT_CANDIDATE / NOT_OPERATIONAL` | M2-0 TARGET과 M2-1 exact-one-project/root admission 뒤, M2-2 후보가 별도 expected pilot grant, complete project-source reference manifest, explicit common→policy bindings, role-roster pin과 root-bound canonical packet을 한 zero-write command로 결속한다. command는 launch와 canonical packet만 읽고 project source-body는 읽지 않으며, 기존 deterministic AX·SE subject를 한 번 호출한다. RAG/Wiki/LLM call과 ERP/TaskDriver effect·authority는 0이고 source truth·freshness·actual-project approval은 여전히 증명하지 않는다 | M2-2 fresh B/V/Claude 수락, Owner-frozen actual launch/packet/root provenance와 수동 zero-write pilot, 이후 P5 context resolver·project-local RAG/thin Wiki·ERP accepted generation·TaskDriver 승격 |
