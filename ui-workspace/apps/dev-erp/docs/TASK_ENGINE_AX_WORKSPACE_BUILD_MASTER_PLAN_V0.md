...
Hard gate는 세 가지다. P5의 deterministic acceptance receipt 전에는 P6를 시작하지 않고,
...
않는다. C00은 owner 승인 시점의 exact HEAD/origin/candidate/count를 다시 pin하고 불일치하면 중단한다.
...
| --- | --- | --- |
| `C00` | exact owner-gated preflight candidate | owner가 `public_only_stdout`을 고르면 §18.1 literal public command와 stdout receipt만 실행 가능. 현재 CV-02 live proof가 남으므로 예상 결과는 `BLOCKED`, P1 unlock `false`; `owner_authorized_query_only`는 별도 승인 |
| `H00` | exact pinned `canon_candidate`의 ratification-only gate | C00 PASS 뒤 현재 contract/helper/test를 그대로 ratify/hold. 파일 편집은 새 exact packet 필요 |
...
allowed_read_paths: [public_only_stdout은 HEAD/origin-main refs, .git/index.lock 존재 여부, §18.1의 literal public tracked file만; candidate ref/content, root-wide status/search, private/live path 제외]
allowed_commands: [public_only_stdout은 §18.1 exact allowlist만; query-only 명령은 별도 packet 전 없음]
...
folder_delta: [public_only_stdout은 none; owner_authorized_query_only만 approved metadata report directory]
docs_contract_changelog_delta: [not_applicable]
owner_and_writers: [inventory agent; source/DB/file writer 없음]
acceptance_checks: [approved SHA==HEAD==origin/main, literal public five-lane candidate map, fixed live-proof list, receipt field allowlist와 raw sentinel 0, tracked before/after tree·diff equality, CV matrix evidence; candidate ref는 별도 승인 전 제외]
regression_checks: [tracked HEAD/tree/diff unchanged, receipt YAML parse, target plan unchanged by execution]
migration_or_backfill: none
rollback: public_only_stdout은 delta가 없어 not_applicable; query-only metadata packet만 revert/removal, source/runtime state delta 0
stop_conditions: [profile/ACL absent for required live proof, raw/private value required, writer ambiguity, baseline drift, tracked dirty/mutation, index.lock, command/path allowlist 이탈]
...
| ID | 현재 | 다음 gate |
| --- | --- | --- |
| HP-01 | `HISTORICAL_REPORTED`: 계획 관찰 당시 main `9df7e577...`, candidate `927b3fb0...`, merge-base `15e988b4...`, count `25/2` | C00 승인 시 exact ref/count 재-pin; 불일치 시 중단 |
| HP-02 | `PASS(pinned plan baseline)` / candidate oracle drift | integration patch에서 S24 제거, blob diff 0 재검증 |
| HP-03 | `HISTORICAL_REPORTED`: correction 시작 시 index lock 없음; target은 owner-designated untracked | C00 승인 시 status/worktree/overlap 재검증 |
...
### 18.1 C00 `public_only_stdout` exact preflight (승인 전 미실행)
...
| proof ID | C00이 닫아야 할 live 사실 | public-only 상태 / next proof |
| --- | --- | --- |
| `C00-LIVE-01` | lane별 current physical owner와 default root | `UNRESOLVED`; separately approved query-only owner/path inventory |
| `C00-LIVE-02` | lane별 current writer·direct caller 전체 집합 | `UNRESOLVED`; read-only caller graph with authority |
| `C00-LIVE-03` | lane별 current consumer 전체 집합 | `UNRESOLVED`; read-only consumer graph with authority |
| `C00-LIVE-04` | lane별 current coverage availability와 exact unknown reason | `UNRESOLVED`; metadata-only coverage aggregate with authority |
...
result: PASS | BLOCKED
reason_codes: []
p1_unlocked: true | false
exit_code: 0 | 3 | 20 | 21 | 22 | 23 | 24
```

Exit `0`은 baseline·literal path/symbol map·모든 required live proof·raw sentinel·tracked zero-mutation이
전부 PASS이고 `p1_unlocked: true`일 때만 쓴다. Exit `3`은 안전한 `BLOCKED`, `20`은
...
```mermaid
flowchart LR
  P["현재: owner wait<br/>plan-scope validation PASS"] --> O0{"Owner: C00/P0 inventory?"}
  O0 -- "보류" --> STOP
  O0 -- "승인" --> P0["P0 C00 read-only receipt"]
  P0 --> C0{"C00 receipt PASS?"}
  C0 -- "BLOCKED" --> STOP
...
| `npm.cmd run validate:path-policy` | `HISTORICAL_REPORTED PASS`; 5 pass, Windows symlink 환경 1 skip, violation `0` |
...
| plan structural invariant check | latest working delta `PASS`; D01~D26 26개, AC-01~22 22개, CV-01~09 9개, D26-FX-01~20 20개, code fence 96개 |
| fresh `fork_turns="none"` inspector/judge | 최초 correction 2명과 후속 bounded slice별 fresh review; latest verdict를 publish 전 다시 받음, claim ceiling은 document-level only |
...
| AC-12 | `VERIFIED_FOR_PLAN_SCOPE` | §12 full YAML cards와 non-executable phase-card claim ceiling을 분리. H01~H06·split row는 literal path/symbol/command/full field child packet 전 write 승인 금지 |
