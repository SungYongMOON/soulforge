...
| 2026-07-30 AI 작업 결과 누락 복구 candidate | `PUBLIC_SYNTHETIC / FEATURE_OFF`; 최근 24시간·foreground pull 의존을 제거한 exact `{repo,ref,source_lane}` cursor planner, source `occurred_at`/first-ledger `recorded_at`, canonical full-record digest conflict HOLD와 synthetic recovery tests를 추가했다. private cursor·ledger·runtime·scheduler·network·production activation `0`; 기존 `five_field_session_capture_v0`, `five_field_log.jsonl`, `bounded_work`는 호환 이름을 유지한다. |
