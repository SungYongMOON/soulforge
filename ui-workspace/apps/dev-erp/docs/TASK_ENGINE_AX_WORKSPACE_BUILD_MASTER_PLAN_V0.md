...
| Slack 대화·첨부 | `LIVE_READ_ONLY_COLLECTION / BLOCKED_BY_ABANDONED_WRITER_LEASE` | 예약 batch 수집이 해제되지 않은 batch writer lease 때문에 막혀 있다. launcher·runtime manifest·node·binding pin과 no-network preflight는 통과하므로 collector 코드·manifest·binding 결함이 아니다. 이번 변경은 blocked lease가 `store_slack_custody` lane에 exact safe blocker code `batch_lease_unavailable`를 남기도록 collector 관측 계약만 바꾼다. lock을 stale로 추정하거나 제거하지 않고, lease-guarded custody store를 읽지 않으며, batch state를 덮어쓰지 않는다. 실측 수집량·시각·custody/HOLD/page 지표는 private runtime plane에만 둔다 | stale lease 해제는 별도 승인된 private runtime 행동이며 이 변경은 production·recovery authority를 부여하지 않는다. backup 축적 형태와 향후 Slack Archive MCP 준비도는 `PARTIAL/HOLD`다. thread reply·delete coverage는 현재 read-only polling 계약으로는 구조적으로 불가하며, source-arrival+attachment ref의 프로젝트 시간장부 자동 투영과 H07 acceptance는 계속 별도다 |
