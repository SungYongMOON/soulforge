...
| Linear LB1 backup foundation + Owner Gate | `PUBLIC_SYNTHETIC / FEATURE_OFF / OWNER_GATE_PENDING / ACTUAL_LB1_HOLD` | offline contract가 immutable revision·manifest·coverage·restore completeness를 검증하고, 별도 pure Gate가 Owner decision, exact workspace/read-only credential, Drive target/storage authority, retention/RPO/partial HOLD, human restore reviewer와 trusted full-packet pin을 결속한다. focused gate 8/8, 전체 backup-controller 63/63 PASS | 제안 기본안 전체 workspace/Drive `Soulforge Linear Backup`/일별30·월별12/RPO24h에 대한 Owner 승인과 exact provider/target/reviewer refs 뒤 actual one-shot. provider/storage/network/scheduler/Task/P5 effect 0 |
| Linear LB1 v2 Bound Runner | `PUBLIC_SYNTHETIC / FEATURE_OFF / STORED_BYTE_RESTORE_CANDIDATE / EXTERNAL_ADAPTERS_HOLD` | 별도 v2가 18개 복원 차원과 bounded body/history를 보존하고, Gate v2가 writer/epoch·claim store·synthetic Adapter refs·artifact layout·limits·expiry를 full pin으로 결속한다. async Runner는 claim-before-read→in-memory create-only generation→exact-byte readback→independent restore를 수행하며 모든 post-claim 실패를 `HOLD_CONSUMED`로 닫는다. main v2 44/44, backup-controller 107/107 PASS, fresh Opus 최종 ACCEPT | actual Linear reader·Drive writer·cross-process claim store·credential/target refs·Owner decision·human restore review. external provider/storage/network/filesystem/scheduler/Task/P5 effect 0 |
...
실제 Linear collector/poller, Drive backup writer, actual restore runner, webhook, scheduler,
live `LinearTaskProvider`, Linear writer는 모두 `0`이다.
