...
| P1 readiness 재검토 ref | `main@16190bff6c1dd9e101c11a078b97e84f1c1c43ea` (`origin/main`과 동일); H00 candidate와 H02~H05 public 코드·계약만 후속 대조 |
...
| H05 run/log | `_workmeta/<project>/runs/**`는 위치 owner지만 모든 run에 공통인 manifest schema가 아니다. strict `soulforge.workflow_receipt.v1`은 현재 report-authoring 전용이고 five-field는 draft/차단 훅 OFF다. | 승인된 exact schema+explicit ref allowlist만 입력으로 받고 run directory recursion, raw/stage log, transcript, arbitrary manifest를 거부한다. daily-ledger occurrence는 H03만 소유하고 H05에서는 관계 ref로만 쓸 수 있다. unknown schema나 raw artifact가 필요하면 중단한다. |
...
있지만 H03/H06 합격을 단독 주장할 수 없다. H05의 최초 allowlist 후보는 strict metadata-only
workflow receipt와 boundary-validated five-field receipt뿐이며, schema가 없는 `runs/**` 파일은 이름이나
위치만으로 자동 수용하지 않는다.
...
| `H03` | structured PC work + external SE schedule history / `H00`; internal `H03A→H03B` | H03A ERP MCP WorkSession/daily-ledger/metadata-only instruction fixtures + H03B owner-approved schedule current/revision/event fixture → combined structured history+gap receipt | `erp_mcp_service.mjs`, server/tests, daily-ledger modules/tests, proposed schedule contract, README, CHANGELOG | WorkSession idempotency, task-chat/full-conversation payload reject, daily-ledger single-lane ownership, schedule stale revision/current-event replay | adapter OFF+revert / H03A alone cannot close H03; stop on surveillance or D20 exact owner/path/writer gap / D19+D20 |
| `H04` | file history envelope / `H00` | explicit immutable file event/checkpoint + bounded projection/coverage refs → common envelope adapter | `guild_hall/file_activity/**`, owner README, tests, CHANGELOG | existing file tests + complete_with_events/complete_no_events/not_collected/not_applicable + exact revision + bounded projection/truncation gap | adapter OFF+revert / stop on filesystem scan, live collector need, or complete-ledger overclaim / file owner approves |
| `H05` | run/log history envelope / `H00` | explicit allowlisted run metadata/five-field/validator receipt refs → run occurrence+coverage adapter; no `runs/**` recursion | existing run/five-field public workflow modules, tests, READMEs, CHANGELOG | missing/partial run, validator ref, schema allowlist, H03/H05 lane uniqueness, raw/stage log exclusion; AgentRun not required | adapter OFF+revert / stop on unknown schema, directory scan, or raw artifact need / run owner+D26 approve |
...
| HP-HISTORY-08 | five technical source owners and single-lane ownership preserved | voice/file/run/mail/structured source record의 second truth `0`; 같은 native occurrence의 H03/H05 중복 산입 `0` |
...
| HP-HISTORY-12 | raw exclusion | mail/voice/file/run payload, run-directory recursion, stage log, whole-conversation/completion-summary/OS-surveillance field `0` |
...
| D26 | source-to-lane identity와 H05 run schema allowlist | voice `recording_id` occurrence 후보와 session/bundle relation 분리; daily-ledger occurrence는 H03만 소유; Codex task-chat/completion summary는 coverage 아님; H05는 exact schema+explicit ref만 허용하고 `runs/**` recursion 금지. 모두 owner ratification 전 candidate | H02/H03/H05/H06 | public/synthetic / unknown schema·cross-lane duplicate·private run scan 금지 |
...
| run/log common source schema and lane assignment | `runs/**`는 위치 owner일 뿐 공통 manifest가 아니고 daily-ledger가 H03/H05에 중복될 수 있음 | `DEFER` | D26 + H05 exact schema allowlist/explicit-ref/raw-sentinel fixture; directory recursion `0` |
