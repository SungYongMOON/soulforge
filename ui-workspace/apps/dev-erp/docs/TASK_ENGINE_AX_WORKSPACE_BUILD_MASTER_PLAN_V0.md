...
> 2026-08-20 CURRENT: three parallel public-synthetic foundations were integrated
> without activating a live route. Engineering Engine release integrity now binds the exact
> Git-index source set; one admitted project PDF can produce trusted RAG/Thin Wiki and a
> non-accepted P5 input candidate; and `backup_controller` has an offline Linear LB1
> manifest/restore contract. Actual project persistence, P5 acceptance, Linear/Drive/NAS,
> Task dispatch/write, MCP/runtime, scheduler, and AgentRun activation remain `HOLD`.
...
| P4 PDF RAG·Thin Wiki projection | `PUBLIC_SYNTHETIC / FEATURE_OFF / P5_NOT_ACCEPTED` | admitted PDF revision 하나를 exact trusted candidate/source-receipt digest와 upstream admission commitments에 결속해 project-local RAG·2-page 이하 Thin Wiki sibling candidate 및 P5 input candidate를 만든다. recomputed citation forgery, nested getter/TOCTOU, alias, foreign project/ref mismatch를 search 전 거부하고 bitemporal·coverage/gap·supersession·reviewer·writer-epoch 부족을 명시한다. main focused 13/13, root isolation 21/21, 기존 PDF family PASS | actual KVDS project persistence와 source-body pilot receipt, P5 reviewer/writer epoch와 accepted generation. Engine/ERP/Task effect·model/network/write 0 유지 |
| Linear LB1 backup foundation | `PUBLIC_SYNTHETIC / FEATURE_OFF / ACTUAL_LB1_HOLD` | `backup_controller`의 순수 offline contract가 immutable revision, deterministic manifest·coverage, duplicate/conflict, partial/failure, forged coverage, restore completeness와 Sheet/CSV-only 불완전을 검증한다. main LB1 11/11, 전체 backup-controller 55/55; raw/path-like error code·deterministic revision ID·status consistency hostile-run controls PASS | exact storage-write authority, minimum Linear read scope, retention/RPO/partial-failure policy, human restore acceptance 뒤 actual one-shot LB1. provider/storage/network/scheduler/Task/P5 effect 0 |
...
| `LB1` one-shot read-only pilot | exact backup target/storage-write authority + 최소 read-only Linear credential와 workspace/team/project scope + retention/RPO/partial-failure/restore-acceptance 정책 승인 | bounded snapshot/API read, 승인된 private target에 새 revision 저장, manifest/hash/coverage, restore check | Linear mutation, webhook 등록, scheduler, overwrite, P4~P8 unlock 금지 |
