...
| `C5` proposal shadow | Engine/Task Core + C3 live-only synthetic evaluator foundation 구현; accepted-generation query와 real adjudicated corpus 없음 | accepted generation 기반 read-only query + Reactive/SE proposal-only route와 실제 품질 관찰 | Official Task/Linear mutation 0 | 현재는 synthetic contract invariant/quality receipt만; live metric은 `C3+C4+HB-D5` 뒤 | `1~2주`, actual `C3+C4` 뒤 |
