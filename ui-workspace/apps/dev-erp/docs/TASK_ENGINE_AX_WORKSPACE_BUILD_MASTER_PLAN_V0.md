...
| 이번 실행 결과 | foundation-first 순서를 유지한 채 source custody/promotion, personal WorkSession lifecycle, accepted-generation query/knowledge 보정; 구현·데이터 변경·운영 활성화는 owner 승인 전 금지 |
...
| --- | --- | --- | --- |
| `erp_get_my_tasks` | account-scoped task assignments와 revision refs | bearer self; cursor generation pin | business row `0`; current agenda compatibility 유지 |
...
```text
source/work PC ── pointer 또는 upload ──> custody inbox/source owner
...
#### 14.6A HP-INGRESS·HP-SESSION·HP-QUERY acceptance
...
2. H00~H06 + MAIL/HP-HISTORY와 HP-INGRESS의 contract/shadow schema, custody, coverage, replay/export tests; live caller-zero/payload promotion/cutover/failover claim 없음
...
| `D27` / I8 feature-OFF contract 전 | source-kind별 custody/staging/quarantine/promoter/destination, operation allowlist, retention/legal hold/ACL/scan/backup/delete authority | pointer/reference 기본, central upload inbox custody, move/delete·live promotion `OFF` | public custody matrix와 synthetic receipt fixture | path 생성, payload read/copy/move/delete, malware scan overclaim | HP-INGRESS synthetic receipt → I8 contract only; live는 P9/P10 별도 |
| `D28` / AX-G1 전 | exact assignment epoch owner, opaque thread-ref capability, node binding, local outbox writer/path/fsync/encryption/retention, missing SLA, official completion approver | one active primary, multiple checkpoints, closeout≠completion; exact binding은 `VERIFY_HP` | pure lifecycle/outbox/adversarial fixture | current record 소급 migration, raw thread ID, team activation | HP-SESSION synthetic → AX-G2 feature-OFF; one-seat/team은 별도 |
| `D29` / Q8 feature-OFF adapter 전 | project/common grant authority, 403/404 leak policy, generation manifest/current-pointer/cursor owner, candidate reviewer/approver/writer | ERP UI/MCP primary query, files audit snapshot, explicit scope/no fallback, team candidate-only | schema/ACL/generation parity fixture | live endpoint/pointer advance, implicit fallback, direct truth write | HP-QUERY synthetic → Q8 feature-OFF; Q9/Q10 별도 |
...
이번 후속 correction의 publish 범위는 이 파일, current/target 경계를 공유하는
`AX_WORKSPACE_TASK_ENGINE_INTEGRATED_VALIDATION_PLAN_V0.md`,
`TASK_ENGINE_CONTEXT_FOUNDATION_CROSS_VALIDATION_V0.md`, `CODEX_TEAM_WORKSPACE.md`,
`slices/ERP-MCP-V0.md`, root 정책상 필요한 `CHANGELOG.md`의 bounded 항목뿐이다. 독립 리뷰와
...
아래 표는 최초 correction의 historical receipt와 이번 후속 public 문서 delta의 fresh 결과를 구분한다.
...
| 검증 | 결과 |
| --- | --- |
| `git diff --check -- <six scoped files>` | 후속 working delta `PASS` |
...
최초 correction의 reviewer와 이번 후속 correction의 ingress·MCP/session·query/RAG inspector가 독립
검토했다. Deterministic validator는 위 receipt처럼 통과했거나 unrelated blocker를 분리했고, 요청된
보정 뒤 latest diff를 fresh inspector와 independent judge가 각각 `ACCEPT`했다. 따라서 아래 행은
`VERIFIED_FOR_PLAN_SCOPE`다. 구현 완료·C00A/C00Q/C00B 승인·private/runtime 검증·운영 승인을 뜻하지
않으며, CV-02의 current writer/default-root inventory는 `UNKNOWN_BLOCKED`로 P0 acceptance를 계속 막는다.
...
| AC-23 | `VERIFIED_FOR_PLAN_SCOPE` | §3.5/§6.2A/§7.1 ingress custody·promotion, §6.5A/§8/§10/§11.2 personal lifecycle, §9.3 query/knowledge, §12 side-card DAG, HP-INGRESS/SESSION/QUERY와 D27~D29; 구현·binding·activation authority `0` |
