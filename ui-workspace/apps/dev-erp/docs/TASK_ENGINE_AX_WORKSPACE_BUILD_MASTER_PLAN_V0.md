...
| --- | --- | --- |
| H00 | `PROJECT_HISTORY_ENVELOPE_V0.md`와 pure helper는 event와 coverage receipt를 분리한 `canon_candidate`다. §3.4.1의 nested coverage 예시는 아직 owner-ratified canon이 아니다. | owner가 independent coverage receipt를 ratify하기 전 H01~H05 adapter를 구현하거나 기존 source에 소급 적용하지 않는다. ratify 시 §3.4.1의 nested 예시는 독립 receipt 계약으로 교체한다. |
...
| H02 voice | Mac voice primary, recording/session/library, accepted route, producer receipt와 consumer ack는 구현돼 있다. common envelope, route/retranscription supersession, D25 gap mapping은 없다. | `recording_id`를 native occurrence 후보로 두고 session과 meeting bundle은 relation/grouping ref로 분리한다. 이 매핑은 voice owner 승인 전 candidate이며 route/coverage ambiguity가 남으면 adapter를 시작하지 않는다. |
...
| H04 file | five-ID, exact content/revision, observation/event/checkpoint, sole reconciler candidate와 bounded life-tree projection이 구현돼 있다. projection은 live collector/ERP correlation 부재를 포함해 의도적으로 `partial`이다. | adapter는 caller-supplied immutable event/checkpoint/projection ref만 받고 filesystem을 scan하지 않는다. bounded projection을 complete ledger로 승격하지 않고 truncation/live gaps를 D25 receipt에 보존한다. |
...
current/revision/event contract+fixture → H03 combined acceptance`다. H03A는 H00 ratification 뒤 진행할 수
있지만 H03/H06 합격을 단독 주장할 수 없다. H05의 최초 allowlist 후보는 report-authoring 전용
...
| HP-HISTORY-12 | raw exclusion | mail/voice/file/run payload, run-directory recursion, stage log, whole task-chat·task-chat completion-hook/full-message summary·OS-surveillance field `0`; bounded WorkSession `summary`는 허용 schema로 별도 검증 |
...
10. §12.1 slice별 minimum independent review. 공개 기준선 `main@7f02523900adf38ca7f18d6540a12111ad88f8af`의
...
fresh root validation/review (공개 기준선 `main@7f02523900adf38ca7f18d6540a12111ad88f8af`까지 PASS;
...
| HP-21 | 공개 `main@7f02523900adf38ca7f18d6540a12111ad88f8af` plan-scope validation `PASS`; 구현 full regression 미실행 | 각 후속 문서 delta publish 전 재검증, 각 approved implementation slice는 별도 full regression |
...
`main@7f02523900adf38ca7f18d6540a12111ad88f8af`까지 계획 범위 상태는 `PASS`다. 명령명이
...
| plan structural invariant check | latest working delta `PASS`; D01~D26 26개, AC-01~22 22개, CV-01~09 9개, D26-FX-01~20 20개, code fence 94개 |
