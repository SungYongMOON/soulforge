...
| Chat 1-hour Shadow Bot | `OWNER_REPORTED_CONFIGURED_V0_4_0 / BOUNDED_MULTI_APP_WRITE / FIRST_RUN_READBACK_PENDING` | Owner는 기존 `업무 인입 감시` ID·매시간·활성 상태를 유지하고 Linear create/update/comment/status, Slack post/reply/mention/reaction/edit, Calendar create/update/invite, Drive create/update/move/designated-share, Gmail read/label/Draft-only를 허용했다고 보고했다. Gmail send/reply/forward, Done/Cancel/Archive/Delete, 계약·비용·기술기준·대외약속 자동 확정은 금지하며 run당 외부 변경 상한은 5다. repo/Codex는 설정·첫 run을 독립 관찰하지 않았다 | B0 exact task/prompt digest·permission snapshot, first v0.4.0 Cycle Receipt와 앱별 readback, `gmail_sent=0`, effect≤5, prohibited-action 0, correction Evidence 확인. 관찰 전 actual mutation 성공/실패는 `UNKNOWN/HOLD` |
...
| `C4` actual P5 | `ACCEPTANCE_FOUNDATION_DONE_FOR_SYNTHETIC_SCOPE`: candidate content/review/coverage/supersession binding + append-only in-memory manifest/receipt/current-pointer CAS. actual writer/persistence 0 | actual P4/M2/timeline+registered reviewer+HPP fenced writer/epoch로 generation 1개 human acceptance | synthetic store only; actual accepted-context sole-writer만, ERP/P6 0 | acceptance/query 30/30 + candidate 12/12; actual generation receipt 미생성 | foundation 완료; actual `2~4일`, `C1` 뒤 |
| `C5` proposal shadow | C3 live-only evaluator + mandatory-ACL/no-fallback/uniform-no-leak Accepted Context Query synthetic foundation 구현; actual accepted generation·corpus 없음 | actual generation current pointer/manifest/receipt binding + Reactive/SE proposal-only route와 실제 품질 관찰 | Official Task/Linear mutation 0 | current query foundation 30/30; live metric은 actual `C3+C4+HB-D5` 뒤 | `1~2주`, actual `C3+C4` 뒤 |
...
- Owner 보고: 기존 Chat Scheduled Task는 v0.4.0 bounded multi-app write로 설정됐고 first run/readback은 대기 중이다.
- repo 검증: P4 Preparation 18/18·Runner 13/13, Linear LB1 runtime 22/22·v2 50/50,
  P5 candidate 10/10, Task Core POC 28/28, C3 synthetic shadow 53/53 PASS. C3 persistent
  Decision Ledger와 C5 Accepted Context Query는 아직 없다.
