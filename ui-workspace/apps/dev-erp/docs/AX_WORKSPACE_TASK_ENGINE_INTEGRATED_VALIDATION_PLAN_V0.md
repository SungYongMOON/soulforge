...
### 0.8 HPP MCP/storage/access 계획 경계 (2026-07-16)

Owner-confirmed CURRENT baseline은 HPP·회사 업무 PC·MacBook Air·Mac mini가 같은 logical
`_workspaces/<project>` actual logical project worksite/body를 기존 OneDrive junction으로 physical
materialize/shared-sync한다는 사실이다. 이 program은 topology를
rename/rematerialize/bootstrap/repair하지 않는다. Exact junction target와 sync/runtime health는 private
`VERIFY_HP`이며 CURRENT 정상 상태로 과장하지 않는다. 근거와 세부 gate는
[`MULTI_PC_DEVELOPMENT_V0.md`](../../../../docs/architecture/workspace/MULTI_PC_DEVELOPMENT_V0.md)와
master plan §0/§3.5/§5/§7/§12/§14의 HPP addendum을 함께 따른다.

| 영역 | `TARGET` | 검증 중단선 |
| --- | --- | --- |
| storage ownership | `_workspaces/<project>`는 actual project payload/artifact owner이고 OneDrive junction은 physical materialization/shared sync; HPP TARGET은 별도 private active DB/accepted central RAW/quarantine/central queue/outbox/runtime custody | topology delta, narrow runtime exclusions의 cloud sync, non-HPP D/UNC/SMB/SQLite access 모두 `0`; exact HPP binding/health는 `VERIFY_HP` |
| transport | MCP control plane + strict private office LAN의 mTLS reverse proxy/authenticated HTTPS binary plane; HPP backend loopback | VPN/Tailscale/remote/public Internet/port-forward/Funnel/direct SMB, URL-only bearer authority, client destination path `0` |
| identity/delegation | one-time human device enrollment, OS broker silent refresh, distinct actor chain, human/device/agent/task-object-action intersection, earliest expiry+cascading revoke | routine per-chunk prompt, delegation ceiling 초과, revoked child grant/ticket `0` |
| authorization/query | artifact/revision/action existence policy, ACL-aware RAG pre/post field/chunk filtering+cache, immutable redacted derivative lineage | raw/latest fallback, unauthorized cache/locator, hidden/formula/comment/embed policy 누락 `0` |
| `A8-SYNTH` | D27~D29+exact public security packet 뒤 core-independent public/pathless/feature-OFF verifier fixtures | P1/P8/P9/TaskEngine/live binding unlock `0` |
| `A8-CANARY` | SYNTH PASS+accepted private `VERIFY_HP` exact binding receipt+strict office LAN+explicit owner+Level 3 뒤 one seat/project/small file exact revision | VPN/remote, mail/voice bulk, team/production/core unlock `0` |
...
| HP-STORAGE-08 | owner-confirmed logical topology가 보존되나 | 모든 named PC의 `_workspaces/<project>` path-set delta 0 | pathless private junction inventory | 🟥 `GATE` |
| HP-STORAGE-09 | active runtime class가 cloud sync에 들어가나 | DB/WAL/SHM/RAW/queue/outbox/quarantine sync 0 | HPP custody-class receipt | 🟥 acceptance |
| HP-STORAGE-10 | client가 HPP storage를 직접 여나 | D/UNC/SMB/SQLite direct access 0 | MCP/API actor-bound receipt만 | 🟥 acceptance |
...
후속 correction은 추가로 master plan의 `HP-INGRESS-01..16`, `HP-SESSION-01..18`,
`HP-QUERY-01..16`을 그대로 acceptance로 사용한다. 핵심 claim ceiling은 다음과 같다.
...
- Actor/delegation/ticket swap·replay·revoke race, path traversal/archive bomb/hash/size/media mismatch는
  fail closed하고 exact-revision range download·RAG field/chunk/cache·redacted derivative lineage를 검증한다.
...
- D27~D29와 exact public security child packet이 accepted되면 `A8-SYNTH` enrollment/mTLS→broker→
  delegation→ticket verifier를 public/pathless/feature-OFF로 core와 무관하게 실행한다. 이 receipt는
  P1/P8/P9/TaskEngine을 unlock하지 않는다.
...
- 첫 live HPP access implementation인 `A8-CANARY`는 SYNTH PASS+accepted private `VERIFY_HP` exact binding
  receipt+strict office-LAN binding+explicit owner approval+Level 3 live gate 뒤에만 한 small file의 prepare
  upload→HTTPS transfer→hash/readback→idempotent
  finalize→query→exact revision download→optional validated redacted derivative/XLSX canary다. Mail/voice bulk와
  team activation은 제외한다.
...
- established topology delta, narrow runtime exclusion의 cloud sync, non-HPP D/UNC/SMB/SQLite access,
  VPN/Tailscale/remote/public ingress/Funnel이 필요하다.
- `A8-CANARY`가 SYNTH PASS/private exact binding receipt/strict office LAN/owner/Level 3 중 하나 없이 실행된다.
- actor/delegation/revoke/ticket/exact-revision/RAG filter/redacted-lineage guard가 fail open한다.
