...
- `slices/B9-STEM-RIVER-VIEW.md` — **줄기 강(江) 뷰 설계 정본** (2026-07-06 owner 구술): 기둥=시간축 SE 흐름·게이트=큰 점·가지=맥락 한 줄·잎=기록(기원/경로/종결 즉답)·모양=진단·지식그래프와 잎 수준 양방향 접점. 구현 순서 B9a(가지 타임라인)→b(강줄기 렌더)→c(진단)→d(지식 역링크). B9a **done 2026-07-11**(branch_story API+지도 렌즈 이야기 뷰)·B9b core done 2026-07-07·B9c **done 2026-07-11**(`/api/context/graph` 진단 집계+네 번째 렌즈+후속 사용 미관찰 후보 회색 표시, codex_gpt-5). **다음 착수 = B9d(지식 역링크).**
...
- `slices/ENGINE-13-TASK-DRIVER-CLOSED-LOOP.md` — TaskDriver `why/why-now`, 판단/적용과
  작업 상태의 두 축, exact refs, 승인·멱등·replay, completion feedback을 닫는 cold-start
  packet. 상세 public-safe 설계는 `task_engine_redesign/README.md`부터 읽는다.
  (**status: canon_candidate; high-performance PC inventory/dry-run/pilot 전 runtime 미구현**)
...
- ④ event_log append-only 라벨링: 모든 쓰기 이벤트에 used_refs[]+data_label 부착. CURRENT
  상태 전이는 `core_item` current row UPDATE와 event append를 함께 수행한다. event evidence를
  생략하거나 current row만 과거 truth로 쓰지 않는다(P-4-ai-B approve/reject도 append).
  ENGINE-13 target replay 계약은 별도 migration이다. 읽기 전용 계산기
  (riskAlerts/nudges/workload/inputFulfillment/eolImpact 등)는 이벤트 0(미저장 확인).
...
- [`REPORT-AUTHORING-WORKFLOW-SHELL.md`](slices/REPORT-AUTHORING-WORKFLOW-SHELL.md) — Soulforge body/dev-ERP metadata shell, one-draft `final_polish`, canonical runner single-writer path, `_workspaces` body / `_workmeta` receipt owner 분리, production/live attestation 미완료(default-off).
