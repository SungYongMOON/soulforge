...
- [event_log kind 정본] ai_proposal/ai_proposal_approve/ai_proposal_reject(P-4-ai), recommender_run(P-19), person_skill_set(P-6), knowledge_upsert(P-10), calculator_activate(P-11), ics_export·embed_register(P-17/P-18, data_label='meta'), schedule_spawn·anchor_move·attachment_add(기존, 추가 0).
...
- ③ 원문 미저장: 첨부·지식·임베드·ICS 는 포인터+sha256+요약/메타만. core_attachment.pointer, core_knowledge.source_ref/pointer, embed_view URL, ICS due/title/project_id — 본문·secret·도면 원문 0. llm.mjs 화이트리스트 외부전송은 메타/요약만(이 종합 범위에서 외부전송 신규 0).
...
- [data/real_meta.json 존재로 인한 fixture purge — 수동 verify 경로 정정] server.mjs 부팅 시 data/real_meta.json(현재 존재)이 있으면 purgeSynthetic 실행 → 기본 DB 에 합성 fixture 없음. 따라서 모든 '수동 verify' acceptance 는 node server.mjs --db :memory: 로 기동(메모리 DB 는 real_meta 미적재 분기에서 fixture 로드). curl 검증도 --db :memory: 기준. node:test 는 freshStore()=openStore(':memory:')+loadFixture 라 영향 없음.
...
- [entity_type 화이트리스트 단일 소스] core_attachment 검증 배열은 현재 ['item','project','purchase','meeting','part'](store.mjs L1210, 'part'는 P-1 기 추가). P-10 만 'knowledge' 를 1회 추가. U-1b 는 기존 'part' 사용(추가 0). 중복 편집 금지.
