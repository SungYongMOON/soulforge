...
추가 확인(2026-06-17, codex_gpt-5.3): 할일_장부는 메일 기반 자동 생성을 위해
검토상태, 라우트 후보/신뢰도, 필요 역할·역량, 제안 담당자, 소스 후보/메일/스레드/그룹,
생성런/규칙, 동기화 상태/오류/리비전/해시/시각 메타데이터를 보존한다. ERP 인입은 신규
행만 자동 추가하고, 기존 `할일키` 의 내용이 달라지면 덮어쓰기 대신 `conflict` 로 표시해
사람 수정 이력을 보호한다.
