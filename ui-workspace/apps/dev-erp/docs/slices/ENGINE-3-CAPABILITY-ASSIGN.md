...
- status: done 2026-07-02 / parallel_group: G-intake-cycle (E1 과 같은 파일 — 직렬) / depends_on: 없음
...
- [x] (확인완료 2026-07-02) person_skill / core_person.capability_label 실데이터 유무:
      runtime DB 에 person_skill 테이블은 있으나 0행, core_person 6행 중 capability_label 채움 0행.
      person_skill 컬럼은 person_id/capability_label/source_ref/weight. 따라서 v1 은 규칙 파일의
      role/capability/suggested_assignee_ref 만 사용하고 개인 매칭·점수화는 하지 않는다.
- [x] (확인완료 2026-07-02) ERP UI 는 이미 `static/app.js` itemAutomationHints 에서
      `suggested_assignee_ref` 를 "추천담당", `required_role|required_capability` 를 "필요" 힌트로
      표시한다. 별도 UI 슬라이스 없이 후보 필드가 보인다.
...
3. 배선: tools/auto_intake_cycle.mjs 분류 결과 candidates 에 프로젝트별 규칙으로 enrich.
   ⚠️ 정정(검증 반영): buildProjectContextLines 는 내부에서 loadContextHintRules 를 호출하지만
   **문자열 라인만 반환하고 규칙 객체는 반환하지 않는다** — enrich 단계는 프로젝트별로
   loadContextHintRules 를 별도 호출해 rules 를 얻은 뒤 enrichCandidateWithRules 를 적용한다.
   (원하면 buildProjectContextLines 시그니처를 {lines, rules} 반환으로 확장해 이중 로드 제거 —
   기존 소비자 파괴 없는 방식으로만.)
```
...
- `loadContextHintRules` 가 기존 branch/priority/keywords 에 더해 id, work_types,
  required_role, required_capability, suggested_assignee_ref 를 반환한다.
- `enrichCandidateWithRules` 는 제목 키워드로 첫 규칙을 골라 빈 제안 필드만 채운다.
  LLM 이 이미 채운 role/assignee/work_type 은 덮지 않고, work_type 은 review 폴백일 때만
  허용목록 값으로 보강한다.
- `runCycle` 은 분류 직후 프로젝트별 rules 로 candidates 를 보강한 뒤 ledger writer 에 넘기고,
  summary.capability_assign.enriched 로 보강 건수를 보고한다.
- 확정 담당 `assignee_ref` 는 생성하지 않는다. 제안 필드만 사용한다.
