...
- [x] project_context CSV v1 소비자 전수(그래프 탭·branch_summaries 소비 지점) — v2 컬럼 추가가
      깨뜨리는 reader 유무. (표면 스레드 메모: 그래프 탭은 branch_summaries.csv + sources.csv 읽음)
- [x] confirm된 할일의 anchor 충족률(anchor 없는 open 할일 비율) — 골격 소속 불가분의 크기.
- [x] "실무협의" 정규화 제목의 실제 주기(월/주) 표본 — 이력줄기 감지 파라미터 검증.

## 구현 결과 (2026-07-05)

- `tools/haengbogwan_project_context.mjs`가 additive v2 메타데이터를 쓴다:
  `project_context/branches.csv`, `project_context/occurrences.csv`,
  `sources.csv.branch_ref`, `sources.csv.suggested_branch_ref`,
  `summaries/branch_summaries.csv`의 v2 컬럼.
- `review_status=approved` 또는 명시 confirm 신호가 있는 task 메타데이터는
  `branch_kind=work`, `anchor_ref=item:<id>` 작업줄기가 된다. 완료 상태 task는
  `status=closed`, `closed_at`으로 닫힌다. 명시 skeleton anchor는
  `branch_kind=skeleton`으로 기록된다.
- 정규화 제목이 8주 창 안에서 3회 이상 반복되는 mail은
  `branch_kind=history`, `status=proposed` 이력줄기와 `occurrences.csv` 회차로 기록된다.
  이는 사람 확정 전 제안 표면이며 자동 확정이 아니다.
- anchor 없는 mail은 제목 조각 branch를 새로 만들지 않는다. source에는 빈 `branch_ref`와,
  있을 경우 약한 `suggested_branch_ref`만 남긴다.
- `tools/haengbogwan_run.mjs`는 승인 task/anchor 메타를 project_context로 전달하고,
  `src/context_graph.mjs`는 v1 파일을 허용하면서 v2 branch metadata와 occurrence를 노출한다.
  `tools/auto_intake_cycle.mjs`는 `branch_summaries.csv`를 header 기반으로 읽어 additive column에
  깨지지 않는다.
- 회귀 검증:
  `node --test test/haengbogwan_project_context.test.mjs`,
  `node --test test/haengbogwan_run.test.mjs`,
  `node --test test/auto_intake_cycle.test.mjs`,
  `node --test --test-name-pattern CTX-GRAPH test/core.test.mjs`.
