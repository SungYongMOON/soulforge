...
| K-2 | E4 팔로업 기본 정책(무응답 판정 N일, 대상 메일함) | **✅ 확정됨 (2026-07-03 owner)** — 무응답 3일(달력일), 기본 담당=원 발신자 제안(확정 아님), 대상=수집 전체(기본). **보강(2026-07-03 owner): 이미 할일로 변환된 발신 메일도 추적 포함 — 기존 할일에 followup_due 부착 방식, 후속 패킷 대기.** 상세: ENGINE-4 패킷 "owner 결정 기록" 절 |
