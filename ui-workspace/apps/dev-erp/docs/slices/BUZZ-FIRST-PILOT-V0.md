...
2. 봇이 확인 질문을 실제로 전달하면 작업대에 답변 대기가 표시된다. 대기 시작 시각과
   ...
   응답 대기함에서 읽음·30분/2시간/하루 미루기를 관리할 수 있다. 이는 보기 설정이며
   봇을 일시정지하거나 답변을 대신 보내는 기능이 아니다.
...
`buzz_pilot_owner_attention.mjs`는 같은 읽기 포트의 질문·답변 근거를 기존 응답 대기함에
연결한다. 질문의 고정 ref와 binding으로 보기 상태를 식별하여 새로 고침·재시작 뒤에도
읽음·미루기를 유지한다. 실제 Owner 답변 관측으로 대기를 해제하며, 만료·수집 불명은
답변이나 완료로 바꾸지 않는다. 원문은 보호 reader에서 화면으로만 전달하고 보기 DB에는
식별 해시와 선호만 남긴다. 이미 기록된 Buzz 질문 전달과 별도 알림 기능을 구분한다.
