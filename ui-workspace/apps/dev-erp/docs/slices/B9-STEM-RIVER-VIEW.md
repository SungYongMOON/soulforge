...
2. B9b 강줄기 렌더 — 방사형 → 가로 시간축: 기둥 위 게이트 큰 점, 가지가 born_at 좌표에서 갈라져 closed_at 에서 닫힘, 이력줄기는 나란한 점선 레일(회차=점). (B8 방사형은 조감 토글 존치 여부 owner 결정. 참고: 작업가지의 기둥 좌표 = item anchor_stage_code, 이력가지 = 레일이므로 단계 좌표 불요)
