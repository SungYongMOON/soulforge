...
- Tailscale HTTPS 운영이면 작업 스케줄러 → 작업 만들기 → 트리거 "시스템 시작 시" →
  기본 core-only 동작은 `powershell.exe -NoProfile -ExecutionPolicy Bypass -File
  <runtime-checkout>\ui-workspace\apps\dev-erp\ops\run-dev-erp-background.ps1
  -SecureCookie` 실행 →
  "작업이 실패하면 다시 시작" 옵션. 이 launcher는 기본 `127.0.0.1`, 외부 통합 off이고,
  정확한 `node.exe` + 절대 runtime `server.mjs` + 전체 argv가 일치하는 listener만 교체한다.
  미식별 port owner는 종료하지 않고 시작을 차단한다. 등록 전 같은 명령에 `-DryRun`을
  붙여 점검한다. 실제 작업 스케줄러 변경은 별도 owner 승인 작업이다.
  로그인 세션 의존도는 낮게 "사용자 로그온 여부 관계없이 실행"으로 둔다.
- LAN, 로컬 LLM, 메일 수집/자동 인입, autosync, morning brief, dedicated Codex worker는
  각각 launcher의 명시적 opt-in switch가 필요하다. 전체 목록과 필수 인자는
  [`RUNTIME_MAINTENANCE_RUNBOOK_20260618.md`](RUNTIME_MAINTENANCE_RUNBOOK_20260618.md)의
  `Background launcher safe default` 절을 따른다. worker switch는 worker-first readiness나
  release audit를 대신하지 않는다.
- 작업 스케줄러로 direct LAN HTTPS를 유지할 때는 Action의 launcher 인자에 `-ListenOnLan`,
  `-TlsCertPath`, `-TlsKeyPath`, 필요하면 `-TlsCaPath`의 **절대 경로**를 그대로 보존한다.
  실행 계정에 인증서/CA read 권한과 제한된 private-key read 권한이 있는지 확인하고, key는
  tracked checkout에 복사하지 않는다. 등록 전과 경로/ACL 변경 후 같은 인자에 `-DryRun`을
  붙여 `tls=explicit`을 다시 확인한다.
