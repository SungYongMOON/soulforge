...
- **마이크(음성 입력) 조건**: 브라우저는 마이크를 보안 컨텍스트(HTTPS 또는 localhost)에서만
  허용한다 — `http://<IP>:4300` 접속에서는 🎤 버튼이 비활성(사유 툴팁)으로 표시된다.
  해결 ①(파일럿, 접속 PC별 1회): Chrome 주소창 `chrome://flags/#unsafely-treat-insecure-origin-as-secure`
  → 텍스트박스에 `http://<서버IP>:4300` 입력 → Enabled → 브라우저 재시작(Edge 는 edge://flags 동일).
  해결 ②(정석): Tailscale HTTPS 경로로 접속. 음성 인식 자체는 브라우저 내장(벤더 서버 처리
  가능)이므로 민감 내용 구두 입력 주의 고지는 버튼 툴팁에 포함되어 있다.
