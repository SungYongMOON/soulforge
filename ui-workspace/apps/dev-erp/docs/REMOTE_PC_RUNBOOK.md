...
> **2026-08-14 정책 갱신:** ERP의 로컬/LAN LLM 실험은 중지되었다. ERP는
> `stub`/비모델 경로만 사용한다. 과거 `qwen3.5:9b` RAG 후보도 현재 M2에서는
> 선택·활성화되지 않으며, M2-0~M2-2는 모델 호출 없이 진행한다.
...
ERP만 실행 중이면 모델 목록이 비어 있어야 한다. 현재 M2 작업에서
`qwen3.5:9b` 또는 다른 모델을 적재하지 않는다. 향후 선택적 RAG/LLM 활성화는
별도 모델 비교와 Owner 결정 이후의 새 절차로만 수행한다.
