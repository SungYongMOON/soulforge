...
**Row2(2분류) 배치 실측·교정 (owner: "본문과 2번행 사이에 올라탄, 맨 왼쪽부터 안 채워짐")**
- ecount 실측(vw=1920): L2 띠 UL `x=109 · w=548`(탭 폭만, 전폭 아님), 사이드바(견적서 `x=7,y=98,w=200`)보다 **위**, 본문 위에 떠 있음.
- dev-erp 교정: grid `"side subbar" / "side main"` 으로 변경 → 2분류를 **본문(main) 열**에 배치(사이드바 다음 x에서 시작), 배경 투명, 전폭 채움 제거. 검증: sub-bar `x=200(사이드바 뒤)·전폭 아님·투명`, 첫 탭 `x=216`.
