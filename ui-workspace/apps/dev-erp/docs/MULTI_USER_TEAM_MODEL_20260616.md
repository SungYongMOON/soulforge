...
- 로그인한 사용자는 `POST /api/auth/password` 로 현재 비밀번호 확인 후 본인 비밀번호를 변경할 수 있다.
  임시 비밀번호로 온보딩한 뒤 팀원이 직접 비밀번호를 넘겨받는 흐름을 지원한다.

## 2. 관리자 모드

- `admin` 역할 보유 계정. `isAdmin()` 게이트로 계정 관리 surface 접근.
- 관리자만: 계정 생성/수정(이메일·표기명·역할)/비활성, 전체 계정·전체 보기범위 열람.
- 관리자만: `POST /api/accounts/password` 로 팀원 비밀번호를 새 임시 비밀번호로 초기화할 수 있다.
  비밀번호 평문은 저장하지 않고 해시만 갱신하며, 타 계정 초기화 시 기존 세션은 무효화된다.
...
4. **표시(ERP)**: `scan_mail_ledger.mjs`/`ingestMail` 이 `메일함` 값을 `core_mail.mailbox` 로 보존한다.
   ...
   `mailbox` 는 계정 이메일 exact 값뿐 아니라 `계정이메일/받은 편지함`, `계정이메일\보낸 편지함`
   같은 하위 폴더 prefix 도 같은 계정 범위로 본다.
...
- 운영 형태: 회사 PC 한 대를 신뢰된 호스트로 두고, 팀원들은 그 호스트의 dev-ERP에 접속한다.
- 기본은 `127.0.0.1`(안전). 팀 공유는 회사 PC에서 `--host 0.0.0.0`(사내 LAN).
- 메일 ID/비밀번호/토큰은 회사 PC의 비공개 env 파일에 저장한다. ERP DB에는 계정 이메일과
  `mailbox_env_ref` 상대 포인터만 저장하고, secret 값은 저장하지 않는다.
- 신뢰된 사내 LAN 내부에서만 직접 HTTP 파일럿을 할 때는 `node server.mjs --host 0.0.0.0` 로 실행한다.
- HTTPS reverse proxy/tunnel 뒤에서는 `DEV_ERP_COOKIE_SECURE=1 node server.mjs --host 127.0.0.1`
  로 실행한다. HTTP 직접 LAN에서는 `DEV_ERP_COOKIE_SECURE=1` 을 켜지 않는다.
- 신뢰된 사내 LAN을 벗어나거나 휴대폰/외부망/넓은 공유로 접속한다면 HTTPS reverse proxy/tunnel 뒤에서
  공개하고 `DEV_ERP_COOKIE_SECURE=1` 또는 `--secure-cookie` 로 세션 쿠키 Secure 옵션을 켠다.
- 회사 PC 최초 릴리즈는 개발 checkout 이 아니라 `<runtime-checkout>` runtime clone 에서 실행한다.
  운영 authority, Tailscale HTTPS, owner mail secret 경계는
  [RUNTIME_OPERATING_CONTRACT_20260617.md](RUNTIME_OPERATING_CONTRACT_20260617.md)를 따른다.
- 첫 관리자 생성 뒤 자가 가입은 기본 차단이다. 팀 운영 계정은 관리자 패널 또는
  `dev-erp:import-team-roster` 로 만든다. localhost 파일럿에서만 `DEV_ERP_ALLOW_SELF_REGISTER=1`
  또는 `--allow-self-register` 를 사용한다.
...
- 초대코드/승인 기반 가입 흐름. 현재는 첫 관리자 bootstrap 이후 자가 가입 기본 차단,
  관리자 생성 또는 roster import 를 사용한다.
- 실제 운영 등록부 작성은 secret 값을 보지 않는 owner/operator 절차가 필요하다. agent 는 env 파일 경로와
  필요한 변수명을 안내할 수 있지만 값은 열람하지 않는다.

## 8. 내일 사용 전 체크리스트

1. 회사 PC 호스트에 팀원 메일 credential env 파일을 준비한다. 이 파일들은 public repo에 넣지 않고,
   실제 값은 operator 가 직접 입력한다.
2. private roster(JSON/CSV)를 준비한다. 임시 비밀번호가 들어가므로 public repo에 두지 말고,
   import 후 삭제하거나 회사 PC의 ignored/private 위치에만 보관한다.
3. roster 필드: `username`, `display_name`, `email`, 임시 `password`,
   `mailbox_provider`, `mailbox_enabled`, `mailbox_env_ref` 를 담는다.
4. `dev-erp:import-team-roster` dry-run 으로 생성/수정 계획을 확인하고, 문제가 없으면 `--apply` 로 반영한다.
5. 팀원에게 임시 비밀번호를 전달하고, 첫 로그인 후 **비밀번호 변경**으로 본인 비밀번호를 직접 설정하게 함.
6. 팀원이 다른 PC/휴대폰에서 접속한다면 HTTPS reverse proxy/tunnel 뒤에서만 공개하고
   `DEV_ERP_COOKIE_SECURE=1` 또는 `--secure-cookie` 로 세션 쿠키 Secure 옵션을 켬. 직접 LAN HTTP면 켜지 않음.
7. `dev-erp:export-team-mailboxes` 로 `team_mailboxes.json` 생성.
8. `dev-erp:team-preflight` 로 회사 PC 호스트 기준 설정 점검을 실행한다. 이 점검은 DB·계정·메일함
   포인터·메일함 등록부·env 파일 존재 여부만 확인한다. SQLite는 read-only/query-only로 열어
   스키마를 초기화하거나 마이그레이션하지 않으며, credential env 파일 내용도 읽지 않는다.
   메일 수집 전 첫 실행은 `configuration_ready` 를 확인하는 용도다.
9. `guild-hall:gateway:fetch:team -- --once --dry-run --json` 으로 등록부/수집 경로 점검.
10. `guild-hall:gateway:fetch:team -- --once --json` 으로 실제 팀 메일 수집을 1회 실행.
11. `dev-erp:team-preflight` 를 다시 실행해 `team_use_ready` 가 true 인지 확인.
12. 실제 수집 후 `scan-mail-ledger`, `mail-to-task-ledger`, `task-ledger` 순서로 메일 장부와 할일 장부를 반영.
13. 관리자 패널의 **팀 사용 준비** 영역 또는 `GET /api/accounts/readiness` 에서 활성 관리자,
   활성 팀원 수, 팀원별 이메일/메일함/env ref, 최근 수집시각, 메일 원장 건수, 미분류 큐 경고를 확인.
   차단 사유가 남아 있으면 팀 공개 전 먼저 보정.
14. 팀원 계정으로 로그인해 메일/검색/대시보드/최근 이벤트가 본인 범위만 보이는지 확인.

메일 credential env 예시(값은 operator 가 회사 PC에서 직접 입력):

```bash
# private-state/mail/team/alice.env
EMAIL_FETCH_SOURCE_GMAIL_ENABLED=true
EMAIL_FETCH_SOURCE_HIWORKS_ENABLED=false
GMAIL_TOKEN_URI=https://oauth2.googleapis.com/token
GMAIL_ACCESS_TOKEN_FILE=./alice_gmail_token.json
EMAIL_FETCH_GMAIL_INITIAL_DAYS=3
```

```bash
# private-state/mail/team/bob.env
EMAIL_FETCH_SOURCE_GMAIL_ENABLED=false
EMAIL_FETCH_SOURCE_HIWORKS_ENABLED=true
HIWORKS_POP3_HOST=pop3.mail.example.com
HIWORKS_POP3_USERNAME=bob@example.com
HIWORKS_POP3_PASSWORD_FILE=./bob_hiworks_password.txt
```

roster 예시(실제 파일에는 임시 비밀번호가 들어가므로 public repo에 두지 않음):
  ...
  "accounts": [
    {
      ...
      "password": "<operator-input-temp-password>",
      "mailbox_provider": "gmail",
      "mailbox_enabled": true,
      "mailbox_env_ref": "private-state/mail/team/alice.env"
    }
  ]
}
```
...
`P00-000_INBOX` 는 미배정 인박스 예시다. 프로젝트별 메일 장부에 대해 할일을 만들 때는
