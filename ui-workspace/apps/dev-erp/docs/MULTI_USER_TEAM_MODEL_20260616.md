...
- 직접 사내 LAN HTTP 파일럿은 `node server.mjs --host 0.0.0.0` 로 실행한다.
...
- 팀원이 다른 PC/휴대폰에서 접속한다면 HTTPS reverse proxy/tunnel 뒤에서 공개하고
  `DEV_ERP_COOKIE_SECURE=1` 또는 `--secure-cookie` 로 세션 쿠키 Secure 옵션을 켠다.
...
8. `guild-hall:gateway:fetch:team -- --once --dry-run --json` 으로 등록부/수집 경로 점검.
9. 실제 수집 후 `scan-mail-ledger`, `mail-to-task-ledger`, `task-ledger` 순서로 메일 장부와 할일 장부를 반영.
10. 관리자 패널의 **팀 사용 준비** 영역 또는 `GET /api/accounts/readiness` 에서 활성 관리자,
   활성 팀원 수, 팀원별 이메일/메일함/env ref, 최근 수집시각, 메일 원장 건수, 미분류 큐 경고를 확인.
   차단 사유가 남아 있으면 팀 공개 전 먼저 보정.
11. 팀원 계정으로 로그인해 메일/검색/대시보드/최근 이벤트가 본인 범위만 보이는지 확인.
