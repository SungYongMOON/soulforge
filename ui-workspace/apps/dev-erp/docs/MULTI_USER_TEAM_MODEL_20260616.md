...
- 초대코드/승인 기반 가입 제한 흐름. 현재는 관리자 생성, bootstrap, 또는 localhost 파일럿용 자가 가입을 사용한다.
...
2. 팀원에게 임시 비밀번호를 전달하고, 첫 로그인 후 **비밀번호 변경**으로 본인 비밀번호를 직접 설정하게 함.
3. 팀원이 다른 PC/휴대폰에서 접속한다면 HTTPS reverse proxy/tunnel 뒤에서만 공개하고
   `DEV_ERP_COOKIE_SECURE=1` 또는 `--secure-cookie` 로 세션 쿠키 Secure 옵션을 켬.
4. 각 계정의 메일함 provider/enabled/env ref 저장. env ref 는 상대경로 포인터만, secret 값은 입력하지 않음.
5. `dev-erp:export-team-mailboxes` 로 `team_mailboxes.json` 생성.
6. `guild-hall:gateway:fetch:team -- --once --dry-run --json` 으로 등록부/수집 경로 점검.
7. 실제 수집 후 `scan_mail_ledger.mjs`, `mail_to_task_ledger.mjs`, `task_ledger.mjs --apply` 순서로 메일 장부와 할일 장부를 반영.
8. 관리자 패널의 **팀 사용 준비** 영역 또는 `GET /api/accounts/readiness` 에서 활성 관리자,
   활성 팀원 수, 팀원별 이메일/메일함/env ref, 최근 수집시각, 메일 원장 건수, 미분류 큐 경고를 확인.
   차단 사유가 남아 있으면 팀 공개 전 먼저 보정.
9. 팀원 계정으로 로그인해 메일/검색/대시보드/최근 이벤트가 본인 범위만 보이는지 확인.
