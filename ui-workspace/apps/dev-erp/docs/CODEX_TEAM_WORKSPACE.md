...
## Personal MCP 후속 목표의 세 경계

다음은 계획 승인용 `TARGET`이며 현재 personal MCP 구현 완료 주장이 아니다.

1. **Ingress/custody**: 로컬 파일은 pointer/hash가 기본이다. Raw upload를 하면 중앙 service inbox가
   새 storage binding의 custody를 갖지만, project output·ArtifactRevision·knowledge로의 승격은 별도
   promoter authority와 receipt가 필요하다. Source writer, promoter, HPP history projector, ERP task
   writer는 서로의 권한을 빌리지 않는다.
2. **WorkSession lifecycle**: 팀원은 exact ERP assignment를 받아 personal session을 start/bind하고 여러
   checkpoint와 terminal closeout을 보낸다. 기본 cardinality는 `{assignment epoch,account}`당 active
   primary 하나다. Closeout과 completion proposal은 ERP 공식 완료가 아니며 task row를 바꾸지 않는다.
   현행 `erp_publish_work_session` one-shot record를 이 lifecycle로 소급 해석하지 않는다.
...
```mermaid
flowchart LR
  PC["팀원 PC/Codex"] -->|"session event·file upload"| MCP["personal ERP MCP"]
  MCP -->|"durable receipt"| PC
  MCP --> ERP["ERP task/session/query service"]
  ERP --> Q["accepted generation"]
  Q -->|"explicit project 또는 common"| PC
  MCP -. "closeout/proposal은 task status write 0" .-> ERP
```

Client-local outbox는 durable server receipt를 검증하기 전 지우지 않는다. Server-side
missing-closeout candidate는 accepted start가 있는 session에서만 계산하며 client-local pending outbox와
같다고 추정하지 않는다. Exact personal thread-ref capability, node registration, outbox path/fsync/encryption/
retention과 stale SLA는 `VERIFY_HP`; 실제 팀 rollout은 별도 owner 승인 전 `OFF`다.
...
10. Personal query는 explicit account/project/common grant와 accepted generation에 고정한다. Project가
    없거나 권한이 없을 때 다른 project/common으로 fallback하지 않고 존재 정보를 노출하지 않는다.
11. WorkSession/closeout/knowledge candidate는 중앙 worker의 thread binding, ERP task completion,
    Wiki/RAG/canon authority를 자동 상속하지 않는다.
12. Upload completion 뒤 project promotion이 필요하면 inbox path를 client에 노출하거나 직접 이동하지
    않고 approved promoter가 opaque storage binding/ref와 rollback receipt로 처리한다.
