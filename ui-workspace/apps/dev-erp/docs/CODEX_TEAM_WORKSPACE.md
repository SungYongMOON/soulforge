...
ERP runtime은 업무 원문을 소유하지 않는다. runtime-local 등록부가 `workspace_id`를
팀 PC의 승인된 로컬/UNC 폴더에 연결하고, ERP 할일의 Codex 스레드는 처음 선택한
`workspace_id + workspace revision + root fingerprint`에 고정된다.

## 파일의 실제 위치

- ERP 서버 코드는 별도 runtime checkout에 있다.
- 업무 원문과 산출물은 Soulforge `_workspaces/<과제>/...` 또는 owner-approved
  팀 공유 폴더에 남는다.
...
3. 공유 폴더가 offline이거나 mapping이 바뀌면 다른 폴더로 fallback하지 않고
   중단한다.
4. 기본 sandbox는 read-only다. 기존 `danger-full-access` API는 비활성화한다.
5. 쓰기는 등록부의 `allowed_write_prefixes`와 OS ACL 안에서만 관리자 승인,
   할일/과제/작업실/기존 상대 하위 폴더에 고정되며 최대 8시간 후 만료한다.
   turn cwd는 항상 workspace root로 유지하고 승인된 하위 폴더만 writable root로
   추가한다. turn 제한시간은 가장 먼저 끝나는 승인 시각을 넘지 않으며, 수동 철회는
   그 승인을 사용 중인 Codex 프로세스도 중단한다.
6. read-only와 workspace-write 모두 Codex sandbox의 network access는 끈다.
...
`private-state`, 개인 프로필과 다른 과제 비밀영역은 OS ACL로 거부한다. worker에는
Soulforge의 service-owned attachment root 읽기만 허용하고 쓰기는 허용하지 않는다. 서로 다른
기밀영역은 별도 worker/등록부로 분리한다. 계정/SMB secret은 사용자가 Windows에
직접 설정하며 ERP JSON이나 Git에 복사하지 않는다.

worker가 시작하는 각 Codex app-server는 `dev_erp_bounded` named permission profile을
강제한다. 이 profile은 전체 디스크를 기본 거부하고, 최소 도구 경로와 선택한 workspace를
읽기, 승인된 기존 하위폴더만 쓰기, 검증된 첨부 경로만 읽기로 다시 연다. network는
비활성화한다. app-server가 돌려준 `activePermissionProfile`, `runtimeWorkspaceRoots`,
빈 `instructionSources`를 매 turn 확인한다. worker 시작 시 exact-path probe v3는
workspace 읽기, 승인 `Output` 쓰기, 비승인 workspace 쓰기 거부, exact attachment
파일 읽기, 같은 폴더 sibling 읽기·parent 열거·attachment 쓰기 거부, root 밖 읽기·쓰기
거부뿐 아니라 workspace/Output junction, hardlink 읽기, attachment 삭제·이동 거부를
실제 실행한다. Probe가 통과한 Codex runtime의 aggregate SHA-256은 owner 기대값에
고정되고, health/attestation/model discovery/turn 전후에 다시 계산한다. 이 probe가 실패하거나
실행 파일이 바뀌면
`worker_permission_boundary_unproven`으로 기동을 중단하고 release audit도 차단한다.
현재 개발 PC의 Codex 0.144.1 native Windows sandbox는 이 root 밖 읽기 거부 probe를
통과하지 못했으므로, 실제 팀 PC와 실제 UNC mapping의 worker-identity sentinel preflight가
모두 통과하기 전에는 production 배포하지 않는다. WSL/container는 현재 구현된 대안이
아니며 별도 worker permission profile과 검증이 필요한 향후 후보다.

등록된 root는 Codex가 전체를 읽을 수 있는 sanitized 전용 share로 취급한다. metadata-only
preflight는 보호 이름, symlink/junction, hardlink와 filesystem object 교체를 차단하지만
scan 직후 다른 계정이 share를 바꾸는 race 자체를 없애지는 못한다. 운영에서는 turn 동안
다른 identity의 변경을 막는 SMB/NTFS ACL 또는 immutable snapshot projection 영수증이
필수다. 각 실제 UNC mapping마다 harmless sentinel로 root read, 승인 Output write,
sibling/outside/parent 열거 거부를 worker identity에서 별도로 증명한다.
...
별도 process, registry revision, 보호 root, exact-path permission profile,
실제 root 밖 read/write 거부 probe,
`app-server` mode가 모두 맞지 않으면 차단한다.
UNC receipt는 raw path 없이 registry revision, worker identity, probe v3, server-side share
...
기존 inline message와 workspace 증명이 불완전한 binding은 owner mapping을 만든 뒤
반드시 dry-run부터 실행한다. payload root는 Soulforge의 ERP 전용 영역이다.
...
`--apply`는 restore-verified coherent backup 뒤에만 실행하고, dry-run이 모든 legacy
row에 정확히 bind 또는 retire 결정을 가진다고 보고할 때만 허용한다.
...
- worker ACL이 ERP DB/mail secret/private-state 및 다른 trust domain을 거부하고 service-owned attachment root는 읽기만 허용하는가
- 전용 home에 hooks/plugins/marketplaces/MCP connector가 없는가
- 전용 home이 source/runtime/team workspace 밖에 있고 `config.toml`이 없으며 exact-path probe v3가 `proven:true`인가
