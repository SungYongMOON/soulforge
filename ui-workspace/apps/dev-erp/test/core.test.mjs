...
import { buildMetaContext, contextToText, erpLlmProvider, runLlm } from "../src/llm.mjs";
...
test("ERP 모델 정책: provider 환경변수 override도 stub으로 닫힘", () => {
  assert.equal(erpLlmProvider({ ERP_CHAT_PROVIDER: "ollama" }), "stub");
});
...
test("챗봇 UI: ERP와 챗봇 릴리즈 번호를 별도 컴포넌트 버전으로 표시한다", () => {
  ...
  assert.match(server, /thinking: false/);
...
test("docs: 릴리즈 런북은 /api/version과 4300/4310 포트 경계를 안내한다", () => {
  ...
  assert.match(chatbotDoc, /dev-ERP/);
  assert.match(chatbotDoc, /stub/);
  assert.match(chatbotDoc, /none/);
  assert.match(chatbotDoc, /qwen3\.5:9b/);
  assert.match(chatbotDoc, /ollama stop qwen3\.5:9b/);
...
test("server: 운영 4300은 runtime checkout 전용이고 개발 기본 포트는 4310이다", () => {
  ...
  assert.match(watchdog, /\[ValidateSet\("stub"\)\]/);
...
test("TEAM-ACCT: teamReadiness 가 팀 사용 전 계정·메일함·장부 누락을 집계", () => {
  ...
  assert.ok(ready.next_actions.some((x) => x.code === "configure_member_mailboxes" && x.actual === 1 && x.expected === 3), "메일함 설정 다음 행동");
...
test("D2/S8-2: nudges — 마감 미지정 미착수는 no_due 사유(오늘마감 아래·일반 위) + 신규 우선", () => {
  const store = freshStore();
  store.upsertProject({ id: "P99-300", title: "노출 레인 검증", data_label: "synthetic" });
  const today = new Date().toISOString().slice(0, 10);
  store.upsertItem({ id: "nd1", project_id: "P99-300", title: "오늘 마감", assignee_ref: "김민재", status: "open", due: today, data_label: "synthetic" });
  store.upsertItem({ id: "nd2", project_id: "P99-300", title: "먼저 온 미지정", assignee_ref: "김민재", status: "open", data_label: "synthetic" });
  store.upsertItem({ id: "nd3", project_id: "P99-300", title: "나중 온 미지정", assignee_ref: "김민재", status: "open", data_label: "synthetic" });
  store.upsertItem({ id: "nd4", project_id: "P99-300", title: "먼 마감 일반", assignee_ref: "김민재", status: "open", due: "2099-01-01", data_label: "synthetic" });
  store.upsertItem({ id: "nd5", project_id: "P99-300", title: "미지정이지만 시작함", assignee_ref: "김민재", status: "doing", data_label: "synthetic" });
  store.db.prepare("UPDATE core_item SET created_at=? WHERE id=?").run("2026-07-10T00:00:00.000Z", "nd2");
  store.db.prepare("UPDATE core_item SET created_at=? WHERE id=?").run("2026-07-11T00:00:00.000Z", "nd3");

  const ns = store.nudges({ person: "김민재", limit: 10 });
  const reasons = Object.fromEntries(ns.map((n) => [n.id, n.reason]));
  assert.equal(reasons.nd1, "due_today");
  assert.equal(reasons.nd2, "no_due");
  assert.equal(reasons.nd3, "no_due");
  assert.equal(reasons.nd4, "open");
  assert.equal(reasons.nd5, "open", "시작한 건은 미지정이어도 no_due 아님(미착수 한정)");
  const order = ns.map((n) => n.id);
  assert.ok(order.indexOf("nd1") < order.indexOf("nd3"), "오늘 마감이 no_due 위");
  assert.ok(order.indexOf("nd3") < order.indexOf("nd2"), "no_due 는 신규 우선(created_at desc)");
  assert.ok(order.indexOf("nd2") < order.indexOf("nd4"), "no_due 가 일반 open 위");

  // mine 위젯 노출 레인 + nudges 위젯 라벨 배선(소스 계약)
  const app = readFileSync(join(APP_DIR, "static", "app.js"), "utf8");
  assert.match(app, /mine_nodue_head/);
  assert.match(app, /no_due: L\.nudge_no_due/);
});

test("D1/S7-1: 보관=치움 — 보관된 승격 항목은 재승격을 막지 않는다(활성 중복은 차단)", () => {
  const store = freshStore();
  store.upsertProject({ id: "P99-400", title: "재승격 검증", data_label: "synthetic" });
  store.ingestMail({ id: "mailD1", project_code: "P99-400", at: "2026-07-11T09:00:00+09:00", subject: "재승격 대상", data_label: "synthetic" });
  const p1 = store.promoteMail("mailD1", "tester");
  assert.equal(p1.ok, true);
  store.archiveItem(p1.item.id);
  assert.equal(store.promotedMailIds().includes("mailD1"), false, "보관 후 메일함 ✓ 해제(기존 동작)");
  const p2 = store.promoteMail("mailD1", "tester");
  assert.equal(p2.ok, true, "보관 후 재승격 허용(owner A안 2026-07-11) — 교착 해소");
  assert.notEqual(p2.item.id, p1.item.id);
  assert.equal(store.promoteMail("mailD1", "tester").error, "already_promoted", "활성 중복 승격은 여전히 차단(코드 dedup)");
  // 명시적 복원은 허용 — 활성 2개 공존은 행보관 split 계약(메일 1→할일 N, TASK-LEDGER)과 일관.
  const r = store.restoreItem(p1.item.id);
  assert.equal(r.ok, true);
