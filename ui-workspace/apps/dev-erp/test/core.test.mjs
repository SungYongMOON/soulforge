...
test("챗봇 UI: ERP와 챗봇 릴리즈 번호를 별도 컴포넌트 버전으로 표시한다", () => {
  ...
  assert.match(app, /const ERP_RELEASE_VERSION = "v1\.0\.2"/);
  assert.match(app, /const ERP_UI_VERSION = "ui-2026\.06\.18-version-split\.8"/);
  assert.match(app, /const ERP_CHATBOT_RELEASE_VERSION = "v1\.1\.0"/);
  assert.match(app, /const ERP_CHATBOT_UI_VERSION = "chatbot-2026\.06\.18-version-split\.8"/);
  ...
  assert.match(app, /\$\{esc\(state\.lex\.chat_version_label\)\} \$\{ERP_CHATBOT_RELEASE_VERSION\}/);
