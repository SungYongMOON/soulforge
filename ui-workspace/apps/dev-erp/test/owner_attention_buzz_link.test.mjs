...
const syntheticFileUrl = pathToFileURL(join(tmpdir(), 'synthetic-unopened-file')).href;
...
test('credentials, unknown arguments, invalid identifiers and path normalization cannot widen a Buzz link', () => {
  for (const value of [null, '', `buzz://other/${channel}`, `buzz://channel/${channel}?other=1`,
    ...
    'javascript:alert(1)', syntheticFileUrl, 'http://public.example.invalid/', 'https://user:secret@example.invalid/']) assert.equal(safeOwnerAttentionBuzzUrl(value), null, String(value));
});
