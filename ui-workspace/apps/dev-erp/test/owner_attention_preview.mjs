...
f.publish({client_session_ref:`oa1:review_scope:1:${Math.floor((f.now()-60000)/1000)}`,
  summary:'외부 공유할 수신 범위에 추가 업체를 포함해도 되는지 확인해 주세요.',
  next_actions:['허용할 외부 수신 범위를 알려 주세요.'],
  stop_conditions:['추가 업체로의 반출만 보류합니다. 내부 작업은 계속합니다.']});
let controller;
...
const server=createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&url.pathname==='/synthetic/buzz'){res.setHeader('Content-Type','text/html; charset=utf-8');res.end(shell('<h1>합성 Buzz 대화</h1><p>이 페이지는 실제 Buzz가 아닙니다. 응답 기록만 검증하며 실제 외부 반출을 허가하거나 실행하지 않습니다.</p><form method="POST" action="/synthetic/respond"><button>합성 공개 범위로 답변 기록</button></form><a href="/owner-attention.html">응답 대기함</a>'));return;}
  if(req.method==='POST'&&url.pathname==='/synthetic/respond'){
    ...
    f.publish({request_kind:'owner_attention/response',summary:'합성 공개 범위로 진행하는 답변 예시입니다. 실제 반출은 없습니다.',outputs:[`owner-request:${original.work_session_id}`]},f.owner);
...
process.stdin.setEncoding('utf8');process.stdin.on('data',chunk=>{
  for(const line of chunk.trim().split(/\r?\n/u)){
    ...
    else if(line==='revision')f.publish({client_session_ref:`oa1:review_document:${++count+1}:none`,summary:'새 외부 제출 판본의 공개 범위를 확인해 주세요.'});
