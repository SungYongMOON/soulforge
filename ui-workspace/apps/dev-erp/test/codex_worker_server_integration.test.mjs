...
async function waitForWorkerHealth(url, child, stderr) {
  ...
  for (let attempt = 0; attempt < 300; attempt += 1) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        lastBoundary = health?.attestation?.codex_execution_boundary ?? null;
        if (health?.attestation?.codex_worker_ready === true) return health;
      }
    } catch {}
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
  throw new Error(`worker_not_ready:${lastBoundary ?? "unknown"}:${stderr()}`);
}
...
async function waitForProjectionManifest(projectionRoot) {
  for (let attempt = 0; attempt < 500; attempt += 1) {
    try {
      const projects = readdirSync(projectionRoot, { withFileTypes: true })
        .filter((entry) => entry.isDirectory() && entry.name !== ".projection-operation-lock");
      if (projects.length === 1) {
        const projectRoot = join(projectionRoot, projects[0].name);
        const revisions = readdirSync(projectRoot, { withFileTypes: true })
...
test("ERP delegates model, workspace, attachment, and turn execution to the attested dedicated worker", async () => {
  ...
  for (const directory of [
    workerHome, workspaceRoot, join(workspaceRoot, "Output"), attachmentRoot, messageRoot,
    turnProjectionRoot, dirname(dbPath), workerKeyOwner, registryOwner,
  ]) {
    mkdirSync(directory, { recursive: true });
  }
  assert.deepEqual(readdirSync(workspaceOwnerRoot), ["dev-erp"]);
  assert.deepEqual(readdirSync(payloadOwner).sort(), ["codex-message-payloads", "codex-task-attachments"]);
  ...
  const envKeys = [
    ...
    "DEV_ERP_CODEX_TURN_PROJECTION_ROOT",
  ...
  try {
    Object.assign(process.env, {
      ...
      DEV_ERP_CODEX_TURN_PROJECTION_ROOT: turnProjectionRoot,
    ...
    const health = await waitForWorkerHealth(`${base}/api/health`, erp, () => stderrText);
    ...
    assert.equal(health.attestation.codex_worker_payload_deny_binding_match, true);
    ...
    assert.deepEqual(readdirSync(turnProjectionRoot), []);

    response = await request("/api/codex-task/write-grant", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        item_id: item.id,
        relative_prefix: "Output",
        reason: "The first worker slice must reject this grant.",
        ttl_minutes: 60,
      }),
    });
    assert.equal(response.status, 409);
    assert.deepEqual(await response.json(), { error: "workspace_write_unsupported" });

    const attachmentBytes = Buffer.from("bounded worker attachment", "utf8");
    ...
    const emptyProjection = await waitForProjectionManifest(turnProjectionRoot);
    assert.equal(emptyProjection.schema, "dev_erp.codex_turn_projection.v1");
    assert.equal(emptyProjection.item_id, item.id);
    assert.deepEqual(emptyProjection.files, []);
    ...
    const attachmentTurn = request("/api/codex-task/message", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        item_id: item.id,
    ...
    const attachmentProjection = await waitForProjectionManifest(turnProjectionRoot);
    assert.equal(attachmentProjection.schema, "dev_erp.codex_turn_projection.v1");
    assert.equal(attachmentProjection.item_id, item.id);
    assert.equal(attachmentProjection.files.length, 1);
    assert.equal(attachmentProjection.files[0].attachment_id, attachment.attachment_id);
    assert.equal(attachmentProjection.files[0].name, "worker-note.txt");
    assert.equal(
      attachmentProjection.files[0].sha256,
      createHash("sha256").update(attachmentBytes).digest("hex"),
    );
    response = await attachmentTurn;
    assert.equal(response.status, 200, await response.clone().text());
    assert.deepEqual(readdirSync(turnProjectionRoot), []);
    ...
    try {
      ...
      assert.equal(db.prepare("SELECT COUNT(*) AS n FROM codex_workspace_write_grant WHERE item_id=?").get(item.id).n, 0);
