...
test("buildProjectContextLines: 전체 count가 같아도 branch별 source count가 다르면 summary를 거부", (t) => {
  const root = mkdtempSync(join(tmpdir(), "ai-ctx-branch-count-"));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const context = join(root, "P77-009", "project_context");
  mkdirSync(join(context, "summaries"), { recursive: true });
  writeFileSync(join(context, "sources.csv"),
    "source_id,project_code,source_kind,external_ref,source_time,title,branch_key,branch_ref,suggested_branch_ref,summary_hint,pointer_ref,metadata_hash,body_access,created_at,updated_at\n"
    + "s1,P77-009,mail,M1,2026-07-01T08:00:00Z,event-1,k1,b1,,,mailcsv:M1,h1,metadata_only,2026-07-02T09:00:00Z,2026-07-02T09:00:00Z\n"
    + "s2,P77-009,mail,M2,2026-07-01T08:30:00Z,event-2,k1,b1,,,mailcsv:M2,h2,metadata_only,2026-07-02T09:30:00Z,2026-07-02T09:30:00Z\n");
  writeFileSync(join(context, "summaries", "branch_summaries.csv"),
    "branch_id,project_code,branch_key,label,source_count,task_count,open_review_count,updated_at\n"
    + "b1,P77-009,k1,과소 집계 줄기,1,1,0,2026-07-02T10:00:00Z\n"
    + "b2,P77-009,k2,근거 없는 줄기,1,1,0,2026-07-02T10:00:00Z\n");

  const lines = buildProjectContextLines(root, "P77-009");
  assert.ok(lines.some((line) => line.includes("[context_gap:stale_summary]")));
  assert.ok(!lines.some((line) => line.includes("과소 집계 줄기")));
  assert.ok(!lines.some((line) => line.includes("근거 없는 줄기")));
});

test("buildProjectContextLines: 다른 branch의 최신 summary가 branch별 stale source를 가리지 못함", (t) => {
  const root = mkdtempSync(join(tmpdir(), "ai-ctx-branch-time-"));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const context = join(root, "P77-010", "project_context");
  mkdirSync(join(context, "summaries"), { recursive: true });
  writeFileSync(join(context, "sources.csv"),
    "source_id,project_code,source_kind,external_ref,source_time,title,branch_key,branch_ref,suggested_branch_ref,summary_hint,pointer_ref,metadata_hash,body_access,created_at,updated_at\n"
    + "s1,P77-010,mail,M1,2026-07-01T08:00:00Z,new-k1,k1,b1,,,mailcsv:M1,h1,metadata_only,2026-07-03T09:00:00Z,2026-07-03T09:00:00Z\n"
    + "s2,P77-010,mail,M2,2026-07-01T08:30:00Z,old-k2,k2,b2,,,mailcsv:M2,h2,metadata_only,2026-07-01T09:00:00Z,2026-07-01T09:00:00Z\n");
  writeFileSync(join(context, "summaries", "branch_summaries.csv"),
    "branch_id,project_code,branch_key,label,source_count,task_count,open_review_count,updated_at\n"
    + "b1,P77-010,k1,오래된 k1 요약,1,1,0,2026-07-02T10:00:00Z\n"
    + "b2,P77-010,k2,최신 k2 요약,1,1,0,2026-07-04T10:00:00Z\n");

  const lines = buildProjectContextLines(root, "P77-010");
  assert.ok(lines.some((line) => line.includes("[context_gap:stale_summary]")));
  assert.ok(!lines.some((line) => line.includes("오래된 k1 요약")));
  assert.ok(!lines.some((line) => line.includes("최신 k2 요약")));
});
