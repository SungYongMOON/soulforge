...
RAG의 기본 진입은 전체 과제 개요이며, 원천별 문서 구성·과제 비교·현재 조치 대상을 함께
보여준다. 공통 저장소의 별도 연결 정보가 없으면 미연결로 설명하고 과제 집계를 공통
RAG라고 부르지 않는다. 과제 상세의 그래프는 `?view=graph&project=<code>`로 실제 Neo4j
노드·관계를 조회하며, 선택적 `document=sha256:...`는 해당 과제 manifest에 있는 문서만
허용한다. 최대 80개 노드·160개 관계, 60초 캐시, 동시 조회 2개, 20초 worker 제한을 둔다.
전후 과제·DB 버전과 root table·binding·pointer를 검사하고 불일치 시 거부한다.
이름·종류·문서/청크 참조만 표시하며 원문·벡터·임의 쿼리·쓰기·모델 호출을 추가하지 않는다.
관계 자체에 버전 속성이 없던 기존 기록은 양끝 노드의 현재 과제·버전으로 범위를 확인한다.
원천→과제 구성도와 개별 DB 관계, 실제 질문의 검색 경로를 서로 구분한다.

모든 메뉴는 가용 폭을 사용한다. 넓은 화면에서 파일 미리보기와 검색 결과 상세는 목록 옆에
배치하고, RAG의 별도 화면에도 운영 화면의 테마를 전달한다. 과제 선택과 기존 조회 기능은
유지하며, 저장소·메모리 기능이 없을 때 공간을 채우기 위한 가짜 수치를 추가하지 않는다.
...
운영 콘솔은 소스별 `idle/loading/success/error`를 관리한다. 최초 의존 소스 조회가 끝나기
전에는 판단 숫자를 렌더링하지 않고, 재조회·실패 시 마지막 정상 응답과 확인 시각을 유지한다.
전체 진행률과 6초 지연 표시, API·영향 범위를 가진 소스별 재시도를 제공한다. 카탈로그의 기존
읽기 API만 사용하며 수집·복구 writer를 새로 호출하지 않는다. 진단처럼 여러 소스를 합친 판정은
필요한 소스가 모두 끝날 때 교체하고 화면 조작은 유지한다.

공개 hash는 overview/diagnostics/data/usage/rag/search/memory이며 기존 system/directory/evidence도
호환한다. RAG 과제 선택은 부모가 URL·history를 소유하고 iframe을 다시 만들지 않고 전달한다.
폴더 트리는 로드한 범위의 필터·50개씩 표시·키보드 이동을 제공한다. 검색 중복 정리는 표시 계층만
묶고 원래 근거를 보존한다. 누락된 날짜·Slack 표시명을 추정하지 않으며 검색 범위·예산은 그대로다.

배치는 **PC 자원 여유 → 개입할 항목 → 사용량 변화 → 자료 처리 진행**의 판단 순서다.
맨 위 PC 띠는 기존 호스트 스냅샷의 CPU·메모리·모든 관측 드라이브와 가동시간을 재사용하며,
추가 수집 호출을 만들지 않는다. 실패 또는 3분이 지난 관측은 보존값으로 표시한다.

운영 현황 최상단은 한도·토큰·수집기·검색 DB·모델 API·확인할 항목의 압축 요약이다.
넓은 화면에서 여섯 범위를 한 줄로 비교하고, 작은 화면은 줄바꿈한다. 요약 선택은 같은
화면의 해당 상세로 이동한다. 제공자별 한도는 현재 확인된 기간 중 가장 적게 남은 값을
기간과 함께 표시하며 서로 합산하지 않는다. 토큰 요약의 최근 7일 KST 범위는 상세 차트의
기간 선택과 독립적이다. 수집기 수·과제 수·API 응답 수의 서로 다른 분모를 유지하며,
조회 실패·일부 확인·미측정은 정상 0으로 대체하지 않는다.
작은 추세선은 실제 7일 토큰·RAG 문서 이력만 사용하고 빠진 관측은 연결하지 않는다.
한도 막대와 수집기·모델 상태 도형은 현재 비율이며 시간 추세가 아니다. 상세 한도는
오른쪽 그래프 높이와 독립적으로 위부터 모아 배치한다.

첫 화면은 **AI 자원 → 운영 판단 → 자료 수집·전처리·RAG → 로컬 모델** 순서다.
한도와 사용 추이는 같은 윗층에 두고, 넓은 화면의 가용 폭을 사용한다. 핵심 수치를 접지 않는다.
사용 추이의 좌표계는 실제 표시 폭을 따라 축 글자를 변형하지 않는다. 토큰 모델 범례는
모델명·수치가 정렬된 목록이며, 요청 횟수 범례는 그 아래의 별도 단위 행으로 읽는다.
