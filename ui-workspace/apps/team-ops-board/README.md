...
미리보기다. 운영 현황 → 같은 항목의 시스템 진단·직접 입출력 → 근거·조치 이력을
연결하고, 데이터 폴더·기존 사용 이력을 같은 탐색 안에서 유지한다. 기존 `/`, 운영 지도,
...
- 폴더는 기존 Path Registry reader만 사용한다. 처음 최대 8개 하위 폴더를 2개씩 읽고
  화면에는 최대 500행을 표시한다. reader의 직접 자식 200개 검사·보호 이름·링크 차단·캐시
  계약은 유지한다. 여러 폴더 펼침과 화면 왕복 상태를 유지하며 원문은 열지 않는다.
...
- 밝게/어둡게 보기, 좁은 화면의 서비스 목록 접기, 키보드 폴더 조작은 화면에만 영향을 준다.
  3D, 새 수집·진단 실행·자동 복구·LLM 호출은 추가하지 않는다.
