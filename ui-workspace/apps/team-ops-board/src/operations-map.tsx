...
function Detail({ node, model, close }: { node: Row | undefined; model: Row; close: () => void }) {
  ...
  return <aside className="op-detail" aria-label="선택 노드 상세"><header><small>NODE INSPECTOR</small><button aria-label="상세 닫기" onClick={close}><X size={18}/></button></header><h2>{node.label}</h2><code>{node.id}</code>
    <dl className="op-status-axes"><dt>구현·연결</dt><dd>{node.implementation}</dd><dt>최근 검사 판단</dt><dd>{node.assessment?.label ?? healthLabel[node.health] ?? '미확인'}</dd><dt>근거 신선도</dt><dd>{node.freshness === 'fresh' ? '현재 관측' : node.freshness === 'retained' ? '저장된 관측 · 아래 시각 기준' : '관측 근거 없음'}</dd></dl>
...
function App() {
  ...
  const flowEdges = scene.edges.map((e: Row) => ({ id: e.id, source: e.from, target: e.to, type: 'default', animated: false, markerEnd: { type: MarkerType.ArrowClosed },
    sourceHandle: e.sourceHandle, targetHandle: e.targetHandle, label:e.relation==='control'?'검증 요청':undefined, labelStyle:{fontSize:10}, className: e.relation==='control'?'op-edge-control':e.evidenceMode === 'implementation_contract' ? 'op-edge-implementation' : 'op-edge-declared', style: { strokeWidth: focusEdges?.has(e.id)?2:1.2, opacity:focusEdges&&!focusEdges.has(e.id)?.12:.75 } }));
  ...
  return <main className={`op-app ${embedded?'op-embedded':''}`}><header className="op-topbar"><a className="op-brand" href="/"><GitBranch size={25}/><span>SOULFORGE<small>OPERATIONS ATLAS</small></span></a><nav aria-label="화면 선택"><a href="/">대시보드</a><button className={tab === 'map' ? 'active' : ''} onClick={() => setTab('map')}><GitBranch size={16}/>운영 지도</button><button className={tab === 'directory' ? 'active' : ''} onClick={() => setTab('directory')}><Folder size={16}/>디렉터리</button></nav><div className="op-mode"><LockKeyhole size={13}/>로컬 · 읽기 전용<a href="http://127.0.0.1:4192/" target="_blank" rel="noreferrer">운영 설치본 ↗</a></div></header>
    ...
    {tab === 'directory' ? <Directory/> : <div className="op-reading-layout"><section className="op-status-reading" aria-label="진단 해석 요약"><strong>최근 검사 기준 · {at(model.observedAt)}</strong><span>이상 신호 {model.assessments.problem}곳</span><span>처리 보류/재시도 {model.assessments.pending}곳 ({model.assessments.pendingItemsUnknown?'건수 미확인':`${model.assessments.pendingItems}건`})</span><span>관측/검사 문제 {model.assessments.observation_error}곳</span><span>미확인 {model.assessments.unknown}곳</span><small>미확인은 정상으로 세지 않습니다. 수집기 정상과 업무 처리 완료는 다릅니다.</small></section><section className="op-map-main"><div className="op-map-toolbar"><div><Layers size={16}/><strong>자료가 지나가는 순서</strong><span>왼쪽 → 오른쪽</span></div><label><Search size={15}/><input aria-label="노드 필터" placeholder="세부 항목 찾기" value={query} onChange={e => {setQuery(e.target.value);setSelected(null);}}/></label><button onClick={() => { setOpenStage(null); setSelected(null); setPositions({}); setQuery(''); }}>전체 흐름으로</button></div>
      ...
      <div className="op-overview-gaps"><div><Unplug size={17}/><strong>입력 연결</strong><span>보관 → 맥락 엔진 준비<br/>전달 연결 미확인</span></div><div><Unplug size={17}/><strong>응답 연결</strong><span>맥락 → 에이전트 응답<br/>전달 연결 미등록</span></div><p>수집 감독·원장 검증·Gmail 출력은 중간 단계를 표시했습니다.<br/>선이 없다는 것만으로 미구현·고장을 단정하지 않습니다.</p></div>
      <div className="op-map-legend"><span><i className="op-legend-line"/>등록된 자료 연결</span><span><i className="op-legend-line dashed"/>구현 계약 · 실행 미확인</span><span><i className="op-legend-line control"/>제어·검증 요청</span><span>노드 선택으로 직접 연결과 진단 확인</span></div>
    </section>
