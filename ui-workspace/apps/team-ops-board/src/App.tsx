...
function App() {
  ...
  const exactTaskLabels = useMemo(
    () => new Map(
      [...projection.threads, ...projection.history]
        .filter((thread: any) => typeof thread.thread_id === "string" && typeof thread.display_label === "string")
        .map((thread: any) => [thread.thread_id, thread.display_label])
    ),
    [projection]
  );
  ...
  function updateProjection(force = false) {
    ...
    void aiUsageProjectionRequest.load({ force }).then(
      (next: any) => setAiUsageProjection(next),
  ...
  return (
    <div className="inbox-app live-board-app">
      ...
      <section className="live-health-strip" aria-label="local projection health" data-live-dialog-background>
        ...
        <span data-testid="lifecycle-source-health">lifecycle {lifecycleSourceHealthLabel(projection.scope.lifecycle_source_health)} · exact {projection.scope.lifecycle_matched_enrolled_count}/{projection.scope.lifecycle_exact_identity_count}</span>
        ...
        <section className="live-board-controls" aria-label="업무와 기록 필터" data-live-dialog-background>
          ...
          <label>
            ...
            <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
              <option value="stopped">{"\uacb0\uacfc \ubbf8\ud655\uc815 \uc815\uc9c0 \uad00\uce21"}</option>
              <option value="all">전체</option>
      ...
      {surface === "work" && <AiUsagePanel projection={aiUsageProjection} exactTaskLabels={exactTaskLabels} />}
...
function AiUsagePanel({ projection, exactTaskLabels }: { projection: any; exactTaskLabels: ReadonlyMap<string, string> }) {
  ...
  return (
    <section className={`ai-usage-panel ai-usage-panel-${projection.state}`} aria-label="AI Usage Meter read-only projection" data-live-dialog-background>
      ...
      {projection.history && <AiUsageHistoryPanel history={projection.history} selectedWindow={historyWindow} onSelectWindow={setHistoryWindow} exactTaskLabels={exactTaskLabels} />}
    </section>
  );
}
...
function AiUsageHistoryPanel({ history, selectedWindow, onSelectWindow, exactTaskLabels }: { history: any; selectedWindow: string; onSelectWindow: (value: string) => void; exactTaskLabels: ReadonlyMap<string, string> }) {
    ...
    <section className="ai-usage-history" aria-labelledby="ai-usage-history-heading">
      <header className="ai-usage-history-header">
        <div>
          ...
          <p>KST 기준 통화 지표와 project/work/task 순위를 보여줍니다.</p>
        </div>
      ...
      <section id={panelId} role="tabpanel" aria-labelledby={`ai-usage-period-${selectedWindow}`} className="ai-usage-history-panel" tabIndex={0}>
        <dl className="ai-usage-history-summary">
          <div><dt>Turns</dt><dd>{formatUsageNumber(window.totals.turns)}</dd></div>
          <div><dt>Tokens</dt><dd>{formatUsageNumber(window.totals.total_tokens)}</dd></div>
          <div><dt>Credits</dt><dd>{formatUsageCredits(window.totals.credits)}</dd></div>
          <div><dt>Unknown credits</dt><dd>{formatUsageNumber(window.totals.credit_unknown_turns)}</dd></div>
        </dl>
        ...
        <div className="ai-usage-history-grid">
          <UsageHistoryRows heading="Project ranking" rows={window.breakdowns.projects} labelKey="project_id" />
          <UsageHistoryRows heading="Work ranking" rows={window.breakdowns.works} labelKey="work_id" />
          <UsageHistoryRows heading="Task ranking" rows={window.breakdowns.tasks} labelKey="task_id" exactTaskLabels={exactTaskLabels} />
        </div>
      </section>
    </section>
  );
    ...
    <section aria-label={heading}>
      ...
      <div className="ai-usage-table" role="table">
        <div className="ai-usage-row ai-usage-row-head" role="row">
          <span role="columnheader">ID</span><span role="columnheader">Turns</span><span role="columnheader">Tokens</span><span role="columnheader">Credits</span>
        </div>
