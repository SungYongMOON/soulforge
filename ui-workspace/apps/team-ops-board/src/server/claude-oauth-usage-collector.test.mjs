...
test("collector is disabled by default and never requests or reads credentials", async () => {
  let calls = 0;
  const result = await collectClaudeOauthUsage({ gatePath: "C:\\safe\\gate.json", receiptPath: "C:\\safe\\provider_quota.receipt.v1.json", read: async () => { calls += 1; throw new Error("missing"); }, fetchImpl: async () => { throw new Error("must-not-run"); } });
...
test("collector sanitizes 5h weekly and Fable without returning secret or raw response", async () => {
  let captured;
  const result = await collectClaudeOauthUsage({ gatePath: "C:\\safe\\gate.json", receiptPath: "C:\\safe\\provider_quota.receipt.v1.json", read: async (path) => reader(path), fetchImpl: async (_url, init) => { assert.match(init.headers.Authorization, /^Bearer /u); return response(payload); }, now: () => Date.parse("2026-08-10T10:00:00.000Z"), store: { persistAcceptedSnapshot: async (snapshot) => { captured = snapshot; return { write_state: "written" }; } } });
...
test("auth, redirects, malformed, oversized, timeout, and future data fail closed with fixed codes", async () => {
  const common = { gatePath: "C:\\safe\\gate.json", receiptPath: "C:\\safe\\provider_quota.receipt.v1.json", read: async (path) => reader(path), now: () => Date.parse("2026-08-10T10:00:00.000Z") };
