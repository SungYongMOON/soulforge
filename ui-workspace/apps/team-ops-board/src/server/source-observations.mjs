...
export function observationSeries(id,label,basis,records,observedAt,scope,partial=true){
  const end=day(observedAt),days=Array.from({length:14},(_,i)=>day(new Date(Date.parse(`${end}T00:00:00+09:00`)-(13-i)*86400000).toISOString()));
    ...
    pending=(async()=>{
      ...
      const sources=await Promise.all([
        source('slack','Slack',async()=>{
          ...
          for(const project of projects){try{const state=await json(`ingress/slack/channels/${project}/state/slack-continuous.json`,['soulforge.slack_continuous.state.v1']);if(!Array.isArray(state.revisions))throw Error('shape');for(const r of state.revisions){if(typeof r.message_ref!=='string'||!/^[0-9]{10,16}\.[0-9]{6}$/u.test(r.message_ts??''))continue;const key=`${r.channel_id}:${r.message_ref}`;if(seen.has(key))continue;seen.add(key);const ms=Number(r.message_ts)*1000;if(!Number.isFinite(ms)||!Number.isFinite(new Date(ms).getTime()))continue;rows.push({id:key,source:'Slack',title:`${project} · 메시지`,at:new Date(ms).toISOString(),value:1,status:'보관 원장에 기록',basis:'메시지 작성 시각'});}}catch{missing++;}}
        ...
        source('linear','Linear',async()=>{
          ...
          const rows=Object.entries(state.object_index).filter(([key])=>/^issues:[a-f0-9-]{36}$/u.test(key)).map(([id,r])=>({id,source:'Linear',title:`이슈 · ${id.slice(7,15)}`,at:dateOf(r.updated_at),value:1,status:'보관 인덱스 기록',basis:'이슈 최종 수정 시각'}));
