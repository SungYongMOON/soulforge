...
test("a request carrying a proxy-passage header is rejected 403 even from a loopback socket, before the method check (M1/M8)", async () => {
  // A binding path that does not exist: the projection fails closed to
  // `unavailable` and the test never reads the Owner's state root.
  const missingRoot = path.join(tmpdir(), "team-ops-receipt-expiry-guard-missing");
  const plugin = createReceiptExpiryServerAdapter({
    ownerRoot: missingRoot,
    bindingPath: path.join(missingRoot, "receipt_expiry_binding.v1.json"),
  });
  const loopbackRequest = (overrides = {}) => ({
    method: "GET",
    url: RECEIPT_EXPIRY_PATH,
