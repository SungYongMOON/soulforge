...
test("a request carrying a proxy-passage header is rejected 403 even from a loopback socket, before the method check (M1/M8)", async () => {
  const directory = await mkdtemp(join(tmpdir(), "team-ops-topology-guard-"));
  try {
    let reads = 0;
    const plugin = createTopologyAdapterPlugin({
      readOnlyPilot: true,
      pointerPath: join(directory, "binding.pointer.json"),
      now: () => NOW,
      resolveBinding: async () => {
        reads += 1;
        return "configured";
      ...
      readSnapshot: async () => {
        reads += 1;
        return validateTopologyHealthSnapshot(sampleSnapshot(), { now: NOW });
      },
    });
    const loopbackRequest = (overrides = {}) => ({
      method: "GET",
      url: TOPOLOGY_SNAPSHOT_PATH,
    ...
    // A plain loopback GET still reaches the synthetic binding/snapshot reads and is answered 200.
