...
export function aggregateMailRuns(runs,observedAt,diagnostics={}){
  const end=kst(Date.parse(observedAt)),startMs=Date.parse(`${end}T00:00:00+09:00`)-13*86400000;
  const dates=Array.from({length:14},(_,i)=>kst(startMs+i*86400000));
  ...
  return {id:'mail',label:'메일',basis:'수집일',state:measuredRuns?'partial':'unavailable',observed_at:observedAt,
    scope:`최근 14일 기존 수집 영수증 · 확인 ${measuredRuns}회 / 건수 미확인 ${unmeasuredRuns}회 · 신규 등록 항목 합계 · 메일 수신일·보관 전체 건수·RAG 완료와 별도`,
    ...
    timeline:{state:'partial',as_of:observedAt,start:dates[0],end,daily,total,records,records_total:records.length,records_limited:false}};
}
function admit(root){
...
export function createMailCollectionHistoryReader(){
  ...
  return {async read(root,observedAt){
    ...
    return aggregateMailRuns(results.filter(Boolean),observedAt,{examined_files:examined,selected_files:selected.length,unreadable_files:unreadable,invalid_receipts:invalid,limited:truncated});
  }};
}
