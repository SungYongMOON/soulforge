...
import {
  CLAUDE_USAGE_SNAPSHOT_PATH,
  createClaudeUsageAdapterPlugin,
} from "./claude-usage-adapter.mjs";
...
test("a request carrying a proxy-passage header is rejected 403 even from a loopback socket, before the method check (M1/M8)", async () => {
  // Synthetic roots that do not exist: the collector has nothing to scan and
  // the test never reads the Owner's real ~/.claude session transcripts.
  const missingRoot = join(tmpdir(), "team-ops-claude-usage-guard-missing");
  const plugin = createClaudeUsageAdapterPlugin({
    projectsRoot: join(missingRoot, "projects"),
    claudeConfigPath: join(missingRoot, ".claude.json"),
  });
  const loopbackRequest = (overrides = {}) => ({
    method: "GET",
    url: CLAUDE_USAGE_SNAPSHOT_PATH,
  ...
  // A plain loopback GET still reaches the collector and is answered 200 JSON.
  ...
  assert.doesNotThrow(() => JSON.parse(served.body));
