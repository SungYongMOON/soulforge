...
test("a request carrying a proxy-passage header is rejected 403 even from a loopback socket, before the method check (M1/M8)", async () => {
  // Unconfigured (no private binding pair): the endpoint answers the fixed
  // metadata-only `unavailable` to a direct loopback caller and nothing at all
  // to a proxied one.
  const plugin = createStorageMapServerAdapter();
