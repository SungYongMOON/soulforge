...
test("a request carrying a proxy-passage header is rejected 403 even from a loopback socket, before the method check (M1/M8)", async () => {
  const directory = await mkdtemp(join(tmpdir(), "team-ops-ai-usage-guard-"));
  try {
    let reads = 0;
    const plugin = createAiUsageAdapterPlugin({
      registryPath: join(directory, "missing-visibility.json"),
      env: ENV,
      readUsageProjection: async () => {
        reads += 1;
        return readOnlyFixture();
    ...
    const loopbackRequest = (overrides = {}) => ({
      method: "GET",
      url: `${AI_USAGE_SNAPSHOT_PATH}?${AI_USAGE_READ_ONLY_QUERY_KEY}=1`,
      socket: { remoteAddress: "127.0.0.1" },
      headers: {},
      ...overrides
    });
    ...
    // Past the guard and the method check the exact read_only query is still
    // required: a plain loopback GET without it reports 400, not 403.
    for (const surface of ["configureServer", "configurePreviewServer"]) {
      const middleware = captureGuardMiddleware(plugin, surface);
      assert.equal((await invokeGuard(middleware, loopbackRequest({ url: AI_USAGE_SNAPSHOT_PATH }))).statusCode, 400, surface);
      assert.equal((await invokeGuard(middleware, loopbackRequest({ url: AI_USAGE_SNAPSHOT_PATH, headers: undefined }))).statusCode, 400, surface);
    }
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
