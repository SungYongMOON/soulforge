...
export const DEFAULT_LIFECYCLE_POSITIVE_LEASE_MAX_AGE_MS = DEFAULT_LIFECYCLE_SNAPSHOT_MAX_AGE_MS;

const DEFAULT_LIFECYCLE_SNAPSHOT_MAX_FUTURE_MS = 60 * 1_000;
const SAFE_LIFECYCLE_TURN_ID = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,239}$/u;
...
function safePositiveLeaseMaxAgeMs(value) {
  if (!Number.isSafeInteger(value) || value < 0) return DEFAULT_LIFECYCLE_POSITIVE_LEASE_MAX_AGE_MS;
  return Math.min(value, DEFAULT_LIFECYCLE_SNAPSHOT_MAX_AGE_MS);
}

function hasFreshPositiveLifecycleEvidence(identity, nowMilliseconds, positiveLeaseMaxAgeMs) {
  if (typeof identity?.turn_id !== "string" || !SAFE_LIFECYCLE_TURN_ID.test(identity.turn_id)) return false;
  const observedAt = Date.parse(identity.observed_at);
  // Lifecycle receipts are event evidence, not a validated heartbeat. Positive
  // Board status therefore fails closed when this lease expires.
  return Number.isFinite(observedAt)
    && observedAt <= nowMilliseconds
    && nowMilliseconds - observedAt <= positiveLeaseMaxAgeMs;
}
...
function exactEnrolledThreadId(identity, enrolledThreadIds) {
  if (identity.agent_id) return enrolledThreadIds.has(identity.agent_id) ? identity.agent_id : null;
...
export function projectLifecycleSnapshotRuntime({
  source,
  enrolledThreadIds = new Set(),
  now = Date.now,
  positiveLeaseMaxAgeMs = DEFAULT_LIFECYCLE_POSITIVE_LEASE_MAX_AGE_MS
} = {}) {
  ...
  const observedNow = safeNowMilliseconds(now);
  const safePositiveLease = safePositiveLeaseMaxAgeMs(positiveLeaseMaxAgeMs);
  ...
  const runtime_threads = [...selectedByThreadId.entries()]
    .map(([threadId, identity]) => {
      ...
      if (
        ...
        && !hasFreshPositiveLifecycleEvidence(identity, observedNow, safePositiveLease)
      ) return null;
      return {
        thread_id: threadId,
        status,
