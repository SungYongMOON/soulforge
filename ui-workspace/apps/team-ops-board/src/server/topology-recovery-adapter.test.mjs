...
test("a request carrying a proxy-passage header is rejected 403 even from a loopback socket, before the method check (M1/M8)", async () => {
  // A relative owner root never resolves to an evidence directory: the read
  // fails closed to `unavailable` and the test never touches the Owner's state root.
  const plugin = createTopologyRecoveryAdapterPlugin({ ownerRoot: "relative", now: () => NOW });
  const loopbackRequest = (overrides = {}) => ({
    method: "GET",
    url: TOPOLOGY_RECOVERY_PATH,
  ...
  assert.equal(JSON.parse(served.body).state, "unavailable");
