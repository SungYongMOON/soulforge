...
import { Activity, ArrowLeft, ArrowRight, ArrowUpRight, Check, ChevronDown, ChevronRight, CircleHelp, Clock3, Database, FileCheck2, FolderOpen, GitBranch, LayoutDashboard, List, LockKeyhole, Moon, RefreshCw, Search, ShieldCheck, Sun, TriangleAlert, X } from 'lucide-react';
...
import { ConsoleDirectory } from './operations-console-directory';
import './team-ops.css';
...
type Screen='overview'|'system'|'directory'|'usage';
type Navigation={screen:Screen;node:string|null};
const screens=[['overview','운영 현황',LayoutDashboard],['system','시스템 진단',GitBranch],['directory','데이터 폴더',FolderOpen],['usage','사용 이력',Activity]] as const;
...
const compact=(n:any)=>typeof n==='number'&&Number.isFinite(n)?new Intl.NumberFormat('ko-KR',{notation:'compact',maximumFractionDigits:1}).format(n):'미확인';
...
function useSources(){
  ...
  const load=useCallback(async()=>{if(inFlight.current)return;inFlight.current=true;setLoading(true);
    const entries=[['federation','/topology-federation.snapshot.json'],['health','/operations-health.snapshot.json'],['recovery','/topology-recovery.snapshot.json'],['graph','/operations-graph-receipts.json'],['host','/host-stats.snapshot.json']];
...
function Overview({model,inputs,failed,go}:{model:Row;inputs:Row;failed:string[];go:(nav:Navigation)=>void}){
  const usage=inputs.usage,graph=inputs.graph,attention=model.attention.filter((n:Row)=>n.status.key!=='unknown');
  const host=buildHostStatsViewModel(inputs.host);
  const headline=!model.sourceAvailable?'시스템 구조를 아직 읽지 못했습니다':!model.healthAvailable?'지금 상태를 확인할 근거가 부족합니다':attention.length?'확인할 항목이 있습니다':'최근 검사에서 이상 신호가 없습니다';
  return <><section className="cx-overview-lead"><div><span className="cx-eyebrow">최근 검사에서 확인할 것</span><h2>{headline}</h2><p>{model.healthAvailable?`검사 기준 ${at(model.observedAt)} · ${model.healthCurrent?'최근 관측':'저장된 검사 결과'} · 미확인 ${model.counts.unknown}곳은 별도로 남아 있습니다.`:'저장된 구조와 이전 관측은 볼 수 있습니다. 최신 상태로 해석하지 않습니다.'}</p></div><ShieldCheck size={36}/></section>
    <div className="cx-summary-counts">{['problem','pending','observation_error','unknown'].map(key=><button key={key} onClick={()=>go({screen:'system',node:model.attention.find((n:Row)=>n.status.key===key)?.id??null})}><span className={`cx-dot is-${CONSOLE_ASSESSMENTS[key].tone}`}/><span>{CONSOLE_ASSESSMENTS[key].label}</span><strong>{model.sourceAvailable?number(model.counts[key]):'—'}</strong><ChevronRight size={14}/></button>)}</div>
    <section className="cx-card"><header className="cx-section-heading"><div><h2>확인이 필요한 곳</h2><p>등록된 검사 대상 {model.watchedCount}곳 기준 · 업무 영향은 상세에서 확인합니다.</p></div><button onClick={()=>go({screen:'system',node:null})}>모두 보기 <ArrowUpRight size={15}/></button></header>
      {(attention.length?attention:model.attention).slice(0,4).map((n:Row)=><button className="cx-attention-row" key={n.id} onClick={()=>go({screen:'system',node:n.id})}><span className={`cx-row-icon is-${n.status.tone}`}>{n.status.key==='pending'?<Clock3 size={20}/>:n.status.key==='unknown'?<CircleHelp size={20}/>:<TriangleAlert size={20}/>}</span><span className="cx-grow"><strong>{shortName(n.label)}</strong><small>{n.healthReasons?.length?n.healthReasons.map(describeTopologyReason).join(' · '):n.status.next}</small></span><Status status={n.status}/><ChevronRight size={17}/></button>)}
      {!model.attention.length&&<p className="cx-empty-inline">{model.sourceAvailable?'이 범위에서 별도 확인 항목이 없습니다.':'자료를 읽으면 이곳에 표시합니다.'}</p>}
    </section>
    <section className="cx-card"><header className="cx-section-heading"><div><h2>시스템을 따라가기</h2><p>읽는 순서입니다. 선택하면 실제 입출력과 검사 범위를 봅니다.</p></div></header><div className="cx-stage-rail">{model.stages.filter((s:Row)=>s.id!=='support').map((s:Row,i:number)=><button key={s.id} onClick={()=>go({screen:'system',node:s.members[0]?.id??null})}><small>{String(i+1).padStart(2,'0')}</small><strong>{s.title}</strong><span>{s.members.length}개 항목 <ArrowRight size={14}/></span></button>)}</div></section>
    <div className="cx-overview-bottom"><section className="cx-card"><header className="cx-section-heading"><div><h2>자료 반영</h2><p>설정된 과제의 최근 처리 영수증</p></div><Database size={18}/></header><div className="cx-inline-metrics"><div><span>반영 검증</span><strong>{failed.includes('graph')?'미확인':number(graph?.summary?.completed)}</strong></div><div><span>대기</span><strong>{failed.includes('graph')?'미확인':number(graph?.summary?.pending)}</strong></div><div><span>실패</span><strong>{failed.includes('graph')?'미확인':number(graph?.summary?.failed)}</strong></div></div><p className="cx-footnote">{graph?.rows?.length??0} / {graph?.expected??'미확인'}개 과제 · 최근 반영 {at(graph?.summary?.last_reflected_at)}<br/>DB 생존·검색 품질과는 별도입니다.</p><button className="cx-text-link" onClick={()=>go({screen:'system',node:'context_engine::neo4j'})}>반영 근거 확인 <ArrowRight size={15}/></button></section>
    <section className="cx-card"><header className="cx-section-heading"><div><h2>사용 흐름</h2><p>등록된 TASK의 측정 원장</p></div><Activity size={18}/></header><div className="cx-inline-metrics"><div><span>오늘 토큰</span><strong>{compact(usage?.history?.windows?.calendar_day?.totals?.total_tokens)}</strong></div><div><span>이번 주</span><strong>{compact(usage?.history?.windows?.calendar_week?.totals?.total_tokens)}</strong></div></div><p className="cx-footnote">{failed.includes('usage')?'새 관측 읽기 실패 · 이전 값':'부분 계측은 합계와 함께 유지합니다.'}<br/>기준 {at(usage?.history?.generated_at??usage?.snapshot?.observed_at)}</p><button className="cx-text-link" onClick={()=>go({screen:'usage',node:null})}>모델·시간별로 보기 <ArrowRight size={15}/></button></section></div>
    {host.available&&<div className="cx-host-strip">{host.cells.map((cell:Row)=><span key={cell.key}>{cell.label==='MEM'?'메모리':cell.label==='UP'?'가동 시간':cell.label}<strong>{cell.value}</strong></span>)}<small>{failed.includes('host')?'이전 ':''}호스트 관측 {at(host.observedAt)}</small></div>}
  </>;
}
...
function UsageView({usage,failed}:{usage:Row|undefined;failed:boolean}){
  ...
  return <div className="cx-usage">{failed&&<p className="cx-notice">최신 사용량을 읽지 못했습니다. 아래에 값이 있다면 이전 관측입니다.</p>}
    <section className="cx-card"><header className="cx-section-heading"><div><h2>어느 때, 어떤 모델을 사용했나요?</h2><p>기존 측정 원장 · 날짜 또는 모델을 선택해 세부 값을 확인하세요.</p></div></header><UsageTrendChart usage={usage} onSelection={onSelection}/></section>
...
function ConsoleApp(){
  ...
  const [nav,setNav]=useState<Navigation>({screen:'overview',node:null}),[inspector,setInspector]=useState(false),[theme,setTheme]=useState('light');
  ...
  useEffect(()=>{window.history.replaceState({consoleNavigation:{screen:'overview',node:null}},'','#overview');const pop=(event:PopStateEvent)=>{if(event.state?.consoleNavigation){setNav(event.state.consoleNavigation);setInspector(false);setPast(old=>old.slice(0,-1));}};window.addEventListener('popstate',pop);return()=>window.removeEventListener('popstate',pop);},[]);
  const title=screens.find(s=>s[0]===nav.screen)?.[1];
  return <div className="cx-app" data-theme={theme}><a className="cx-skip" href="#console-content">본문으로 건너뛰기</a><aside className="cx-sidebar"><a className="cx-brand" href="#overview" onClick={e=>{e.preventDefault();go({screen:'overview',node:null});}}><span><GitBranch size={22}/></span><strong>Soulforge<small>Operations</small></strong></a><nav aria-label="운영 화면">{screens.map(([id,label,Icon])=><button key={id} aria-current={nav.screen===id?'page':undefined} className={nav.screen===id?'is-selected':''} onClick={()=>go({screen:id,node:nav.node})}><Icon size={18}/><span>{label}</span>{id==='system'&&model.healthAvailable&&(model.counts.problem+model.counts.pending+model.counts.observation_error)>0&&<small>{model.counts.problem+model.counts.pending+model.counts.observation_error}</small>}</button>)}</nav><div className="cx-sidebar-bottom"><span className="cx-preview-label">UX 미리보기</span><p>기존 관측을 읽습니다.<br/>서비스는 변경하지 않습니다.</p><a href="http://127.0.0.1:4192/" target="_blank" rel="noreferrer">기존 업무·조직·대시보드 <ArrowUpRight size={14}/></a><button onClick={()=>setTheme(t=>t==='light'?'dark':'light')}>{theme==='light'?<Moon size={15}/>:<Sun size={15}/>} {theme==='light'?'어둡게 보기':'밝게 보기'}</button><span className="cx-local"><LockKeyhole size={12}/>이 PC · 읽기 전용</span></div></aside>
    ...
    <main ref={main} id="console-content" className="cx-main">{failed.length>0&&<div className="cx-source-notice" role="status"><CircleHelp size={16}/><span>일부 근거를 읽지 못했습니다: {failed.map(k=>({federation:'구조',health:'상태',recovery:'조치 이력',graph:'자료 반영',host:'호스트',usage:'사용량'})[k]).join(' · ')}. 다른 관측은 유지합니다.</span></div>}
      <div hidden={nav.screen!=='overview'}><Overview model={model} inputs={inputs} failed={failed} go={go}/></div>
      <div hidden={nav.screen!=='system'}><SystemView model={model} nav={nav} go={go} inspect={()=>{lastTrigger.current=document.activeElement as HTMLElement;setInspector(true);}}/></div>
      <div hidden={nav.screen!=='directory'}><ConsoleDirectory active={nav.screen==='directory'} context={nav.node?selected:undefined} onReturn={()=>go({screen:'system',node:nav.node})}/></div>
      <div hidden={nav.screen!=='usage'}><UsageView usage={inputs.usage} failed={failed.includes('usage')}/></div>
      <footer className="cx-main-footer"><span>실제 관측 · 범위별 시각 유지</span><span>화면 선택과 실제 서비스 동작은 별개입니다.</span></footer>
