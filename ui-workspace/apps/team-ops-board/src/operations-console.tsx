...
import {createSourceStore,sourceGroupState} from './core/operations-source-state.mjs';
import {parseConsoleRoute,consoleRouteUrl} from './core/operations-routes.mjs';
import {formatAmount,formatExact,displayModelName} from './core/operations-format.mjs';
import {SOURCE_CATALOG,SourceStateContext,SourceBoundary,DIAGNOSTIC_SOURCES} from './operations-source-ui';
...
const screens=[['overview','운영 현황',LayoutDashboard],['system','구조·진단',GitBranch],['directory','데이터 공간',FolderOpen],['usage','사용 이력',Activity],['rag','RAG 처리',Database],['evidence','자료 검색',Search],['memory','기억·맥락',FileCheck2]] as const;
...
const number=(n:any)=>formatAmount(n);
...
async function readJson(url:string,signal?:AbortSignal){const r=await fetch(url,{cache:'no-store',credentials:'omit',redirect:'error',signal:AbortSignal.any([signal??new AbortController().signal,AbortSignal.timeout(url.startsWith('/rag-operations')?45000:20000)])});if(!r.ok)throw Object.assign(new Error(),{code:`HTTP_${r.status}`});if(!r.headers.get('content-type')?.includes('application/json'))throw Object.assign(new Error(),{code:'INVALID_RESPONSE'});return r.json();}
function useSources(){
  const [store]=useState(()=>createSourceStore(Object.fromEntries(Object.entries(SOURCE_CATALOG).map(([key,source])=>[key,async({signal}:{signal:AbortSignal})=>{
    const value=key==='usage'?await aiUsageProjectionRequest.load({signal:AbortSignal.any([signal,AbortSignal.timeout(20000)]),force:true}):await readJson(source.url,signal);
    if(!value||typeof value!=='object'||Array.isArray(value)||!Object.keys(value).length||['unavailable','denied','hold','invalid','error'].includes(value.state)||['unavailable','invalid','error'].includes(value.status)||(key==='usage'&&value.state!=='ready'))throw Object.assign(new Error(),{code:'SOURCE_UNAVAILABLE'});
    return value;
  }]))));
  const state=useSyncExternalStore(store.subscribe,store.snapshot,store.snapshot);
  const load=useCallback(()=>store.refresh(),[store]);
  useEffect(()=>{void load();const timer=setInterval(()=>{if(!document.hidden)void load();},60000);return()=>{clearInterval(timer);store.dispose();};},[load,store]);
  const entries=Object.entries(state.sources) as [string,Row][];
  const inputs=Object.fromEntries(entries.filter(([,row])=>row.value!==undefined).map(([key,row])=>[key,row.value]));
  const failed=entries.filter(([,row])=>row.status==='error'&&row.value===undefined).map(([key])=>key);
  const readAt=entries.map(([,row])=>row.lastSuccessAt).filter(Boolean).sort().at(-1)??null;
  return {inputs,failed,loading:state.progress.active,readAt,load,sources:state.sources,progress:state.progress,retry:store.retry};
}
...
function FocusMap({model,selected,onSelect}:{model:Row;selected:string;onSelect:(id:string)=>void}){
  ...
  return <div className="cx-focus-map" aria-label="선택 서비스의 직접 연결 지도"><ReactFlow ariaLabelConfig={{'controls.zoomIn.ariaLabel':'확대','controls.zoomOut.ariaLabel':'축소','controls.fitView.ariaLabel':'전체 보기','controls.interactive.ariaLabel':'연결 조작 전환'}} key={selected} nodes={nodes} edges={edges} nodeTypes={types} fitView fitViewOptions={{padding:.13}} minZoom={.85} maxZoom={1.4} nodesDraggable={false} nodesConnectable={false} panOnScroll={false} zoomOnScroll={false} onNodeClick={(_,node)=>onSelect(node.id)}><Background color="#d9e0e8" gap={24} size={1}/><Controls showInteractive={false}/></ReactFlow></div>;
}
...
function UsageView({usage,failed,taskLabels=new Map<string,string>()}:{usage:Row|undefined;failed:boolean;taskLabels?:ReadonlyMap<string,string>}){
  ...
  return <div className="cx-usage">{failed&&<p className="cx-notice">최신 사용량을 읽지 못했습니다. 아래에 값이 있다면 이전 관측입니다.</p>}
    <section className="cx-card"><header className="cx-section-heading"><div><h2>사용 추이</h2><p>기존 측정 원장 · 날짜 또는 모델을 선택해 세부 값을 확인하세요.</p></div></header><UsageTrendChart usage={usage} onSelection={onSelection} compact/></section>
    {chosen&&<section className="cx-card cx-selection-detail" aria-label="선택 날짜 모델 상세"><header className="cx-section-heading"><div><h2>{chosen.date} · {selection?.modelId?displayModelName(selection.modelId):'모든 모델'}</h2><p>선택 날짜의 모델별 측정 · 작업별 귀속은 이 자료에 없습니다.</p></div><button onClick={()=>setSelection(null)} aria-label="사용량 선택 해제"><X size={17}/></button></header><div className="cx-table-wrap"><table><thead><tr><th>모델</th><th>토큰</th><th>회차</th><th>토큰 미측정 회차</th></tr></thead><tbody>{chosen.rows.map((r:Row)=><tr key={r.model_id}><td>{displayModelName(r.model_id)}</td><td title={formatExact(r.total_tokens)}>{number(r.total_tokens)}</td><td>{number(r.turns)}</td><td>{number(r.token_unknown_turns)}</td></tr>)}</tbody></table></div></section>}
    <section className="cx-card cx-reused-usage"><header className="cx-section-heading"><div><h2>기간별 작업 이력</h2><p>위의 모델 선택과 별개인 기간 집계입니다. 정확한 ID로 기록된 사용량을 유지합니다.</p></div></header>{usage?.history?<AiUsageHistoryPanel history={usage.history} selectedWindow={period} onSelectWindow={setPeriod} exactTaskLabels={taskLabels} exactTaskAttribution={new Map()} exactTaskAttributionState="unavailable"/>:<p className="cx-empty-inline">사용 이력 근거를 아직 읽지 못했습니다.</p>}</section>
...
function RagFrame({theme,project}:{theme:string;project?:string}){
  ...
  const [src]=useState(()=>`/rag-operations.html?embedded=1&theme=${themeRef.current}${project?`&project=${encodeURIComponent(project)}`:''}`);
  ...
  useEffect(()=>{if(loaded)frameRef.current?.contentWindow?.postMessage({type:'rag-select-project',project:project??''},window.location.origin);},[loaded,project]);
  return <iframe
    ref={frameRef}
...
function ConsoleApp(){
  const {inputs,failed,loading,readAt,load,sources,progress,retry}=useSources();
  const [nav,setNav]=useState<Navigation>(()=>parseConsoleRoute(location.hash,location.search) as Navigation),[inspector,setInspector]=useState(false),[theme,setTheme]=useState(()=>{try{return localStorage.getItem('soulforge.operations.theme')==='light'?'light':'dark';}catch{return 'dark';}});
  ...
  const candidateModel=useMemo(()=>buildConsoleView(inputs,failed),[inputs,failed]);
  const stableModel=useRef<Row|null>(null),diagnosticGroup=sourceGroupState(sources,DIAGNOSTIC_SOURCES);
  if(!diagnosticGroup.pending&&!diagnosticGroup.missing&&!diagnosticGroup.errors.length)stableModel.current=candidateModel;
  const model=(diagnosticGroup.pending||diagnosticGroup.errors.length>0)&&stableModel.current?stableModel.current:candidateModel;
  const selected=model.nodes.find((n:Row)=>n.id===nav.node)??model.attention[0]??model.nodes[0];
  const go=useCallback((next:Navigation)=>{const resolved={...next,project:Object.hasOwn(next,'project')?next.project:nav.project};scrollPositions.current[nav.screen]=main.current?.scrollTop??0;setPast(old=>[...old,nav]);setNav(resolved);setInspector(false);window.history.pushState({consoleNavigation:resolved},'',consoleRouteUrl(resolved,location.search));requestAnimationFrame(()=>{if(main.current)main.current.scrollTop=scrollPositions.current[resolved.screen]??0;});},[nav]);
  const closeInspector=useCallback(()=>{setInspector(false);lastTrigger.current?.focus();},[]);
  useEffect(()=>{window.history.replaceState({consoleNavigation:nav},'',consoleRouteUrl(nav,location.search));const restore=(event:Event)=>{const parsed=parseConsoleRoute(location.hash,location.search) as Navigation;setNav(parsed);setInspector(false);if(event.type==='popstate')setPast(old=>old.slice(0,-1));};window.addEventListener('popstate',restore);window.addEventListener('hashchange',restore);return()=>{window.removeEventListener('popstate',restore);window.removeEventListener('hashchange',restore);};},[]);
  useEffect(()=>{
    const onMsg=(e:MessageEvent)=>{
      if(e.origin!==window.location.origin||e.source!==document.querySelector<HTMLIFrameElement>('iframe.ow-embedded-rag')?.contentWindow)return;
      if(e.data?.type==='rag-project'){
        const project=typeof e.data.project==='string'?e.data.project||undefined:undefined;
        if(project!==nav.project)go({...nav,project});
      }
    };
  ...
  return <SourceStateContext.Provider value={{sources,retry:key=>{void retry(key);}}}><ProjectNamesContext.Provider value={projectNames}><div className="cx-app" data-theme={theme}><a className="cx-skip" href="#console-content" onClick={event=>{event.preventDefault();main.current?.focus();}}>본문으로 건너뛰기</a><aside className="cx-sidebar"><a className="cx-brand" href="#overview" onClick={e=>{e.preventDefault();go({screen:'overview',node:null,project:nav.project});}}><span><GitBranch size={22}/></span><strong>Soulforge<small>Operations</small></strong></a><nav aria-label="운영 화면">{screens.map(([id,label,Icon])=><button key={id} aria-current={nav.screen===id?'page':undefined} className={nav.screen===id?'is-selected':''} onClick={()=>go({screen:id,node:nav.node,project:nav.project})}><Icon size={18}/><span>{label}</span></button>)}<a className="cx-rag-link" href="/rag-operations.html"><Database size={18}/><span>RAG 처리 상태</span></a></nav><div className="cx-sidebar-bottom"><span className="cx-preview-label">UX 미리보기</span><p>기존 관측을 읽습니다.<br/>서비스는 변경하지 않습니다.</p><a href="http://127.0.0.1:4192/" target="_blank" rel="noreferrer">기존 업무·조직·대시보드 <ArrowUpRight size={14}/></a><button onClick={()=>setTheme(t=>t==='light'?'dark':'light')}>{theme==='light'?<Moon size={15}/>:<Sun size={15}/>} {theme==='light'?'어둡게 보기':'밝게 보기'}</button><span className="cx-local"><LockKeyhole size={12}/>이 PC · 읽기 전용</span></div></aside>
    <div className="cx-shell"><header className="cx-topbar"><div><button className="cx-back" aria-label="이전 화면" disabled={!past.length} onClick={()=>window.history.back()}><ArrowLeft size={17}/></button><h1>{title}</h1></div><div className="cx-topbar-actions"><a href="/operations-manual.html" target="_blank" rel="noreferrer" className="cx-manual-link">사용 설명서</a><div className="os-progress" role="status">{loading?<><progress max={progress.total} value={progress.completed}/><span>{progress.completed}/{progress.total}개 항목 확인 중{progress.delayed.length?` · 응답 지연 ${progress.delayed.length}개`:''}</span></>:<span>{readAt?`마지막 확인 ${at(readAt)}`:'조회 준비 중'}</span>}</div><button aria-label="전체 데이터 다시 조회" disabled={loading} onClick={()=>void load()}><RefreshCw size={16} className={loading?'cx-spin':''}/><span>전체 새로고침</span></button></div></header>
    <main ref={main} id="console-content" className="cx-main" tabIndex={-1}><SourceNotice failed={failed}/>
      <div hidden={nav.screen!=='overview'}><OperationsDashboard model={model} inputs={inputs} failed={failed} go={go}/></div>
      <div hidden={nav.screen!=='system'}><SourceBoundary keys={['models']} label="모델 서버"><div className="vd-system-models"><ServerStrip data={inputs.models} failed={failed.includes('models')}/></div></SourceBoundary>{nav.screen==='system'&&<SourceBoundary keys={DIAGNOSTIC_SOURCES} label="구조·진단" allowPartial><OperationsSystem model={model} inputs={inputs} selected={selected} onSelect={id=>go({screen:'system',node:id,project:nav.project})} go={go} inspect={()=>{lastTrigger.current=document.activeElement as HTMLElement;setInspector(true);}}/></SourceBoundary>}</div>
      <div hidden={nav.screen!=='directory'}><DataSpaces active={nav.screen==='directory'}/></div>
      <div hidden={nav.screen!=='usage'}><SourceBoundary keys={['limits','codexQuota','agQuota']} label="남은 한도" allowPartial><QuotaStrip inputs={inputs} failed={failed}/></SourceBoundary><SourceBoundary keys={['usage']} label="사용 이력"><UsageView usage={inputs.usage} failed={failed.includes('usage')} taskLabels={new Map((inputs.threads?.threads??[]).filter((r:Row)=>typeof r.thread_id==='string'&&typeof r.display_label==='string').map((r:Row)=>[r.thread_id,r.display_label]))}/></SourceBoundary></div>
    ...
    </main></div>{inspector&&nav.screen==='system'&&selected&&<Inspector node={selected} close={closeInspector} folder={()=>go({screen:'directory',node:selected.id,project:nav.project})}/>}</div></ProjectNamesContext.Provider></SourceStateContext.Provider>;
}
