...
function TrendPlot({title,unit,series,points}:{title:string;unit:string;series:Row[];points:Row[]}){
  ...
  return <figure className="rt-plot"><figcaption><strong>{title}</strong><span>{n(latest?.[series[0].key])}<small> {unit}</small></span></figcaption>
    ...
    </svg><div className="rt-readout">{active?`${active.label??time(active.at)} · ${series.map(s=>`${s.label} ${n(active[s.key])}`).join(' · ')}`:'관측 이력 미확인'}</div>
  </figure>;
}
export function RagTrend({projects,expected,asOf}:{projects:Row[];expected:number;asOf?:string}){
  ...
  return <section className="rt-trends" aria-label="RAG 축적과 처리 추세"><header><h3>RAG 축적 추세</h3><div className="op-period" role="group" aria-label="RAG 조회 기간">{[1,7,30].map(d=><button key={d} aria-pressed={days===d} onClick={()=>setDays(d)}>{d}일</button>)}</div><small>{expected}개 과제 · KST</small></header>{trend.points.length?<div className="rt-grid-plots" key={days}>
    <TrendPlot key={`${days}-documents`} title="검색 판본의 문서" unit="개" points={trend.points} series={[{key:'documents',label:'문서',color:'var(--blue,#79afff)'}]}/>
    ...
    </div>:<p>추세를 그릴 실행 기록이 아직 없습니다.</p>}<p className="rt-note">문서·청크는 시점별 보유량입니다. {days===1?'시간':'날짜'}별 마지막 관측 · 보존된 최근 30일 실행 기록 · 전 과제 근거가 없는 구간은 연결하지 않습니다.{trend.partial?' 이전 이력은 일부만 표시됩니다.':''}</p></section>;
}
