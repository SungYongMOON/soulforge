...
async function loadOrganizationCatalog(path) {
  return readOrganizationCatalog(path);
}

function organizationCatalogOptions(loaded) {
  return loaded.status === "available" ? { organizationCatalog: loaded.catalog } : {};
}

function organizationCatalogValidationError(status) {
  if (status === "available") return null;
  return "organization_catalog_" + status;
}
...
async function run() {
  ...
  const organizationCatalogPath = String(
    process.env.TEAM_OPS_BOARD_ORGANIZATION_GOVERNANCE_OVERLAY
    || flagValue(flags, "organization-catalog", defaultOrganizationCatalogPath())
  );

  if (command === "register-existing") {
    const registry = await loadRegistry(registryPath, { permitMissing: true });
    const organizationCatalog = await loadOrganizationCatalog(organizationCatalogPath);
    const result = registerExistingThread(registry, entryOptions(flags), organizationCatalogOptions(organizationCatalog));
    ...
    safeOutput({
      ...
      entry: result.entry,
      registry_revision: result.registry.registry_revision,
      organization_catalog_status: organizationCatalog.status
    });
    return;
  }
  ...
  if (command === "rollover") {
    ...
    }, organizationCatalogOptions(organizationCatalog));
    if (result.error) throw new Error(result.error);
    if (result.changed) await writeThreadEnrollmentRegistryAtomic(registryPath, result.registry);
    safeOutput({
      command,
  ...
  if (command === "validate") {
    const registry = await loadRegistry(registryPath);
    const organizationCatalog = await loadOrganizationCatalog(organizationCatalogPath);
    const catalogError = organizationCatalogValidationError(organizationCatalog.status);
    const result = validateThreadEnrollmentRegistry(
      registry,
      organizationCatalog.status === "available" ? { organizationCatalog: organizationCatalog.catalog } : {}
    );
    const valid = result.valid && catalogError === null;
    safeOutput({
      command,
      valid,
      error: result.error ?? catalogError,
  ...
  if (command === "reconcile") {
    ...
    if (flags.snapshot) {
      ...
      const adapter = createLiveThreadAdapter({ registryPath, organizationCatalogPath });
    ...
    safeOutput({
      command,
      ...result,
      adapter_health: projection.adapter.health,
      organization_catalog_status: organizationCatalog.status,
      organization_health: projection.organization?.health ?? "invalid"
    });
    return;
  }
