export const LIVE_THREAD_PROJECTION_SCHEMA = "soulforge.team_ops_board.live_threads.v3";
...
const LIFECYCLE_SOURCE_HEALTH = new Set(["available", "missing", "invalid", "disabled", "stale"]);
...
function resultStateForGate(gate) {
...
function createProjectedThread(entry, runtime, bindingsByThreadId, gate, childResultCount = 0) {
  ...
  return {
    ...
    result_state: resultStateForGate(gate),
    attention_target: gate?.stage === "result_ready" ? gate.target : "none",
...
export function buildLiveThreadProjection({
  ...
  const effectiveResultGateHealth = derivedResultGates.health === "invalid"
    ? "invalid"
    : (RESULT_GATE_HEALTH.has(resultGateHealth) ? resultGateHealth : derivedResultGates.health);
...
export function liveThreadStatusLabel(status) {
  const labels = {
    stopped: "\uacb0\uacfc \ubbf8\ud655\uc815 \uc815\uc9c0 \uad00\uce21",
    active: "실행 중",
    waiting: "입력·승인 대기",
    error: "관측 불가",
    parent_result_ready: "하위 결과 도착/취합 중",
    owner_attention: "Owner 확인 필요",
    accepted_closed: "수락·종료 이력",
    not_loaded_unknown: "관측 불가"
  };
...
export function liveThreadResultStateLabel(resultState) {
  const labels = {
    none: "명시적 결과 게이트 없음",
    started: "시작 수신",
    delivered_to_parent: "상위 수락 대기",
    owner_attention: "Owner 대상 결과",
    accepted: "수락됨",
    closed: "종료됨"
  };
