...
export function operationIssue(node){
  ...
  if(node.collection?.recovering){cause='PLAUD 수집 재개 · 후속 처리 대기';impact=`최근 실행: 새 등록 ${node.collection.imported??'미확인'}건 · 목록 ${node.collection.catalog??'미확인'}건 확인 · 원본 보관 검증 통과`;next='기존 ID와 새 ID의 연결을 확인하며 순차 처리합니다. 미대조 ID를 모두 새 녹음으로 세지 않습니다. 수집 주기와 처리 한도는 유지합니다.';}
  ...
  const recovery=node.collection?.recovering?'기존 수집 주기로 순차 처리':!r?.available?'자동 조치 정책 조회 미확인':r.stateKey==='not_targeted'?'기존 자동 복구 대상 아님':r.stateLabel??'자동 조치 상태 미확인';
