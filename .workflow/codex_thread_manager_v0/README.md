...
- **Validation State Provenance & Truthfulness**: `last_validation_state` reflects caller-supplied metadata when declared (`last_validation_state_source: "declared"`). Validator declaration or script availability is NOT execution proof. Missing/unresolvable validator refs produce state `"unvalidated"`, malformed or unsafe rows produce state `"unknown"`, and if scanner overrides/downgrades caller state, provenance is set to `"scanner_override"`. Report `status: PASS` signifies metadata coverage completeness rather than proof that validators executed.
- **Enumerated Status Values**: `changelog_status` enum values are `"recorded"`, `"pending"`, `"missing"`. `roadmap_status` enum values are `"active"`, `"proposed"`, `"mission_bound"`, `"missing"`. `last_validation_state_source` enum values are `"declared"`, `"absent"`, `"scanner_override"`.
...
- **Default Catalog**: `DEFAULT_LIFECYCLE_RETENTION_FEATURE_CATALOG` defines explicit, public-safe feature rows for the retention slice (`codex_lifecycle_retention_core`, `codex_feature_manual_inventory`, and `codex_retention_automation`).
- **Atomic File Output**: Atomic writes are restricted to approved activity roots (`reports/codex_retention/current.json`). Escape attempts outside approved activity roots throw `write_root_escape_attempt`.
- **Activity Event**: Appends a sanitized Activity event (`scope: "codex_retention"`, `action: "prepare_report"`, `result: "prepared"|"hold"`, `refs: ["reports/codex_retention/current.json"]`) to the activity ledger before publishing `current.json` as an attempt/preparation record.
...
- **Shared owner-root / state-root override (defaults only)**: the CLI, `runLifecycleRetentionReport`, `activity_log.loadNodeIdentity`, and `activity_log.defaultActivityRoot` (so the retention Activity event and every other default-root activity writer land beside the report) read the two variables resolved by `guild_hall/shared/soulforge_state_root.mjs`, shared with the Workspace Board runtime and the AI usage meter so one state root moves the whole cluster. Precedence: an explicit flag (`--local-root`, `--activity-root`, `--enrollment`, `--lifecycle`, `--result-gate`, `--binding`) > `SOULFORGE_STATE_ROOT` > `SOULFORGE_OWNER_ROOT` > this checkout. Without `--activity-root` the activity root is `<state root>/operations/soulforge_activity`; the state files the report reads (`team_ops_board/thread_visibility.v1.json`, `thread_result_gate.v1.json`, `task_worktree_binding.v1.json`, `ai_usage_meter/lifecycle/current.json`) and `local/node_identity.yaml` resolve under the same state root, where the state root is `SOULFORGE_STATE_ROOT`, else `SOULFORGE_OWNER_ROOT/guild_hall/state`, else `<repo root>/guild_hall/state`. `--local-root` defaults to `SOULFORGE_OWNER_ROOT` or this checkout and is never redirected by `SOULFORGE_STATE_ROOT`: it keeps naming the checkout used for `git worktree list` and the docs scan. A variable that is set but empty, relative, missing, or not a directory exits 2 with `soulforge_root_override_invalid` before any root is touched (`lifecycle_retention_cli.mjs` reports the same code); values are trimmed first and a driveless Windows `\path` spelling is refused (`drive_or_unc_required`); with both unset every path is byte-identical to before. The override roots must be real directories, not a junction, symlink, or `subst` alias: the report writer's realpath/reparse seam refuses an aliased activity root downstream, so an aliased state root fails at write time rather than silently landing elsewhere. The Phase 6 registrar and `ops/run-codex-retention-refresh.ps1` keep their explicit, mandatory `-LocalRoot`/`-ActivityRoot` parameters unchanged.
