...
workflow 는 실행 절차 canon 이고 party 는 그 절차를 수행할 reusable composition canon 이다. 따라서 한글 호출 alias 가 실제 실행 선택으로 이어지는 경우에도 workflow 이름과 party 이름을 섞지 않는다.

문서상 권장 resolve 흐름은 다음과 같다.

```text
workflow global_name_ko 후보 -> workflow_id -> party_id -> path
```

- workflow alias 후보는 먼저 `.workflow/index.yaml` 의 canonical `workflow_id` 로 resolve 되어야 한다.
- party 선택은 `.party/index.yaml` 의 canonical `party_id -> path` catalog 로 resolve 되어야 한다.
- `.party/<party_id>/party.yaml` 의 `default_workflow_id` 는 routing hint 이며 workflow body 를 embed 하지 않는다.
- party naming 과 mapping 초안은 `.party/docs/PARTY_NAMING_CONTRACT_V0.md` 와 `.party/docs/PARTY_NAME_MAPPING_TABLE_V0.md` 가 소유한다.
- 이 문서는 party id, folder, index 등록값을 변경하지 않으며 alias 후보 저장 위치도 확정하지 않는다.
...
- 2026-05-18 기준 기존 44개 workflow 전체에 대한 한글 전역 호출명 inventory 초안은 `WORKFLOW_NAME_MAPPING_TABLE_V0.md` 에 따로 둔다.
