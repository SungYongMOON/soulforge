...
4. **Rendered visuals** — a strict `artifact_render_receipt_v0` from an independent visual
   reviewer must cover every positive-count page, slide, or sheet and affirm dimensions,
   clipping, overlap, blank-surface, and style-drift checks.
...
The validator proves exact semantic-packet identity and consumption evidence. For HWPX it also
compares normalized text similarity and record-ID sets. It does not independently interpret every
fact, number, unit, negation, status, source pointer, or record order.
