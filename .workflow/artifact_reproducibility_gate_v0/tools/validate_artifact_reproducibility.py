...
def inspect_render_receipt(path: Path | None) -> dict:
    ...
    expected_keys = {
        "schema_version", "reviewer_kind", "complete_surface_coverage", "surface_count",
        "reviewed_surface_count", "dimensions_checked", "checks",
    }
    expected_checks = {"clipping", "overlap", "blank_surface", "style_drift"}
    counts_valid = (
        type(data.get("surface_count")) is int
        and data["surface_count"] > 0
        and type(data.get("reviewed_surface_count")) is int
        and data["reviewed_surface_count"] == data["surface_count"]
    )
    checks = data.get("checks")
    valid = (
        isinstance(data, dict)
        and set(data) == expected_keys
        and data.get("schema_version") == "artifact_render_receipt_v0"
        and data.get("reviewer_kind") == "independent_visual_reviewer"
        and data.get("complete_surface_coverage") is True
        and data.get("dimensions_checked") is True
        and counts_valid
    ...
    return {
        "present": True,
        "valid": valid,
        "complete_surface_coverage": valid,
...
def evaluate(manifest: dict) -> dict:
    ...
    if family in {"hwpx", "docx", "pptx", "xlsx", "zip_xml_package"}:
        ...
        for item in inspected:
            ...
            if family == "docx":
                required_components.append(findings.get("has_document_xml") is True)
            elif family == "pptx":
                required_components.append(type(findings.get("slide_count")) is int and findings["slide_count"] > 0)
            elif family == "xlsx":
                required_components.append(
                    type(findings.get("sheet_component_count")) is int and findings["sheet_component_count"] > 0
                )
            elif family == "zip_xml_package":
                required_components.append(any(
                    name.lower().endswith((".xml", ".rels", ".hpf"))
                    for name in findings.get("entry_names", [])
                ))
            else:
                required_components.append(True)
        if not all(required_components):
            gates["package_structure"] = "fail"
            differences.append({"code": "required_native_component_missing", "values": required_components})
...
def validate_manifest_shape(manifest: dict) -> None:
    ...
    if set(manifest) != required:
        raise ValueError("manifest keys must match the contract exactly")
    ...
    if manifest["format"] not in {"hwpx", "docx", "pptx", "xlsx", "zip_xml_package", "generic_binary"}:
        raise ValueError("unsupported format")
    if not isinstance(manifest["candidates"], list) or len(manifest["candidates"]) < 2:
    ...
    optional_contract_keys = {"minimum_text_similarity"}
    contract = manifest["comparison_contract"]
    if not isinstance(contract, dict) or not contract_keys.issubset(contract) or not set(contract).issubset(contract_keys | optional_contract_keys):
        raise ValueError("comparison_contract keys are invalid")
    if any(type(contract[key]) is not bool for key in contract_keys):
        raise ValueError("comparison_contract flags must be booleans")
    threshold = contract.get("minimum_text_similarity")
    if threshold is not None and (type(threshold) not in {int, float} or isinstance(threshold, bool) or not 0 <= threshold <= 1):
        raise ValueError("minimum_text_similarity must be between zero and one")
    candidate_required = {
        "candidate_id", "artifact_ref", "semantic_packet_refs", "template_ref", "builder_ref",
        "toolchain_identity", "prompt_contract_identity", "declared_input_refs",
    }
    candidate_optional = {"input_consumption_receipt_ref", "render_receipt_ref", "worker_self_check_ref"}
    for candidate in manifest["candidates"]:
        if not isinstance(candidate, dict) or not candidate_required.issubset(candidate):
            raise ValueError("candidate keys are incomplete")
        if not set(candidate).issubset(candidate_required | candidate_optional):
            raise ValueError("candidate contains unknown keys")
        scalar_keys = {"candidate_id", "artifact_ref", "template_ref", "builder_ref", "toolchain_identity", "prompt_contract_identity"}
        if any(not isinstance(candidate[key], str) or not candidate[key] for key in scalar_keys):
            raise ValueError("candidate scalar fields must be non-empty strings")
        for key in ("semantic_packet_refs", "declared_input_refs"):
            values = candidate[key]
            if not isinstance(values, list) or (key == "semantic_packet_refs" and not values) or any(not isinstance(value, str) or not value for value in values):
                raise ValueError(f"{key} must contain strings")
        for key in candidate_optional:
            if key in candidate and candidate[key] is not None and (not isinstance(candidate[key], str) or not candidate[key]):
    ...
    with tempfile.TemporaryDirectory(prefix="artifact-repro-") as temp:
        ...
        for index in (1, 2):
            ...
            render.write_text(json.dumps({
                "schema_version": "artifact_render_receipt_v0",
                "reviewer_kind": "independent_visual_reviewer",
                "complete_surface_coverage": True,
                "surface_count": 1,
                "reviewed_surface_count": 1,
                "dimensions_checked": True,
                "checks": {"clipping": True, "overlap": True, "blank_surface": True, "style_drift": True},
            }), encoding="utf-8")
        ...
        receipts[0].write_text(json.dumps({"complete_surface_coverage": True}), encoding="utf-8")
        forged_render_rejected = evaluate(manifest)["gates"]["rendered_visual"] == "fail"
        invalid_manifest = json.loads(json.dumps(manifest))
        invalid_manifest["unexpected"] = True
        try:
            validate_manifest_shape(invalid_manifest)
        ...
        missing_component_manifest = json.loads(json.dumps(manifest))
        missing_component_manifest["format"] = "docx"
        missing_component_manifest["comparison_contract"]["visual_receipt_required"] = False
        missing_component_rejected = evaluate(missing_component_manifest)["gates"]["package_structure"] == "fail"
        passed = (
            receipt_a["verdict"] == "pass"
            and stable
            and forged_render_rejected
            and invalid_manifest_rejected
            and missing_component_rejected
        )
        print(json.dumps({
            "self_test_pass": passed,
            "cold_replay_stable": stable,
            "forged_render_rejected": forged_render_rejected,
            "invalid_manifest_rejected": invalid_manifest_rejected,
            "missing_component_rejected": missing_component_rejected,
            "receipt_hash": receipt_a["deterministic_receipt_hash"],
