...
- [`external_reasoning_workspace_v0/workflow.yaml`](external_reasoning_workspace_v0/workflow.yaml): public-safe unregistered workflow draft with private pilot evidence for using a session-aware external ChatGPT browser conversation as an advisory reasoning workspace with sanitized prompt packets, DOM message-role readback, private pointer refs, and no source-truth, registration, production-ready, or default-route-safety claim.
