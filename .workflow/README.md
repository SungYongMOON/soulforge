...
- [`external_reasoning_workspace_v0/workflow.yaml`](external_reasoning_workspace_v0/workflow.yaml): registered public-safe workflow with private pilot evidence for using a session-aware external ChatGPT browser conversation as an advisory reasoning workspace with sanitized prompt packets, DOM message-role readback, private pointer refs, and no source-truth, validation-authority, production-ready, or default-route-safety claim.
...
- [`presentation_artifact_render_v0/workflow.yaml`](presentation_artifact_render_v0/workflow.yaml): registered non-default candidate for rendering one approved, hash-bound presentation packet or storyline into an editable PPTX with pinned template identity, fidelity checks, visual QA, and metadata-only receipts.
- [`document_artifact_publisher_v0/workflow.yaml`](document_artifact_publisher_v0/workflow.yaml): registered non-default candidate for publishing one approved strict content packet as editable DOCX, XLSX, and/or self-contained HTML through format-native adapters under one shared design-token contract, with deterministic cross-reference validation, semantic-parity and visual QA, metadata-only receipts, and PPT explicitly excluded.
- [`outbound_mail_authoring_v0/workflow.yaml`](outbound_mail_authoring_v0/workflow.yaml): registered structure-only workflow, short alias `/outbound-mail`, for drafting owner-style outbound business mail, resolving project mail keywords, checking mandatory signature plus security footer, and preparing owner-approved send handoff without granting default external send authority.
- [`outlook_mail_reconcile_v0/workflow.yaml`](outlook_mail_reconcile_v0/workflow.yaml): registered metadata-only workflow, short alias `/outlook-reconcile`, for updating Codex-managed project sent-mail history from Outlook sent-folder metadata and cross-validating received-mail history without Outlook mutation, raw body reads, `.msg` export, attachment export, or public project mail rows. If no project is named, the default scope is all Codex-managed project ledgers, excluding `P00-000_INBOX`.
...
- [`knowledge_source_audit_v0/workflow.yaml`](knowledge_source_audit_v0/workflow.yaml): registered script-backed workflow for generating private metadata-only source storage audit reports, owner decision queues, duplicate hash reviews, and orphan workspace file candidates without moving, copying, deleting, uploading, or decoding source payloads.
...
- [`daily_work_ledger_capture_v0/workflow.yaml`](daily_work_ledger_capture_v0/workflow.yaml): registered metadata-only workflow for writing company project, `P00-000_INBOX`, and Soulforge sub-ledger daily work ledgers after activity sync so reports read prepared ledgers instead of rediscovering work.
- [`development_team1_project_bootstrap_v0/workflow.yaml`](development_team1_project_bootstrap_v0/workflow.yaml): preview-first Development Team 1 project bootstrap for classification, internal D1 identity or exact formal-code admission, separated payload/metadata/Bot workspaces, starter packets, onboarding worklog, minimum runtime planning, compatibility HOLDs, and one bounded first-work canary without granting people, external, budget, route, baseline, or acceptance authority.
- [`project_folder_indexing_v0/workflow.yaml`](project_folder_indexing_v0/workflow.yaml): registered private-pilot-executed workflow for binding a project worksite, checking missing or stale file search indexes, running bounded incremental or controlled full indexing, separating blocked files, and writing metadata-only reports without mutating originals or granting scheduler/default-route authority.
- [`project_password_unlock_copy_only_v0/workflow.yaml`](project_password_unlock_copy_only_v0/workflow.yaml): registered copy-only workflow for binding a project folder, using its project-local owner-provided password candidate file, dry-running encrypted-file routes, and writing metadata-only unlock reports without exposing password values or mutating originals.
- [`knowledge_ingest_pipeline_v0/workflow.yaml`](knowledge_ingest_pipeline_v0/workflow.yaml): registered owner-gated orchestration workflow for chaining optional copy-only unlock preprocessing, source storage audit, wiki/RAG preparation, owner decision gates, and closeout review without granting upload, NotebookLM, public promotion, index-build, migration, secret, or original-mutation authority.
