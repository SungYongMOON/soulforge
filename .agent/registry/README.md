...
- `active_class_binding.yaml`, `workspace_binding.yaml`, `capability_index.yaml`
- 본체 자산 색인과 참조 테이블
...
- `registry/` 는 source of truth 본문을 소유하지 않고, `identity/`, `policy/`, `communication/`, `protocols/`, `engine/`, `sessions/` 같은 실소유 경계로 연결만 한다.
