...
- `packages/renderer-core/` 와 `apps/renderer-web/` 안에서 `.agent`, `.agent_class`, `_workspaces` 정본을 직접 읽거나 import 하지 못하게 한다.
- 허용 예외는 `apps/renderer-web/vite.config.ts` 의 `derive-ui-state --json` bridge 뿐이다.
- canonical 경로 문자열이 있어도 producer bridge 가 아니면 FAIL 로 본다.

## package boundary lint

- `apps/renderer-web` 는 `@soulforge/renderer-core` package 경계만 사용해야 한다.
...
- palette token 정의는 `apps/renderer-web/src/theme.css` 에만 둔다.
- `apps/renderer-web/src/app.css` 와 TSX 파일에 raw hex color 를 넣지 못하게 한다.
- `apps/renderer-web/src/main.tsx` 에서 `theme.css` 를 `app.css` 보다 먼저 import 하는지 검사한다.
