...
- owner 는 위키 페이지를 정본으로 직접 선언할 수 있다. frontmatter 에 `claim_ceiling: canon_entry`, `canon_surface: private_workspace_canon`, `owner_declared_canon: true`, `public_registry_excluded: true` 를 둔다.
- 비공개 정본은 공개 `.registry/knowledge` 등록과 무관하다. 공개 canon 은 별도의 public-safe 추상만 다룬다(국방 구체값 제외).
- owner 가 `auto_cite_allowed: true` 로 표시한 정본은 응답·작업에서 **자동 인용·재사용**해도 된다(owner 사전 승인). 인용 시 source 위키 ref 를 남긴다.
