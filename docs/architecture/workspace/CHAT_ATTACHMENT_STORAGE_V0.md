...
4. ERP 서비스는 인증된 item과 workspace 권한을 다시 확인한 뒤 manifest의 크기·hash·
   realpath를 검증한다. 선택된 첨부만 disposable turn projection으로 copy+rehash하고,
   Codex worker에는 원본 경로가 없는 descriptor와 projected file만 전달한다. worker는
   canonical attachment root를 stat/read하지 않는다. 브라우저가 제출한 경로는 사용하지 않는다.
...
- 운영에서는 root가 Soulforge `_workspaces/system/dev-erp` 안에 있다는 증거와 별도
  ERP owner identity를 release audit가 확인해야 한다. worker identity는 이 canonical
  root를 읽지 못해야 하며 별도 single-active projection root만 읽는다. worker가 서명한
  pathless payload-deny binding은 ERP가 exact attachment/message lexical root로 독립
  계산한 기대값과 일치해야 하고, 두 계산 모두 canonical root를 stat/read하지 않는다.
...
- 구현: `ui-workspace/apps/dev-erp/server.mjs`,
  `src/codex_attachment_registry.mjs`, `src/codex_turn_projection.mjs`
- 정본 테스트: `test/codex_attachment_registry.test.mjs`, item 인증/업로드 통합 테스트,
  `test/codex_turn_projection.test.mjs`, payload 포함 backup/restore 검증
