...
- `workspace/schema/` 는 `_workmeta` example contract, binding 파일, owner-only shared metadata event stream 의 사람이 읽는 canonical schema 문서를 둔다.
...
- battle event stream 은 `_workmeta/<project_code>/log/events/YYYY/MM/battle_events.jsonl` 아래 owner-only shared metadata append stream 으로 둔다.
