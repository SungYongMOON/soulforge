...
| D46(제안 2026-08-18) | 규칙 행의 종류 확장 — 산출물(문서)만이 아니라 활동·결정 노드(예: PBS 확정, 기능분석, 트레이드 스터디, 기본설계, 리서치, 이해관계자 확인)를 같은 표에 두고 선후 관계(depends_on)를 붙일지 | 확장. , 활동·결정의 증거는 기록(회의록·결정 기록·산출물)이며 판정 어휘는 그대로(satisfied/gap_missing/gap_unknown), 어휘에 activity/decision 계열 추가, 컴파일러가 depends_on으로 순서 계산 |
| D47(제안 2026-08-18) | 엔진 mission 후보를 서브 에이전트가 읽는 지시서(무엇을·왜·입력·산출·근거·담당·기한) 계약의 owner | 엔진 owner의 별도 계약(, zero-write). 지시서 = mission 후보 + 가이드 카드(정본 인용) + 맥락 채움(기한·담당·사연); 판단을 바꾸지 않음 |
