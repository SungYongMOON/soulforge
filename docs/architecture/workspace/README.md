...
| `PROJECT_REQUIREMENT_TRACE_MODEL_V0.md` | (DRAFT/canon_candidate) 과제 맥락 = append-only 사실 원장 + 재생 가능한 타입 그래프·RTM 투영 + 별도 수락 게이트 + 얇은 카드; 계약 요구 ID→파생→산출물→시험 커버리지 계산 규칙과 Graph DB(Neo4j)·GraphRAG 채택 조건(조회 전용 projection, D41 2026-09-12)을 고정한다. `PROJECT_CONTEXT_GRAPH_MODEL_V0.md`의 SE 확장. |
...
| `SOULFORGE_ARTIFACT_TEMPLATE_SYSTEM_V0.md` | authoring과 artifact rendering을 분리하고, template family/revision/SHA-256, 포맷별 네이티브 템플릿, `_workspaces` payload와 `_workmeta` receipt 경계를 고정한다. |
...
- [`SOULFORGE_ARTIFACT_TEMPLATE_SYSTEM_V0.md`](SOULFORGE_ARTIFACT_TEMPLATE_SYSTEM_V0.md)
