...
| `pc_local_runtime_tool` | Local LLM installs, venvs, tool drops, executable runtime folders, or machine-only tooling. | Move to `_workspaces/_local/<node_id>/...`, OS/env tooling, or `guild_hall/state/tools/**`. |
...
3. Preserve the existing normal folder under `_workspaces/_local_hold/...`.
4. Create the shared-view link at the original repo-relative path.
5. Verify the link view without printing host-local target paths.
6. Compare preserved local holds against the shared target by metadata and
   hash.
7. Classify preserved content as shared keep, project move, regenerate,
   PC-local keep, or conflict review.
