...
- raw truth와 artifact는 workspace/worksite에 존재하고, local `_workmeta/<project_code>/runs/<run_id>/`에는 public-safe example과 같은 compact receipt만 존재한다.
...
- 이 `runner/` 는 설명용 sample packet bundle 이며 local `_workmeta/<project_code>/` 의 required folder 가 아니다.
- actual local prototype와 실행 script는 workspace/worksite 또는 public-safe code owner에 두며 `_workmeta/<project_code>/tools/`를 만들지 않는다.
