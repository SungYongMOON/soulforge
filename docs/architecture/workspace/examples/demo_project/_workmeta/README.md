...
- raw truth 는 local `_workmeta/<project_code>/runs/<run_id>/` 아래에만 존재한다.
...
- actual local prototype 는 한 예시로 `_workmeta/<project_code>/tools/` 아래 script 로 존재할 수 있지만, 이 경로를 required implementation convention 으로 고정하지는 않는다.
