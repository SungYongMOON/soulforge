...
- actual local runner implementation 은 한 예시로 dedicated `runner/` folder 대신 `.project_agent/tools/` 아래 prototype script 로 존재할 수 있지만, 이 경로를 required implementation convention 으로 고정하지는 않는다.
