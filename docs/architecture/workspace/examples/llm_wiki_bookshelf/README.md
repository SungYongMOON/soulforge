...
bookshelf flow. It mirrors the operating shape for an owner-managed Google Drive
bookshelf and NotebookLM packet planning without storing source payloads, live
Drive IDs, NotebookLM account state, or runtime absolute paths.

## Files

- `canonical_source_intake_checklist.md` is the manual intake checklist for
  deciding whether a source can move from candidate to canonical bookshelf use.
...
1. Place or identify candidate source material in the owner-held bookshelf
   surface. Keep drafts, raw mail, local-only working files, and uncertain
   versions out of active NotebookLM source sets.
...
- Google Drive is the owner-held source bookshelf surface.
- NotebookLM is an advisory question and synthesis interface over selected
  approved sources.
