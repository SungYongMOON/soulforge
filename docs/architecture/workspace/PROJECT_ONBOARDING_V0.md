...
- 실제 프로젝트 현장 materialization 과 `.project_agent` 도입 순서를 분리해 과한 초기 반영을 막는다.
...
8. 사람에게 보여줄 full project title 은 `.project_agent/contract.yaml` 의 `display_name` 에 둔다.
9. 첫 실제 프로젝트 온보딩은 `.project_agent` 를 바로 active 로 만들지 않고, 먼저 read-only intake 로 구조와 민감 경계를 확인한 뒤 아주 작은 first run/use 로 들어간다.
...
6. 그다음에만 `.project_agent/` minimal shape 를 draft 로 만든다.
...
- 사람용 실험 메모와 온보딩 문서는 `_workspaces/<project_code>/.project_agent/reports/onboarding/` 아래에 둔다.
- 사람과 Codex 가 같이 진행한 시작 단계 판단, blocker, 다음 액션은 `_workspaces/<project_code>/.project_agent/reports/onboarding/project_start_worklog.md` 에 append 한다.
- 사용자가 따로 요청하지 않아도 새 시작 행위의 실제 작업 순서와 절차 초안은 `project_start_worklog.md` 와 관련 onboarding note 에 함께 남긴다.
- 근거 파일, 임시 export, 비교 산출물은 `_workspaces/<project_code>/.project_agent/artifacts/onboarding/` 아래에 둔다.
