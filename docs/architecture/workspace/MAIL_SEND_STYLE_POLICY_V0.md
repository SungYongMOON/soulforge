...
## 대용량 첨부파일 및 Google Drive 링크 전송 규칙

- 실제 첨부파일 총용량이 약 20MB 이하이면 일반 메일 첨부를 우선한다.
- 약 20MB를 초과하거나 수신·발신 메일 서버에서 반송될 가능성이 있으면 Google Drive 링크 방식을 우선한다.
- Drive 링크 방식은 전용 폴더 `메일_대용량_임시공유`를 사용한다.
- 메일 본문에는 전체 폴더 링크가 아니라 해당 메일에 필요한 정확한 파일별 링크만 넣는다.
- 서로 다른 업무의 파일을 하나의 링크나 공유본 묶음에 무분별하게 섞지 않는다.
- 수신자에게는 `뷰어` 권한만 부여하고, 링크로 전달한 파일을 같은 메일에 다시 첨부하지 않는다.
- 본문에는 `첨부파일 용량 관계로 Google Drive 링크로 전달드립니다.` 정도로 자연스럽게 안내한다.
- `draft_only` 단계에서는 파일을 Drive에 업로드하거나 외부 공유 링크를 생성하지 않는다.
- owner가 실제 발송을 요청한 경우에만 필요한 파일의 발송본 또는 복사본을 전용 공유 폴더에 업로드하고 파일별 링크를 생성한다.
- owner가 지정한 파일은 임의로 다른 버전이나 유사 파일로 바꾸지 않는다.
- 기밀, 대외비 또는 외부 링크 공유가 부적절할 수 있는 자료는 업로드·공유하지 않고 owner에게 먼저 확인한다.
- 기존 프로젝트 원본 파일은 이동, 삭제 또는 권한 변경하지 않는다.
- Drive의 공유 파일은 메일 전달용 복사본 또는 발송본으로 취급하며, 프로젝트 원본의 저장 위치나 권한을 대신하지 않는다.
