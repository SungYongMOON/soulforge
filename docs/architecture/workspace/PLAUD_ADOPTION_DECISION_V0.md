...
- 원본 오디오를 내려받을 수 있어 provider 결과가 틀려도 독립 전사, 화자분리,
  사실 확인을 다시 수행할 수 있다.
- 전사 완료 메일을 신호로 사용하면 active always-on voice writer가 공식 CLI 수집을
  시작할 수 있다.
...
### 원음 우선 보관 — 2026-09-10 Owner 결정

- 다운로드 가능한 원본 오디오는 제공자 전사 유무와 분리하여 먼저 보관하고 독립 ASR을 수행한다.
- `readiness.require_transcript`는 profile 선택이다. 기존 strict 기본값 `true`는 유지하며,
  승인된 원음 우선 경로는 `false`를 명시하고 세션의 기존 `post_import_contract`에
  `provider_transcript_required: false`를 기록한다. 필드가 없는 과거 세션은 strict로 해석한다.
- 원음 보관, 제공자 전사 가용성, 독립 ASR, 전달 파일 집합의 완료를 각각 판정한다.
  제공자 미가용을 처리 중이라고 추정하거나, 빈 제공자 파일·독립 ASR로 제공 완료를 꾸미지 않는다.
- 제공자 전사가 나중에 실제로 생기면 같은 provider ID에 보완한다. 원음을 중복 다운로드하거나
  기존 독립 ASR 결과를 제공자 자료로 바꾸지 않는다. 실제 제공된 파일은 필수 검증을 통과해야 한다.
- 새 원음 우선 세션을 쓴 뒤에는 이전 strict-only 설치본(예: 0.1.13)의 완전한 호환성을 가정하지 않는다.
  쓰기 전 동기 실패와 쓰기 후 복구를 구분하고, 후자는 새 data/cache를 보존한 상태에서 호환성을 검증한다.
...
## 2026-07-18 owner 장비 역할 보정

2026-07-13의 AI 임시 확정 처리선은 유지하되, 맥미니 단일 writer 장비 지정은
다음 target으로 대체한다.

- 회사 NAS 접근과 장시간 고성능 처리가 가능한 HPP의 별도 always-on identity를
  정상 voice processing operational-primary로 둔다.
- 맥미니는 경량 감시, source-local HOLD/outbox, fallback/mirror와 임시 개발면을
  맡는다.
- HPP가 현재 꺼져 있거나 exact cutover receipt가 없으면 맥미니의 기존 수집을
  끄지 않는다. 이 기간에는 맥미니가 temporary failover 단일 writer다.
- HPP 복귀 뒤 shared workspace materialization, producer receipt/consumer ack,
  queue freeze, catch-up/replay, lease/epoch가 모두 확인된 뒤에만 정상 writer를
  HPP로 넘긴다. 두 장비의 동시 shared writer는 금지한다.
- NAS 접근 가능성은 source read capability이며 NAS mutation, 자동 ingest, source
  승인 또는 canon 승격 권한을 자동으로 만들지 않는다.
...
active writer 외 PC는 원음 처리 surface를 동시에 쓰지 않는다. 다른 PC는
public/private Git을 pull하고 shared worksite의 payload를 확인한 뒤 자기 node
이름의 consumer ack와 허용된 프로젝트 metadata만 기록한다.
...
5. 하이웍스 메일에서 active voice writer 수집까지 끊김 없이 이어지는가.
...
이 절은 HPP unavailable 기간의 임시 연속성 절차다. 정상 TARGET writer를
맥미니로 되돌리는 운영 결정이나 HPP failback receipt를 대신하지 않는다.
