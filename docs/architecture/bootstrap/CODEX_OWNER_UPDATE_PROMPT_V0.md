...
- public repo 는 Soulforge root, private repo 는 Soulforge root 아래 nested `_workmeta/` 와 `private-state/`
- behind 인 repo 만 `git pull --rebase origin main` 으로 갱신해
- local env, token, password, cookie, session, credential JSON 은 읽거나 바꾸지 마
- pull 이 끝나면 `npm run skills:sync -- --all` 를 실행해
...
- Windows PowerShell 에서 `npm.ps1` execution policy 로 막히면 위 `npm run ...` 명령은 같은 의미로 `npm.cmd run ...` 형태로 실행해

끝나면 아래만 짧게 보고해
- public/`_workmeta`/`private-state` 각각 before ahead/behind
- public/`_workmeta`/`private-state` 각각 pull 여부
- skills sync 결과
- final doctor 결과
```
