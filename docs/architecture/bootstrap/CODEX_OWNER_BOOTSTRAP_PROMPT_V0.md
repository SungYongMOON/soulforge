...
아래 프롬프트를 다른 PC 의 Codex 에 그대로 넣으면, 기존 설치를 정리하고 Soulforge root 아래 owner-only nested `_workmeta/`, `private-state/` 구조로 다시 깨끗하게 bootstrap 하도록 지시할 수 있다.
...
- project metadata repo 는 Soulforge root 아래 nested `_workmeta/` 로 둔다
- cross-project continuity repo 는 Soulforge root 아래 nested `private-state/` 로 둔다
...
작업 규칙:
- 먼저 현재 디스크에 Soulforge, `_workmeta`, `private-state` 관련 기존 폴더가 있는지 확인해
- 기존 설치가 잘못되어 있으면, public repo 와 private repo 의 Git 상태를 먼저 확인하고 보고해
- 보존해야 할 local env 가 있으면 먼저 알려주고, 그 뒤 잘못된 설치 흔적은 정리해
- 최종 상태는 Soulforge root 아래 nested `_workmeta/`, `private-state/` 만 남기고, 이전 잘못된 배치는 남기지 마
- private repo 는 public repo 의 nested git 이지만, public Git 에 올라가면 안 되므로 public repo 의 `.gitignore` 가 `_workmeta/`, `private-state/` 를 무시하는지 확인해
...
5. `_workmeta/` private repo 가 있으면 Soulforge root 아래 `_workmeta/` 로 재배치하고, nested Git repo 인데 `origin` remote 가 비어 있으면 private remote 를 먼저 연결해
   - `cd _workmeta && git remote add origin <workmeta-repo-url> && git fetch origin main && git switch -C main --track origin/main`
6. `private-state/` private repo 가 있으면 Soulforge root 아래 `private-state/` 로 재배치하고, nested Git repo 인데 `origin` remote 가 비어 있으면 private remote 를 먼저 연결해
   - `cd private-state && git remote add origin <private-state-repo-url> && git fetch origin main && git switch -C main --track origin/main`
7. private repo 가 없으면 Soulforge root 에서 아래처럼 clone
   - `git clone <workmeta-repo-url> _workmeta`
   - `git clone <private-state-repo-url> private-state`
8. public repo 에서 `npm install` 실행
9. sync 가능한 Soulforge Codex skill 전체를 sync
   - `npm run skills:sync -- --all`
10. owner-with-state 기준으로 doctor safe 실행
   - `npm run guild-hall:doctor -- --profile owner-with-state`
11. GitHub/remote 상태 확인
   - `npm run guild-hall:doctor -- --profile owner-with-state --remote`
12. 필요한 local env 파일이 없으면 example 에서 복사해 자리만 만들어
13. 내가 실제 값을 넣어야 하는 파일 목록과, 내가 직접 열어 복사해야 하는 원본 파일 경로만 따로 알려줘
14. local env 를 내가 채운 뒤 live doctor 실행
   - `npm run guild-hall:doctor -- --profile owner-with-state --live`
15. 끝나면 아래를 짧게 보고해
   - public repo 경로
   - `_workmeta` repo 경로
   - `private-state` repo 경로
   - 기존 설치 정리 여부
   - skills sync 결과
   - doctor safe 결과
   - doctor remote 결과
...
- `_workmeta/`, `private-state/` private repo 는 Soulforge root 아래 있어야 한다
- 최종 상태는 Soulforge + nested `_workmeta/` + nested `private-state/` 구조로 정리할 것
- 기능 코드/문서/public-safe sample 은 public repo 로, project metadata 는 `_workmeta/` 로, continuity data 는 `private-state/` 로 저장할 것
