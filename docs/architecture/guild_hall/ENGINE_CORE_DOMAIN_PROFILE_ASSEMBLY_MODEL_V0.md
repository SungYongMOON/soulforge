...
`engines/reliability_maintainability/`, `engines/calibration_measurement_validity/`, `engines/configuration_change_impact/`, `engines/pcb_compliance/`와 `.registry/engineering_profiles/`로 물리 migration이
...
- 공통 Core와 Domain Engine(`systems_engineering`, `quality_readiness`, `database_engineering`, `material_procurement_readiness`, `reliability_maintainability`, `calibration_measurement_validity`, `configuration_change_impact`, `pcb_compliance`) 및 Profile schema·adapter 물리 분리가 완료됐다.
...
- E01 Quality Readiness, Database Engineering, E03 Material Procurement Readiness, E04 Configuration Change Impact, E06 Reliability & Maintainability, E11 Calibration & Measurement Validity, E10 PCB Compliance의 `source_supported` candidate integration 및 multi-domain adapter conformance가 완료됐다. 이 통합은 live DB·ERP writer, production release, 표준 채택·준수·인증, 제품/품질 합격, 변경 승인·baseline mutation·closure·repair·release authority 또는 project acceptance를 의미하지 않는다.
