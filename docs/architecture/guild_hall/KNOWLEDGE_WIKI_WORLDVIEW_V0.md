...
### Source Revision과 시간 동기

같은 문서명도 개정판·효력 시점·원본 byte가 달라질 수 있으므로 Wiki와 RAG는
mutable path나 현재 파일만 가리키지 않는다. 새로 만들거나 갱신하는 sourcebound
projection은 `source_id`, exact `source_revision_id`, full `content_id`를 출발점으로
삼는다.

- private Wiki는 `wiki_page_id`, `wiki_revision_id`, `source_revision_refs`를 가진다.
- RAG index/chunk도 같은 `source_revision_id`로 돌아갈 수 있어야 한다.
- source revision이 바뀌면 기존 Wiki를 덮어쓰지 않고 새 revision을 만든다.
- legacy projection에 exact revision이 없으면 읽을 수는 있어도 point-in-time
  source support를 증명한 것으로 보지 않는다.
- legacy `compiled_projection`/`projection` ref는 새 ID 축으로 복제하지 않는다.
  verified `source_id + stable page key`로 `wiki_page_id`를 만들고,
  `source_revision_id + wiki body content hash`를 포함해 `wiki_revision_id`를
  결정적으로 mapping한 뒤 legacy 값은 typed ref로 남긴다.
- 업무 사용은 bare `knowledge_id`가 아니라 public registry의 exact git/content revision
  또는 owner-declared private canon의 exact Wiki revision을
  `knowledge_revision_ref`로 고정한다.
...
그 결정/포인터는 `_workmeta`에 metadata-only로 남긴다.
