...
- active design follow-up(2026-07-14): `ENGINE-13-TASK-DRIVER-CLOSED-LOOP`는 기존
  `core_item`/append-only event task truth 위에 `why/why-now` TaskDriver, 판단/적용과
  작업 상태의 두 축, completion feedback을 잇는 `canon_candidate`다. ID/TaskDriver/RAG
  synthetic 구현과 project-local RAG 단일 pilot은 완료했다. TaskDriver live DB pilot은
  operational-primary에서만 수행하며 writer/scanner/scheduler는 별도 activation gate다.
