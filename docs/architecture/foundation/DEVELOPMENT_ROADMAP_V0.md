...
설치/업데이트·매뉴얼을 먼저 출시 후보로 완성한다. 현재 한글 작업은 끝까지 진행하며,
Buzz의 Claude 한글 공방과 기존 HWPX 스킬을 연결해 실제 작성·출력·검사·결과 전달을
검증한다. 코드·합성 검사·봇 등록·실제 실행·사람 수락은 구분한다.

나머지 전문 공방의 신규 실행 연결과 고도화는 Owner가 명시한 **출시 이후 순차 확장**
범위다. 기존 구현·검증 성과는 보존하고 공방별로 하나씩 붙인다. 미완료를 완료 또는
Owner Gate로 바꾸지 않으며, 이 후속 공방들을 기본 운영 출시의 선행조건으로 두지 않는다.
현재 한글은 이 연기 범위에 포함하지 않는다. 운영·키·원문·정본·외부 반출의 기존 권한
경계는 유지하되, 명시된 Claude 한글 공방 생성과 합성 관통 시험을 계속 진행한다.
...
- adjacent Internal RC contract build(2026-09-01): OD-11 pure taxonomy는 기존 A0~A6와
  ...
  검증한다(17 tests, actual acceptance/NAS/RPO/RTO 0). Current tracked Pack specs는 HPP 1,022,
  Universal Client 19, Backup-Recovery 76 files로 재계산됐고 focused validators가 통과했다.
  Active HPP는 그 시점 기준 `0.1.2`였고, 2026-09-02 항목대로 `0.1.6`으로 전환됐다. 이는
  one-seat physical Gate의 준비 증거이며 설치·RC 수락은 아니다.
...
### First usable pilot priority (Owner 2026-09-08)

첫 운영 후보는 **Buzz 단일 지시·대화 → 봇 수행 → 작업대의 대기·진행·이력·결과
조회**에 집중한다. Owner와 선택된 소수 팀원, 확실히 작동하는 업무 한 종류로 시작해
실사용 피드백을 받으며 SW별로 계속 개발·업데이트한다. 작업대의 native 직접 실행
버튼은 첫 출시 범위에서 제외하고 별도 개발자 실험으로 보존한다. 공통 내부 실행기·
권한·검증·기록은 재사용한다. 아래 첫 pilot gate는 전체 로드맵 완료율과 별개의 분모다.

| Gate | 첫 pilot 완료에 필요한 관측 |
| --- | --- |
| FP1 | Buzz의 실제 요청·요청자·현재 역할/권한·입력 판본이 같은 업무ID와 원지시 ref로 결속됨 |
| FP2 | 선택한 봇/모델/도구가 현재 실행 환경에서 인증·범위 사전 검사 후 실제 업무 한 종류를 수행함 |
| FP3 | 사람 답변이 필요한 질문만 정확한 수신자에게 표시·알림; 이유·질문ref·대기시작·경과시간·Buzz 링크를 확인함 |
| FP4 | 같은 대화의 답변 후 정확히 한 번 재개하고, 읽음·미루기·의도적 pause·완료 idle·프로세스 생존을 업무 대기와 구별함 |
| FP5 | 실패·불명 실행·재시도·취소·재시작의 기록과 중복방지가 작동하며 기술 막힘은 운영 담당으로 전달됨 |
| FP6 | 실제 산출물/검증과 원문 지시·공개 응답·실제 도구 입출력·변경을 권한 있는 사람이 보호된 작업영역에서 조회함; thinking/reasoning은 제외 |
| FP7 | 코드와 작업 원문을 분리한 정확한 후보의 필요한 검사·독립 검토·설치/되돌리기·기록 복구와 간단한 사용 절차가 검증됨 |
| FP8 | 위 한 바퀴의 실제 시험 근거, 남은 운영 활성화·사람 수락 조치, 첫 사용자 피드백 경로를 제시함 |

전봇 명부를 재구축하지 않고 활성 업무·대기·정체를 우선 표시한다. 모든 공방·모델,
2.5D 최종 품질, 전문 엔진 확장은 전체 로드맵에 유지하되 첫 pilot의 선행조건으로 두지
않는다. 이 우선순위는 main/운영 전환·원천 접근·외부 반출·사람 수락의 기존 권한을
새로 승인하지 않는다. 첫 pilot의 통과를 전체 Goal 완료나 전체 SW 운영 완료로 보고하지 않는다.

### Owner-directed integrated release-candidate completion (2026-09-07, retained roadmap)
...
결속은 아직 진행 중이다. 기본 Claude 한글 공방의 대표 합성 문서는 실제 작성·교정·
한글 COM PDF 출력·전체 페이지 확인을 마쳤다. 실제 업무 발견 모델·자료 공급,
자동 개선 전체 실행과 첫 Buzz 질문→답변→결과 관통은 별도 기술 잔여로 유지한다.

후속 기본 운영 통합에서는 회사의 공개 WorkPacket을 기존 Linear·현재 규칙과 대조하는
업무 발견, 기존 로그인에 결속한 읽기 화면, UNKNOWN·현재 철회를 보존하는 합성 복구를
연결한다. 개발 관리의 한국어 상태·상세 이동·검토/전달/수락 구분도 보완한다. G1의 발행
현재성은 인증된 SENDER 응답을 요구하며 실제 계정·설치·응답시간은 따로 검증해야 한다.
한글은 기존 Buzz 기본 Claude의 역할·작업폴더·스킬·기본 기능을 우선 재사용한다.
별도 scoped 연결기·문서 job 관리·broker 후보는 보존하되 첫 사용·출시 선행에서 제외한다.
반복 문서 작업·첨부·과거 결과 회수도 기본 기능을 먼저 확인하며 새 연결기를 중복 제작하지 않는다.
대표 문서 성공을 모든 문서 유형, 강제 보안 격리 또는 전체 제품 완료로 확대하지 않는다.
...
| 24 | **현재 RC 필수 연결(Owner 2026-09-08)** — 할일 생성의 내부 흡수 — Chat 예약작업(1시간 업무인입·3시간 Backlog 점검, 현재 A0 Shadow)이 Slack·Plaud·Gmail·Linear에서 만들던 후보를 `guild_hall/forge_intent` 발견 어댑터(P6)로 옮김: 근거 1순위는 Rune 기대 산출물·게이트·커버리지, 자료 원천은 Tributary custody, 세 조각 분해(준비·내부 인계·최종 결정), 자료 인계 패키지 = Work Brief 입력 manifest, 정책 grant는 C6 tuple 규칙 | 결속 계약 커밋과 Rune 계층 계약 커밋 2가 main에 있음; 골든 사례는 private, 저장소는 합성 fixture만 | `guild_hall/forge_intent`, `docs/architecture/foundation/team_member_engineering_program/04_*`, `ui-workspace/apps/dev-erp/docs/TASK_ENGINE_*` |
...
| 30 | 보호 가공 업무 lane 사이클 2호 (외부 작업자 실제 1왕복 · Tongs 보관 · 독립 의미 검토) | 사이클 1호 합성 한 바퀴가 닫혔음(2026-09-05, UTC). 사이클 1호 fresh 검토가 permit 자기참조(B1)를 지적해 신뢰 키 결속(BIND09)으로 고쳤으니, 사이클 2호 시작 전에 그 신뢰 키의 신원 결속까지 닫는다. Owner가 (a) 외부 provider 키 한 줄 파일 배치와 route 활성화, (b) Tongs bearer 발급, (c) 합성 유지 여부를 정하면 시작. 실자료 canary는 별도 결정이며 이 후보에 포함하지 않음 | `guild_hall/secure_work`, `docs/architecture/guild_hall/SECURE_WORK_CYCLE_V0.md`, `<private_root>/config/secure_work` |
