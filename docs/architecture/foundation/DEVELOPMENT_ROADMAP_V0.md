...
| 23 | knowledge/RAG 파이프라인 병목 burndown (B1 promotion-ready 가시화, B2 ontology 후보 통합, B3 stage 커버리지·retrieval 지표, B4 NotebookLM advisory 레인 결정, B5 pending-work 단일 뷰) | owner 가 promotion 자동 가시화 허용 여부·throughput 단위·NotebookLM 재투자 여부를 정함 | `guild_hall/knowledge_access`, `guild_hall/rag`, `.workflow`, `docs/architecture/foundation`, `ui-workspace`, `_workmeta/system` |
...
후보 23 의 상세 후보 패킷은 `_workmeta/system/dev_worker_candidate_queue/knowledge_pipeline_bottleneck_burndown_v0.yaml`
(`status: proposed`, B1~B5 sub-slice) 에 있으며, owner 승인 전에는 실행 큐로 승격하지 않는다. 각 sub-slice 는 실행 전 개별 execution-ready dev_worker_request 로 분할한다. 출처는 2026-06-13 Fable5 지식 서브시스템 심층 검토다.
