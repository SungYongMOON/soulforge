...
| 28 | 보호 가공 업무 lane 사이클 2호 (외부 작업자 실제 1왕복 · Tongs 보관 · 독립 의미 검토) | 사이클 1호 합성 한 바퀴가 닫혔음(2026-09-05). Owner가 (a) 외부 provider 키 한 줄 파일 배치와 route 활성화, (b) Tongs bearer 발급, (c) 합성 유지 여부를 정하면 시작. 실자료 canary는 별도 결정이며 이 후보에 포함하지 않음 | `guild_hall/secure_work`, `docs/architecture/guild_hall/SECURE_WORK_CYCLE_V0.md`, `<private_root>/config/secure_work` |
