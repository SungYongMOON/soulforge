...
2026-09-09 정정: 기존 Buzz 뒤의 자료·맥락·권한→업무 발견·배분→실행→검증·수락→정본·지식 연결이 목표다. 단일 실행 조회 화면·질문 재개·검사 수는 전체 연결의 완료가 아니다. 기본 운영과 기존 builtin Claude 한글을 우선하고 다른 공방은 출시 후다. 로드맵 상단의 실제 검증 한계와 전체 워크트리 색인을 먼저 읽는다. 현재 요청은 기존 변경의 검증·병합 정리이며 새 기능 개발과 운영 전환을 자동 재개하지 않는다.
