...
| `driver_id` | task 후보/전이의 `왜/왜-지금` causal record | ENGINE-13 candidate |
...
경계만 하네스가 고정하고, 해석·분류·일회 계산은 가능한 한 모델에 남긴다.

| 층 | 기본 동작 | 예시 |
| --- | --- | --- |
| 작은 중심 맥락 | 매 질의에 짧게 포함 | project/gate/branch 현재 상태, 얇은 SE 흐름 지도, 현재 적용 규칙·용어 |
| 필요시 근거 | 질문 범위를 고른 뒤 top-k만 포함 | 방위사업청 가이드북 exact page/chunk, 소나 설계자료, 사내 규칙, 과거 해결 근거 |
| 필요시 도구 | 수치 업무가 요구할 때만 호출 | 기존 workbook, 임시 계산 코드, 검증된 O-ring·잡음 계산기 |

계산기는 RAG/Wiki나 기본 과제 맥락이 아니다. 다음 중 하나가 반복해서 확인될 때만
등록 도구로 강화한다.

- 동일 계산의 반복 사용
- 동일 입력에 같은 결과가 필요한 재현성
- 규격·안전·품질·감사 때문에 식과 단위 검증이 필요한 경우
- 입력 범위와 검증 fixture를 고정할 수 있는 경우

그 전에는 LLM이 계산 방법을 설명하거나 거친 추정치를 제안할 수 있고, bounded 임시
스크립트나 기존 spreadsheet를 사용한다. 다만 실제 결정·할일·산출물에 쓰는 수치는
자유 형식 LLM 산술을 그대로 신뢰하지 않고 결정론적 스크립트·spreadsheet·등록 도구로
다시 계산해 단위와 입력 범위를 검증한 뒤 시간축 사건과 근거로 연결한다.

### 10.2 자료가 늘어날 때의 조회 순서

RAG와 Wiki 중 하나를 지식 전체로 삼지 않는다.
...
먼저 거르는 catalog route를 우선한다. BM25/vector/reranker/GraphRAG는 고정 평가셋에서
현재 lexical retrieval의 부족이 확인된 뒤 추가한다.

사용 원장 집계는 원문을 읽지 않는 한 번의 streaming pass로 수행한다. 원장 규모가
5만 행을 넘거나 snapshot 집계가 반복해서 500ms를 넘으면 월별 rollup을 만들고 최근
원장만 증분 집계한다. 그 전에는 별도 집계 DB를 선행 도입하지 않는다. 집계 화면에는
유효 행뿐 아니라 무효 행·중복 event·읽지 못한 file 수도 함께 표시해 coverage를 숨기지
않는다.

### 10.3 지식 방명록과 보존 판단

접근 횟수 하나로 사용과 중요도를 동시에 판단하지 않는다.

| 사건 | 의미 | 기본 가중치 |
| --- | --- | --- |
| `retrieve` | 검색 결과가 실제 답변 context로 선택됨 | 낮음 |
| `read` | 사람·도구가 지식 ref를 열어 확인함 | 낮음 |
| `cite` | 답변·문서에 근거로 제시됨 | 중간 |
| `apply` | 과제 판단·할일·산출물에 실제 반영됨 | 높음 |
| `validate` / `promote` | 검증 또는 정본 승격 절차에 사용됨 | 높음, 별도 authority 필요 |

각 metadata-only event는 `timestamp_utc`, actor, project/gate/branch/task, exact target 또는
opaque target id, revision ref, retrieval run/rank, 사용 이유 label, output ref, outcome을
가진다. raw 질문, 원문, chunk body, 메일·음성 본문, secret은 기록하지 않는다. 검색된
모든 후보가 아니라 **답변 context에 선택된 근거**부터 기록하고, 실제 적용은 별도
`apply` event로 남긴다.

물리 저장 위치인 ledger/shard ref는 사건 identity와 logical dedupe key에 넣지 않는다.
저장되는 RAG 답변은 실행별 불변 output path와 `output_revision_ref`를 사용한다. 답변을
쓰기 전 shared workspace output은 project code와 무관한 output-global coordination
surface에서, project-private output은 해당 `_workmeta` owner에서 원자적으로 선점한다.
여러 PC의 상호배제는 모두 같은 shared workspace filesystem을 볼 때만 성립한다. 그 뒤 과제별 `_workmeta` 접근이력 owner에
metadata-only pending receipt를 만든다. 원장 append 뒤 expected event를 다시 읽어 모두
유효하고 실제 `ledger_ref`도 일치할 때만 `recorded`로 닫는다. append 실패 시 pending receipt가 남아 LLM 답변을 다시
만들지 않고 재조정할 수 있다. 재조정은 원래 PC shard가 아니라 복구를 수행한 PC의 별도
recovery shard에 기록하고 partial tail을 유효 사건으로 오인하지 않는다.

정리 후보는 두 축을 분리한다.

| | 중요도 높음 | 중요도 낮음 |
| --- | --- | --- |
| 사용 많음 | 핵심 지식 | 편의 지식·중복 검토 |
| 사용 적음 | `cold_essential`: 보호하고 발견성 점검 | archive/retire 검토 후보 |

중요도는 authority, 현재·향후 applicability, 결정/할일/산출물 dependency, uniqueness,
lifecycle/conflict, access-log coverage로 판단한다. 특히 적용 대상인데 검색 이력이 없는
자료는 폐기 후보가 아니라 `expected_but_not_retrieved` 검색·라우팅 실패 후보로 본다.
법령·계약·공식 지침·품질·안전 자료, 설계 baseline, 승인·시험 근거, exact historical
revision은 저사용이어도 자동 폐기하지 않는다.

RAG index/cache처럼 원본과 lineage에서 재생성 가능한 파생물만 정책에 따라 자동
정리할 수 있다. source original, source revision, 정본, 업무 적용 근거의 archive/retire는
owner decision 또는 명시된 policy gate를 요구한다.

현재 첫 자동 연결은 저장되는 metadata RAG answer run과 source-text answer run이 선택한
근거를 PC별 월간 shard JSONL에 `retrieve`로 남기는 범위다. 일반 파일 open, editor read, Wiki 조회,
실제 `cite/apply` 판정까지 모두 자동화되었다는 뜻은 아니다. source-text index의 exact
`source_revision_id + content_id` 전파와 Wiki 조회 adapter는 아래 migration gate에서
계속 닫는다.
...
- [`../workspace/PROJECT_TASK_ENGINE_LIFECYCLE_V0.md`](../workspace/PROJECT_TASK_ENGINE_LIFECYCLE_V0.md)
...
- [`../guild_hall/KNOWLEDGE_OPERATING_MODEL_V0.md`](../guild_hall/KNOWLEDGE_OPERATING_MODEL_V0.md)
