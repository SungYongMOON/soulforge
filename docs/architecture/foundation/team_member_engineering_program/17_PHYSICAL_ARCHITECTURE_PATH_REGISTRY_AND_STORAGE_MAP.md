...
| 20_문서검색 | Project-owned extracted text/tables, rebuildable lexical/vector/graph retrieval assets, source-location maps, extraction warnings and quality records. Accepted source artifacts remain in their original owner. `검색_색인/extraction_checkpoints/` holds per-document graph extraction results keyed by text, units, profile and exact model revision, so a stopped graph run does not repeat accepted extractions; it is created on demand by the index writer (not part of the materialized template above), is never selected or served, is pruned by the index writer after each commit (embodied ones) and after 14 days (orphans), and is backup-classified **excluded because deterministically rebuildable** (losing it costs re-extraction, never a fact). |
