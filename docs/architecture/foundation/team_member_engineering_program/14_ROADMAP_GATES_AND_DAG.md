...
| Board/4192 watch vertical | DONE `L-BOARD-VIEWMODEL`,`L-BOARD-PAGE` (기본 OFF `?watch=1`) + `L-WATCH-SUP-3`(backup_restore_readiness 공급자 — R3 overlay 집계 verbatim 번역·계약 결속 완성) | n/a (vite 앱) | PARTIAL — 실 read-only 공급자 3/9 domain (`L-WATCH-SUP-1`,`L-WATCH-SUP-2`); backup_restore_readiness는 클라이언트 배선 완료·`/storage-map.snapshot.json` endpoint가 private binding/ACL gate 뒤(공급 시작 시 추가 코드 없이 활성); watchtower binding 등 잔여 5는 환경 gate | GATED |
| Mail source-lane capture adapter | DONE (accepted continuous/store receipt→refs-only capture generation; backup/restore/acceptance 생성 불가, capture-only R3=`degraded`) | — | GATED (실 receipt caller·private record store) | GATED |
| Agent workforce lineage | DONE (pure Family→Mark→Deployment→Run→Memory `PREPARED_CONTRACT`; agent_record≠Mark·effects 0) | GATED (`project_ai_team_pack`) | GATED (durable registry/writer·approved deployment) | GATED |
| Tool workshop | DONE `L-WORKSHOP-CORE` | DONE `L-PACK-BUILDER` (4-file pack, install+smoke green) | GATED (물리 Tool PC·runner) | GATED |
| Deployment pack (contract·builder·spec 4종/5종 중 project_ai_team만 외부 gate) | DONE `L-PACK-CONTRACT` | DONE `L-PACK-BUILDER`,`L-HPP-PACK`,`L-DEP-DELIVERY`,`L-GITFREE-ATTEST`,`L-LIFECYCLE` + start/stop 증명 + Team Client·Backup-Recovery spec(Backup-Recovery의 격리 설치 evidence는 역사적 49파일·14/14 smoke·제외 0; 2026-08-31 current generated closure는 physical one-shot까지 포함한 **54파일·16 declared smoke·제외 0·reviewed pin 11**. 연결된 Linear one-shot 결과는 `PARTIAL_TECHNICAL_RESTORE_CANDIDATE`이며 exact-byte 복구 사본은 있으나 누락된 history/evidence 차원과 human acceptance=false 때문에 full backup·recurring operation·recovery-ready를 주장하지 않음) (hpp **941파일**: 실 unit gate·양방향 install 검증·**전 suite 91/91 smoke green·제외 0**·**initial gate 7다리 전부 receipt**(install/start/stop/smoke/upgrade/rollback/restore — 실규모 생애주기 체인은 939-세대 digest 왕복 backup→restore→upgrade→rollback→boot로 실증, 941-세대는 install/smoke/start-stop 재실증) — worker와 server 양쪽이 동봉 pack manifest 검증 후 **재계산** pack_digest를 소스 신원으로 attest(health 라이브 확인); 무서명 manifest의 잔여 신뢰는 외부 pin·전달 채널 소관; gate 자체는 무주장. team_client **214파일 소스 pack**: 실 unit gate(Board 723 tests)·양방향 install 검증·**전 suite 77파일 smoke green·제외 0**(UI 빌드는 npm install 요구 — package/sbom gate 소관 무주장); 2026-08-31 storage-map projection 의존 3파일이 계산 폐포에 추가됨) | — (ring 승격·서비스 상시 기동 없음) | GATED (물리 ring) |
| Module operability gate (manifest·의존·cycle·preflight) | DONE `L-MODOP-GATE` + risk-based 등재 (check-only; current manifest **26종**, import scan 1,167파일/2,590 edge·cycle 0 — 최소권한 manifest와 실제 validator로 검증; 잔여 24 dir는 의도적 미등재로 보고) | n/a (게이트 자체는 pack 대상 아님) | — | — |
