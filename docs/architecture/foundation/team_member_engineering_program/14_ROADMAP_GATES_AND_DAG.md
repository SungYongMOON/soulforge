...
| `validate:deployment-pack` | 34 | 계약 7 + builder 14(파티션·pin·builder↔reader 라운드트립·E2E 포함) + start/stop 증명 가드 4(env 중화·identity 결속·post-stop 재결속·pre-boot 거부) + 생애주기 9(backup/upgrade/rollback/restore·영수증 정직성·half-state coded 거부·traversal 거부·digest recipe pin; reader측 traversal 회귀는 dev-erp identity suite에 별도) + spec drift `--check` |
