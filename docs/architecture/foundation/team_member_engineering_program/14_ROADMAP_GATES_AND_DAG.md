...
| Mail source-lane capture adapter | DONE `L-MAIL-SOURCE-LANE` (accepted continuous/store receipt→refs-only capture generation; backup/restore/acceptance 생성 불가, capture-only R3=`degraded`) | — | GATED (실 receipt caller·private record store) | GATED |
