...
| --- | --- | --- | --- |
| RED-01 | `node --test ui-workspace/apps/dev-erp/test/context_life_tree.test.mjs` failed: the non-admin life-tree scope test loses an owned mailbox row after more than 500 newer foreign rows. | Confirmed current defect; it precedes client/Watch rollout. | Red test first, correct source filtering before cap, add sibling lane regressions, then fresh review. |
| RED-02 | Fable5 reported a 4192 topology oracle mismatch. Current public `topology_provider_adapters.test.mjs` passes and asserts a 291-node/900-edge federated snapshot. | Preserve as a source-of-truth/oracle reconciliation problem, not as a claim that the Board is healthy or unhealthy. | Name the intended topology contract, pin the producer/view expectation, and add one test that rejects count/source drift. |
| RED-03 | `renderer-web/controlCenterPlugin.ts` contains a token-gated `PUT` path that calls `fs.writeFile`; it does not itself demonstrate the repository's protected write guard. | Keep the capability `HOLD`; do not use it as a Vault, workmeta, or operational writer. | Explicit allowed-path/owner/guard contract, negative tests, review, and separate activation authority. |
