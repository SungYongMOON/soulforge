...
| `guild_hall/backup_controller` | Feature-gated backup/recovery, bounded Linear one-shot, generic Source/Buzz/Hermes generation-readiness contracts | Reuse for policy/restore gates | New contracts are metadata/evidence only; actual Buzz/Hermes/Source bytes, restore execution and human acceptance remain HOLD |
...
| `guild_hall/path_registry` | R1–R5 source/asset ledgers, 40-row registry and Mail/Slack/Voice/PC-activity capture adapters | Reuse as whole-estate organizing spine | Cloud/Git/NAS and private persistent record/backup stores remain HOLD |
