...
| `npm.cmd --prefix ui-workspace/apps/dev-erp-mcp test` | BLOCKED at planning time | Local dependencies for the package test were missing; no installation was attempted then. Lockfile-only installs (`npm ci` at repo root and `ui-workspace`) were later recorded by the build loop for validator execution. |
