...
| Path Registry/resolver | Canonical-root preservation, exact physical-root enum, logical/physical/scope/binding-revision separation, owner coverage, ambiguity/expiry/unavailable HOLD, no fallback, registry-revision/epoch/writer/operation-aware authorization, append-vs-overwrite/delete and current-target fencing |
| Folder materializer | Approved empty canary root, junction/reparse/UNC/device/ADS/Unicode/traversal/realpath negatives, HPP backup classification, dry-run/apply parity, idempotent replay, existing-payload move 0, empty-created-directory rollback |
| 4192 Storage Map | Registry snapshot digest, all registered root/source rows including HOLD/N/A, root/record identity, coverage totals/unclassified/drift, deterministic Watch precedence, unknown-without-evidence, no raw/secret/writer fields, safe owner pointers |
