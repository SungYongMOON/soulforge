...
| --- | --- | --- | --- | --- |
| Linear | Official Task status currently; task, relation, assignee, priority, due, Work Brief revision refs, comment/history, approved attachment refs as permitted | Read-only collector/backup before any writer migration; future task writer remains separately authorized | Immutable generation, ordered cursor/dedupe, manifest/hash, create-only generation, isolated restore and reconciliation | Collection lane v1 (`guild_hall/linear_history/`, read-only 15-minute HPP source lane with a `credentials.api_key_file` pointer, create-only custody, refs-only receipts, read-evidence envelopes) activated on the Main Node 2026-09-02 under the Owner's explicit chat authorization: Scheduled Task `Soulforge-HPP-Linear-Collect` (`PT15M`) registered and active, private binding v3, Owner-placed read-only key file (never read by an agent), custody under `<private_root>/ingress/linear/<url_key>/` holding 84 issues / 185 comments / 12 projects after a one-time backfill, idempotent runs; evidence in the private activation receipt. Claim ceiling: activated internal_rc candidate, observed; Human/Level-3 independent review pending; not production-ready. Backup generation/Human restore remain HOLD (LB1 in `backup_controller/`) |
...
| Buzz | Collaboration message/thread/attachment source | Collection lane v1 (`guild_hall/buzz_history/`, the read-only 15-minute HPP Tributary) built 2026-09-03: the relay's PostgreSQL is reached over the container's local socket by a manifest-pinned exporter inside a session PostgreSQL holds read-only, so the lane holds **no credential at all** and a binding declaring one is refused; two reads per run (a loopback `_liveness` GET pinned to the `127.0.0.1` literal, and one bounded export process whose every staged file is re-hashed before parsing); create-only content-addressed custody under `<private_root>/ingress/buzz/<relay_key>/`; the community `signing_key` is removed inside PostgreSQL and never leaves it | Postgres/media/Git/Redis-classified backup manifest, isolated restore, audit and identity recovery. A `buzz_backup_generation_index` recorder (`backup_controller/`) now indexes the Buzz controller's own daily backup by refs, digests and byte counts with **zero byte duplication** (`claim_ceiling: index_only`) | Collection lane built and locally validated (observed); **not registered, not activated** — Scheduled Task registration, binding placement and any `--apply` await Owner approval. Actual backup capture, isolated restore and human acceptance remain VERIFY_PHYSICAL: Soulforge has performed no restore, so the index emits a restore-test **observation**, never a `restore_test` lane record |
...
| Hermes | Bot profile instruction/config/capability source (`<hermes_home>/profiles/`) | Read-only profile inventory; the runtime itself is not a Soulforge writer | Sigil snapshot v0 (`backup_controller/hermes_profile_snapshot.mjs`) built 2026-09-03: `SOUL.md` bytes copied and verified by isolated readback; config by digest and size; skills/hooks/plans/cron by name; sessions/memories/workspaces/databases by count and bytes only; `.env`/`auth.json`/`auth.lock` by existence and size, never opened and never hashed; logs and cache not walked | `claim_ceiling: inventory_v0`, **not** a restorable agent backup. No database byte is captured: 45 of the profiles' 159 SQLite files had a live write-ahead log held open by a running `serve` process, so a file copy would be torn and would miss the WAL. A future generation including databases must use `sqlite3` online backup or `VACUUM INTO`. Liveness is supplied by the operator, never enumerated here |
