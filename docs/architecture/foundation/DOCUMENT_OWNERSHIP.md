...
- 기존 single-body, single-loadout 문서 해석을 정본 기준에서 제거한다.

## 기본 원칙

- 루트 `README.md` 는 저장소 전체 상위 지도만 다룬다.
- `docs/architecture/foundation/` 는 여섯 축의 cross-root canon 을 고정한다.
- 각 owner 루트가 materialize 되면 해당 루트 바로 아래 `README.md` 와 owner-local 문서가 그 owner의 상세 운영 정본이 된다.
- `_workspaces/<project_code>/` 아래 문서와 실행 산출물은 local/private owner 영역이며 public canon 이 아니다.
...
| `.agent/` | species / hero catalog | `docs/architecture/foundation/TARGET_TREE.md`, `docs/architecture/foundation/DOCUMENT_OWNERSHIP.md` | `.agent/README.md`, `.agent/docs/architecture/` |
...
| `.agent_class/` | class / package catalog | `docs/architecture/foundation/TARGET_TREE.md`, `docs/architecture/foundation/DOCUMENT_OWNERSHIP.md` | `.agent_class/README.md`, `.agent_class/docs/` |
...
| `dev/` | migration plan 과 작업 로그 | `dev/plan/`, `dev/log/` 하위 문서 | 해당 없음 |

## owner별 문서 경계

| owner | 문서에 반드시 남겨야 할 것 | 문서에서 제거해야 할 것 |
| --- | --- | --- |
| `.agent` | species, hero, catalog 구조와 설명 | active runtime, memory, sessions, raw run owner 서술 |
| `.unit` | unit 구조, policy, protocols, runtime, memory owner 설명 | species catalog 나 class catalog owner 서술 |
| `.agent_class` | class/package 정의, refs, profiles, manifests | loadout root, workflow owner, project run owner 서술 |
...
| `_workspaces` | local-only mission site mount 정책, `_workspaces/<project_code>/` 구조 | `company/`, `personal`, repo-tracked sample baseline 정본화 |

## 적용 규칙

1. foundation 문서는 owner-local 세부 운영을 복제하지 않고 owner 경계만 고정한다.
2. `.agent/body.yaml`, `.agent/body_state.yaml`, `.agent_class/loadout.yaml` 의미는 새 정본 owner 정의로 사용하지 않는다.
