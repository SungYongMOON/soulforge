...
| `guild_hall/context_engine/` | 맥락 APP의 공개 진입점·공통 검사·교체 알고리즘·profile·하니스·설치 명세 구현 owner | `guild_hall/context_engine/README.md`, 앱 `docs/`; 프로젝트 데이터 배치는 Plan 17 | 원문·수락 기록은 기존 owner에 보존, 파생 데이터는 승인된 프로젝트 store, 설치 코드와 개발·평가 출력은 분리 |
| `guild_hall/workspace_ledgers/` | 과제별 메일 라우팅 규칙(021)과 관리 대장 CSV(023·027) 생성·버전관리·refresh 엔진 구현 owner | `guild_hall/workspace_ledgers/README.md`, `docs/architecture/workspace/PROJECT_ONBOARDING_V0.md`(관리 폴더 quick map) | 실제 project code·조직명·사람 이름·host-local 경로(합성 예시만 `examples/`에), 실제 메일 본문·발신자·첨부 바이트 |
...
| `docs/reviews/` | 비정본 검토 기록: 외부 검토 대응표와 리뷰어 패킷 산출물(생성물, 미추적)·문서 왕복 기록(exchange/)과 읽기 순서표(READING_LIST) | `docs/reviews/README.md` | 해당 없음 (판정은 참고이며 정본 변경은 owner 문서에서만) |
...
| `guild_hall/context_engine` | APP 내부 실행/알고리즘 책임, 정확한 외부 의존·caller 계약, 전략·하니스 판본과 설치·재생성·교체/복구의 실제 검증 범위 | 수집기·RUNE 업무 규칙·사람 수락 권한의 중복 소유, runtime의 평가 gold 의존, 실제 source·credentials·프로젝트 데이터의 설치본 포함 |
| `guild_hall/workspace_ledgers` | 규칙 스키마·분류기·CSV 대장 빌더·refresh/preview-rule/save-rule 엔진 계약과 CLI 표면, 합성 example | 실제 project code·조직/발신자·host-local 경로, 메일 본문·첨부 바이트(counts만 허용), 실제 라우팅 규칙 draft 원문 |
