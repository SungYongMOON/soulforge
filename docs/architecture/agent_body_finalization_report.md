...
권장 저장 경로: `docs/architecture/AGENT_BODY_FINALIZATION_REPORT.md`
...
> 단, 저장소 churn을 줄이고 싶으면 경로는 `engine/`으로 유지하고 README와 문서에서 runtime 의미로 재정의하는 절충도 가능하다.  
> 이 보고서의 **target concept 이름은 `runtime/`** 으로 둔다.
...
## 6.6 `runtime/`  **(`engine/`의 target rename)**
...
last_decision: export는 제거하고 protocols를 추가한다
next_action: registry와 runtime README 정리
open_risks:
  - runtime 경로를 실제로 rename할지 미정
...
status:
  summary: draft-aligned
  warnings:
    - runtime path rename pending
```
...
- engine은 runtime 의미로 재정의하거나 runtime으로 rename하는 방향으로 설계한다.
...
- `.agent/runtime/README.md` 또는 `.agent/engine/README.md`
...
### 작업
- `export/` 삭제 또는 deprecate
- `engine/`을 `runtime/`으로 rename 하거나, rename이 어렵다면 `engine/README.md`와 body 메타에서 runtime 의미로 재정의
- `protocols/` 신규 추가
- `registry/` 범위 축소
...
1. export 폴더는 삭제 권장이다.
   - 삭제가 어렵다면 README에 deprecated 표시를 하고 body target tree에서는 제외하라.
2. engine은 runtime으로 rename하는 것을 우선 검토하라.
   - rename cost가 크면 경로는 유지하되 README와 architecture 문서에서 runtime 의미로 재정의하라.
...
- [ ] `communication/` 유지
- [ ] `protocols/` 신규 추가
- [ ] `runtime/` 도입 또는 `engine` 재정의
- [ ] `memory/` 유지
- [ ] `sessions/` 유지
- [ ] `autonomic/` 유지
- [ ] `artifacts/` 유지
- [ ] `export/` 제거 또는 deprecated 처리
