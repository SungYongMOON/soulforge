...
JSONL 관측은 마지막 최대 4MiB 범위에서 가장 최근 행만 읽는다. 전체 로그가 커져도
관측할 수 있지만 최신 행이 한도를 넘거나 손상됐거나 읽는 도중 파일이 바뀌면 실패한다.
실패를 과거 정상 행으로 덮지 않으며 JSON 단일 파일의 기존 4MiB 한도는 유지한다.
CLI의 종료 코드 2는 down 노드를 포함한 검사 완료다. 실행 영수증은 이를 감시기 충돌과
구분하지만 복구 재검증 게이트의 실패·허용 정책은 바꾸지 않는다.
...
| `mail_new_event_notice.mjs` | 살핌이 운영감시 메일 알림. Watchtower `store_mail_events`의 `count_store_unchanged_new_event_count_<n>` 판정을 읽어 기존 `planAlerts`(전환 시 발송·1h/4h/1d 재보고·보고된 것만 해소 통지)로 고정 문장 한 줄을 낸다. 불일치는 이후 `store_changed` 영수증이 올 때까지 미해결로 유지하며, 신규 0건·비교 불가로는 해소하지 않는다. 모델 호출 없음. `--report` 모드는 예약 실행마다(정상이어도) 정기 보고를 낸다: 확인 시각·범위, 정상/이상/확인 불가, 새 이상·계속 미해결, 지난 보고 이후 수집 실행 영수증 요약(사이 구간 대조는 확인 불가로 명시), 직전 보고 전달 실패 여부, Hermes 작업 주기로 계산한 다음 보고 시각 |
