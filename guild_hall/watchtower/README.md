...
| `tools/emit_federated_topology.mjs` | 두 tracked source만 읽어 단일 derived projection을 생성하거나 byte parity를 검사하는 도구 |
...
- 현재 producer allowlist는 Watchtower native topology와 Engineering Engine generated topology 두 개뿐이다.
...
- fragment와 projection은 프로젝트 식별자, 실제 경로, payload, source text, NotebookLM answer, account/session,
  credential을 포함하지 않는다. RAG·Wiki·NotebookLM은 각 owner contract가 준비된 뒤 별도 provider로 추가한다.
