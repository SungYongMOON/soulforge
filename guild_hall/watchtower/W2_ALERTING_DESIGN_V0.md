...
## 3A. 그리고 고장 자체도 알림 사유가 아니다 (2026-09-04 실측)

자기 복구는 이 정책보다 먼저 돌고, 대개 옳다. 09-04 새벽 2시간 동안 코디네이터는
200건을 연속으로 판정해 **전부 정확히 분류하고 전부 거부**했다 — 원인이 어긋난
경로였고 어떤 재시작으로도 고쳐지지 않기 때문이다. 멀쩡한 작업을 200번 재시작하지
않은 것이 옳은 행동이었다.

그러므로 알릴 것은 고장이 아니라 **자기 복구가 못 고치는 고장**이다.

| 코디네이터 판정 | 알림 |
| --- | --- |
| `owner_action_required` · `fail_closed_quiesce` | **즉시** — 사람이 필요하다고 스스로 말한 것이다 |
| `auto_repairable` · `bounded_retry` · `quarantine_and_continue` | 침묵 — 처리 중인 것은 사건이 아니다 |
| 판정 없음 | **알린다** — 침묵이 증거 부재의 결과가 되면 안 된다 |
| 처리 중이지만 유예(1시간)를 넘김 | **알린다** (`node_repair_stalled`) |

마지막 줄이 이 표의 이유다. **영원히 재시도하며 한 번도 성공하지 못하는 코디네이터는
밖에서 보면 건강한 것과 구분되지 않는다.** 그 침묵이 이 게이트가 막으려는 바로 그
실패 모양이므로, 유예를 넘긴 고장은 코디네이터가 뭐라 하든 보고 대상이 된다.

스스로 고친 건은 알리지 않는다. 이력에만 남는다.
