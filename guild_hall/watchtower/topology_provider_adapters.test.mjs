...
test("allowlisted adapters map exact Watchtower and Engineering Engine inventories", () => {
  ...
  assert.equal(engineeringEngine.nodes.length, 33);
  assert.equal(engineeringEngine.edges.length, 151);
...
test("CLI emits canonical bytes only on stdout by default and --check remains read-only", () => {
  ...
  assert.equal(parsed.summary.node_count, 71);
  assert.equal(parsed.summary.edge_count, 199);
