...
test("allowlisted adapters map exact Watchtower and Engineering Engine inventories", () => {
  ...
  assert.equal(engineeringEngine.nodes.length, 236);
  assert.equal(engineeringEngine.edges.length, 803);
...
test("CLI emits canonical bytes only on stdout by default and --check remains read-only", () => {
  ...
  assert.equal(parsed.summary.node_count, 275);
  assert.equal(parsed.summary.edge_count, 853);
