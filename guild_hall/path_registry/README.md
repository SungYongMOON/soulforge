...
- **Linear capture adapter** (`src/linear_source_lane_adapter.mjs`): `guild_hall/linear_history/`가 run마다 남기는 refs-only `soulforge.linear_collect.run_receipt.v1`를 lane/workspace/generation/digest/time으로 결속해 `source.linear`의 `capture_generation` 한 건만 만든다. status `error`·preflight·forged digest·stale/future·body/path 포함 receipt는 거부하며, runner가 run 시점에 쓰는 lane record와 같은 9-key 파생이라 lane test가 두 파생의 동일성을 고정한다. backup pointer/restore/acceptance는 만들지 않으므로 R3는 `degraded`다.
