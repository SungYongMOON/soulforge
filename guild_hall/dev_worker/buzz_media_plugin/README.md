...
Buzz desktop voice notes arrive as `video/mp4` attachments. For an authorized,
hash-verified attachment whose signed filename has the `voice-note-<timestamp>.mp4`
shape, the plugin probes the local file with `ffprobe`. Only an MP4 with an audio
stream and no video stream is presented to Hermes as `audio/mp4`, which enters
the existing gateway speech-to-text route. Missing `ffprobe`, invalid media and
ordinary videos retain their original `video/mp4` classification. The verified
voice-note link is removed from the agent-facing text after the local file is
attached, so the agent does not retry the private media URL without credentials.
...
```text
python -m unittest guild_hall.dev_worker.buzz_media_plugin.test_inbound_voice_note -v
```

The voice-note test checks signed attachment metadata, a mocked authenticated
download with hash verification, and MP4 audio-versus-video classification using
synthetic media. It requires Hermes' Python dependencies plus `ffmpeg` and
`ffprobe` on PATH. Mocked transport does not prove live relay delivery or
speech-to-text provider availability. Live validation should use an authorized
test voice note in the intended profile; do not use business files as fixtures.
