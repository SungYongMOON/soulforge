...
_VOICE_NOTE_NAME = re.compile(r"voice-note-[0-9]{13}\.mp4\Z", re.IGNORECASE)


def _audio_only_mp4(path):
    """Require a real audio stream and no video stream before routing to STT."""
    ...
    try:
        result = subprocess.run(
            [probe, "-v", "error", "-show_entries", "stream=codec_type",
             "-of", "json", str(path)],
            capture_output=True, timeout=8, env=child_env, check=False,
        )
...
def _effective_mime(item, path):
    if (item["voice_note"] and item["mime"] == "video/mp4"
            and _audio_only_mp4(path)):
        return "audio/mp4"
    return item["mime"]
...
def _descriptors(raw, event, origin):
    ...
    for tag in tags:
        ...
        if not re.fullmatch(r"\.[a-z0-9]{1,12}", extension):
            extension = ""
        descriptors.append(dict(url=url, mime=mime, digest=digest, size=int(size),
                                extension=extension, name=name,
                                voice_note=bool(_VOICE_NOTE_NAME.fullmatch(name))))
    ...
    try:
        ...
        async with asyncio.timeout(120), httpx.AsyncClient(follow_redirects=False, trust_env=False, timeout=60) as client:
            ...
            for item in descriptors:
                ...
                effective_mime = _effective_mime(item, target)
                types.append(effective_mime)
                if effective_mime == "audio/mp4" and item["mime"] == "video/mp4":
                    voice_links.append(f'[{item["name"]}]({item["url"]})')
            if paths:
                event.media_urls = paths
                event.media_types = types
                for link in voice_links:
                    event.text = (event.text or "").replace(link, "", 1).strip()
    except Exception:
