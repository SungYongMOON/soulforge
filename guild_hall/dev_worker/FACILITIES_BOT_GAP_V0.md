...
2026-09-08 개발 보완: 아래는 원래 gap 설명이다. 전역 금지 목록은 이제
`guild_hall/shared/agent_write_boundary.mjs`에 구현되어 있다. 이번 보완은 root/glob/
대소문자·경로 별칭으로 그 목록을 빠져나가던 판정을 고쳤고, 이미 승인된 후보도
금지 경로이면 ready 큐로 승격하지 않는다. 자기 발견·자동 개선 실행·독립 감시의
실제 연결까지 완료했다는 뜻은 아니다. 내부 가역 개발은 먼저 진행·후보고하며,
기술적 실패를 곧바로 Owner 확인 요청으로 넘기지 않는 최신 위임을 적용한다.
