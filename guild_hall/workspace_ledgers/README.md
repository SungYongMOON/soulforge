...
- **영수증이 없으면 줄도 없다(S-7).** 한도의 장부가 영수증 디렉터리 하나뿐이므로, 그
  디렉터리가 계속 못 쓰는 상태이면 예전에는 매 `decide`가 줄만 남기고 한도는 영영 오르지
  않았다(한도 3에 줄 3·영수증 0). 이제 `decide`는 모든 검사와 설정 재대조를 마친 **뒤,
  붙이기 직전에** 영수증 디렉터리에 고유 이름의 probe 파일을 만들었다 지우며 쓸 수 있는지
  증명한다. 실패하면 `..._receipts_unwritable_before_append`로 **한 줄도 붙이지 않고**
  멈춘다(끝값 2). 디렉터리가 없으면 만든다(실제 영수증 쓰기와 같은 동작). probe 파일 이름은
  한도 계수기가 읽는 `bot_triage-*.json` 형태와 양쪽 모두 어긋나므로, 충돌로 남은 잔재가
  세어지거나 계수를 깨뜨리지 않는다. 붙인 **뒤**의 실패(아래)는 두 번째 방어선으로 남는다.
...
`receipts_dir`에 원자적으로 쓰인다. **검사를 통과한 메일 id는 잘리지 않고 통째로** 들어간다
(S-6 -- 영수증이 판독표의 그 줄과 대조되지 않으면 영수증으로서 할 일이 없다). id 자체는
`assertId`의 명시 상한(512자, `..._id_too_long`)으로 여전히 유한하며, 거부된 호출에서만
검사 안 거친 값이 80자로 잘리고 제어문자가 걷힌다. 담는 것은 명령·메일 id·판정·분류 어휘·
결과 코드·계수뿐,
