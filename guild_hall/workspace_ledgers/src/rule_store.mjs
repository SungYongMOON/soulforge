...
export function acquireRuleSaveLock(ruleDir, now) {
  ...
  if (existsSync(lockFile)) {
    ...
    if (ageMs <= RULE_SAVE_STALE_LOCK_MS) return { held: true, age_ms: ageMs };
...
export function saveRuleVersion({ workspacesRoot, workmetaRoot, code, draft, by, note, now = new Date().toISOString(),
  ...
  try {
    ...
    const decided = extractSection(previous.md, '## Owner 확인 기록');
    const open = extractSection(previous.md, '## Owner 확인이 필요한 것');
