...
if(!emitted){Emit(new {type="system",subtype="init",tools=names,cwd=Directory.GetCurrentDirectory(),model=Get(args,"--model"),mcp_servers=new[]{new{name="soulforge_workspace",status="connected"}}});emitted=true;}
...
if(m.type!=='user')return;fs.writeFileSync('received-user-count.txt',String(++users));if(!emitted){console.log(JSON.stringify({type:'system',subtype:'init',tools:names,cwd:process.cwd(),model:get('--model'),mcp_servers:[{name:'soulforge_workspace',status:'connected'}]}));emitted=true;}
...
export function fixture({ mode = 'synthetic instructions', tools = ['workspace_list', 'workspace_read_text', 'workspace_write_text'] } = {}) {
