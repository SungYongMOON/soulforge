...
export function validatePresentationPacket(packet) {
  ...
  for(const slide of packet.slides) {
...
export function pinPptxRunnerBinding({artifactRoot,pythonExecutable,templatePath,templateApprovalRef,templateProvenance}) {
  ...
  return {code_root:ROOT,node_executable:directPath(process.execPath),node_version:process.version,node_sha256:sha256(readFileSync(process.execPath)),python_executable:pythonExecutable,python_version:python.version,python_files:python.files,python_sha256:sha256(readFileSync(pythonExecutable)),artifact_root:artifactRoot,artifact_version:info.version,artifact_files:treeFiles(artifactRoot),template_path:templatePath,template_sha256:sha256(boundedRead(templatePath,8*1024*1024)),template_approval_ref:templateApprovalRef,template_provenance:templateProvenance,template_family:'workshop.two_slide_text',template_revision:'template:v1',sources:SOURCES.map(relative_path=>({relative_path,sha256:sha256(boundedRead(path.join(ROOT,relative_path),4*1024*1024))}))};
}

export function verifyPptxRunnerBinding(binding) {
  exactKeys(binding,['code_root','node_executable','node_version','node_sha256','python_executable','python_version','python_files','python_sha256','artifact_root','artifact_version','artifact_files','template_path','template_sha256','template_approval_ref','template_provenance','template_family','template_revision','sources']);
  ...
  if(binding.code_root!==ROOT || binding.node_executable!==process.execPath || binding.node_version!==process.version || binding.template_family!=='workshop.two_slide_text' || binding.template_revision!=='template:v1' || !/^3\.12\.\d+$/.test(binding.python_version))reject('binding_drift');
...
export function createPptxWorkshopRunner({queue,binding,projectRef,inputRoot,workRoot,outputRoot}) {
  ...
  return Object.freeze({async runNext(){
    ...
    try {
      ...
      const packet=validatePresentationPacket(JSON.parse(bytes.toString('utf8')));
      ...
      const python=mode=>runBoundedToolProcess({executable:binding.python_executable,args:['-I','-S','-B',path.join(runRoot,PY),mode,input,template,output],runRoot,lease,queue,deadline});
      ...
      const outputBytes=boundedRead(output,8*1024*1024),renders=[1,2].map(index=>boundedRead(path.join(runRoot,`slide-${index}.png`),4*1024*1024));
      if(native.sha256!==sha256(outputBytes) || checked.sha256!==native.sha256 || checked.size_bytes!==outputBytes.length || checked.slide_count!==2 || checked.template_sha256!==binding.template_sha256 || !Array.isArray(checked.renders) || checked.renders.length!==2)reject('validator_failed');
      for(let index=0;index<2;index++)if(checked.renders[index].sha256!==sha256(renders[index]) || checked.renders[index].size_bytes!==renders[index].length || checked.renders[index].width!==1280 || checked.renders[index].height!==720)reject('validator_failed');
      stage='binding_drift';verifyPptxRunnerBinding(binding);
