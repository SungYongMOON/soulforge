...
function safeCycleSummary(result, cycle) {
  return event("cycle_completed", {
    ...
    asr_processed_count: result?.asr?.processed_count ?? null,
    asr_failed_count: result?.asr?.failed_count ?? null,
    asr_remaining_pending_count: result?.asr?.remaining_pending_count ?? null,
    label_processed_session_count: result?.labels?.processed_session_count ?? null,
    label_duplicate_session_count: result?.labels?.duplicate_session_count ?? null,
    label_no_content_session_count: result?.labels?.no_content_session_count ?? null,
    label_failed_session_count: result?.labels?.failed_session_count ?? null,
    label_failure_codes: Array.isArray(result?.labels?.failure_codes)
      ...
      : [],
    timeline_annotation_count: result?.labels?.timeline_annotation_count ?? null,
