...
// Buzz collection CLI entrypoint. Only reachable through the exact-runtime
...
export function parseBuzzCollectArguments(argv) {
  const request = {
    mode: null,
...
function assertRuntimeAttestation(runtimeRoot) {
  const attestation = globalThis[Symbol.for("soulforge.buzz_collect.runtime_attestation")];
...
async function main() {
  try {
    const request = parseBuzzCollectArguments(process.argv.slice(2));
    assertRuntimeAttestation(request.runtime_root);
    const result = request.mode === "preflight"
      ? await preflightBuzzCollect(request)
      : await runBuzzCollect(request);
    process.stdout.write(`${JSON.stringify(result)}\n`);
  } catch (error) {
    const candidate = error instanceof BuzzCollectError
      ? error.code
      : String(error?.code ?? "");
    const code = /^[a-z][a-z0-9_]{0,95}$/u.test(candidate)
      ? candidate
      : "unknown_failure";
    process.stderr.write(`buzz_collect_rejected:${code}\n`);
    process.exitCode = 1;
  }
}
