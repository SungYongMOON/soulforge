...
test('isSafeRelativePosixPath strictly accepts valid relative POSIX paths and rejects hostile shapes', () => {
  ...
  assert.equal(isSafeRelativePosixPath(['file:', '', '', 'etc', 'hosts'].join('/')), false);
  ...
  for (const alias of ['CON', 'nul.txt', 'folder/COM1.log', 'folder/lpt9', 'folder/file.']) {
    assert.equal(isSafeRelativePosixPath(alias), false, alias);
  }
});

test('case aliases in a Windows manifest are rejected before payload hashing', () => {
  const files = [
    { path: 'folder/file.mjs', sha256: sha256('x'), bytes: 1 },
    { path: 'FOLDER/FILE.mjs', sha256: sha256('x'), bytes: 1 },
  ];
  const tree = makeTree({
    [`${PACK_ROOT}\\pack.manifest.json`]: {
      content: JSON.stringify({ pack_digest: recomputePackDigest(files), files }),
    },
  });
  const port = makePort(tree);
  let hashes = 0;
  port.hashFile = () => { hashes += 1; return null; };
  const result = buildTopologyV2Evidence({ privateBinding: frozenPrivateBinding(), port, clock: CLOCK });
  assert.deepEqual(result.holds, [HC.INSTALLED_PACK_MANIFEST_INVALID]);
