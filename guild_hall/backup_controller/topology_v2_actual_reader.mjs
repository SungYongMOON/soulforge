...
export function isSafeRelativePosixPath(relPath) {
  ...
  for (const segment of segments) {
    if (/^\.+$/u.test(segment) || segment.endsWith('.')
...
export function buildTopologyV2Evidence({ privateBinding, port, clock } = {}) {
  ...
  for (const entry of manifest.files) {
    ...
    const manifestIdentity = pathIdentity(entry.path, port.platform);
    if (seenManifestPaths.has(manifestIdentity)) {
      return hold([...holds, HC.INSTALLED_PACK_MANIFEST_INVALID], evaluatedAt);
    }
    seenManifestPaths.add(manifestIdentity);
  }
