...
function makeTempDir(label) {
  return realpathSync.native(mkdtempSync(join(tmpdir(), `soulforge-test-${label}-`)));
}
...
test('writeCreateOnlyAtomic writes atomically with readback and cleans up on partial failure', () => {
  ...
  try {
    ...
    assert.deepEqual(readdirSync(controlDir), ['output.json']);
  } finally {
    rmSync(controlDir, { recursive: true, force: true });
  }
});

test('writer revalidates confinement itself and rejects planted directory links', () => {
  const controlDir = makeTempDir('ctrl');
  const outside = makeTempDir('outside');
  try {
    const linked = join(controlDir, 'linked');
    symlinkSync(outside, linked, process.platform === 'win32' ? 'junction' : 'dir');
    const throughLink = join(linked, 'leak.json');
    assert.equal(validateOutputDestination({
      destination: throughLink, approvedControlDir: controlDir,
    }).code, 'CONTROL_DIRECTORY_UNSAFE');
    for (const destination of [throughLink, join(outside, 'direct.json')]) {
      assert.throws(() => writeCreateOnlyAtomic({ destination, content: 'synthetic', approvedControlDir: controlDir }));
    }
    assert.deepEqual(readdirSync(outside), []);
    const protectedDir = join(controlDir, 'protected');
    mkdirSync(protectedDir);
    assert.throws(() => writeCreateOnlyAtomic({
      destination: join(protectedDir, 'out.json'), content: 'synthetic',
      approvedControlDir: controlDir, forbiddenRoots: [protectedDir],
    }), /protected/);
    assert.deepEqual(readdirSync(protectedDir), []);
  } finally {
    rmSync(controlDir, { recursive: true, force: true });
...
test('a directory replaced after validation is refused when the writer starts', () => {
  const controlDir = makeTempDir('ctrl');
  const outside = makeTempDir('outside');
  try {
    const parent = join(controlDir, 'output');
    mkdirSync(parent);
    const destination = join(parent, 'out.json');
    assert.equal(validateOutputDestination({ destination, approvedControlDir: controlDir }).ok, true);
    renameSync(parent, join(controlDir, 'preserved'));
    symlinkSync(outside, parent, process.platform === 'win32' ? 'junction' : 'dir');
    assert.throws(() => writeCreateOnlyAtomic({ destination, content: 'synthetic', approvedControlDir: controlDir }), /parent/);
    assert.deepEqual(readdirSync(outside), []);
  } finally {
    rmSync(controlDir, { recursive: true, force: true });
    rmSync(outside, { recursive: true, force: true });
...
test('a competing destination created at publication is never overwritten and staged bytes are cleaned', () => {
  const controlDir = makeTempDir('ctrl');
  try {
    const destination = join(controlDir, 'out.json');
    assert.throws(() => writeCreateOnlyAtomic({
      destination, content: 'candidate bytes', approvedControlDir: controlDir,
      linkImpl(source, target) {
        writeFileSync(target, 'competing bytes', { flag: 'wx' });
        linkSync(source, target);
      },
    }), { code: 'EEXIST' });
    assert.equal(readFileSync(destination, 'utf8'), 'competing bytes');
    assert.deepEqual(readdirSync(controlDir), ['out.json']);
  } finally { rmSync(controlDir, { recursive: true, force: true }); }
});

test('a protected root alias cannot bypass the resolved-path fence', () => {
  const controlDir = makeTempDir('ctrl');
  const aliases = makeTempDir('alias');
  try {
    const protectedAlias = join(aliases, 'protected');
    symlinkSync(controlDir, protectedAlias, process.platform === 'win32' ? 'junction' : 'dir');
    assert.equal(validateOutputDestination({
      destination: join(controlDir, 'out.json'), approvedControlDir: controlDir,
      forbiddenRoots: [protectedAlias],
...
test('CLI rejects malformed private input without exposing the input path or contents', () => {
  const controlDir = makeTempDir('ctrl');
  try {
    const inputPath = join(controlDir, 'private-draft.json');
    writeFileSync(inputPath, 'synthetic-private-marker');
    const result = spawnSync(process.execPath, [
      fileURLToPath(new URL('./topology_v2_cli.mjs', import.meta.url)),
      'generate', '--draft', inputPath, '--out', join(controlDir, 'out.json'),
    ], { encoding: 'utf8' });
    assert.equal(result.status, 1);
    assert.equal(result.stdout, '');
    assert.equal(result.stderr, 'failed to read draft JSON\n');
    assert.deepEqual(readdirSync(controlDir), ['private-draft.json']);
  } finally { rmSync(controlDir, { recursive: true, force: true }); }
});
