...
Antigravity의 요청 전용 기록은 대화를 이어 쓰며 바뀌는 대화/파일 시각을 개별 요청의 불변 실행 시각으로 취급하지 않는다. 동일 요청 ID·조직·모델·과제·사용량인 재관측은 최초 저장 시각을 보존한다. 모델 세부 수준·조직의 기존 보존 규칙은 유지하고, 실제 토큰·소속·모델 충돌은 계속 거부한다. 인덱스가 없는 대화는 최근 파일 관측 시각으로 수집 창을 판단해 오래된 대화의 새 활동도 읽는다. 그래프 날짜는 관측 기준이며 정확한 요청 실행 시각이 아니다.
