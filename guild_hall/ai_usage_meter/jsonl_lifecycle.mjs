...
const IDENTITY_KEYS = [
  ...
  "result_state", "observed_at", "source", "source_event",
];
...
async function selectSessionFiles({ sessionsRoot, sessionFiles, threadIds, maxSessionCount, afterThreadId }) {
  ...
  if (exactThreadIds.length) {
    ...
    for (const threadId of exactThreadIds) {
      const matching = explicitFiles
        ? explicitFiles.filter((file) => path.basename(file).endsWith(`-${threadId}.jsonl`))
        : await findSessionFilesById(sessionsRoot, threadId);
...
function normalizeObservations(observations) {
  ...
  for (const observation of observations) {
    const fingerprint = canonicalJson(observation);
...
function buildObservations(sessions) {
...
export function validateJsonlLifecycleSnapshot(snapshot) {
  ...
  for (const identity of snapshot.identities) {
    if (!hasExactKeys(identity, IDENTITY_KEYS) || !IDENTITY_KINDS.has(identity.identity_kind)) {
