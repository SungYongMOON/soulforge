...
export async function persistLifecycleReceipt(stateRoot, receipt) {
  const persisted = await persistLifecycleReceipts(stateRoot, [receipt]);
  return {
    status: persisted.created_count === 1 ? "created" : "replayed",
    receipt_id: persisted.receipt_ids[0],
    receipt_count: persisted.receipt_count,
    latest_identity_count: persisted.latest_identity_count,
  };
}

export async function persistLifecycleReceipts(stateRoot, receipts) {
  const root = path.resolve(stateRoot);
  if (!Array.isArray(receipts)) fail("lifecycle_receipt_batch_invalid");
  const acceptedById = new Map();
  for (const receipt of receipts) {
    const accepted = validateLifecycleReceipt(receipt);
    const existing = acceptedById.get(accepted.receipt_id);
    if (existing && !sameReceiptPayload(existing, accepted)) fail("lifecycle_receipt_conflict");
    if (!existing || accepted.observed_at < existing.observed_at) acceptedById.set(accepted.receipt_id, accepted);
  }
  const accepted = [...acceptedById.values()].sort((left, right) => left.receipt_id.localeCompare(right.receipt_id, "en"));
  const before = await loadLifecycleReceipts(root);
  const existingById = new Map(before.map((item) => [item.receipt_id, item]));
  let createdCount = 0;
  let replayedCount = 0;
  let requiresReload = false;
  for (const receipt of accepted) {
    const existing = existingById.get(receipt.receipt_id) ?? null;
    if (existing) {
      if (!sameReceiptPayload(existing, receipt)) fail("lifecycle_receipt_conflict");
      replayedCount += 1;
      continue;
    }
    const month = receipt.observed_at.slice(0, 7);
    const target = path.join(root, "lifecycle", "receipts", month, `${receipt.receipt_id}.json`);
    const status = await writeReceiptExclusive(target, receipt);
    if (status === "created") createdCount += 1;
    else {
      replayedCount += 1;
      requiresReload = true;
    }
    existingById.set(receipt.receipt_id, receipt);
  }
  const all = createdCount === 0 && !requiresReload ? before : await loadLifecycleReceipts(root);
  ...
  return {
    ...
    receipt_ids: accepted.map((item) => item.receipt_id),
