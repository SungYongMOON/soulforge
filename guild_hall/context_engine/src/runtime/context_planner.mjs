...
import { createLocalChat, installedModelDigest, validateChatBinding } from '../adapters/local_model/ollama_chat.mjs';
...
// A request or a binding can only lower the profile's budget.
function narrowBudget(base, ...limits) {
  const budget = { ...base };
  for (const limit of limits) {
    for (const [key, value] of Object.entries(limit ?? {})) {
      if (!Object.hasOwn(base, key) || !Number.isSafeInteger(value) || value < 0) fail('planner_budget_invalid');
...
export async function composeWorkingContext({ view, request, binding, profile = CONTEXT_PLANNER_PROFILE, fetchImpl = fetch } = {}) {
  ...
  if (plan.status !== 'ok') {
    ...
    for (let round = 2; round <= budget.max_search_rounds && budget.max_model_calls - callsMade() > 1; round++) {
      const review = await local.chat({ step: 'review', system: profile.prompts.review, schema: profile.schemas.review,
        user: JSON.stringify({ questions, evidence: modelEvidence() }) });
      if (review.status !== 'ok') break;
      answered = (Array.isArray(review.value.answered) ? review.value.answered : []).filter(id => questionIds.has(id));
      missing = (Array.isArray(review.value.missing) ? review.value.missing : []).filter(row => questionIds.has(row?.question_id))
        .map(row => ({ question_id: row.question_id, reason: text(row.reason, 300) }));
      const followUp = cleanSearches(review.value.searches, questionIds);
  ...
  const body = { schema_version: CONTEXT_PACK_V2_SCHEMA, status, code, claim_ceiling: 'observed', project_key: view.manifest.project_key,
    ...
    deliverables, questions, answered, missing, sections, open_questions: openQuestions, statement_kinds: STATEMENT_KIND_LABELS,
    ...
    rune: { status: 'not_run', reason: 'no_applicable_rule_or_verified_input' },
    enforcement: stats,
  ...
  const content = { status, code, index: body.index, planner: body.planner, deliverables, questions, answered, missing, sections,
