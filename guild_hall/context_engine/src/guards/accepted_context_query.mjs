...
import { projectAcceptedMembership } from '../../algorithms/representation/accepted_typed_v1.mjs';
import { compareCodePoints } from '../../../engineering_engine/kernel/canonical.mjs';
import { exactRefIdentityKey, sameExactRef } from '../../../engineering_engine/kernel/identity.mjs';
...
} from '../../../engineering_engine/kernel/project_context_acceptance_gate.mjs';
...
function decodeCursor(value) {
  ...
  try {
    ...
    const temporal = parsed && Object.hasOwn(parsed, 'valid_at') && Object.hasOwn(parsed, 'known_at');
    if (!parsed || !keys(parsed, temporal ? [...CURSOR_FIELDS, 'valid_at', 'known_at'] : CURSOR_FIELDS) || !ref(parsed.generation_ref)
        || (temporal && (!instant(parsed.valid_at) || !instant(parsed.known_at)))
...
export function createAcceptedContextQuery({ store, readModel, aclPolicy } = {}) {
  ...
  return {
    async query(request) {
      ...
      const temporal = safe && Object.hasOwn(safe, 'valid_at') && Object.hasOwn(safe, 'known_at');
      if (!safe || typeof safe !== 'object' || !keys(safe, temporal ? [...QUERY_REQUEST_FIELDS, 'valid_at', 'known_at'] : QUERY_REQUEST_FIELDS)) {
      ...
      const validAt = temporal ? safe.valid_at : asOf;
      const knownAt = temporal ? safe.known_at : asOf;
      ...
      if (!actorToken || !projectBinding || !targetGenerationRef
          ...
          || !instant(asOf) || !instant(validAt) || !instant(knownAt)
          || epoch(validAt) > epoch(knownAt) || (temporal && knownAt !== asOf) || !token(purpose)
      ...
      if (cursor && (!sameExactRef(cursor.generation_ref, targetGenerationRef) || cursor.scope !== scope
          || cursor.valid_at !== safe.valid_at || cursor.known_at !== safe.known_at
      ...
      const filtered = memberships.filter(function (member) {
        ...
        if (epoch(member.valid_at) > epoch(validAt) || epoch(member.known_at) > epoch(knownAt)) return false;
        return true;
      });
      ...
      const nextCursor = nextOffset < filtered.length ? encodeCursor({
        ...
        ...(temporal ? { valid_at: validAt, known_at: knownAt } : {}),
        actor_ref: actorToken, grant_revision_ref: grant.grant_revision_ref, offset: nextOffset,
      }) : null;

      const hits = sliced.map(projectAcceptedMembership);
