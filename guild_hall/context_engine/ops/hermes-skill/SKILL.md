...
**구간 초안으로 (먼저 이것)** — 라벨 run이 나눠 둔 의미 단위로 읽는다.
...
**원 전사 구간 그대로** — 초안이 없거나 초안의 경계를 다시 봐야 할 때.
...
**세션 id를 받았으면 이 순서가 아니라 §음성 구간 검토로 간다.** 그 길은 품질을 먼저 보고,
대화를 구간으로 나누고, 단서로 과제를 좁히는 — 방향이 반대인 절차다(7형은 `--units`부터).
...
첨부 텍스트 1 → 렌더 1. 음성이면: 구간 초안 읽기 1~2 → 단서로 과제 하나씩 검색 1~2 → 본문 1.
...
한 녹음에 여러 과제 얘기가 섞여 있고, 녹음마다 업무·팀 운영·아이디어·잡담이 함께 들어 있다.
그래서 **낱말을 고치는 것이 아니라 대화를 나누는 것이 먼저다.** 순서를 지킨다.

### ① 녹음·전사 품질부터 본다

출력 머리의 `transcript`·`quality_metrics`·`evidence_gate`를 먼저 읽는다.

- `평균 토큰 확률`이 낮거나 `flags`에 `low_token_probability_signal`이 있으면 **그 전사는 군데군데 틀렸다.**
- 같은 말이 계속 반복되는 구간은 **환각 반복**일 수 있다(반복 필터가 켜져 있어도 남는다). 그런 구간은
  내용을 옮기지 말고 `판독 불가(반복 의심)`로 적는다.
- 잘 안 들려 토막 난 구간을 **잡담으로 분류하지 않는다.** 안 들리는 것과 중요하지 않은 것은 다르다.
  그런 구간은 `판독 불가`로 두고 **원음 재확인 필요**를 적는다.
- `evidence_gate`가 `stronger_local_asr_required`면 라벨 엔진 스스로 "더 좋은 전사가 필요하다"고 말한 것이다.
  그 말을 답에 옮긴다.

### ② 의미가 이어지는 대화 구간을 나눈다 (초안)

`--units`가 주는 것은 **초안**이다(`claim_ceiling machine_generated_reviewable`). 그대로 쓰지 않는다.

- **한 구간 = 하나의 대화**다. 질문 → 답변 → 정정 → 결론은 **한 구간**으로 묶는다.
- **고정 시간·글자 수·화자 바뀜만으로 경계를 정하지 않는다.** 초안 경계가 대화 중간을 자르고 있으면 합친다.
- 화제가 A → B → A로 돌아오면 **A를 한 덩이로 뭉치지 말고 두 구간으로 두고** "같은 화제(앞 구간 id와 관련)"로 잇는다.
- 초안이 없으면(`labels_absent`·`labels_other_revision`·`labels_ambiguous`) 원 전사 구간으로 직접 나눈다.

### ③ 구간마다 성격과 짧은 설명을 붙인다

성격은 다섯 중 하나다: **과제 업무 / 팀 운영 / 아이디어 / 일상 / 판독 불가**.

- 제목과 설명은 **파생 요약**이다 — 실제 발언도 아니고 승인된 회의록도 아니다. 답에 그 한 줄을 반드시 적는다.
- **아이디어를 무가치로 버리지 않는다.** 결론이 안 난 제안도 `아이디어`로 남긴다.
- 일상으로 분류하는 것은 내용이 실제로 일상일 때뿐이다. **안 들려서 모르는 것은 `판독 불가`다.**

### ④ 관련 과제 후보를 대조한다 — 한 번에 하나씩

- 구간의 단서로 **후보 과제 하나씩** 검색한다(1형). 메일·Slack·Linear가 같은 DB에 있다.
- **공통 용어로는 과제를 정하지 못한다.** 출력에 `공통(과제 N개) <용어>` 표시가 붙은 낱말이 그것이고,
  등록부가 없거나(`shared_terms not_configured`·`unavailable`) 그 낱말이 등록부에 없더라도 규칙은 같다.
- **후보를 내려면 다음 중 두 가지 이상**이 그 과제를 가리켜야 한다:
  **장비 / 보드·부품 / 논의 목적 / 시험 조건 / 산출물 / 후속 기록(메일·이슈)**.
  하나뿐이면 후보가 아니다.
- 불명확하면 **중립 제목**(그 낱말을 그대로 쓴 제목)으로 두고 `candidate`가 아니라 `unclassified`로 남긴다.
- **`confirmed`는 쓰지 않는다.** 확정은 사람이 한다. `candidate`가 되어도 그 과제의 다른 자료를 더 열지 않는다.

### ⑤ 용어 교정과 재확인 표시

- 기존 지시·문서의 표기와 전사의 표기가 다르면 **전사 쪽 철자를 그대로 두고** "표기 다름(문서: …)"으로 적는다.
  비슷한 이름으로 고쳐 적지 않는다.
- 인명·수치는 기계 전사라 틀릴 수 있다 → `전사 오인 가능`.
- 판정이 그 한 낱말에 걸려 있으면 **원음 재확인 필요**를 적는다. (이 봇은 오디오를 듣지 못한다.)

### ⑥ 필요하면 구간과 과제 판단을 고친다

④·⑤에서 나온 것이 ②·③을 뒤집으면 **경계와 성격을 다시 잡는다.** 처음 나눈 대로 밀고 가지 않는다.
...
1은 과제 코드 대신 `세션 id · 읽은 창(초) · 어느 전사(local/provider) · 전사 품질`을 적고,
근거 목록 자리에 **대화 목록 표**를 둔다. 열은 이 일곱이다.
...
- 제목과 설명은 **파생 요약이며 실제 발언도 승인된 회의록도 아니다.**
- 발언자 라벨은 정렬 힌트다 — **발언자 ≠ 담당자**.
- 공통 용어(`공통(과제 N개)` 표시)로는 과제를 정하지 않았다 / 정할 수 없었다.

못 본 창이 있으면 마지막 줄 앞에 `다음 회차: --from <N> --to <M>`을 둔다.
