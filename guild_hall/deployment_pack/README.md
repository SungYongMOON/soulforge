...
- **Fresh-spec preflight**(`PACK_CATALOG[*].spec_emitter`): emitter로 만드는 pack은 그 emitter 경로를 spec이 아니라 코드 쪽 catalog(`src/deployment_pack_contract.mjs`)가 pack_id별로 박는다 — 감사 대상인 spec이 자기 감사자를 지명하거나 지울 수 없게 하기 위해서다. builder는 scan·hash·unit gate보다 먼저 그 emitter를 `--print`(stdout만; "무쓰기"는 emitter 계약이지 builder가 강제하지 않는다)로 자식 프로세스 실행해, 지금 tree에서 다시 계산한 spec과 tracked spec을 byte 비교한다(emitter `--check`와 같은 비교를 build 시점에 강제). 다르면 `spec_drifted_from_tree`로 거부하고 메시지에 무엇이 달라졌는지(tree에만 있는 파일·spec에만 있는 파일·역할 배정 변경·stale/미등재/소멸 pin·vendored hash·기타 필드 — 경로만, 각 3개+N)와 재검토 뒤 실행할 emitter 명령을 적는다. emitter 부재·실패·비객체 출력은 `spec_emitter_failed`(stderr는 첫 줄 200자만 인용). catalog 행이 `spec_emitter: null`인 손편집 spec(`tool_workshop_pack`·`project_ai_team_pack`; 모든 행이 이 키를 선언한다)은 재계산 대상이 없어 건너뛰되 build receipt의 `spec_freshness`가 `not_recomputed_no_emitter`로 이를 가시화한다(재계산 통과는 `matches_live_tree`). 이 gate가 새로 잡는 drift는 마지막 emit 뒤 tree에 추가된 파일(pack에서 조용히 빠짐), 새 test(smoke에서 조용히 빠짐), vendored byte pin 변경(builder는 `vendored_file_sha256`을 여기서만 본다)이며, secret-regex hit가 있는 pin 파일의 byte 변경은 전부터 `scan_review_pin_stale`/`unused`로 잡혔다. 주의 두 가지: `matches_live_tree`는 spec의 신선도이지 내용의 신선도가 아니며(예: tracked Universal Client bundle이 stale이어도 spec은 fresh일 수 있다), emitter로 만드는 pack에서 미등재 hit는 언제나 drift로 먼저 잡히고 재emit이 그 hit를 자동 pin하므로 "재검토"는 사람의 약속이다 — drift 메시지의 `new scan hit(s) not yet pinned` 목록을 보고 나서 재emit한다. hpp emitter는 `static/`을 live tree에서 쓸어 담으므로 `.gitignore`의 로컬 스킨 규칙(`static/skins/dungeons/`, `static/skins/main.*`)을 emitter 안에서 같은 조건으로 제외해 Owner tree와 청소 checkout이 같은 spec을 내게 했다(그 스킨을 pack에 실으려면 추적 자산 승격 또는 byte pin이 필요한 Owner 판단).
...
- tracked spec 4종: `packs/tool_workshop_pack.spec.json`(4 파일), `packs/backup_recovery_extension.spec.json`(**94 파일·30 smoke·제외 0·pin 29**, read-only topology v2 actual reader와 NAS DR preflight/runner 포함), compatibility ID를 유지한 Universal Client `packs/team_client_pack.spec.json`(**19 파일·5 smoke·pin 3**, self-contained mTLS/MCP bundle과 durable outbox 포함), `packs/hpp_server_pack.spec.json`(**1,012 파일·97 smoke entry·pin 72**, dev-ERP와 Voice/Ingress/Local Activity 운영 entrypoint 및 shared runtime-path contract 포함). 모든 Pack은 reboot/driver/system-update를 금지하고 자기 서비스만 재기동할 수 있다. Project AI Team Pack은 actual approved input 전 absent다.
