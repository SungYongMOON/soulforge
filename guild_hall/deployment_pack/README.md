...
- tracked spec 2종: `packs/tool_workshop_pack.spec.json`(4 파일), `packs/hpp_server_pack.spec.json`(dev-ERP 서버 pack **267 파일**, reviewed pin 60). dev-ERP는 app 디렉터리로 자족하지 않는다 — 청소 설치본 smoke 2라운드가 가르친 대로 guild_hall 공유 모듈 25종 cross-root import + served static + **fs-read 데이터 폐포**(contract schema 8·manual·ps1 helper 2·doctor가 검사하는 `.gitignore`·phase-1 checklist·verify gate 필수 설계/QA 문서 2·workspace 예제 fixture)까지 필요하다 — 그래서 `tools/emit_hpp_spec.mjs`는 모듈 그래프와 데이터 읽기 **두 폐포**를 계산하고, `--check`가 validate에서 drift를 fail-closed로 잡는다(packed 파일 수정 시 spec 재검토·재emit이 의도된 마찰).
- **Installed-smoke 선언 파티션**(`installed_smoke_entries`/`installed_smoke_excluded`): 전체 suite를 청소 설치본에서 다 돌릴 수 없는 pack은 실행 가능한 부분집합과 **사유 있는 제외 장부**를 선언해야 한다 — 두 리스트가 smoke 전체를 빠짐없이 분할하지 않으면(누락·중복·무사유) spec 로드가 거부되고, 장부는 smoke receipt에 동봉되어 green smoke가 제외분의 통과를 절대 암시하지 못한다. hpp는 78 실행 + 11 제외(ajv 4·yaml 5 — pack은 node_modules를 싣지 않으며 의존성 배송은 package/sbom gate의 몫; git-checkout source attestation 2)이고 89 전체 green은 그 gate들 뒤의 TARGET으로 기록된다. CLI: `node guild_hall/deployment_pack/tools/build_pack.mjs --spec <spec> --out guild_hall/deployment_pack/dist [--install-verify <dir>] [--smoke]`.
