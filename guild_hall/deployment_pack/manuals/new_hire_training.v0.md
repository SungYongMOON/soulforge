...
- `027_수신이력_이동이력`: 메일 수신 이력과 파일 이동 이력을 두는 자리. 메일 이력은 매 refresh마다 수집 메일에서 다시 만드는 현재 시점 view이고(Owner기입 칸 보존, 이전 파일은 `history/`에 보관), append-only인 것은 그 아래 수집 메일 custody와 정정 이력이다.
...
- `025_통합로그_의사결정조치/작업_장부.csv`: 실제로 한 일을 적는 장부. 누가, 언제, 어떤 task, 무엇을 근거로, 무엇이 나왔는지, 다음 action을 한 행씩 남긴다.
- `026_상태_진행현황/할일_장부.csv`: 앞으로 해야 할 일을 적는 장부. 항목, 담당자, 마감일, SE stage, 출처, 완료 기준, 상태를 한 행씩 남긴다.

그 일을 한 사람이든 AI든 직접 행을 쓴다. AI가 작업을 끝내면 작업 장부에 행을 남기고, 후속이 필요하면 할일 장부에 제안으로 추가한다 — AI가 만든 할일은 사람이 수락·수정·거절하기 전까지 확정이 아니다. AI 작업이 끝났다는 것도 사람이 확인한다.

장부에는 경로·링크·메일 이력 key 같은 포인터만 적고, 메일 본문이나 첨부, 개인정보, secret은 적지 않는다.
