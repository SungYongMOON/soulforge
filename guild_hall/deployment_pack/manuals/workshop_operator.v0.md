...
Claude 연결기는 세션이 열릴 때 두 자리를 훑는다. 설정 폴더 아래 `skills/`와, 봇이
실행되는 작업 폴더 아래 `.claude/skills/`다. 클라이언트 둥지의 `.agents/skills/`는 그
클라이언트 자체 규약 자리이며 연결기가 읽지 않는다. 그 자리에 둔 스킬은 봇 스킬 목록에
나타나지 않는다.

봇마다 다른 작업 폴더를 주고 나면 **자리 폴더 쪽에 둔다.** 자리가 갈려 있으므로 그
자리에서 뜨는 봇만 그 스킬을 본다. 지침·스킬·MCP 정책·작업이 한 폴더에 모이므로 자리를
통째로 백업하거나 옮길 수 있고, 자리 문서가 말하는 "이 자리의 스킬"과 실물이 일치한다.

설정 폴더를 봇마다 따로 두는 방법도 스킬을 가르지만 권하지 않는다. 그 폴더에는 로그인
자격 파일이 함께 있어야 봇이 기동하는데, 자격을 봇 수만큼 복사하면 갱신이 한 곳에서만
일어나 나머지가 낡는다. 로그인이 한 번 끊기면 봇 수만큼 손이 간다. 실제로 한 번 끊겼을
때 봇마다 따로 복구해야 했다. 자격은 한 벌로 두고 스킬은 자리로 가른다.

스킬 본체는 `SKILL.md` 한 파일이고 앞머리에 `name`과 `description`이 있어야 한다. 다른
도구용 스킬 트리에서 가져올 때는 그 파일을 복사한다. 복사본은 원본과 자동으로 동기화되지
않으므로, 자리 전용본으로 삼을 때는 이름을 달리하고 출처를 파일에 적어 갈래를 분명히
한다.

### 자리에서 외부 MCP를 막는 방법

봇이 사람 계정의 설정을 읽으면 거기 등록된 MCP 서버가 함께 들어온다. 계정 커넥터를 끄는
환경변수는 이쪽에 적용되지 않는다. 서로 다른 경로이므로 각각 막아야 한다. 실제로 자격을
한 벌로 되돌린 직후 사람 계정에 등록된 서버들이 봇 세션에 결속됐고, 그중에는 녹음 전사
도구가 있었다.

자리 폴더의 `.claude/settings.json`에 거부 규칙을 두면 그 자리에서만 막힌다. 서버 이름을
`mcp__<서버>` 형태로 적는다. 적용 뒤 봇에게 결속된 MCP 도구를 물어 0개인지 확인한다.
서버가 설정에 등록돼 있다는 것과 도구가 결속됐다는 것은 다르므로, 봇의 답에서 그 둘을
구분해 읽는다.

이 방식은 이름을 적은 서버만 막는다. 사람 계정에 서버가 새로 등록되면 그것은 막히지
않으므로, 자리를 점검할 때 결속 도구 수를 함께 본다.
...
한 봇을 세울 때 봇마다 달라지는 것은 아래와 같다. 공용 중계기 스크립트와 harness 등록은
하나씩만 두고 재사용하므로, 봇이 늘어도 코드는 늘지 않는다.

| 갈리는 것 | 두는 곳 | 효과 |
| --- | --- | --- |
| 작업 자리 | 작업 폴더 지정 환경변수 | 그 자리의 컨텍스트 파일이 지침으로 주입된다 |
| 일감 규칙 | 자리 폴더의 컨텍스트 파일 | 그 자리의 작업 규칙이 봇에게 들어간다 |
| 스킬 | 자리 폴더의 `.claude/skills/` | 그 자리에서 뜨는 봇만 그 스킬을 본다 |
| 외부 MCP | 자리 폴더의 `.claude/settings.json` 거부 규칙 | 사람 계정에 등록된 서버가 그 자리에서 결속되지 않는다 |
| 계정 커넥터 | 커넥터 사용 환경변수 | 계정에 붙은 커넥터가 그 봇에서만 빠진다 |
| 작업 시간 | 침묵 허용치·절대 상한 환경변수 | 문서 작업 길이에 맞춘 상한을 두지 않는다 |
| 표시 이름 | 관리 화면의 편집·저장 | 기록 파일 편집으로는 바뀌지 않는다 |

로그인 자격은 봇마다 가르지 않는다. 사람 계정의 한 벌을 그대로 쓴다. 봇 수만큼 복사하면
갱신이 한 곳에서만 일어나 나머지가 낡고, 한 번 끊길 때 봇 수만큼 손이 간다.

새 자리를 세우는 순서는 자리 폴더와 컨텍스트 파일 준비, 자리의 스킬과 MCP 거부 규칙 배치,
봇 등록의 실행기 전환과 환경변수 기재, 표시 이름이 필요하면 관리 화면에서 변경, 기동 뒤
확인이다. 확인은 봇에게 작업 폴더·주입된 규칙·결속된 스킬·결속된 MCP 도구 수를 묻고
harness 로그의 실제 실행 명령과 대조한다. 하나라도 확인 전에는 적용됐다고 세지 않는다.

### 봇 이름은 파일로 바뀌지 않는다

봇의 표시 이름은 봇 키로 서명해 relay에 올린 프로필에서 온다. 봇 기록 파일의 이름 값을
손으로 고치면 관리 화면의 표시만 바뀌고 대화에 보이는 이름은 옛 이름 그대로 남는다.
기동해도 갱신되지 않는다. 앱은 이름이 바뀐 사실을 모르므로 프로필을 다시 올릴 대상으로
표시하지 않는다. 실제로 손 편집 뒤 정지·시작을 반복해도 relay의 프로필은 이틀 전 값
그대로였고, 관리 화면에서 이름을 바꿔 저장하자 그 즉시 새 프로필이 올라갔다.

그러므로 이름만은 관리 화면의 편집·저장 경로로 바꾼다. 나머지 — 실행기, 작업 폴더,
설정 폴더, 커넥터, 턴 길이, 지침, 소개 문구 — 는 기록 파일 편집으로 반영된다. 이름을
바꾼 뒤 다른 설정이 초기화되지 않았는지는 저장 후 한 번 대조한다. 실제 사례에서는
유지됐다.

대화 채널의 이름은 채널을 열 때 정해지며 프로필을 바꿔도 따라오지 않는다. 자리의 용도가
바뀐 봇에게는 채널을 새로 여는 편이 낫다. 옛 채널을 그대로 쓰면 이전 용도의 대화가 최근
메시지로 딸려 들어와 봇이 지난 지시를 자기 할 일로 읽는다. 실제로 그런 관측이 있었다.

### 봇이 자기 이름을 아는 경로

봇은 자기 이름을 따로 알려 주지 않으면 주변에서 주워 쓴다. 자리 문서의 제목이나 대화
채널의 이름을 자기 이름으로 착각한다. 자리 이름과 봇 이름은 다른 것이고, 한 자리에 다른
봇이 앉을 수 있으므로 자리 문서에 봇 이름을 적어 해결하지 않는다.

harness는 봇의 표시 이름을 환경변수로 넘기며 봇 프로세스가 그것을 물려받는다. 그러므로
정체성 쪽 지침에 "네 이름은 그 환경변수에 있다, 자리 문서의 제목과 채널 이름은 자리와
통로의 이름이지 네 이름이 아니다"를 한 줄 둔다. 그러면 이름이 바뀌어도 문서를 고칠 필요가
없고, 자리와 봇이 각각 자기 이름으로 불린다. 확인은 봇에게 자기가 누구이고 어디서
일하는지를 묻고, 이름과 자리를 나누어 답하는지로 본다.
