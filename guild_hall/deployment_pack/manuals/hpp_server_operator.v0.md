...
- 현재 `/workbench.html`은 결속된 Buzz 업무 한 건의 기록 조회 화면이다. 오늘 할 일,
  전체 응답 대기, 진행·막힘·담당, 결과물과 과제 자료를 한곳에서 관리하는 실사용
  통합 작업대는 아직 완성되지 않았다. 이 조회 화면이나 검사 통과로 제품 완성을 주장하지 않는다.
- 로그인 계정의 **내 연결**에서는 본인 MCP 연결을 조회하고 대상·영향을 확인한 뒤
  철회한다. 서버 재조회가 철회를 확인해야 완료로 표시한다. 연결키 발급·평문 노출은 없다.
  기존 cookie HTTP 철회 호출자는 먼저 목록 GET의 CSRF를 받고 같은 Origin으로 보내야 한다.
  관리자 CLI 직접 호출은 유지하며 외부·private 호출자의 이전 여부는 별도 확인한다.
- Windows 링크 생성 권한이 필요한 네 검사는 미실행 근거로 남는다. 공개 CI의 승인된
  업무 발견 kit 준비와 실제 계정 분리 검증도 로컬 합성 검사로 대체하지 않는다.
