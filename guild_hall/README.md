...
- `deployment_pack/`: 배포 pack 5종 카탈로그·릴리스 gate 사다리 15단(단조 evidence 강제)·rollout ring 8단·runbook 카탈로그 13종의 **계약** owner + **pack builder CLI**(tracked spec→미추적 dist; validate-before-write, secret 내용 스캔+exact-pin scan-review 장부(stale/unused pin fail-closed), `test_cwd` 지원, timestamp 없는 byte-동일 manifest, `claimed_gate: contract`까지만 주장하고 격리 install digest 양방향 전수검증·installed-copy smoke는 out-of-ladder receipt로만 기록). tracked spec 4종: tool_workshop_pack(4 파일)·backup_recovery_extension(49 파일, 14/14 smoke·제외 0)·team_client_pack(Board 소스 211 파일, 전 suite 77파일 smoke green·제외 0)·**hpp_server_pack(dev-ERP 모듈+데이터+vendored-npm 세 폐포 941 파일 — guild_hall 공유·vendored 6패키지 656파일 포함, pin 62, installed-smoke **91 전체 실행·제외 0**(git-free pack-manifest attestation으로 장부 해소) 파티션 강제, emit `--check` drift gate)**. `tools/prove_start_stop.mjs`는 설치본의 **격리 start/stop 증명**(pre-gate 전 파일 byte 검증→ephemeral port 기동→health가 manifest와 동일한 64-hex pack digest attest→종료·port 해방→payload 양방향 byte-clean 재검증)을, `tools/pack_lifecycle.mjs`는 **backup/upgrade/rollback/restore**(verify-before-mutate·이전 세대 1개 보존·검증 불가 세대는 rollback 약속에서 제외)를 out-of-ladder receipt로 남긴다 — initial gate 문구의 7개 다리(install/start/stop/smoke/upgrade/rollback/restore) 전부 증거 확보, 상시 기동·등록 없음. ring 승격·발행·서비스 상시 기동·물리 ring은 Owner gate 뒤. 검증은 `npm run validate:deployment-pack`.
...
- `module_operability/`: manifest 완전성 스키마(26필드·lifecycle probe 규칙·release_digest stale fail)·선언 의존 validator(`port:*` caller-주입 규약, Board→Watch 명시 의존 포함)·import-cycle validator(재수출 포함; repo 스캔 cycle 0 고정 — pcb_compliance evaluator⇄adapter cycle을 fact_admission leaf 추출로 절단)·preflight CLI(validator 실재성 = fixture/integration 계약, `--stamp` 명시 각인)의 **check-only** owner. 검증은 `npm run validate:module-operability`.
