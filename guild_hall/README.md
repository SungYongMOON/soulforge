...
- `module_operability/`: manifest 완전성·선언 의존·import-cycle·aggregate preflight와 PC1~PC3 **no-move product composition** 검사의 check-only owner. 현재 Universal Client를 포함한 31 enrolled Module을 8 Product-owned/23 Shared로 exact 분류하고 새 Module 미분류·path traversal·stale Interface·source move·release claim을 거부한다. 검증은 `npm run validate:module-operability`과 `npm run validate:product-composition`.
