...
- `context_engine/`: 맥락 제작 APP의 공개 호출·CLI 진입점, 공통 권한·판본·예산 검사,
  준비·표현·검색·기억·조립 알고리즘, profile·하니스·검사·설치 명세를 모으는 owner.
  공통 RAG·validator는 명시 의존으로 사용한다. 수집·RUNE 업무 규칙·정본 수락 권한은 기존 owner에 남고,
  프로젝트 데이터 배치는 Plan 17을 따른다. 기본 off이며 명시적 합성 binding 또는 Owner 작업 관찰 binding만 받는다.
  dev-ERP caller 연결(shim·행보관 CLI)은 HPP 릴리스 gate 전까지 보류하고, 그래프 저장·추출·의미/그래프 검색은
  D41과 같은 날 Owner 지시(2026-09-12)에 따라 Neo4j GraphRAG를 연결해 쓴다(아직 미연결). 실자료·운영 적용은 HOLD다.
...
- `deployment_pack/`: 배포 pack 5종, 15단 release gate, 8단 rollout ring, exact 16-role runbook catalog와 isolated pack builder의 계약 owner. 16개 역할의 digest-bound candidate manual과 deterministic HTML projection을 관리하되 actual exercise·release promotion은 별도 Gate다. tracked spec: tool_workshop 4파일, Backup-Recovery 72파일/24 smoke/pin19, Universal Client(`team_client_pack` compatibility ID) 19파일/5 smoke/pin3, HPP 1,018파일/93 smoke/pin72; Project AI Team spec은 actual approved binding 전 absent다. Pack spec·격리 install/smoke/lifecycle 증거는 release·physical rollout을 뜻하지 않는다. 검증은 `npm run validate:deployment-pack`, `validate:manual-release`, `validate:manual-projection`이다.
...
- `watch_panel_contract/`: Watch/4192 coarse projection **계약** owner(패널 enum·freshness 의미론·safe pointer·승인요청 filing·no-writer 구조 증명). probe·render·writer 없음. Board 채택: `ui-workspace/apps/team-ops-board/src/core/watch-panel-view.mjs`가 이 계약을 단일 원천으로 소비해 전 domain full-coverage 스트립 view-model(무증거=unknown 행, display severity 요약)을 제공하며 mutation은 계약 request surface(filing)뿐이다 — 검증은 `npm run validate:watch-panel-board`. 페이지 배선 완료: `?watch=1` 명시 flag에서만 렌더되는 기본 OFF strip(`src/watch-strip.tsx`, `main.tsx`에서 **lazy 로드** gate — flag 없으면 strip 모듈 체인을 아예 로드하지 않아 기존 Board와 동일하고, flag 토글은 리로드 필요)이며 preview로 OFF(모듈 로드 0)/ON·콘솔 클린을 검증함 — team-ops-board dev 표면이 곧 4192 포트다(측정: vite 7.3.1은 module-graph 내 cross-root import를 `fs.allow` 없이 서빙하므로 config 변경 불필요). 실 증거 공급자 3종 배선됨(`src/core/watch-evidence-suppliers.mjs`, flag 경로 한정 읽기 전용 GET 3건): `connector_freshness`←receipt-expiry projection의 source-asserted status(ready→healthy, partial→degraded, **unavailable→unknown** — 투영이 못 보는 것을 connector 다운으로 단언하지 않는 의도적 비대칭), `hpp_host`←host-stats(측정치는 단언이 아니므로 unknown+evidence_at, 살아있는 피드=as_asserted vs 없는 피드=no_evidence 구분), `hermes_runtime`←agent-runtime envelope(source-asserted **hold**는 clock 없이도 hold_asserted로 생존 — 라이브 검증됨; ready는 per-bot 관측가능성 이진 규칙: 전 bot observed→healthy, 미관측/불량 row→degraded, bot 0→unknown; 활동값 working/idle은 절대 매핑 안 함). 불인식 envelope·형태 불량·fetch 실패는 전부 무공급(no_evidence)이고 render는 contract throw에 대해 all-unknown으로 강등 방어. 나머지 6 domain 공급자(watchtower topology 포함 — 이 PC엔 binding이 없어 ok-경로 관측 불가)는 후속 leaf. 계약 자체 검증은 `npm run validate:watch-bastion`.
...
- `authority_taxonomy/`: OD-11의 **pure/default-OFF contract** owner. 기존 A0~A6 effect shape와 별도 R0~R4 risk·EV1~EV3 evidence를 결속하고, R3 grant와 R4 authoring을 거부하며 STOP을 subtract-only로 처리한다. ERP writer·Bastion runtime·Console action·persistence 없음. 검증은 `npm run validate:authority-taxonomy`.
- `agent_platform/`: `product.agent`의 composition-only product root. 현재 구현을 이동·복제하지 않고 Agent Observation·AI Usage Meter·Codex Work Directory·Tool Workshop manifest를 묶는다. README와 product manifest 외 runtime 구현·release 없음.
- `module_operability/`: manifest 완전성·선언 의존·import-cycle·aggregate preflight와 PC1~PC3 **no-move product composition** 검사의 check-only owner. 현재 맥락 APP을 포함한 35 enrolled Module을 8 Product-owned/27 Shared로 exact 분류하고 새 Module 미분류·path traversal·stale Interface·source move·release claim을 거부한다. 검증은 `npm run validate:module-operability`과 `npm run validate:product-composition`.
- `integration_dogfood/`: **cross-module 통합 테스트 전용** owner(plan 13 test ladder의 module integration/default-off 단 첫 합성 조각) — 합성 work item 1건이 forge→vault(bundle)→forge brief→workshop receipt로 흐르며 같은 digest·task_ref가 보존됨, MCP read facade가 실제 코어 record를 egress 계약 안에서 서빙함, redaction/external gate 결합, 균일 거부 어휘 동일성을 증명. 실행 코드·상태·authority·manifest 없음(test-only). 검증은 `npm run validate:integration-dogfood`.
...
- `watchtower/`: 각 owner가 선언한 public-safe 구조를 결정론적으로 합성하고, local probe·receipt가
  ...
  `topology/federated_topology.v1.contract.json`이 **단일 topology oracle pin**(provider별 노드/간선 수·artifact sha256)이며 producer·Board 테스트가 기대값을 전부 여기서 유도한다 — 구조 성장은 의도적 pin 갱신, 무언의 drift는 양쪽 fail-closed.
