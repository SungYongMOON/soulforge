...
- `deployment_pack/`: 배포 pack 5종, 15단 release gate, 8단 rollout ring, exact 16-role runbook catalog와 isolated pack builder의 계약 owner. Manual resolver는 actual artifact가 없는 16개 role을 모두 `HOLD/manual_absent`로 유지하며 build_pack integration은 후속 Gate다. tracked spec: tool_workshop 4파일, Backup-Recovery 72파일/24 smoke/pin19, Team Client 216파일/78 smoke/pin19, HPP 964파일/93 smoke/pin67; Project AI Team spec은 actual approved binding 전 absent다. Pack spec·격리 install/smoke/lifecycle 증거는 release·physical rollout을 뜻하지 않는다. 검증은 `npm run validate:deployment-pack`과 `npm run validate:manual-release`.
...
- `module_operability/`: manifest 완전성·선언 의존·import-cycle·aggregate preflight와 PC1~PC3 **no-move product composition** 검사의 check-only owner. 현재 30 enrolled Module을 7 Product-owned/23 Shared로 exact 분류하고 새 Module 미분류·path traversal·stale Interface·source move·release claim을 거부한다. 검증은 `npm run validate:module-operability`과 `npm run validate:product-composition`.
