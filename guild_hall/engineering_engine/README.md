...
- 계약 문서와 그 JSON Schema(draft 2020-12 subset)·무의존 검증기·시험은 투영 원천이 있는
  package 안 `contracts/`·`schemas/`·`tests/`에 둔다(루트 `contracts/`는 포인터 전용,
  `tools/validate_no_duplicate_authority.mjs`). 예: `engines/systems_engineering/contracts/task_hierarchy_v1.md`
