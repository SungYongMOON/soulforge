...
2. **`engines/`**: Domain Engine implementations adhering to the Core Interface.
   ...
   - `engines/quality_readiness/`: Quality Readiness (QR) domain package with direct-source derivation, locator-only non-authoritative RAG, Core-compatible Typed Facts/Profile compilation, fixed-point replay-bound assessment/observation/guidance/read tools, seven closed deepening schemas, public-synthetic fixtures, tests, a bounded manual, and `engine.yaml`. The 56-row corpus aggregate and caller-admitted Profile lanes remain `observed`/HOLD; the local MCP-shaped dispatcher is unregistered and read-only.
   ...
   - `engines/field_failure_corrective_action/`: Field Failure Corrective Action (E09) evidence-readiness package with public eCFR/FDA source bindings, closed Project Binding/Typed Facts/rules/results/receipts, deterministic containment/RCA/corrective/effectiveness/recurrence readiness evaluation, public synthetic fixtures, hostile/replay/zero-write tests, and a bounded manual.
   - `engines/safety_hazard/`: Safety Hazard (E07) evidence-readiness package with public MIL-STD-882E source binding, obsolete/context source guards, closed Project Binding/Typed Facts/rules/results/receipts, deterministic hazard evidence evaluation, public synthetic fixtures, hostile/replay/zero-write tests, and a 12-chapter manual.
   - `engines/bom_supply_chain_risk/`: BOM & Supply-Chain Risk (E08) evidence-readiness package with public DoD/DFARS/NIST source boundaries, exact BOM identity, dual-gated DFARS applicability evidence, closed Core Typed Facts/Profile/rules/results/receipts, public synthetic fixtures, hostile/schema/replay/zero-write tests, and a bounded manual.
   - `engines/interface_consistency/`: Interface Consistency (E02) structural evidence-readiness package with public ECSS/NASA/DLA/RFC source boundaries, exact Core Typed Facts/Profile replay, canonical interface/pair/rule projections, strict schemas plus a package semantic verifier, public synthetic fixtures, hostile/replay/zero-write tests, and a bounded manual.
...
- `engineering_engine/` 은 Soulforge 의 cross-project 증거기반 Engineering Engine Core와 전문 Domain Engine 구현을 소유한다. 현재 통합된 Domain Engine은 체계공학·품질 준비도·데이터베이스 공학·자재조달 준비도·신뢰성/정비성·교정/측정 유효성·형상변경 영향·제조 준비도·현장고장 시정조치·안전위험·PCB 증거준비도이며, Interface 등 후속 분야는 같은 Core Interface를 쓰는 독립 package target이다.
...
- `schemas/`: `contracts/`가 인용하는 cross-engine 데이터 계약의 JSON Schema(draft 2020-12
  subset)와 그 무의존 검증기. 예: `contracts/task_hierarchy_v1.md`의 짝인
  `task_hierarchy_v1.schema.json` + `task_hierarchy_v1_schema_validator.mjs`. `emit_manifest.mjs`의
  `INCLUDED_DIRS`에 포함된다
- `fixtures/`: 합성 fixture
