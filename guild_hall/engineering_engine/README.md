...
- `engineering_engine/` 은 Soulforge 의 cross-project 증거기반 Engineering Engine Core와 전문 Domain Engine 구현을 소유한다. 현재 통합된 Domain Engine은 체계공학·품질 준비도·데이터베이스 공학·자재조달 준비도·신뢰성/정비성·교정/측정 유효성·형상변경 영향·제조 준비도·현장고장 시정조치·PCB 증거준비도이며, Interface 등 후속 분야는 같은 Core Interface를 쓰는 독립 package target이다.
