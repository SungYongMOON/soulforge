...
- Graph DB(Neo4j)와 GraphRAG는 §4.4 트리거를 기다리지 않고 채택했다(D41, 2026-09-12 Owner). Neo4j는 조회 전용 projection이라 원장이 여전히 진실이며, Neo4j를 HPP data surface로 넣는 개발 조각에서 backup/restore 분류와 synthetic restore gate를 함께 닫는다.
