...
import {
  ...
  NODE_KINDS,
  SE_FLOORS,
} from '../rules/stage_rule_compiler.mjs';
import { PRESENCE_STATES_PIN } from '../rules/pilot_packet_generator.mjs';
import {
  ...
  nationalTokenFor,
  artifactTypeEntry,
} from '../rules/artifact_vocabulary.mjs';
import {
...
test('the module and everything it imports read no file, clock, network, or model', () => {
  ...
  walk(new URL('../rules/stage_rule_compiler.mjs', import.meta.url));
  ...
  const entry = seen.get(new URL('../rules/stage_rule_compiler.mjs', import.meta.url).href);
