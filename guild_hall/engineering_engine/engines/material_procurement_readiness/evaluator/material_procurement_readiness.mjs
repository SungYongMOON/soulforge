...
import {
  ...
  isPurchaseOrderState,
} from "../rules/material_procurement_readiness_rules.mjs";

export const MPR_TYPED_FACTS_SCHEMA = "soulforge.material_procurement_readiness.typed_project_facts.v0";
...
export const MPR_RECEIPT_SCHEMA = "soulforge.material_procurement_readiness.receipt.v0";
...
const ROOT_FACT_FIELDS = Object.freeze([
  "as_of_date",
  "erp_snapshot_ref",
  "fact_authority",
  "rows",
...
function assertDenseArray(value, label) {
  if (types.isProxy(value) || !Array.isArray(value)
    || Object.getPrototypeOf(value) !== Array.prototype || value.length > 256) {
    fail(MPR_ERROR_CODES.FACTS_INVALID, `${label} must be a bounded ordinary array`);
  }
  for (let index = 0; index < value.length; index += 1) {
    const descriptor = Object.getOwnPropertyDescriptor(value, String(index));
    if (!descriptor || !Object.hasOwn(descriptor, "value") || descriptor.enumerable !== true) {
      fail(MPR_ERROR_CODES.FACTS_INVALID, `${label} must be dense and data-only`);
    }
  }
  if (Reflect.ownKeys(value).some((key) => typeof key !== "string"
    || (key !== "length" && !/^(0|[1-9][0-9]*)$/u.test(key)))) {
    fail(MPR_ERROR_CODES.FACTS_INVALID, `${label} may not carry named or symbol fields`);
  }
}
...
function assertDateOrNull(value, label) {
  ...
  if (typeof value !== "string" || !/^\d{4}-\d{2}-\d{2}$/u.test(value)) {
    fail(MPR_ERROR_CODES.FACTS_INVALID, `${label} must be an ISO calendar date or null`);
  }
  const [year, month, day] = value.split("-").map(Number);
  const parsed = new Date(Date.UTC(year, month - 1, day));
  if (parsed.getUTCFullYear() !== year || parsed.getUTCMonth() !== month - 1 || parsed.getUTCDate() !== day) {
    fail(MPR_ERROR_CODES.FACTS_INVALID, `${label} must be a real ISO calendar date or null`);
  }
  return value;
}

function assertNonnegativeIntegerOrNull(value, label) {
  if (value === null) return null;
  if (!Number.isSafeInteger(value) || value < 0) {
    fail(MPR_ERROR_CODES.FACTS_INVALID, `${label} must be a non-negative safe integer or null`);
  }
  return value;
}
...
function copyFacts(input) {
  const facts = copyExactFields(input, ROOT_FACT_FIELDS, "typed project facts");
  if (facts.schema_version !== MPR_TYPED_FACTS_SCHEMA
    || facts.fact_authority !== "erp_owned_read_only_snapshot") {
    fail(MPR_ERROR_CODES.FACTS_INVALID, "typed facts must identify a read-only ERP-owned snapshot contract");
  }
  const asOfDate = assertDateOrNull(facts.as_of_date, "as_of_date");
  if (asOfDate === null) fail(MPR_ERROR_CODES.FACTS_INVALID, "as_of_date is required");
  const snapshotRef = copyRef(facts.erp_snapshot_ref, "erp_snapshot_ref");
  assertDenseArray(facts.rows, "typed project facts.rows");
  const rowCount = facts.rows.length;
  if (rowCount === 0) fail(MPR_ERROR_CODES.FACTS_INVALID, "typed project facts.rows must be non-empty");

  const seenNeedRefs = new Set();
  const rows = [];
  for (let index = 0; index < rowCount; index += 1) {
    const raw = Object.getOwnPropertyDescriptor(facts.rows, String(index)).value;
    const row = copyExactFields(raw, ROW_FIELDS, `typed project facts.rows[${index}]`);
    if (!TOKEN.test(row.material_need_ref) || !TOKEN.test(row.material_ref) || !TOKEN.test(row.quantity_uom)) {
      fail(MPR_ERROR_CODES.FACTS_INVALID, "material_need_ref, material_ref, and quantity_uom must be bounded tokens");
    }
    if (seenNeedRefs.has(row.material_need_ref)) {
      fail(MPR_ERROR_CODES.FACTS_INVALID, "material_need_ref values must be unique within one snapshot");
    }
    ...
    if (!isPurchaseOrderState(row.purchase_order_state) || typeof row.receipt_required !== "boolean") {
      fail(MPR_ERROR_CODES.FACTS_INVALID, "purchase_order_state or receipt_required is invalid");
    }
    const requiredQuantity = assertNonnegativeIntegerOrNull(row.required_quantity, "required_quantity");
    const availableQuantity = assertNonnegativeIntegerOrNull(row.available_quantity, "available_quantity");
    const openPurchaseQuantity = assertNonnegativeIntegerOrNull(row.open_purchase_quantity, "open_purchase_quantity");
    const receivedQuantity = assertNonnegativeIntegerOrNull(row.received_quantity, "received_quantity");
    const leadTimeDays = assertNonnegativeIntegerOrNull(row.lead_time_days, "lead_time_days");
    if (leadTimeDays !== null && leadTimeDays > 3650) {
      fail(MPR_ERROR_CODES.FACTS_INVALID, "lead_time_days exceeds the candidate engine bound");
    }
    if (NO_OPEN_SUPPLY_STATES.has(row.purchase_order_state) && openPurchaseQuantity !== null && openPurchaseQuantity !== 0) {
      fail(MPR_ERROR_CODES.FACTS_INVALID, "a non-open purchase-order state cannot carry open purchase quantity");
    }
    if (row.purchase_order_state === "unknown" && openPurchaseQuantity !== null) {
      fail(MPR_ERROR_CODES.FACTS_INVALID, "unknown purchase-order state must preserve unknown open purchase quantity as null");
    }
    rows.push({
      ...
      need_date: assertDateOrNull(row.need_date, "need_date"),
      lead_time_days: leadTimeDays,
      order_date: assertDateOrNull(row.order_date, "order_date"),
      planned_receipt_date: assertDateOrNull(row.planned_receipt_date, "planned_receipt_date"),
      promised_delivery_date: assertDateOrNull(row.promised_delivery_date, "promised_delivery_date"),
      confirmed_receipt_date: assertDateOrNull(row.confirmed_receipt_date, "confirmed_receipt_date"),
...
export function evaluateMaterialProcurementReadiness(effectiveRuleSet, typedProjectFacts) {
  const ruleset = copyAndValidateRuleset(effectiveRuleSet);
  const facts = copyFacts(typedProjectFacts);
  const rows = facts.rows.map((row) => evaluateRow(row, facts.as_of_date, ruleset.policy.default_receipt_required));
  const assessment = {
