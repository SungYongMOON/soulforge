...
test("the existing Core Interface preserves Profile provenance and evaluates typed ERP facts without Core changes", () => {
  ...
  const result = evaluate(
    ...
    {},
    { valid_at: request.typed_project_facts.valid_at, known_at: request.typed_project_facts.known_at },
  );
  ...
  assert.equal(materialProcurementReadinessAdapter.revision, "soulforge.material_procurement_readiness.evaluator.v1");
});
...
test("module manifest remains a pre-release deterministic-only contract with caller-pinned build facts", () => {
  ...
  assert.equal(manifest.input_schema_revision, "soulforge.material_procurement_readiness.typed_project_facts.v1");
  assert.equal(manifest.output_schema_revision, "soulforge.material_procurement_readiness.assessment.v0");
});
