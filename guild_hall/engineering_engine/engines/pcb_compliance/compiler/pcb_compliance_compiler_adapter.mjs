...
function assertSafeString(value, label, code = PCB_COMPILER_ERROR_CODES.PROFILE_RULE_INVALID) {
  if (typeof value !== "string" || !SAFE_STRING.test(value) || FORBIDDEN_STRING.test(value) || !isPublicSafeString(value)) {
...
function normalizeProfileRule(rawRule, bindingSourceRefs, existingRuleIds) {
  ...
  if (existingRuleIds.has(rawRule.rule_id)) {
    throw new ContractError(PCB_COMPILER_ERROR_CODES.PROFILE_RULE_DUPLICATE, "duplicate rule_id in profile");
  }
  ...
  assertSafeString(rawRule.source_ref, "profile rule source_ref", PCB_COMPILER_ERROR_CODES.PROFILE_RULE_INVALID);
  assertPublicSafeString(rawRule.source_ref, "profile rule source_ref", PCB_COMPILER_ERROR_CODES.PROFILE_RULE_INVALID);
  assertPublicText(rawRule.source_locator, "profile rule source_locator", PCB_COMPILER_ERROR_CODES.PROFILE_RULE_INVALID);
  assertPublicSafeString(rawRule.source_locator, "profile rule source_locator", PCB_COMPILER_ERROR_CODES.PROFILE_RULE_INVALID);
  assertPublicText(rawRule.source_modality, "profile rule source_modality", PCB_COMPILER_ERROR_CODES.PROFILE_RULE_INVALID);
  assertPublicSafeString(rawRule.source_modality, "profile rule source_modality", PCB_COMPILER_ERROR_CODES.PROFILE_RULE_INVALID);
...
function normalizeBindings(profileBindings) {
  ...
  return profileBindings.map((binding, index) => {
    ...
    for (const ref of binding.source_refs) {
      assertSafeString(ref, "profile binding source_ref", PCB_COMPILER_ERROR_CODES.PROFILE_BINDING_INVALID);
      assertPublicSafeString(ref, "profile binding source_ref", PCB_COMPILER_ERROR_CODES.PROFILE_BINDING_INVALID);
    }
    return binding;
  });
}
...
export function compilePcbComplianceRules(profileBindings = [], compilationScope = {}) {
  ...
  for (const binding of bindings) {
    for (let opIndex = 0; opIndex < binding.operations.length; opIndex += 1) {
      const operation = binding.operations[opIndex];
      ...
      const itemCanon = normalizeProfileOperations([singleOp]);
      profileRuleProvenance[rule.rule_id] = {
        operation_index: opIndex,
        operation_item_digest: itemCanon.operation_digest,
