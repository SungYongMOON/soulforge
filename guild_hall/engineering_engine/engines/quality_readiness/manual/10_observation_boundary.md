...
The projection signature is
`projectQualityReadinessObservations({ effective_rule_set, typed_facts, assessment_run,
observation_run_ref, known_at })`. It uses the fixed evaluator replay verifier, so the supplied
assessment must be reproduced from that exact Core assembly and Typed Facts envelope before its
Typed Facts digest is used. Its receipt carries the bound assessment digest, so a stale or
mismatched observation cannot be paired with a different assessment in guidance.
