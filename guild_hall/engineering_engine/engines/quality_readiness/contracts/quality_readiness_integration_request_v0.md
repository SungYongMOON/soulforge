...
| --- | --- | --- | --- |
| Root validation entrypoint | `package.json` | Add an opt-in `validate:quality-readiness-deepening` script that checks the local source/RAG/Typed-Facts/observation/guidance/MCP/schema surfaces and runs both `tests/quality_readiness_deepening.test.mjs` and `tests/quality_readiness_adversarial.test.mjs`; do not alter existing root validation semantics without owner review. | Direct focused command only; root package untouched. |
| Shared schema registration | Root schema registry selected by the owner | Decide whether the closed QR-local schema IDs and `contracts/schemas/**` become registry-visible and which compatibility/versioning policy applies. | Local descriptor/schema parity only; no registry edit. |
