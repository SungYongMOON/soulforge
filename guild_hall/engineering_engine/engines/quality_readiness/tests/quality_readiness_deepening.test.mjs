...
test('Q1-G4: derived Profile evaluation uses the Core-compatible Typed Facts envelope and retains full per-Profile trace', () => {
  ...
  assert.deepEqual(roundTrip.compilation_trace.compilation_scope, {});
...
test('P2 outer admission: a Proxy Typed Facts wrapper is refused before its getter trap runs', () => {
  const fixture = buildQualityReadinessDeepeningPublicSynthetic();
  let trapCount = 0;
  const hostile = new Proxy({}, {
    get() {
      trapCount += 1;
      throw new Error('outer getter must not run');
    },
  });
  assert.throws(
    () => evaluate(qualityReadinessAdapter, fixture.assembly, hostile, {}),
    (error) => error?.code === 'QR_EVALUATION_INPUT_REFUSED',
  );
...
test('Q1 hardening: derived Profile source bindings refuse missing, downgraded, duplicate, and lane-confused inputs', () => {
  ...
  const assertProfileRefused = (mutate) => {
    ...
    refreshTypedFactsDigest(envelope);
    assert.throws(
  ...
  refreshTypedFactsDigest(widenedClassification);
  assert.throws(
...
test('P1-C: a self-declared official Profile source remains observed and cannot promote itself', () => {
  ...
  refreshTypedFactsDigest(arbitraryOfficial);
  ...
  refreshTypedFactsDigest(selfPromoted);
  assert.throws(
...
test('Q1 B1: aggregate and guidance claim ceilings clamp to the weakest evaluated source lane', () => {
  ...
  refreshTypedFactsDigest(syntheticLaneBaseOnly);
  ...
  assert.equal(
    buildQualityReadinessGuidance({
      ...
      typed_facts: syntheticLaneBaseOnly,
      assessment_run: baseOnlyAssessment,
      observation_projection: baseOnlyObservation,
    }).claim_ceiling,
    'observed',
  );
...
test('Q1-G5: observation and guidance remain zero-write owner-review projections, and local MCP exposes only read tools', () => {
  ...
  for (const tamperedCeiling of ['canon_entry', 'totally_bogus']) {
    ...
    assert.throws(
      () => buildQualityReadinessGuidance({
        effective_rule_set: fixture.assembly,
        typed_facts: fixture.typed_facts,
        assessment_run: tamperedAssessment,
        observation_projection: observation,
      }),
  ...
  const mcpGuidance = callQualityReadinessReadTool({
