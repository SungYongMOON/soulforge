...
const RETRIEVAL_INPUT_KEYS = Object.freeze(['packet', 'query']);
...
const RESULT_KEYS = Object.freeze([
  'schema_version', 'status', 'verdict_authority', 'rule_authority', 'source_packet_sha256',
  'query_id', 'candidate_locators', 'receipt',
]);
const RESULT_RECEIPT_KEYS = Object.freeze(['input_sha256', 'output_sha256', 'effects']);
const RESULT_LOCATOR_KEYS = Object.freeze([
  'source_id', 'direct_record_sha256', 'exact_locator', 'claim_ceiling',
  'confirmation_required_before_rule_or_verdict',
]);
    ...
    if (array) {
      ...
      for (const [key, descriptor] of Object.entries(descriptors)) {
        Object.defineProperty(output, key, {
          value: copyPlain(descriptor.value, `${label}.${key}`, code, depth + 1, ancestors),
          enumerable: true,
          writable: true,
          configurable: true,
        });
...
function validatePacket(packet) {
  ...
  for (const row of copied.retrieval_records) {
    ...
    try {
      assertQualityReadinessPublicLocator(row.locator, 'RAG retrieval locator');
    } catch {
      fail(QUALITY_READINESS_RAG_CODES.PACKET_UNAUTHORIZED, 'RAG retrieval locator crosses the public-safe boundary');
    }
...
export function retrieveQualityReadinessAdvisoryEvidence(input) {
  const admitted = copyPlain(input, 'RAG retrieval input', QUALITY_READINESS_RAG_CODES.PACKET_REFUSED);
  assertExactKeys(admitted, RETRIEVAL_INPUT_KEYS, 'RAG retrieval input', QUALITY_READINESS_RAG_CODES.PACKET_REFUSED);
  const validated = validatePacket(admitted.packet);
  const copiedQuery = copyPlain(admitted.query, 'RAG query', QUALITY_READINESS_RAG_CODES.QUERY_INVALID);
...
/**
 * Recomputes the emitted advisory result before MCP can expose it.  This remains a navigation
 * receipt only: no query/result pair can become verdict or rule authority at this boundary.
 */
export function verifyQualityReadinessRagResult(result) {
  const copied = copyPlain(result, 'RAG result', QUALITY_READINESS_RAG_CODES.PACKET_REFUSED);
  assertExactKeys(copied, RESULT_KEYS, 'RAG result', QUALITY_READINESS_RAG_CODES.PACKET_REFUSED);
  assertExactKeys(copied.receipt, RESULT_RECEIPT_KEYS, 'RAG result receipt', QUALITY_READINESS_RAG_CODES.PACKET_REFUSED);
  if (copied.schema_version !== QUALITY_READINESS_RAG_RESULT_SCHEMA
      || !['advisory_locator_candidates', 'hold_no_admissible_locator'].includes(copied.status)
      || copied.verdict_authority !== false || copied.rule_authority !== false
      || !SHA256_HEX.test(copied.source_packet_sha256) || !TOKEN.test(copied.query_id)
      || !Array.isArray(copied.candidate_locators)
      || !SHA256_HEX.test(copied.receipt.input_sha256) || !SHA256_HEX.test(copied.receipt.output_sha256)
      || !zeroEffects(copied.receipt.effects)) {
    fail(QUALITY_READINESS_RAG_CODES.PACKET_FORGED, 'RAG result is malformed or crosses the advisory authority boundary');
  }
  if ((copied.status === 'hold_no_admissible_locator') !== (copied.candidate_locators.length === 0)) {
    fail(QUALITY_READINESS_RAG_CODES.PACKET_FORGED, 'RAG result status does not match its retained locator rows');
  }
  let prior = null;
  for (const locator of copied.candidate_locators) {
    assertExactKeys(locator, RESULT_LOCATOR_KEYS, 'RAG candidate locator', QUALITY_READINESS_RAG_CODES.PACKET_FORGED);
    if (!TOKEN.test(locator.source_id) || !SHA256_HEX.test(locator.direct_record_sha256)
        || !['observed', 'source_supported'].includes(locator.claim_ceiling)
        || locator.confirmation_required_before_rule_or_verdict !== true) {
      fail(QUALITY_READINESS_RAG_CODES.PACKET_FORGED, 'RAG candidate locator is malformed or claims authority');
    }
    try {
      assertQualityReadinessPublicLocator(locator.exact_locator, 'RAG candidate exact_locator');
    } catch {
      fail(QUALITY_READINESS_RAG_CODES.PACKET_FORGED, 'RAG candidate locator crosses the public-safe boundary');
    }
    const key = `${locator.source_id}\u001f${locator.exact_locator}`;
    if (prior !== null && compareCodePoints(prior, key) >= 0) {
      fail(QUALITY_READINESS_RAG_CODES.PACKET_FORGED, 'RAG candidate locators must be sorted uniquely');
    }
    prior = key;
  }
  const expectedOutput = digest('soulforge.quality_readiness.rag_result.v0', {
    schema_version: copied.schema_version,
    status: copied.status,
    verdict_authority: copied.verdict_authority,
    rule_authority: copied.rule_authority,
    source_packet_sha256: copied.source_packet_sha256,
    query_id: copied.query_id,
    candidate_locators: copied.candidate_locators,
    receipt: {
      input_sha256: copied.receipt.input_sha256,
      effects: copied.receipt.effects,
    },
  });
  if (copied.receipt.output_sha256 !== expectedOutput) {
    fail(QUALITY_READINESS_RAG_CODES.PACKET_FORGED, 'RAG result receipt digest is forged or stale');
  }
