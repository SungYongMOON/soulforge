...
const CORPUS_INPUT_KEYS = Object.freeze(['inventory', 'matrix']);
...
const PRIVATE_LOCATOR_SEGMENT = /(?:^|[\\/])(?:workspace|_workspaces|_workmeta|private-state|private)(?:[\\/]|$)/iu;
const POSIX_ABSOLUTE_PATH = /(?:^|[\s'"`=(])\/(?!\/)(?:home|users|tmp|temp|var|etc|opt|srv|usr|bin|sbin|lib|dev|proc|sys|root|mnt|media|private|workspace|workspaces)(?:[\\/]|$)/iu;
const SECRET_ASSIGNMENT = /(?:^|[^A-Za-z0-9_])(?:api[_-]?key|access[_-]?token|refresh[_-]?token|auth(?:orization)?|bearer|credential|client[_-]?secret|secret|password|passwd|token)\s*[:=]\s*\S+/iu;
const SYNTHETIC_CREDENTIAL_MARKER = /(?:\$\{(?:api[_-]?key|access[_-]?token|refresh[_-]?token|secret|password|credential)[^}]*\}|\{\{\s*(?:api[_-]?key|access[_-]?token|refresh[_-]?token|secret|password|credential)[^}]*\}\}|<(?:api[_-]?key|access[_-]?token|refresh[_-]?token|secret|password|credential)>)/iu;
const SCHEME_LIKE = /^[A-Za-z][A-Za-z0-9+.-]*:/u;
const CANONICAL_INVENTORY_SHA256 = '1addacac80b56d34a666f4dee31662ba6537b35b0c6de0caf65966c31986c2c0';
const CANONICAL_MATRIX_SHA256 = '4a22a7dc7cb5a1c0d46fc0b09691077ab0cdf4287f389df7d6c143f3e1181ec5';
function safeCatalogSnapshot(value, label) {
  try {
    return freezeDeep(copyPlain(value, label));
  } catch {
    return null;
  }
}
const PINNED_CANONICAL_INVENTORY = safeCatalogSnapshot(canonicalInventory, 'package-owned canonical inventory');
const PINNED_CANONICAL_MATRIX = safeCatalogSnapshot(canonicalMatrix, 'package-owned canonical matrix');
const CANONICAL_CATALOG_INTACT = PINNED_CANONICAL_INVENTORY !== null
  && PINNED_CANONICAL_MATRIX !== null
  && digest(
  'soulforge.quality_readiness.source_inventory.v1',
  PINNED_CANONICAL_INVENTORY,
) === CANONICAL_INVENTORY_SHA256 && digest(
  'soulforge.quality_readiness.source_family_matrix.v1',
  PINNED_CANONICAL_MATRIX,
) === CANONICAL_MATRIX_SHA256;
const CANONICAL_INVENTORY_BY_ID = new Map((PINNED_CANONICAL_INVENTORY?.sources ?? []).map((row) => [
  row.source_id,
  row,
]));
const CANONICAL_MATRIX_BY_ID = new Map((PINNED_CANONICAL_MATRIX?.sources ?? []).map((row) => [
  row.source_id,
  row,
]));
...
function assertMatrixRowTypes(row) {
  for (const field of [
    'source_id', 'family_id', 'disposition', 'disposition_reason', 'proof_subset_overlap',
    'body_access_state', 'extraction_review_state', 'common_engine_relevance',
    'research_batch', 'unresolved_or_hold',
  ]) {
    if (typeof row[field] !== 'string') {
      fail(QUALITY_READINESS_SOURCE_CODES.INVENTORY_INVALID,
        `source family matrix ${field} must be bounded public text`);
    }
  }
  if (typeof row.direct_body_confirmation_required !== 'boolean'
      || !Array.isArray(row.candidate_quality_domains)
      || row.candidate_quality_domains.some((value) => typeof value !== 'string')) {
    fail(QUALITY_READINESS_SOURCE_CODES.INVENTORY_INVALID,
      'source family matrix classification fields must retain their exact public value types');
  }
}

function locatorLabel(label) {
  return typeof label === 'string' && label.length > 0 ? label : 'locator';
}

function isBlockedIpv4(hostname) {
  if (!/^(?:0|[1-9]\d{0,2})(?:\.(?:0|[1-9]\d{0,2})){3}$/u.test(hostname)) return false;
...
function ipv6Groups(hostname) {
  ...
  const parseHalf = (half) => half === '' ? [] : half.split(':').map((part) => (
    /^[0-9a-f]{1,4}$/u.test(part) ? Number.parseInt(part, 16) : Number.NaN
  ));
  const leading = parseHalf(halves[0]);
  const trailing = halves.length === 2 ? parseHalf(halves[1]) : [];
  if (leading.some(Number.isNaN) || trailing.some(Number.isNaN)
      || leading.length + trailing.length > 8
      || (halves.length === 1 && leading.length !== 8)) return null;
...
function isBlockedOfficialHostname(hostname) {
  const host = hostname.replace(/\.$/u, '').toLowerCase();
  if (host.length === 0 || isBlockedIpv4(host) || isBlockedIpv6(host)) return true;
  if (host === 'localhost' || host === 'localhost.localdomain' || host === 'local'
      || host === 'internal' || host === 'intranet' || host === 'lan' || host === 'home'
      || host.endsWith('.localhost') || host.endsWith('.local') || host.endsWith('.internal')
      || host.endsWith('.localdomain') || host.endsWith('.intranet')
      || host.endsWith('.lan') || host.endsWith('.home')) return true;
  return !host.includes('.') && !host.includes(':');
}

function hasPrivateUrlPath(pathname) {
  ...
  return PRIVATE_LOCATOR_SEGMENT.test(decoded)
    || FORBIDDEN_TEXT.some((pattern) => pattern.test(decoded))
    || SECRET_ASSIGNMENT.test(decoded)
    || SYNTHETIC_CREDENTIAL_MARKER.test(decoded);
}

function assertPackageOwnedInventoryUrl(value, label) {
  let parsed;
  try {
    parsed = new URL(value);
  } catch {
    fail(QUALITY_READINESS_SOURCE_CODES.PUBLIC_BOUNDARY, `${label} must be an absolute HTTPS public URL`);
  }
  if (parsed.protocol !== 'https:' || parsed.username || parsed.password || parsed.hash
      || isBlockedOfficialHostname(parsed.hostname) || hasPrivateUrlPath(parsed.pathname)) {
    fail(QUALITY_READINESS_SOURCE_CODES.PUBLIC_BOUNDARY, `${label} must be a credential-free public HTTPS URL`);
  }
}

/** Rejects local/private/file locators both at admission and before any public output is emitted. */
export function assertQualityReadinessPublicLocator(value, label, options = {}) {
  const field = locatorLabel(label);
  const officialUrl = options !== null && typeof options === 'object' && options.officialUrl === true;
  if (typeof value !== 'string' || value.length === 0 || value.length > 512
      || value.trim() !== value || value.trim().length === 0
      || /[\u0000-\u001f\u007f]/u.test(value) || FORBIDDEN_TEXT.some((pattern) => pattern.test(value))
      || PRIVATE_LOCATOR_SEGMENT.test(value) || POSIX_ABSOLUTE_PATH.test(value)
      || SECRET_ASSIGNMENT.test(value) || SYNTHETIC_CREDENTIAL_MARKER.test(value)
      || /(?:^|[^A-Za-z0-9])[A-Za-z]:[\\/]/u.test(value)
      || /(?:^|[^\\])\\\\[^\\/\s]+[\\/]/u.test(value)
      || /^(?:[A-Za-z]:[\\/]|[\\/]|file:)/iu.test(value)) {
    fail(QUALITY_READINESS_SOURCE_CODES.PUBLIC_BOUNDARY, `${field} is not a public-safe locator`);
  }
  if (officialUrl || SCHEME_LIKE.test(value)) {
    let parsed;
    try {
      parsed = new URL(value);
    } catch {
      fail(QUALITY_READINESS_SOURCE_CODES.PUBLIC_BOUNDARY, `${field} must be an absolute HTTPS public URL`);
    }
    if (parsed.protocol !== 'https:' || parsed.username || parsed.password || parsed.search || parsed.hash
        || isBlockedOfficialHostname(parsed.hostname) || hasPrivateUrlPath(parsed.pathname)) {
      fail(QUALITY_READINESS_SOURCE_CODES.PUBLIC_BOUNDARY, `${field} must be a credential-free public HTTPS URL`);
    }
  }
  return value;
}
...
function copyPlain(value, label, depth = 0, ancestors = new Set()) {
  ...
  if (value === null || ['boolean', 'number', 'string'].includes(typeof value)) {
    if (typeof value === 'string' && (
      ...
      || SYNTHETIC_CREDENTIAL_MARKER.test(value)
    )) {
  ...
  try {
    ...
    for (const key of Reflect.ownKeys(descriptors)) {
      ...
      if (typeof key !== 'string' || PROTOTYPE_SENSITIVE_KEYS.has(key) || !Object.hasOwn(descriptor, 'value')
    ...
    if (array) {
      ...
      for (const [key, descriptor] of Object.entries(descriptors)) {
        Object.defineProperty(output, key, {
          value: copyPlain(descriptor.value, `${label}.${key}`, depth + 1, ancestors),
          enumerable: true,
          writable: true,
          configurable: true,
        });
...
function directState(row, inventorySource) {
  const holdTokens = new Set((row.unresolved_or_hold.toLowerCase().match(/[a-z0-9]+/gu) ?? []));
...
function hasCanonicalProofIdentity(inventorySource, matrixRow) {
  const sourceId = inventorySource?.source_id;
  return PROOF_SUBSET[sourceId] !== undefined
    && isDeepStrictEqual(inventorySource, CANONICAL_INVENTORY_BY_ID.get(sourceId))
    && isDeepStrictEqual(matrixRow, CANONICAL_MATRIX_BY_ID.get(sourceId));
}

function sourceRecord(inventorySource, matrixRow) {
  const proofSourceRef = hasCanonicalProofIdentity(inventorySource, matrixRow)
    ? PROOF_SUBSET[inventorySource.source_id]
    : null;
  const proofImpostor = PROOF_SUBSET[inventorySource.source_id] !== undefined && proofSourceRef === null;
  const state = proofSourceRef === null
    ? (proofImpostor ? 'hold_canonical_identity_mismatch' : directState(matrixRow, inventorySource))
    : 'proof_subset_packet_bound';
  const record = {
  ...
  if (proofSourceRef !== null) {
    ...
    record.hold_reason = proofImpostor
      ? 'proof-subset identity differs from the package-owned canonical public inventory/matrix record'
      : matrixRow.unresolved_or_hold || matrixRow.disposition_reason;
  }
 ...
 */
export function buildQualityReadinessSourceDirectCorpus(input) {
  const admitted = copyPlain(input, 'source corpus input');
  exactKeys(admitted, CORPUS_INPUT_KEYS, 'source corpus input', QUALITY_READINESS_SOURCE_CODES.INVENTORY_INVALID);
  if (!CANONICAL_CATALOG_INTACT) {
    fail(QUALITY_READINESS_SOURCE_CODES.INVENTORY_INVALID,
      'package-owned source inventory or matrix bytes do not match their immutable digest pins');
  }
  const copiedInventory = admitted.inventory;
  const copiedMatrix = admitted.matrix;
  ...
  for (const source of copiedInventory.sources) {
    ...
    try {
      if (isDeepStrictEqual(source, CANONICAL_INVENTORY_BY_ID.get(source.source_id))) {
        // The checked-in catalog predates B5 and has exact, package-owned public HTTPS query
        // locators. They remain replayable metadata only; callers cannot use this narrow
        // identity exception to admit a new queried direct-source or RAG locator.
        assertPackageOwnedInventoryUrl(source.official_url, 'source inventory official_url');
      } else {
        assertQualityReadinessPublicLocator(source.official_url, 'source inventory official_url', { officialUrl: true });
      }
    } catch {
      fail(QUALITY_READINESS_SOURCE_CODES.INVENTORY_INVALID, 'source inventory rows must have unique public-safe official HTTPS metadata');
    }
...
export function admitQualityReadinessDirectSource(input) {
  ...
  if (typeof copied.source_id !== 'string' || !TOKEN.test(copied.source_id)
      ...
      || typeof copied.official_url !== 'string'
  ...
  assertQualityReadinessPublicLocator(copied.official_url, 'direct source official_url', { officialUrl: true });
  assertQualityReadinessPublicLocator(copied.exact_locator, 'direct source exact_locator');
...
export function verifyQualityReadinessDirectSourceRecord(record) {
  ...
  if (!isDeepStrictEqual(expected, copied)) {
