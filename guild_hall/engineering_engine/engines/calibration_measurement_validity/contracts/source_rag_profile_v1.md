...
Every public E11 ingress uses a descriptor-only snapshot before accessing caller-owned fields. Base evaluation deliberately separates its exact raw-request and complete Typed Facts lanes; wrapper/hybrid shapes cannot switch lanes. Effective-ruleset admission checks the complete Core assembly, trace, nested requirements, immutable refs, provenance rows, derived identity, and Core canonical/null digest together. Observation, guidance, and read-only MCP repeat the same no-getter/no-Proxy admission discipline for their own inputs.
