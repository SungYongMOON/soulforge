...
export function adaptCalibrationMeasurementValidityTypedFacts(input) {
  ...
  try {
    value = snapshotCalibrationMeasurementValidityPlainData(input, {
      code: CMV_TYPED_FACT_CODES.INVALID_INPUT,
      label: 'typed facts input',
      rejectAliases: true,
    });
  } catch {
    refuse(CMV_TYPED_FACT_CODES.INVALID_INPUT, 'typed facts input must be a safely admitted non-aliased CMV data graph');
  }
  assertPlainObject(value, 'typed facts input');
  ...
  for (const factKey of FACT_KEYS) {
    ...
    if (source.binding_kind === 'official_public_direct') {
      refuse(CMV_TYPED_FACT_CODES.SOURCE_HOLD, 'official rule sources support rule semantics only and cannot certify live project facts');
    }
...
export function validateAdaptedCalibrationMeasurementValidityTypedFacts(envelope) {
  let value;
  try {
    value = snapshotCalibrationMeasurementValidityPlainData(envelope, {
      code: CMV_TYPED_FACT_CODES.ADAPTED_ENVELOPE_INVALID,
      label: 'adapted typed facts envelope',
      rejectAliases: true,
    });
  } catch {
    refuse(CMV_TYPED_FACT_CODES.ADAPTED_ENVELOPE_INVALID, 'adapted typed facts envelope must be a safely admitted non-aliased CMV data graph');
  }
  assertPlainObject(value, 'adapted typed facts envelope');
