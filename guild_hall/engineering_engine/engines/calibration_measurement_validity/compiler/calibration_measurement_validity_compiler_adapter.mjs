...
export function deriveCalibrationMeasurementValidityRulesetReference(requirements, profileRuleProvenance = {}) {
  if (requirements.length === 0) return copyReference(CMV_RULESET_REF);
  const digest = createHash('sha256').update(
    `soulforge.calibration_measurement_validity.derived_ruleset.v1\n${canonicalizeCalibrationMeasurementValidity({ base_ruleset_ref: CMV_RULESET_REF, profile_rule_provenance: profileRuleProvenance, requirements })}`,
  ).digest('hex');
  return {
...
export function compileCalibrationMeasurementValidityRules(profileBindings = [], compilationScope = {}) {
  ...
  if (profileBindings.length > 0 && profileCompilation.requirements.length === 0) {
    throw new ContractError(
      CMV_COMPILER_ERROR_CODES.PROFILE_UNSUPPORTED,
      'a non-empty CMV Profile must declare at least one source_bound_requirements operation',
    );
  }
  const effectiveRuleSet = {
    schema_version: CMV_RULESET_SCHEMA_VERSION,
    domain_engine_id: 'calibration_measurement_validity',
    ruleset_ref: deriveCalibrationMeasurementValidityRulesetReference(profileCompilation.requirements, profileCompilation.provenance),
