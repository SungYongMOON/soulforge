...
export function classifyCmvSourceEvidence(input) {
  ...
  try {
    value = snapshotCalibrationMeasurementValidityPlainData(input, {
      code: CMV_SOURCE_CLASSIFICATION_CODES.INVALID_INPUT,
      label: 'source classification input',
      rejectAliases: true,
    });
  } catch {
    refuse(CMV_SOURCE_CLASSIFICATION_CODES.INVALID_INPUT, 'source classification input must be a safely admitted exact source binding');
  }
  assertPlainObject(value, 'source classification input');
  const keys = Object.keys(value).sort();
...
export function validateConsumedCmvSourceClassification(envelope, { requireDirect = false } = {}) {
  ...
  try {
    value = snapshotCalibrationMeasurementValidityPlainData(envelope, {
      code: CMV_SOURCE_CLASSIFICATION_CODES.CONSUMED_ENVELOPE_INVALID,
      label: 'consumed source classification',
      rejectAliases: true,
    });
  } catch {
    refuse(CMV_SOURCE_CLASSIFICATION_CODES.CONSUMED_ENVELOPE_INVALID, 'consumed source classification must be a safely admitted exact envelope');
  }
  assertPlainObject(value, 'consumed source classification', CMV_SOURCE_CLASSIFICATION_CODES.CONSUMED_ENVELOPE_INVALID);
  const keys = Object.keys(value).sort();
