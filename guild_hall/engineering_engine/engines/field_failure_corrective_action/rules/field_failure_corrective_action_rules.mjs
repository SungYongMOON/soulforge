...
export const FFCA_SOURCE_INVENTORY = Object.freeze([
  Object.freeze({
    ...
    revision_basis: "Title 10 view up to date 2026-08-21; accessed 2026-08-26",
...
function sourceInventoryDigest() {
  return sha256Hex(`soulforge.field_failure_corrective_action.source_packet.v0\n${canonicalise({
    schema_version: FFCA_SOURCE_PACKET_SCHEMA,
    sources: FFCA_SOURCE_INVENTORY,
  }, { sources: "insertion_ordered" })}`);
}
...
export const FFCA_SOURCE_PACKET_REF = Object.freeze({
  ...
  content_id: `sha256:${sourceInventoryDigest()}`,
...
export const FFCA_RULE_BY_ID = new Map(FFCA_RULES.map((rule) => [rule.rule_id, rule]));

export function isFfcaRuleId(value) {
  return typeof value === "string" && FFCA_RULE_BY_ID.has(value);
}
