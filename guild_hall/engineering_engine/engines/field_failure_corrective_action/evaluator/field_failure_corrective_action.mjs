...
function assertCoreProjectBindingRef(projectBindingRef) {
  assertExactKeys(projectBindingRef, CORE_PROJECT_BINDING_FIELDS, [], "binding.project_binding_ref",
    FFCA_EVALUATOR_ERROR_CODES.BINDING_REFUSED);
  if (projectBindingRef.schema_version !== "soulforge.project_binding.v0"
      || projectBindingRef.domain_engine_id !== "field_failure_corrective_action"
      || typeof projectBindingRef.binding_revision_hash !== "string"
      || !/^[0-9a-f]{64}$/u.test(projectBindingRef.binding_revision_hash)
      || typeof projectBindingRef.source_manifest_ref !== "string"
      || !/^sha256:[0-9a-f]{64}$/u.test(projectBindingRef.source_manifest_ref)) {
    fail(FFCA_EVALUATOR_ERROR_CODES.BINDING_REFUSED, "binding.project_binding_ref must be an exact Core project binding");
  }
  return Object.freeze({
    binding_revision_hash: projectBindingRef.binding_revision_hash,
    domain_engine_id: projectBindingRef.domain_engine_id,
    project_id: assertRef(projectBindingRef.project_id, "binding.project_binding_ref.project_id"),
    schema_version: projectBindingRef.schema_version,
    source_manifest_ref: projectBindingRef.source_manifest_ref,
  });
}
...
function validateBinding(binding) {
  assertExactKeys(binding, ["project_binding_ref", "ruleset_ref", "source_bindings", "source_packet_ref"], [], "binding",
    FFCA_EVALUATOR_ERROR_CODES.BINDING_REFUSED);
  return Object.freeze({
    project_binding_ref: assertCoreProjectBindingRef(binding.project_binding_ref),
...
function validateRow(raw) {
  const rule = getFfcaRule(raw.rule_id);
...
function projectRow(row) {
  const rule = getFfcaRule(row.rule_id);
...
function validateRequest(request) {
  ...
  if (cutoffs.valid_at > cutoffs.known_at) {
    fail(FFCA_EVALUATOR_ERROR_CODES.INPUT_REFUSED, "cutoffs.valid_at must not be after cutoffs.known_at");
  }
  return Object.freeze({
