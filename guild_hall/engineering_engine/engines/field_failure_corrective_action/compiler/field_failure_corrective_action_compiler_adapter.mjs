...
function requiredString(value, label, { rejectUnversioned = false } = {}) {
...
function copyProfile(rawProfile, expectedOrder, requiredKind) {
  ...
  const sourceRefs = copyDenseArray(profile.source_refs, "profile.source_refs", requiredString);
  ...
  return Object.freeze({
    ...
    revision_or_hash: requiredString(profile.revision_or_hash, "profile.revision_or_hash", { rejectUnversioned: true }),
    extends_or_base_pin: requiredString(profile.extends_or_base_pin, "profile.extends_or_base_pin"),
...
function compileProfileProvenance(profileBindings) {
  ...
  if (profileBindings.length === 2) {
    ...
    provenance.push(copyProfile(descriptors["1"].value, 1, "project"));
  }
  return Object.freeze(provenance);
}

export function compileFieldFailureCorrectiveActionRules(profileBindings = [], _options = {}) {
