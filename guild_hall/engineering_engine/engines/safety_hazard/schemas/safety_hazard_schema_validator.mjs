...
const SUPPORTED_KEYWORDS = new Set([
  ...
  'const', 'enum', 'minLength', 'maxLength', 'pattern', 'format', 'items', 'minItems', 'maxItems',
]);
...
export function validateJsonSchemaSubset(value, schema, path = '$', errors = []) {
  ...
  if (typeof value === 'string') {
    ...
    if (schema.format === 'date-time' && (!/^\d{4}-\d{2}-\d{2}T/u.test(value) || !value.endsWith('Z') || !Number.isFinite(Date.parse(value)))) {
