...
// The adapter fixture uses the actual Core TypedProjectFacts producer. The one fact is the
// closed four-field R&M request; its Core facts digest and bitemporal fields are produced by
// adaptProjectEvidence rather than copied into a custom `{ request }` wrapper.
export function buildReliabilityMaintainabilityPublicSyntheticTypedFacts({
  mutateRequest = null,
  mutateCoreProjectBinding = null,
} = {}) {
  ...
  request.cutoffs = { ...request.cutoffs, valid_at, known_at };
  ...
  if (typeof mutateRequest === 'function') mutateRequest(request, coreProjectBinding);
  if (typeof mutateCoreProjectBinding === 'function') mutateCoreProjectBinding(coreProjectBinding, request);
