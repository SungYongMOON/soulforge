...
import {
  ...
  snapshotReliabilityMaintainabilityAdmission,
  validateAndCloneProjectFactsProvenance,
...
const CUTOFF_REQUIRED_FIELDS = Object.freeze(['accepted_context_generation', 'assessment_cutoff_ref']);
const CUTOFF_OPTIONAL_FIELDS = Object.freeze(['valid_at', 'known_at']);
...
function assertSafeString(value, field, code = RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED) {
  return assertRmPublicSafeString(value, { code, field, maxLength: MAX.string });
}
...
function snapshotPlainData(root) {
  ...
  const walk = (current, depth, field) => {
    ...
    if (valueCount > MAX.values || depth > MAX.depth) {
      fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'input exceeds plain-data limits', { field });
    }
    if (typeof current === 'string') return assertSafeString(current, field);
    if (typeof current === 'boolean') return current;
    if (typeof current === 'number') {
      if (!Number.isSafeInteger(current)) {
        fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'floating-point and unsafe integers are refused', { field });
      }
      return current;
    }
    ...
    if (types.isProxy(current)) {
      fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'Proxy input is refused before reflective access', { field });
    }
    if (seen.has(current)) {
      fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'cyclic and aliased graphs are refused', { field });
    }
    seen.add(current);
    ...
    try {
      ...
      fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'input reflection failed before a safe snapshot could be built', { field });
    }
    if ((array && prototype !== Array.prototype) || (!array && prototype !== Object.prototype)) {
      fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'custom prototypes and host objects are refused', { field });
    }

    const keys = Reflect.ownKeys(descriptors);
    if (keys.some((key) => typeof key !== 'string')) {
      fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'symbol properties are refused', { field });
    }
    const dataKeys = array ? keys.filter((key) => key !== 'length') : keys;
    if (array) {
      const length = descriptors.length?.value;
      if (!Number.isSafeInteger(length) || length < 0 || length > MAX.array) {
        fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'arrays must be bounded dense data arrays', { field });
      }
      ...
      if (dataKeys.length !== expected.size || dataKeys.some((key) => !expected.has(key))) {
        fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'arrays must be dense and may not carry named entries', { field });
      }
      ...
      for (let index = 0; index < length; index += 1) {
        ...
        if (!descriptor || descriptor.enumerable !== true || !Object.hasOwn(descriptor, 'value')) {
          fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'arrays may not carry accessors or hidden entries', { field });
        }
    ...
    if (dataKeys.length > MAX.keys) {
      fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'object has too many fields', { field });
    }
    const copy = {};
    for (const key of dataKeys) {
      if (key.length > 80 || key.normalize('NFC') !== key || PROTOTYPE_SENSITIVE_KEYS.has(key)
          || FORBIDDEN_KEYS.has(key.toLowerCase())) {
        fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'unsafe, payload-bearing, or prototype-sensitive field is refused', { field });
      }
      const descriptor = descriptors[key];
      if (!descriptor || descriptor.enumerable !== true || !Object.hasOwn(descriptor, 'value')) {
        fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'objects may not carry accessors or hidden entries', { field });
      }
...
function assertToken(value, field, code = RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED) {
  const safe = assertRmOpaqueToken(value, { code, field, maxLength: MAX.string });
  if (!TOKEN.test(safe)) fail(code, `${field} must be a bounded opaque token`);
  return safe;
}
...
function assertExactRef(ref, field, code = RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED) {
  ...
  for (const key of ['entity_id', 'revision_id', 'content_id', 'content_hash_alg']) {
    assertSafeString(ref[key], `${field}.${key}`, code);
  }
...
const SOURCE_NATIVE_PROJECTION = Object.freeze({ source_native: true });

function projectCanonicalEvidenceKind(value) {
  if (value === null || typeof value !== 'object') {
    return value;
  }
  if (Array.isArray(value)) {
    return value.map(projectCanonicalEvidenceKind);
  }
  const projected = {};
  for (const [key, child] of Object.entries(value)) {
    if (key === 'evidence_kind' && child === null) {
      projected[key] = SOURCE_NATIVE_PROJECTION;
    } else {
      projected[key] = projectCanonicalEvidenceKind(child);
    }
  }
  return projected;
}
...
function canonicalJson(value) {
  const projected = projectCanonicalEvidenceKind(value);
  const arrayRules = {};
  ...
  inspect(projected);
  return canonicalise(projected, arrayRules);
}
...
function validateManifestBinding(manifest, binding, cutoffs) {
  try {
    ...
    assertExactKeys(cutoffs, CUTOFF_REQUIRED_FIELDS, CUTOFF_OPTIONAL_FIELDS, 'cutoffs', RM_EVALUATOR_ERROR_CODES.BINDING_REFUSED);
    ...
    if (cutoffs.valid_at !== undefined) {
      if (typeof cutoffs.valid_at !== 'string' || !inspectInstant(cutoffs.valid_at).valid) {
        fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'cutoffs.valid_at must be a valid UTC instant');
      }
    }
    if (cutoffs.known_at !== undefined) {
      if (typeof cutoffs.known_at !== 'string' || !inspectInstant(cutoffs.known_at).valid) {
        fail(RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED, 'cutoffs.known_at must be a valid UTC instant');
      }
    }
    return {
      accepted_rule_bindings,
    ...
    fail(RM_EVALUATOR_ERROR_CODES.BINDING_REFUSED,
      ...
      { cause_code: 'UNEXPECTED_BINDING_VALIDATION_ERROR' });
  }
}
 ...
 * @param {object} admittedEffectiveRuleSet
 * @param {object|null} projectFactsProvenance
 * @returns {{assessment: object, domain_result: object, receipt: object}}
 */
function assessReliabilityMaintainabilityInternal(request, admittedEffectiveRuleSet, projectFactsProvenance = null) {
  ...
  const boundProvenance = validateAndCloneProjectFactsProvenance(projectFactsProvenance);
  const domain_result = {
    schema_version: RM_DOMAIN_RESULT_SCHEMA,
    canon_claim_ceiling,
    evidence_claim_ceiling,
    effective_ruleset_digest: admittedEffectiveRuleSet.effective_ruleset_digest,
    results,
    counts: { ...counts },
  };
  if (boundProvenance) {
    domain_result.project_facts_provenance = boundProvenance;
  }
  const assessment = {
  ...
  if (boundProvenance) {
    normalizedInput.project_facts_provenance = boundProvenance;
  }
  if (admittedEffectiveRuleSet.compilation_identity_digest) {
    normalizedInput.compilation_identity_digest = admittedEffectiveRuleSet.compilation_identity_digest;
  }
  const assessmentDigest = canonicalDigest('soulforge.reliability_maintainability.assessment.v0', assessment);
  const domainResultDigest = canonicalDigest('soulforge.reliability_maintainability.domain_result.v0', domain_result);
  const resultDigest = canonicalDigest('soulforge.reliability_maintainability.result.v0', {
    assessment,
    domain_result,
  });
  ...
  const receipt = {
    ...
    digests: {
      ...
      effective_ruleset_sha256: admittedEffectiveRuleSet.effective_ruleset_digest.slice('sha256:'.length),
      assessment_sha256: assessmentDigest,
      domain_result_sha256: domainResultDigest,
    ...
    bindings: {
      ...
      effective_ruleset_ref: cloneRef(admittedEffectiveRuleSet.ruleset_ref),
  ...
  if (boundProvenance) {
    receipt.project_facts_provenance = boundProvenance;
  }
  return freezeDeep({ assessment, domain_result, receipt });
}

const BASE_ADMITTED_EFFECTIVE_RULESET = Object.freeze({
  ruleset_ref: RELIABILITY_MAINTAINABILITY_RULESET_REF,
  effective_ruleset_digest: RELIABILITY_MAINTAINABILITY_RULESET_REF.content_id,
  compilation_identity_digest: null,
});

export function assessReliabilityMaintainability(request) {
  return assessReliabilityMaintainabilityInternal(request, BASE_ADMITTED_EFFECTIVE_RULESET, null);
}

// Adapter-only entrypoint. The caller is the closed admission module, which supplies an
// identity already snapshotted and tied to the exact effective ruleset it admitted.
export function assessAdmittedReliabilityMaintainability(request, admittedEffectiveRuleSet, projectFactsProvenance = null) {
  return assessReliabilityMaintainabilityInternal(request, admittedEffectiveRuleSet, projectFactsProvenance);
}

export function verifyReliabilityMaintainabilityResult(rawResult, suppliedEffectiveRuleSet = null, rawTrustedInput = null) {
  const code = RM_EVALUATOR_ERROR_CODES.INPUT_REFUSED;
  if (!rawResult || typeof rawResult !== 'object' || Array.isArray(rawResult)) {
    fail(code, 'verification requires non-null object result');
  }
  if (!rawTrustedInput || typeof rawTrustedInput !== 'object' || Array.isArray(rawTrustedInput)) {
    fail(code, 'verification requires non-null object trusted input');
  }

  // Safe snapshot rawResult before any property read; reject proxy/accessors/symbols/custom prototype
  const snapshottedResult = snapshotReliabilityMaintainabilityAdmission(rawResult, {
    code,
    rootLabel: 'result',
    allowAliases: true,
  });
  assertExactKeys(snapshottedResult, ['assessment', 'domain_result', 'receipt'], [], 'result', code);
  if (!snapshottedResult.receipt || typeof snapshottedResult.receipt !== 'object'
      || !snapshottedResult.receipt.effects || typeof snapshottedResult.receipt.effects !== 'object'
      || !snapshottedResult.receipt.digests || typeof snapshottedResult.receipt.digests !== 'object'
      || !snapshottedResult.receipt.bindings || typeof snapshottedResult.receipt.bindings !== 'object'
      || !snapshottedResult.assessment || typeof snapshottedResult.assessment !== 'object'
      || !snapshottedResult.domain_result || typeof snapshottedResult.domain_result !== 'object') {
    fail(code, 'result structure is incomplete or malformed');
  }
  for (const [effect, count] of Object.entries(snapshottedResult.receipt.effects)) {
    if (count !== 0) {
      fail(code, `non-zero effect ${effect} is refused in verification`);
    }
  }

  // Exact-admit supplied effective ruleset; reject extra keys
  let admittedRuleset;
  if (suppliedEffectiveRuleSet !== null && suppliedEffectiveRuleSet !== undefined) {
    admittedRuleset = admitReliabilityMaintainabilityEffectiveRuleSet(suppliedEffectiveRuleSet);
  } else {
    admittedRuleset = BASE_ADMITTED_EFFECTIVE_RULESET;
  }

  // Safe snapshot rawTrustedInput before any property read
  const snapshottedInput = snapshotReliabilityMaintainabilityAdmission(rawTrustedInput, {
    code,
    rootLabel: 'trustedInput',
    allowAliases: true,
  });

  let replayed;
  if (snapshottedInput && typeof snapshottedInput === 'object' && snapshottedInput.schema_version === 'soulforge.typed_project_facts.v0') {
    const admittedFacts = admitReliabilityMaintainabilityTypedFacts(rawTrustedInput);
    const { project_facts_provenance, ...directRequest } = admittedFacts;
    replayed = assessAdmittedReliabilityMaintainability(directRequest, admittedRuleset, project_facts_provenance);
  } else {
    assertExactKeys(snapshottedInput, ROOT_FIELDS, [], 'request', code);
    replayed = assessAdmittedReliabilityMaintainability(snapshottedInput, admittedRuleset, null);
  }

  // Deterministically rebuild and byte-canonically compare complete assessment, domain_result, receipt
  if (!isDeepStrictEqual(snapshottedResult.domain_result, replayed.domain_result)
      || !isDeepStrictEqual(snapshottedResult.assessment, replayed.assessment)
      || !isDeepStrictEqual(snapshottedResult.receipt, replayed.receipt)
      || canonicalJson(snapshottedResult.assessment) !== canonicalJson(replayed.assessment)
      || canonicalJson(snapshottedResult.domain_result) !== canonicalJson(replayed.domain_result)
      || canonicalJson(snapshottedResult.receipt) !== canonicalJson(replayed.receipt)) {
    fail(code, 'result does not match deterministic evaluation of trusted input');
  }

  if (suppliedEffectiveRuleSet !== null && suppliedEffectiveRuleSet !== undefined) {
    if (!sameExactRef(admittedRuleset.ruleset_ref, replayed.receipt.bindings.effective_ruleset_ref)) {
      fail(code, 'supplied effective ruleset does not match result effective ruleset binding');
    }
  }
  ...
  return freezeDeep({
    ...
    ruleset_ref: cloneRef(replayed.receipt.bindings.ruleset_ref),
    source_packet_ref: cloneRef(replayed.receipt.bindings.source_packet_ref),
    digests: { ...replayed.receipt.digests },
  });
}
