...
The direct evaluator subject accepts an exact `{ manifest, binding, domain_input, cutoffs }`
envelope. The Core Adapter admits one exact Core TypedProjectFacts envelope and projects its one
closed request fact into that subject only after facts-digest, project/binding, and
bitemporal/cutoff equality checks. Raw `{ request }` wrappers are refused. The evaluator returns
an `assessment`, `domain_result`, and `receipt` with deterministic digests, including the
admitted effective-ruleset identity, and zero-effect counters.
