...
test('R&M Core Interface uses the base adapter but fails closed for derived evaluation', () => {
  ...
  const facts = buildReliabilityMaintainabilityPublicSyntheticTypedFacts();
  ...
  assert.throws(() => evaluate(reliabilityMaintainabilityAdapter, tampered, facts, {}),
    (error) => error.code === 'RM_EFFECTIVE_RULESET_INVALID');
  ...
  const derivedEnvelope = assembleEffectiveRuleSet(reliabilityMaintainabilityAdapter, resolveProfileBindings(makeValidBinding(), null), {});
  assert.throws(() => evaluate(reliabilityMaintainabilityAdapter, derivedEnvelope, facts, {}),
...
test('RED Compiler: profile_rule_provenance records must carry operation_index (0-indexed)', () => {
  const profile = makeValidBinding();
  const compiled = compileReliabilityMaintainabilityRules([profile]);
  const prov = compiled.effective_rule_set.profile_rule_provenance['RM-TEST-01'];
  assert.equal(typeof prov.operation_index, 'number');
  assert.equal(prov.operation_index, 0);
});

test('RED Compiler: calculateReliabilityMaintainabilityProfileOperationItemDigest is exported and binds operation_index', () => {
  const profile = makeValidBinding();
  const compiled = compileReliabilityMaintainabilityRules([profile]);
  const prov = compiled.effective_rule_set.profile_rule_provenance['RM-TEST-01'];
  const expectedDigest = calculateReliabilityMaintainabilityProfileOperationItemDigest(prov, 'RM-TEST-01');
  assert.equal(prov.operation_item_digest, expectedDigest);
});

test('RED Compiler: Profile rule claiming source_supported ceiling is rejected', () => {
  const profile = makeValidBinding({
    operations: [{ op: 'add', rule: { ...VALID_RM_RULE, claim_ceiling: 'source_supported' } }],
  });
  assert.throws(
    () => compileReliabilityMaintainabilityRules([profile]),
    (error) => error.code === RM_COMPILER_ERROR_CODES.RULE_INVALID_FIELD,
  );
});
