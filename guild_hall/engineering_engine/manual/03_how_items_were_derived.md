...
## 3.10 목적 문장은 어떻게 구했나 (`purpose_ko`, 2026-08-19)

행이 **무엇을 요구하는가**와, 그 산출물이 **무엇을 위한 것인가**는 다른 질문이다. 3.1~3.7이 앞의 것을
구했고, 이 절은 뒤의 것을 구한 기록이다. 필요해진 이유는 실측이었다 — 가이드 카드의 "왜"가 근거 등급·기대
상태·정본 대조 결과 세 문장뿐이어서, 인용은 있는데 **이유가 없었다**(11장 §11.10).

방법은 3.0의 파이프라인 그대로다.

1. **정본 확보(S0)**: 새 intake 없음. 이미 확보된 공통 지식 라이브러리의 파생 텍스트를 **읽기 전용**으로 썼다.
2. **사실 추출(S1)**: 정본 계열마다 리더 하나(병렬 6) — 방사청 가이드북 2017/2024 · SE 실무지침서 ·
   방위사업관리규정/국방전력발전업무훈령 · NASA SE Handbook Rev2 + NPR 7123.1D(둘로 나눔) · DoD SEG 2022.
   각 리더는 **자기 정본 안에서만** 읽고, 그 산출물의 목적을 말한 문장이 있을 때만 토큰을 낸다.
   목록·진입기준 표에만 이름이 나오는 산출물은 **비운다**. 산출은 200자 이하 한국어 한 문장과 인용 위치.
3. **합성(S2)**: 토큰마다 층별 정본 우선순위로 **하나만** 고른다(② 규정 > 가이드북 > 실무지침서, ① NASA > DoD).
   앞의 것은 `AGENTS.md`의 우선순위이고, 뒤의 것은 프로세스 입출력을 절 단위로 갖는 정본이 NASA 쪽뿐이기 때문이다(§3.7).
   목적 문장은 **근거 등급을 바꾸지 않는다** — `evidence_level`·`verification_status`는 한 건도 움직이지 않았고
   `purpose_refs`는 `source_refs`와 별도 목록이다.
4. **비판 검토(S3)**: 독립 critic이 층화 표본 10건의 인용 위치를 본문 재독으로 대조 — **10/10 확인, 정정 0건**.
   경고 2건은 인용이 아니라 적용 범위였다(회의록 계열이 회의 자체의 목적을 물려받음).
5. **스펙 반영(S4)**: 행에 `purpose_ko`(≤200자)와 `purpose_refs[{source_key, locator}]`. 컴파일러는 이 둘을
   **읽지 않고** 키만 검증한다(목적은 판정을 바꾸지 않으므로). exporter는 pass-through, `--check` 통과.
   ② `SE_FolderTree_Guide.md` v0.12→v0.13(71/100 토큰, 110행), ① `SE_FolderTree_GenericSE_Base.md` v0.3→v0.4(79/115 토큰, 171행).
6. **검증(S5)**: `--check`, `validate:se-stage-rules`, `validate:se-guidance`, KVDS 두 단계 재실행(판정 불변).

**목적 없는 행은 결함이 아니라 실측이다.** 카드는 그 자리에 `정본에 목적 문장 없음`이라고 적고 일반 지식으로
채우지 않는다. 규칙 4(§3.6)와 같은 이유다 — 채우면 인용된 지시와 구분할 수 없는 지시가 사람 앞에 놓인다.

행별 인용 위치·리더별 커버리지·스팟체크·미결은 `.registry/skills/se_foldertree_generate/codex/references/artifact_purpose_derivation_v0.md`,
작업 파일은 `_workspaces/knowledge/common/systems_engineering/derivations/artifact_purpose_20260819/`
(영수증 `_workmeta/system/reports/se_stage_rules/artifact_purpose_20260819.json`).
