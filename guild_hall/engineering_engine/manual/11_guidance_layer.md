...
| `source_catalog` | 정본 색인 영수증(source_key 목록) | 인용한 출처가 색인에 있나 | `catalog_known: null` |
...
| `how.template` | 스펙 행의 `template` | `how_template_stated` / `how_template_absent` |
| `how.inputs` | 행의 `depends_on` + `dependency_resolution`(범위) + 어휘 표시명 | `how_inputs_listed` / `how_inputs_none` |
...
| `why` | `engine_finding`·`reason_code`·`blocked_by`·`ready` + 카드의 `why` 문장들 | 엔진 + 카드 |
| `inputs` | 입력 토큰·표시명·범위 + `input_state`(`present` / `unknown`) | 카드 + 순서 목록 |
| `output` | 기대 산출물·성숙도·기대 상태·증거 기록 | 카드 |
| `how` | 양식·근거 인용·이 항목을 입력으로 쓰는 항목 | 카드 |
...
| **3 다음 할 일** | 지시서마다 무엇을·왜·입력·산출·어떻게·근거·담당·기한 |
...
| 한계 | 뜻 | 어디서 풀리나 |
| --- | --- | --- |
| "왜"가 규칙 문장 수준 | 카드는 "규정이 요구한다 / 가이드북이 권고한다"까지만 말한다. 그 조문이 실제로 무슨 요건을 말하는지는 인용 위치를 열어야 안다 | 조언 lane(E1) |
| 양식 없는 카드 11/28(CDR) | 스펙 행이 양식을 안 적어서지 이 층이 못 찾아서가 아니다 | 스펙 보강 |
...
검증: `npm run validate:se-guidance`(2026-08-18: 42) · `validate:se-stage-rules` · `validate:canon` ·
