...
| A2 | 선후 관계표 + 순서 계산(파이프라인) — **문서만이 아니라 활동·결정 노드 포함**(예: PBS 확정, 기능분석, 트레이드 스터디, 기본설계, 리서치, 이해관계자 확인; Owner 2026-08-18 "비서처럼 방향을 지시해야 서브 에이전트가 움직인다") | 스펙 행에 `입력 산출물(depends_on)` 열(①은 NASA/DoD, ②③은 방사청·발주처 근거) + 컴파일러 정렬(규정 필수→진입 조건→선후) | 4층 합친 결과에서 과제·사업유형·등급·발주처별 순서가 자동으로 나옴; 빈 과제도 "무엇부터" 답함 |
| A3 | 산출물·활동별 가이드 카드(어떻게) + **지시서 형식** | 스펙 행 + 정본 인용(색인) + 양식 + 앞뒤 관계 + 담당 capability → 카드; 엔진의 mission 후보를 서브 에이전트가 읽는 지시서(무엇을·왜·입력·산출·근거·담당·기한) 형식으로 | 첫 "이제 뭐 해야 해?" 답: 위치·부족·다음 3개(왜·어떻게·근거·담당). Owner용, 스크립트로 |
