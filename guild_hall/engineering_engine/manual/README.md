...
| MCP 문 + 야간 예약 실행 | 밖에서 부르는 문 하나, 밤에 자동 | 호출 | **부분(B1 착지 2026-08-18 + 후속 2026-08-19, 꺼진 채)** — `mcp/`의 stdio JSON-RPC 서버 + 과제 프로필 + 도구 17(읽기 13·쓰기 4), 시험 70, 설명은 [12장](12_mcp_door.md), 사람용 등록 안내는 [§12.A](12_mcp_door.md#12a-등록사용-안내-사람용). 스위치 둘 다 꺼져 있고 어떤 클라이언트에도 등록하지 않았다(켜기는 Owner 결정). 남은 것: 답·맥락 도구(B2) · 야간 예약(B3, 엔진 밖) |
