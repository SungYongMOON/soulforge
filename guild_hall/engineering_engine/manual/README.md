...
| 접근 권한 모델(신원·권한표·집행·파일 권한) | 부르는 사람(역할) × 자료 등급(공개 규칙·팀 판단·기밀 계약·개인) × 동작; 엔진 문은 로컬, 원격은 비서/게이트웨이 층 로그인; 태그 없는 문서는 기밀(fail-closed) | 문 | 없음(합의 2026-08-19, 09장 9.1F; B1 후속에 principal+접근표 최소형) |
