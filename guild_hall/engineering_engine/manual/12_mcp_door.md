...
| `nas_root` | **넷째 뿌리**(2026-08-19). 문 앞 칸 세 폴더만 여기로 옮긴다. UNC는 `\\서버\공유폴더` 형태여야 하고(드라이브 문자 금지 — 로그인 세션에 딸려 있다), 매핑된 절대경로도 된다 | 회사 공유폴더 |
...
이미 가진 세 뿌리 안에 있어야 한다 — **예외는 문 앞 칸 세 폴더뿐이고, 그때도 프로필이 적은 `nas_root`
안이어야 한다**. (3) **`receipts_dir`·`runs_root`는 `_workmeta/<project_code>/` 아래여야
...
| 공유폴더(`nas`) | 프로필의 `nas_root` (예: `\\서버\soulforge_intake`) | **문 앞 칸 세 폴더만.** 과제 자료도 영수증도 여기 두지 않는다 |

**넷째 뿌리는 문만 옮긴다**(2026-08-19). `nas_root`를 적으면 `intake_dir`·`outbox_dir`·`trash_dir`
셋이 그 아래로 가고, 나머지는 하나도 움직이지 않는다 — 관측도 영수증도 `confidential_dirs`도 원래
자리 그대로다. 다섯 줄이 더 붙는다: 셋 다 `nas_root` **아래**여야 하고, 뿌리 자체일 수 없고, 과제 폴더
**안**일 수 없으며(공유폴더인 척하는 과제면 문이 되고 그러면 아래 교차 뿌리 규칙이 전부 헛돈다),
`nas_root`는 메타면 아래일 수 없고(`_workmeta`는 포인터·해시·상태만 담는다), 문을 열지 않은 프로필은
`nas_root`를 적을 수 없다. UNC는 `\\서버\공유폴더` 두 토막을 다 갖춘 형태만 받는다 — `//서버/공유폴더`는
고쳐 주지 않고 거절하고, 드라이브 문자는 애초에 뿌리로 못 쓴다(로그인 세션에 딸려 있어서 무인 실행에서
사라진다, §12.C).

경로 예산은 공유폴더에서도 그대로 잰다(뿌리만 `nas_root`로 바꿔서 잰다). SMB 너머라고 Windows 경로
한계가 사라지지 않는다.
...
`npm run validate:se-mcp` — 2026-08-19(4차, 공유폴더 한 바퀴): **135**
(같은 날 3차: 117 · 2차: 104 · 1차: 70 · 2026-08-18: 28). 3차→4차로 늘어난 18은 넷째 뿌리 6 ·
교차 뿌리 등록·정리·표식·목록 10 · 표 장부에 URL이 없다는 것 2다.
발급기 자체는 별도로 `npm run validate:nas-link-issuer` — **59**(게이트웨이 owner, §12.C).
프로필 검증 21 · 명부·라우팅 10 · 접근 모델 16 · 표 대장 13(순수: 기한·소유·이름·판 붙이기·뿌리·링크 칸) ·
도구 22(합성 fixture 대조·결정론·쓰기 거절·경로 예산·잠금·캐시 무효화·쪽 나누기) ·
파일 문 36(표·이동 등록·폴더 불일치 거절·덮어쓰기 거절·판 붙이기·해시·등급 거절·정리 ·
링크 없음/키 없음/모의 발급/자식 실패의 네 갈래 ·
공유폴더 표·`copy_verify`+치움·`move`·내려받기 outbox·정리·옛 줄 호환·표식 제외·목록·링크 등급) ·
...
언제까지가 적히고 **링크 자체와 비밀번호는 적히지 않는다**(아래 "링크는 어디에 적히나"). 기한이 지나면
그 표로는 등록할 수 없고, 폴더는 남아 있다가 정리 때 휴지통으로 간다.

**칸이 공유폴더에 있어도 절차는 같다**(2026-08-19). 프로필이 `nas_root`를 적으면 문 앞 칸 세 폴더가
회사 공유폴더로 가고, 표의 `folder_ref`는 **그 뿌리 기준** 상대 포인터가 된다. 어느 뿌리인지는 표 줄의
`folder_root`(`project` | `nas`)에 적힌다 — 그 칸이 없는 옛 줄은 `project`라는 뜻이고, 그래서 공유폴더를
나중에 붙인 과제도 옛 표를 그대로 찾아간다.

**등록은 뿌리가 같으면 옮기고, 다르면 복사한 뒤 원본을 치운다.**

| 뿌리 | `transfer_kind` | 무슨 일이 일어나나 |
| --- | --- | --- |
| 같음 (과제면 → 과제면) | `move` | 지금까지와 같다. 옮기고 나면 칸에는 없다 |
| 다름 (공유폴더 → 과제면) | `copy_verify` | 복사 → 양쪽 해시 대조 → **원본을 그 공유폴더의 휴지통** `_trash/consumed/<표>/`로 이동 |

드라이브가 다르면 옮길 방법이 없으니 복사인데, 복사만 하고 두면 다음 사람이 칸에서 "이미 등록된 파일"을
보고 또 등록한다. 그렇다고 지우지는 않는다 — 엔진에는 지우는 도구가 없다. 그래서 **복사·대조·치움** 셋이
한 묶음이고, 영수증에 `transfer_kind`와 **양쪽 해시**(`source_sha256`·`sha256`)가 같이 남는다. 해시가
어긋나면 아무것도 등록되지 않는다.

정리(`file_tickets_gc`)도 같은 뿌리를 따라간다: 공유폴더 표는 공유폴더 휴지통으로 간다.
...
**링크는 어디에 적히나 (Owner 결정 2026-08-19).** 살아 있는 URL은 **메타면에 넣지 않는다.**
`_workmeta`는 포인터·해시·상태만 담는 평면인데(AGENTS.md) 공유 링크는 포인터가 아니라 **작동하는 권한**
이고, 메타면을 읽을 수 있는 사람은 그걸 그냥 쓸 수 있다. 그래서 셋으로 나눈다.

| 어디 | 무엇이 | 왜 |
| --- | --- | --- |
| 표 장부(`_workmeta`) | `link_kind` · `link_expires_at` · `dsm_link_id` | 감사에 필요한 것은 "어떤 링크가 언제까지 있었나"까지다. `link_url` 칸은 **아예 없다**(넣으려 하면 거절한다) |
| 답 | 위 셋 + `link_url` | 표를 낸 사람이 지금 건네줄 것 |
| 칸 안 `.soulforge_ticket.json` | 위 넷 (create-only) | 그 파일을 읽을 수 있는 사람은 이미 그 폴더를 열 수 있는 사람이다. 비밀번호·토큰·세션은 여기에도 없다 |

이 표식 파일은 이름이 점으로 시작해서 등록 대상에서 자동으로 빠지고(`assertUploadFileName`이 점으로
시작하는 이름을 거절한다), `file_register`도 따로 걸러 낸다 — 엔진이 넣어 둔 파일을 사람이 지워야
등록되는 문은 만들지 않는다.

**링크 등급은 링크가 닿는 것을 따른다.** 올리기 링크는 ⓑ다 — **빈 칸 하나**에 대한 권한이고 어디에
있는지 말하지 않으며, 표를 낼 수 있는 역할에게 가리면 기다리는 사람에게 건넬 것이 없어진다. 내려받기
링크는 실제 파일에 닿으므로 **그 파일의 등급**을 가져간다: 기밀 폴더의 파일이면 ⓒ, 아니면 ⓑ. 답에
`link_data_class`로 어느 쪽이 적용됐는지 같이 나온다.
...
### 한 바퀴가 닫혔다 (2026-08-19 후속)

세 조각이 붙어서 **링크로 올라온 파일이 관측까지 간다.**

1. **넷째 뿌리 `nas_root`** (§12.3·§12.B) — 문 앞 칸 세 폴더가 회사 공유폴더에 설 수 있다. 표 폴더는
   NAS에서 `tickets|outbox/<사람>/<표>`이고, 이제 그 자리가 **프로필이 아는 자리**다.
2. **교차 뿌리 등록** — 공유폴더 → 과제면은 `copy_verify`: 복사하고 양쪽 해시를 대조한 뒤 원본을 그
   공유폴더의 `_trash/consumed/<표>/`로 치운다. 지우지 않는다. 같은 뿌리면 지금까지처럼 `move`다.
3. **내려받기·정리도 같은 뿌리를 따른다** — 내려받기 표는 공유폴더 outbox로 복사하고, 정리는 공유폴더
   휴지통으로 옮긴다.

**Owner-reviewable 기본값 셋(2026-08-19 결정, 바꾸려면 이 절을 고친다).**

| # | 기본값 | 왜 |
| --- | --- | --- |
| (a) | 올리기 링크는 ⓑ, 내려받기 링크는 **닿는 파일의 등급**(기밀이면 ⓒ) | 빈 칸에 대한 권한과 실제 문서에 대한 권한은 같은 것이 아니다 |
| (b) | 살아 있는 URL은 `_workmeta`에 넣지 않는다 — 표 장부엔 `dsm_link_id`·`link_kind`·`link_expires_at`만, URL은 답과 칸 안 `.soulforge_ticket.json`에만 | 메타면은 포인터·해시·상태만 담는다. 공유 링크는 포인터가 아니라 권한이다 |
| (c) | 링크 **암호는 당분간 쓰지 않는다** | 발급기는 `--password-from-env`로 받을 수 있지만, 암호를 받은 사람에게 **링크와 다른 경로로** 전달할 방법이 아직 없다. 링크와 같은 메일에 적는 암호는 암호가 아니다 |
...
4. 다른 PC 브라우저로 그 링크에 업로드 → 5. "올렸어" → `file_register` → **`copy_verify`로 정식 자리** →
관측 → 판단 반영 → 6. 공유폴더 `_trash/consumed/<표>/`에 원본이 있는지 → 7. 만료 뒤 링크가 닫히는지 →
8. 정리 미리보기(`dry_run` 기본) → 9. `dry_run:false`로 공유폴더 휴지통까지.

프로필에 넣을 줄은 셋이다(값은 Owner가 채운다):
`"nas_root": "\\\\<서버>\\soulforge_intake"` · 문 앞 칸 세 폴더를 그 아래로 ·
`"link_issuer": {"kind":"synology","env_prefix":"SOULFORGE_NAS"}`.

### 오늘(2026-08-19) 실제 상태
- 파일 문(표·등록·넣기/받기·정리) 병합됨. 그 위에 링크 발급기·문 hook·넷째 뿌리·교차 뿌리 등록이
  올라갔다(`validate:se-mcp` 135 · `validate:nas-link-issuer` 59, 전부 모의 DSM과 임시 폴더).
  **실계정·실호출·실제 공유폴더는 없다.** KVDS 프로필에는 아직 `link_issuer`도 `nas_root`도 넣지
  않았다(Owner가 넣을 줄은 §12.C "만든 것"·"한 바퀴가 닫혔다" 참조).
