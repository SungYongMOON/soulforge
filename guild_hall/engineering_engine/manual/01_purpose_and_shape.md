...
2. **제대로 됐나** — 있는 문서가 필수 절·표·양식을 갖추고 요구를 다뤘는가. 이것이 **문서 내용 검사기**(Owner 제안 2026-08-17, 설계 §2.1A)이며 아직 만들지 않았다. 검사기는 엔진 안이 아니라 관측을 만드는 쪽에 붙어 를 관측으로 넣고, 미검사는 부족으로 찍지 않는다. 모든 기능 표·구조도에 상태 "없음(합의됨)"으로 함께 그린다(README의 합의 목록).
