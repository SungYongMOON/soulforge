...
import "../../engines/interface_consistency/evaluator/interface_consistency_evaluator_adapter.mjs";
...
test("Core Interface: domain engine adapters register and load successfully", () => {
  ...
  const interfaceConsistencyAdapter = loadDomainEngineAdapter("interface_consistency");
  assert.equal(interfaceConsistencyAdapter.domain_engine_id, "interface_consistency");
  assert.ok(validateDomainEngineAdapter(interfaceConsistencyAdapter));
