...
test('Topology Edge Oracle: independently resolves complete static import/re-export graph', () => {
  const roots = ['core', 'engines/systems_engineering', 'engines/quality_readiness', 'engines/database_engineering', 'engines/material_procurement_readiness', 'engines/reliability_maintainability', 'engines/pcb_compliance'];
  ...
  assert.equal(emittedTopology.module_count, 175, 'Canonical module count must equal 175');
  assert.equal(emittedTopology.module_edge_count, oracleResult.edges.length, 'Edge count must match independent scan');
  assert.equal(emittedTopology.module_edge_count, 563, 'Canonical edge count must equal 563');
});
