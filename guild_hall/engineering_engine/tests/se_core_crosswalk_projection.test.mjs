...
} from '../subjects/common_se_corpus_projection.mjs';
...
test('Engine runtime projection excludes evaluator labels and opaque lineage cannot select a row', () => {
  ...
  const subjectSource = readFileSync(resolve(HERE, '../subjects/se_core_crosswalk_case_run.mjs'), 'utf8');
...
test('pure case subject has no filesystem, subprocess, network, Notebook, RAG, or learned-model route', () => {
  const subject = readFileSync(resolve(HERE, '../subjects/se_core_crosswalk_case_run.mjs'), 'utf8');
