...
} from '../subjects/ax_se_project_role_bound_assessment.mjs';

import { buildAxSeProjectRoleRoster } from '../subjects/ax_se_project_role_roster.mjs';
import {
  assessAxSeProject,
  buildAxSeAssessmentInput,
} from '../subjects/ax_se_project_assessment.mjs';
import { candidateHandle, isCandidateHandle } from '../kernel/minting.mjs';
...
const MODULE_URL = new URL('../subjects/ax_se_project_role_bound_assessment.mjs', import.meta.url);
...
test('the pure subject imports no execution adapter and preserves accepted v0 bytes exactly', () => {
  ...
  assert.equal(
    digest(new URL('../subjects/ax_se_project_assessment.mjs', import.meta.url)),
    '27df6edbe37416a8151cad907cf2c1a6b13f1885cf277d9a49e3cebcebf9fbb4',
  );
  assert.equal(
    digest(new URL('../tools/ax_se_project_assessment_runner.mjs', import.meta.url)),
    '2a381a640d1f2907b0b83da5e9473072ab0ed74cf0b0c8f7c2285811b21f78ef',
  );
  assert.equal(
    digest(new URL('../subjects/ax_se_project_role_roster.mjs', import.meta.url)),
    'a7567c713c393a614c5ff0c981ff5e62284fa3ffd3d48e7c1a102fef0b9d3bd3',
  );
});
