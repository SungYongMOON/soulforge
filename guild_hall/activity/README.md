...
- `sync` 는 allowlist 된 activity event 필드만 mirror 한다. legacy row 의 unknown field 는 복사하지 않고, malformed JSONL row 는 원본 ledger 안에 그대로 보존하되 다른 surface 로 복제하지 않는다.
