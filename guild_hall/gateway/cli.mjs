...
function renderMonsterCreatedMessage({ inboxId, sourceRef, subject, monsters }) {
  const lines = [
    "[gateway][monster_created]",
    `inbox_id=${inboxId}`,
  ...
  for (const monster of monsters) {
    lines.push(`- ${monster.monster_id} | ${monster.monster_family} | ${monster.objective}`);
  }
