...
- `mail_body_excerpt.mjs`
  ...
  - Current dev-ERP callers can persist normalized text as runtime-only `core_mail.body_text`; `body_preview` remains the display/search fallback. Raw HTML/provider payloads and attachments stay only in the gitignored event sink.
