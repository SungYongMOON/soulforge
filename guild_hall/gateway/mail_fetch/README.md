...
- `OUTLOOK_SENT_ALLOWED_WINDOWS_KST`를 반드시 지정한다. HPP 운영값은
  `02:00-04:00,12:00-14:00`이며 각 KST 시간대에서 성공 1회만 수행한다.
  이 값은 HPP capsule 실행 시 비밀값 파일보다 우선하는 public-safe 운영
  설정으로 주입되므로, credential 파일을 열거나 수정하지 않고도 전 PC에서
  같은 시각 계약을 유지한다.
