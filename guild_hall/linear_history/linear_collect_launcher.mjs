...
// Exact-runtime launcher for the Linear collection lane (mirror of the Slack
// batch launcher). It pins the Node executable, the runtime manifest, and the
...
export const LINEAR_COLLECT_RUNTIME_MANIFEST_SCHEMA_VERSION = "soulforge.linear_collect.runtime_manifest.v1";
export const LINEAR_COLLECT_RUNTIME_ENTRYPOINT = "guild_hall/linear_history/linear_collect_cli.mjs";
export const LINEAR_COLLECT_RUNTIME_LAUNCHER = "guild_hall/linear_history/linear_collect_launcher.mjs";
export const LINEAR_COLLECT_RUNTIME_ATTESTATION = Symbol.for("soulforge.linear_collect.runtime_attestation");
...
// Broader than the Slack launcher on purpose: a credential file copied into a
// runtime tree under any prefixed name (linear_api_key.txt, user_access_token.txt)
// is refused before its bytes are hashed.
const SECRET_BASENAME_PATTERN = /^(?:\.env(?:\..+)?|.*\.(?:pem|p12|pfx|key)|(?:.*[._-])?(?:secret|secrets|token|tokens|password|passwords|api_key|api_keys)(?:[._-].*)?)$/iu;

export class LinearCollectRuntimeError extends Error {
  constructor(code, target, message) {
    super(`${code} at ${target}: ${message}`);
    this.name = "LinearCollectRuntimeError";
    this.code = code;
    this.path = target;
  }
}
...
function fail(code, target, message) {
  throw new LinearCollectRuntimeError(code, target, message);
}
...
export function validateLinearCollectRuntimeManifest(manifest) {
  exactKeys(manifest, MANIFEST_FIELDS, "$runtime_manifest");
  if (manifest.schema_version !== LINEAR_COLLECT_RUNTIME_MANIFEST_SCHEMA_VERSION) {
    fail("runtime_manifest_schema_invalid", "$runtime_manifest.schema_version", "Unexpected manifest schema");
  }
  if (manifest.entrypoint !== LINEAR_COLLECT_RUNTIME_ENTRYPOINT) {
  ...
  if (!retained.has(LINEAR_COLLECT_RUNTIME_ENTRYPOINT) || !retained.has(LINEAR_COLLECT_RUNTIME_LAUNCHER)) {
    fail("runtime_required_file_missing", "$runtime_manifest.files", "Launcher and entrypoint must be pinned");
  }
  return manifest;
}

export async function verifyExactLinearCollectRuntime({
  ...
  let manifest;
  try {
    manifest = validateLinearCollectRuntimeManifest(
      JSON.parse(manifestBytes.toString("utf8").replace(/^\uFEFF/u, "")),
    );
  } catch (error) {
    if (error instanceof LinearCollectRuntimeError) throw error;
...
async function main() {
  try {
    ...
    const verified = await verifyExactLinearCollectRuntime({
      ...request,
    ...
    globalThis[LINEAR_COLLECT_RUNTIME_ATTESTATION] = Object.freeze({
    ...
    const code = error instanceof LinearCollectRuntimeError ? error.code : "unknown_failure";
    process.stderr.write(`linear_collect_runtime_rejected:${code}\n`);
    process.exitCode = 1;
  }
}
