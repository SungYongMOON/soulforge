// Private custody primitives for the Linear collection lane.
//
// Mirrors the Slack lane's custody contract: every persistent path is resolved
// under one absolute private root, reparse points are refused, state files are
// written atomically, content-addressed objects are create-only, and the
// writer lease is fail-closed (never guessed stale, never auto-deleted).
...
export class LinearCustodyError extends Error {
  constructor(code, target, message) {
    super(`${code} at ${target}: ${message}`);
    this.name = "LinearCustodyError";
    this.code = code;
    this.path = target;
  }
}
...
function fail(code, target, message) {
  throw new LinearCustodyError(code, target, message);
}
