...
param(
  ...
  [string]$TaskName = "Soulforge-HPP-Linear-Collect",
...
function Assert-DisjointPath {
  ...
  if ((Test-SameOrChildPath -Parent $Left -Candidate $Right) `
    ...
    throw "linear collect protected roots overlap"
  }
}
  ...
  while ($true) {
    if (Test-Path -LiteralPath $Cursor) {
      ...
      if (($Item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
        throw "linear collect path contains a reparse point"
      }
    }
...
function Resolve-CanonicalDirectory {
  ...
  if (-not (Test-Path -LiteralPath $Absolute -PathType Container)) {
    throw "linear collect required directory is missing"
  }
  ...
  if (-not $Resolved.Equals($Absolute, [StringComparison]::OrdinalIgnoreCase)) {
    throw "linear collect directory is not canonical"
  }
  return $Resolved
}
...
function Resolve-CanonicalFile {
  ...
  if (-not (Test-Path -LiteralPath $Absolute -PathType Leaf)) {
    throw "linear collect required file is missing"
  }
  $Item = Get-Item -LiteralPath $Absolute -Force
  if (($Item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
    throw "linear collect required file is a reparse point"
  }
  $Resolved = [IO.Path]::GetFullPath($Item.FullName)
  if (-not $Resolved.Equals($Absolute, [StringComparison]::OrdinalIgnoreCase)) {
    throw "linear collect file is not canonical"
  }
  return $Resolved
}
...
function Resolve-PlannedDirectory {
  ...
  while (-not (Test-Path -LiteralPath $Cursor)) {
    ...
    $Parent = [IO.Directory]::GetParent($Cursor)
    if ($null -eq $Parent) {
      throw "linear collect state root has no existing ancestor"
    }
    $Cursor = $Parent.FullName
  }
  if (-not (Test-Path -LiteralPath $Cursor -PathType Container)) {
    throw "linear collect state root ancestor is not a directory"
  }
...
if ($TaskName -ne "Soulforge-HPP-Linear-Collect") {
  throw "linear collect task name is fixed"
}
...
if ((Get-TimeZone).Id -ne "Korea Standard Time") {
  throw "linear collect task requires the Asia/Seoul Windows time zone"
}
...
$Launcher = Resolve-CanonicalFile -Path (
  Join-Path $RuntimeRoot "guild_hall\linear_history\linear_collect_launcher.mjs"
)
...
if (-not (Test-SameOrChildPath -Parent $PrivateRoot -Candidate $StateRoot -Strict)) {
  throw "linear collect state root must be a strict private-root child"
}
if (-not (Test-SameOrChildPath -Parent $PrivateRoot -Candidate $BindingPath -Strict) `
  ...
  throw "linear collect binding must be private and disjoint from state"
}
if (-not (Test-SameOrChildPath -Parent $RuntimeRoot -Candidate $RuntimeManifestPath -Strict) `
  ...
  throw "linear collect runtime files escaped the runtime root"
}

$ActualBindingSha256 = "sha256:" + (Get-Sha256File -Path $BindingPath)
if ($ActualBindingSha256 -ne $BindingSha256) {
  throw "linear collect binding SHA-256 changed"
}
...
if ($ActualRuntimeManifestSha256 -ne $RuntimeManifestSha256) {
  throw "linear collect runtime manifest SHA-256 changed"
}
$ActualNodeSha256 = "sha256:" + (Get-Sha256File -Path $NodePath)
if ($ActualNodeSha256 -ne $NodeSha256) {
  throw "linear collect Node SHA-256 changed"
}

$BindingText = Get-Content -Raw -Encoding UTF8 -LiteralPath $BindingPath
if ($BindingText -match '(?i)lin_(?:api|oauth)_[A-Za-z0-9]{16,}') {
  throw "linear collect binding contains a token-like value"
}
$Binding = $BindingText | ConvertFrom-Json
if ($Binding.schema_version -ne "soulforge.linear_collect.binding.v1" `
    -or $Binding.feature_enabled -ne $true) {
  throw "linear collect binding schema or feature state is invalid"
}
if (-not ([IO.Path]::GetFullPath([string]$Binding.private_root)).Equals(
  ...
  throw "linear collect binding private roots differ from registration inputs"
}
...
if ($LASTEXITCODE -ne 0) {
  throw "linear collect no-network preflight failed"
}
try {
  $Preflight = ($PreflightOutput -join [Environment]::NewLine) | ConvertFrom-Json
} catch {
  throw "linear collect no-network preflight returned invalid aggregate output"
}
if ($Preflight.mode -ne "preflight" `
  ...
  throw "linear collect no-network preflight did not attest the binding"
}
...
$HiddenLauncher = Resolve-CanonicalFile -Path (
  Join-Path $RuntimeRoot "guild_hall\linear_history\ops\run-linear-collect-hidden.vbs"
)
$TaskNodeArguments = @(
  $LauncherCommonArguments
  "--apply"
...
if ($Existing) {
  if ($Existing.State -eq "Running") {
    throw "existing linear collect task is still running"
  }
  if (-not $ExpectedExistingTaskSha256 `
      -or $ExpectedExistingTaskSha256 -notmatch '^[0-9A-Fa-f]{64}$') {
    throw "existing linear collect task replacement requires its exact SHA-256"
  }
  if (-not (Test-Path -LiteralPath $TaskFile -PathType Leaf)) {
    throw "existing linear collect task file is unavailable"
  }
  ...
  if ($ActualExistingTaskSha256 -ne $ExpectedExistingTaskSha256.ToUpperInvariant()) {
    throw "existing linear collect task SHA-256 changed"
  }
...
$Plan = [ordered]@{
  schema_version = "soulforge.linear_collect_task.plan.v1"
  task_name = $TaskName
...
if (-not $Register) {
  Write-Output "linear collect task dry-run attested: plan_digest=$PlanDigest interval=PT15M mutation=false"
  return
}
if (-not $ExpectedDryRunDigest `
    -or $ExpectedDryRunDigest -notmatch '^sha256:[0-9a-f]{64}$' `
    -or $ExpectedDryRunDigest -ne $PlanDigest) {
  throw "linear collect registration requires the matching dry-run plan digest"
}
if (-not $PSCmdlet.ShouldProcess(
    $TaskName,
    "register the fixed hidden 15-minute read-only Linear collection task"
  )) {
  Write-Output "linear collect task registration skipped"
  return
}
...
try {
  $null = Register-ScheduledTask `
    ...
    -Principal $Principal `
    -Settings $Settings `
    -Description "Soulforge HPP read-only Linear collection every 15 minutes (Asia/Seoul); bounded, no mutation, no reboot." `
    -Force `
    -ErrorAction Stop
  ...
  if (-not $RegistrationValid) {
    throw "registered linear collect task failed exported XML attestation"
  }

  $ExportedTaskSha256 = Get-Sha256Text -Value $ExportedTaskXml
  Write-Output "linear collect task registered and XML-attested: interval=PT15M exported_xml_sha256=$ExportedTaskSha256"
} catch {
  ...
  try {
    if ($null -ne $ExistingTaskXml) {
      ...
      if ((Get-Sha256Text -Value $RestoredTaskXml) -ne $ExistingTaskXmlSha256) {
        throw "restored linear collect task XML differs from the prior definition"
      }
    } else {
      ...
      if (Get-ScheduledTask -TaskName $TaskName -ErrorAction SilentlyContinue) {
        throw "new linear collect task remained after rollback"
      }
    }
  } catch {
    $RollbackFailure = $_
  ...
  if ($null -ne $RollbackFailure) {
    throw "linear collect registration failed and rollback failed; the task was disabled"
  }
  throw "linear collect registration failed; the prior task definition was restored or the new task was removed: $($RegistrationFailure.Exception.Message)"
}
