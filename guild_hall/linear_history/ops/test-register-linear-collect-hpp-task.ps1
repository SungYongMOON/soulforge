...
if (-not $RegistrarPath) {
  $RegistrarPath = Join-Path $PSScriptRoot "register-linear-collect-hpp-task.ps1"
}
if (-not (Test-Path -LiteralPath $RegistrarPath -PathType Leaf)) {
  throw "linear collect registrar structural test could not find the source"
}
...
if ($ParseErrors.Count -ne 0) {
  throw "linear collect registrar has PowerShell parse errors"
}
...
if ($TriggerCommands.Count -ne 1) {
  throw "linear collect registrar must construct exactly one task trigger"
}
...
if ($TriggerSource -notmatch '(?i)\s-Once(?:\s|$)' `
  ...
  throw "linear collect trigger must be one time trigger repeating every 15 minutes"
}

$Source = Get-Content -Raw -Encoding UTF8 -LiteralPath $RegistrarPath
foreach ($RequiredPattern in @(
  'TaskName\s*=\s*"Soulforge-HPP-Linear-Collect"',
  ...
  'node_sha256',
  'lin_\(\?:api\|oauth\)_',
  'soulforge\.linear_collect\.binding\.v1',
  ...
  if ($Source -notmatch $RequiredPattern) {
    throw "linear collect registrar is missing a required structural guard: $RequiredPattern"
  }
}
foreach ($ForbiddenPattern in @(
  ...
  if ($Source -match $ForbiddenPattern) {
    throw "linear collect registrar contains a forbidden command or trigger structure: $ForbiddenPattern"
  }
}
if (@($CommandNames | Where-Object { $_ -eq "Register-ScheduledTask" }).Count -ne 2 `
  ...
  throw "linear collect registrar must include registration attestation and fail-closed rollback surfaces"
}
...
if ($RegisterCommand -notmatch '(?i)-Trigger\s+@\(\$TriggerEvery15\)') {
  throw "linear collect registration must receive exactly the one fixed trigger object"
}
...
if ($RestoreCommand -notmatch '(?i)-Force') {
  throw "linear collect rollback must restore the exact prior exported XML"
}

Write-Output "linear collect PowerShell registrar structural checks passed"
