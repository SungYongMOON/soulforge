...
- **Usage trend chart scale decompression and latest-day summary (`team-ops-board`)**: `UsageTrendChart` now renders a compact latest-day summary row above the SVG showing the latest date and nonzero token values (bounded top 3 plus safe remainder) for model and provider tabs. Selecting a legend series dynamically rescales the chart's vertical axis to that series' individual 30-day peak rather than retaining the stacked multi-billion-token aggregate maximum, making recent high-volume days visibly inspectable while preserving stable series colors, hit-grid tooltip navigation, keyboard focus, and AG request overlay.
