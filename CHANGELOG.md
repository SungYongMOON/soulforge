...
- AI Usage Meter (`usage_meter.mjs`) isolates conflicting event identity groups into sanitized `usage_event_duplicate_conflict` quarantine issues without picking conflict winners, persisting clean non-conflicting events. The producer companion validates the collect apply result schema and safe integer counts before advancing projection, holding the Codex lane in degraded error upon duplicate conflict or malformed result.
