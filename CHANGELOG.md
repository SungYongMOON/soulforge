...
- dev-ERP auto-intake가 project context branch summary를 prompt에 넣기 전에 같은 프로젝트·같은 `branch_key`의 `sources.csv`와 source count/updated time을 비교하도록 보강했다. 다른 branch의 최신 summary나 project-wide 합계가 branch별 drift를 가리지 못하며, source/summary 누락, foreign-only 또는 header-only source, producer와 같은 classified-source count 불일치, 엄격한 ISO 달력 timestamp 오류, 다른 project row를 stable `context_gap:*`으로 드러내고 검증되지 않은 branch state를 주입하지 않는다 (worker: codex_gpt-5).
