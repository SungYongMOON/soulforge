...
- **UI clean 30-day chart (`team-ops-board`)**: `UsageTrendChart` in `App.tsx` renders a clean, simple 30-day chart with a truthful textual basis note (` · 토큰 관측일 기준` for complete coverage, ` · 토큰 관측일 우선 · 미근거 항목은 시작일 기준` for partial coverage, and omitted for v3/zero-match fallback), completely avoiding range toggles and latest summary workarounds.
